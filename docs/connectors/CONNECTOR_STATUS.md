...
**Overall Status**: 5 Production-ready (0 live-verified), 1 Beta, 11 Demo-Mode, 1 Stub (18 total connectors), 200+ Planned
...
|--------|-------|------------|
| **production** | 0 | Real HTTP with a live-credential run on record, `static productionStatus = 'production'` declared (none today) |
| **production_ready** | 5 | Real HTTP, readiness-complete (governance, rate limiting, DLP, unit and contract suites), no live-credential run on record; `static productionStatus = 'production_ready'` declared. NetSuite has partial live evidence (2025-10-17 sandbox authentication and reads) |
...
**Current partition (verified 2026-09-23)**: `npm run audit-status-claims` → `audit-status-claims: OK (18 connectors; production=0 production_ready=5 beta=1 demo_only=11 stub=1)`, matching the table above.

**Historical, superseded:** the T4 re-verification (2026-08-07, `docs/claude-md-slim` at `8f046f55c4`) reported `production=5 beta=1 demo_only=11 stub=1`. That was the partition before the five connectors were relabelled `production_ready`, because none has a live-credential run on record.

## Canonical Registry and Wiring

Canonical registry: `src/connectors/connectorRegistry.ts` (PR 6A + 6A-2, ADR-015) — single source of truth for what ships AND canonical wiring. Per-entry `factory(systemId, deps)` closures encapsulate five constructor shapes; `ConnectorManager.createConnector()`, `inversify.config.ts`, `IntegrationService.getConnector()`, and ancillary routes read the closure instead of `new XxxConnector(...)`. CI gate `audit-status-claims --check-wired-connectors` enforces registry↔AST↔proof-card consistency AND wiring drift (any `new <ClassName>(` outside `connectorRegistry.ts` for a factory-wired class fails CI; tests + `scripts/` exempt). Adding #19: see `AGENTS.md` → "How to add connector #19".

## IntegrationConfig Storage

**IntegrationConfig storage** (PR 13c-4): `ConfigurationService` keys configs in-memory by `(tenantId, id)` — `Map<`${tenantId}::${id}`, IntegrationConfig>` — tenant-isolated. On-disk is FLAT `${configDirectory}/${id}.json` (top-level only); `loadConfigurations()` reads top-level `*.json` and IGNORES subdirs — the `integrations/` dir is overloaded with ERP connector-artifact subdirs (`business_central/*.al`, `dynamics365/*.al`, `netsuite/*.js`) that are NOT configs. (A `${tenantId}/${id}.json` subdir layout + boot migration was attempted and reverted — collided with that dual-use dir and crashed boot.) Route layers MUST resolve via `getConfigurationForTenant(req.user.tenantId, id)` — never deprecated `getConfiguration(id)`, which throws `ConfigurationLookupAmbiguousError` (→ 409) on shared id. `/api/configurations` + `/api/integrations` sit behind `authMiddleware`; handlers narrow `req.user?.tenantId` (401 if missing), 404 on cross-tenant collision. Flat `${id}.json` can't hold the same id for two tenants, so `saveConfiguration` rejects a cross-tenant same-id write (409); durable same-id storage deferred until the config store splits from the connector-artifact dir. Migration (PR #869): `SecureConfigurationService` resolves via `getConfigurationForTenant`; dead `exportConfiguration(configId)` deleted. Only remaining `getConfiguration(id)` callers are `IntegrationService`/`IntegrationOrchestrator` `resolveConfiguration`, a **deliberate** background/system escape hatch (e.g. BatchProcessingService) when no request-scoped tenant exists. **Blocking CI gate** (PR #1018): `node scripts/check-route-getconfiguration-abstention.mjs` enforces route-layer abstention (regression: `tests/scripts/check-route-getconfiguration-abstention.test.sh`).
...
- All 16,571 executed tests passing (100%, 26 skipped)
...
|--------|-------|-------------|
| **Production-Ready** | 5 | Real API connectors, readiness-complete and contract- and fixture-tested; 0 live-verified (NetSuite, Salesforce, Business Central, HubSpot, ShipStation) |
...
### 1. NetSuite
**Status**: 🟦 Production-Ready (sandbox connectivity run 2025-10-17: OAuth, system info and customer list passed; no live CRUD on record — see the [proof card](../review/proof-cards/netsuite-connector.md))
...
### 2. Salesforce
**Status**: 🟦 Production-Ready (no live-credential run on record)
...
curl -H "Authorization: Bearer $TOKEN" http://localhost:3000/api/fixtures/available-systems

# Get QuickBooks orders
curl -H "Authorization: Bearer $TOKEN" http://localhost:3000/api/fixtures/quickbooks/orders

# Test Business Central connection
curl -H "Authorization: Bearer $TOKEN" http://localhost:3000/api/fixtures/businesscentral/test-connection
```
...
- 🟦 **5 Production-Ready Connectors** - Real API clients, contract- and fixture-tested; 0 live-verified (NetSuite, Salesforce, Business Central, HubSpot, ShipStation)
...
- ✅ **Current Quality Baseline** - 16,571 / 16,597 tests passed, 26 intentionally skipped, 825 passing suites plus one skipped
- ✅ **Business Central Metadata Client** - 16,571/16,597 tests passing, 24-hour schema caching
...
**Status Legend**:
- ✅ Production Ready — readiness-complete **and** live-credential evidence on record (`production`)
- 🟦 Production-Ready, no live evidence — readiness-complete (governance, rate limiting, DLP, unit and contract suites, docs) with **no live-credential run on record** (`production_ready`); carries every production obligation

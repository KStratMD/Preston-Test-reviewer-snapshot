...
>
> PR-F0: each manifest prefix now also carries a typed `RoutePolicy`
> (auth/lifecycle/rateProfile) in `src/middleware/setup/routePolicy.ts`,
> audited by `npm run audit-route-policy`.
...
## PR 4B + PR 2C-Auth + F6 Stage 5 contract

- **Strict production mode (today).** The production central gate mounts `tenantIsolation` with `strictMode: true, disableHeaderExtraction: true`. A `tenant_required` route without verified tenant context returns `403` with code `TENANT_REQUIRED` before its handler runs. The middleware option default remains permissive only as an explicit compatibility seam for isolated harnesses; production must pass strict mode deliberately.
...
- **Handler identity fallbacks.** `extractIdentityContext` still supports `SYSTEM_IDENTITY` for non-request and explicitly permissive harness contexts, but production `tenant_required` requests can no longer reach that fallback anonymously. Existing handler-local 401 checks remain defense in depth and preserve their behavior on deferred or locally mounted paths that do not use the production central gate.
...
|---|---|---|---|
| `/api/ai/proxy` (+ Agent/BI/MCP/Mapping/Quality sub-routers) | verified `req.user` via `verifiedUserId`/`verifiedIdentity` (F6 sub-project B; no SYSTEM_IDENTITY fallback — anonymous demo callers are diverted by the F2 gate before identity is read) | partial (MCP yes) | ✅ |
...
| `/api/metrics` | Prometheus scrape; gated by `METRICS_SCRAPE_TOKEN` since PR #952 (timing-safe Bearer; unset → 403 unless NODE_ENV is explicitly development/test or HOSTED_DEMO — tenant IDs + per-tenant AI cost appear as metric labels). `/api/metrics/review` (reviewer-evidence JSON) stays open |
...
| `/api/admin/tenants/:tenantId/suitecentral` | PR-A6: platform-admin SuiteCentral control plane for a named tenant; authMiddleware + requirePlatformAdmin. Tenant comes from the path, deliberately NOT `tenant_required` (the central gate must not re-scope it to the admin's own tenant) |
| `/api/admin/suitecentral/allowed-hosts` | PR-A6: platform-global SuiteCentral egress allowlist; authMiddleware + requirePlatformAdmin. Not tenant-scoped — no tenant is accepted or read |
| `/metrics` | gated by ENABLE_METRICS + authMiddleware |
| `/api/disaster-recovery` | ops-only |
| `/api/disaster-recovery/dashboard` | ops-only |
| `/api/statistics` | single-endpoint diagnostic mounted in `src/index.ts` behind route-level REQUIRED `authMiddleware` (PR #952 — it returns cross-tenant aggregate config metadata); reads global configService state, not tenant-scoped |
| `/api/full-pipeline-demo` | platform-admin ERP write family; `authMiddleware` + `requirePlatformAdmin` + shared erp-write limiter via `mountFullPipelineDemoRoutes`. `POST /execute` additionally requires `tenantId` + `user.id` claims |
...
| `/api/suitecentral/prod` | PR-A6: tenant-admin SuiteCentral control plane (replaced the deleted legacy router); authMiddleware + requireSuiteCentralTenantAdmin; target tenant from the verified JWT claim only |

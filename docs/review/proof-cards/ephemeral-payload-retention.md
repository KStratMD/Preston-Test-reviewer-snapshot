...
**Verification recipe:** tests/unit/services/workflowCentral/WorkflowPayloadRetentionJob.test.ts
...
- **No tenant-level rate limit.** Ephemeral rows are admitted only via the `WORKFLOW_ALLOW_EPHEMERAL_PAYLOAD` env flag OR a strict per-tenant `workflow.allow_ephemeral_payload` opt-in — both gating paths are fully implemented (`docs/guides/SECURITY-AND-RATE-LIMITING.md`), so the population stays small by design rather than by an incomplete rollout. If volume becomes a concern, batched/paged SELECTs are a future enhancement.

...
- Implementation: `src/connectors/NetSuiteConnector.ts:43-578`
- Entry point: `src/connectors/NetSuiteConnector.ts:126-160` (`authenticate()`)
- Dependencies:
  ...
  - Base URL is derived from `accountId` when no explicit `base_url`/`baseUrl` is supplied: `https://${dnsNormalizedAccountId}.suitetalk.api.netsuite.com` (lines 115-121). The derived HOSTNAME lowercases the account id and swaps underscores for hyphens per NetSuite's sandbox DNS convention (`1234567_SB1` → `1234567-sb1`); `this.accountId` itself keeps its original form because it doubles as the OAuth1 realm, which NetSuite requires unmodified.

## Tests

- Unit: `tests/unit/__tests__/NetSuiteConnector.test.ts` (14 tests, 27 expects)
- Contract: `tests/unit/contract/NetSuiteConnector.contract.test.ts` (37 tests, 54 expects)
...
- Real HTTP wired? **Yes** · `NetSuiteConnector.ts:122` (`this.httpClient.defaults.baseURL = this.baseUrl`) and `getAuthHeaders()` at line 507 producing OAuth1 Authorization headers signed via `oauth1Helper.getOAuth1AuthorizationHeader()`.
...
- Production credential test on file? **Yes** — sandbox `TSTDRV2698307` per `statusEvidence` field at line 45.

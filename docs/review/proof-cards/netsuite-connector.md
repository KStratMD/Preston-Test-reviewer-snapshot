...
- Entry point: `authenticate()` in `src/connectors/NetSuiteConnector.ts`
- Dependencies:
...
- Production credential test on file? **No.** A **sandbox** credential test is on file (`TSTDRV2698307`, per the `statusEvidence` static field on `NetSuiteConnector`); no production-credential run is recorded. This line previously answered "Yes" for that sandbox test, contradicting the Known Gaps entry below.

...
**Verification recipe:** tests/unit/middleware/syncErrorAssistWebhook.test.ts
...
Rate-limit policy update verified 2026-09-13 against implementation `65ab4cea3b` and integration follow-up `aaee866635`; the broader May verification stamp above is retained. `tests/integration/rateLimitCompatibility.integration.test.ts` proves pre-auth /56 grouping.
...
- The former R19-2 IPv6 deferral is addressed in the [8.7.0 migration](../2026-09-13-maintenance-rate-limit.md): the default-key pre-auth limiter groups IPv6 by /56, verified through the real middleware mount. This differs from the historical /64 proposal; hosted/custom keys retain /64. These are synthetic request tests, not live proxy or vendor acceptance.

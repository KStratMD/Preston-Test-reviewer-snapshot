...
**Verification recipe:** tests/unit/services/security/DLPService.test.ts
...
- **Name detection lost free-text capability** when unified from GovernanceService's former detector. `DLPService.scanText('Mr. John Smith')` no longer flags the name because there's no field context. Per `docs/guides/SECURITY-AND-RATE-LIMITING.md`, this is intentional — field-gating eliminates the free-text false-positive class — but it's a real behavioral diff vs the pre-Commit-2 detector.
- ASCII-only for parity with the old detector; Unicode names (`José`, `李明`) are a future enhancement.
- The 14 patterns cover the categories derived from `DLPService.buildPatternRegistry()` (government_id, financial, contact, health, credential, network) but do not cover, e.g., biometric identifiers, IBAN-shape (vs `bank_account`), or healthcare-specific identifiers beyond medical_record_number. New patterns must be justified against the Codex false-positive risk and gated if structurally ambiguous.

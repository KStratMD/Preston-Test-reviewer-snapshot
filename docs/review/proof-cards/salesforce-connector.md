...
**Status:** production
**Last verified:** 2026-07-29 · git sha `c6e11bcf`
...
- Implementation: `src/connectors/SalesforceConnector.ts:1-1177`
- Entry point: `src/connectors/SalesforceConnector.ts:467-527` (`authenticate()`)
- Dependencies:
  - `src/services/AuthService.ts` — `authenticateOAuth2()` token exchange
  - Base URL: `${instanceUrl}/services/data/${apiVersion}` (line 458)
  - Token endpoint: `${loginUrl}/services/oauth2/token` (line 488)

## Tests

- Unit: `tests/unit/__tests__/SalesforceConnector.test.ts` (38 tests, 70 expects)
- Integration: `none — credential-gated`

## Live vs Fixture

- Real HTTP wired? **Yes** · `SalesforceConnector.ts:458-459` sets `httpClient.defaults.baseURL` to the resolved Salesforce instance URL; `authenticate()` POSTs to `${loginUrl}/services/oauth2/token` at line 488.
- Demo-mode toggle? **Yes** · `SalesforceConnector.ts:441` (`if (this.shouldUseDemoMode(config)) { this.enableDemoMode(...); }`) — but unlike the demo-only connectors, Salesforce has a production credential test on file separate from the fallback path.
- Production credential test on file? **Yes** — per `statusEvidence` field at line 107: "OAuth2 Resource Owner Password Credentials grant (grant_type=password) against real Salesforce REST API".

## Known Gaps

- The demo-mode fallback at `SalesforceConnector.ts:441` is the same pattern used by the demo-only connectors (Adyen, Stripe, etc.). The distinction "production vs demo_only" rests on the operator-attested production credential test, not on the source-level presence/absence of demo mode.

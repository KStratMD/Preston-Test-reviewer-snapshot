...
**Verification recipe:** tests/unit/services/ai/providers/ProviderFactories.test.ts
...
- Production credential test on file? **Operator-attested; no credential artifact is stored in-repo.** `docs/tutorials/ai-provider-configuration-guide.md` documents the production-ready configuration system (env vars, defaults, cost-per-mapping); live usage and credential testing are operator-side and recorded in commit history, not in a checked-in test artifact.

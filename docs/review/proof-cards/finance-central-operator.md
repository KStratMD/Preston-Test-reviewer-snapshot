...
**Verification recipe:** tests/unit/services/financeCentral/FinanceCentralRepository.test.ts
...
- **Identity propagation wired; auth middleware not yet mounted on FC routes**: `extractIdentityContext(req)` is now called on all FC routes (`/dashboard`, `/approvals`, `/approvals/:id/approve`, `/approvals/:id/reject`) and the approve/reject handlers honor an authenticated identity when present (the authenticated `ctxUserId` overrides any body-supplied actor — PR 6 R4/R5 hardening). However, no `authMiddleware`/`optionalAuthMiddleware` is mounted on `/api/finance-central/*`, so unauthenticated callers still resolve to `SYSTEM_IDENTITY` and seeded demo data lives under `tenant_id = '__system__'`. Mounting verified auth on FC routes (so `extractIdentityContext` returns a real tenant) is the remaining step before promoting to `production`.

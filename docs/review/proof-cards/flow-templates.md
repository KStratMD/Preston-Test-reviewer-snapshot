...
- Unit: `tests/unit/flows/templates/FlowExecutor.test.ts` (27 tests, 90+ expects across 7 families: transform/validate; governance decisions (including update-without-id + update-with-whitespace-id pre-governance fail-fasts); dispatch + id resolution (incl. `readId` whitespace-trim); connector contract (systemType match assertion); delete pre-resolve (Codex 5.5 MEDIUM: resolver-throw fail-fast + empty-string rejection + enqueue-uses-pre-resolved-id); governance context plumbing; redactedPayload precedence)
...
- **Lineage hops are not recorded.** `FlowContext` does not yet carry a `LineageRecorder`. Ships when PR 12 lands. Today's audit trail is the OutboundGovernanceService's `auditMetadata` field on the `OutboundDecision`, which is logged but not yet correlated into a multi-hop lineage chain.
- **Ownership checks are not enforced.** `FlowContext` does not yet carry an `OwnershipResolver`. Ships when PR 13 lands. Templates today can target any entity in any system — there's no `SOURCE_OF_TRUTH_MANIFEST` to validate against.
...
```bash
# Unit suite — 27 tests, ~60s on a warm Jest cache:

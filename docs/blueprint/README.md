...
The terminology gate scans Markdown under the singular `docs/blueprint/` directory, the named
tranche specifications and plans, and documents that opt in. JSON Blueprint documents remain
covered by schema and verifier validation. Do not put credentials or customer payloads in examples.
...
The workflows were promoted and the required context was activated after the live event matrix
passed. The [activation record](../review/2026-09-09-blueprint-required-activation.md) records
the protected branches and contexts. A required green `blueprint-verify` result means the draft
is structurally valid; it still does not grant execution approval. Consumers must call
`assertFresh` and act only on the fresh returned result.

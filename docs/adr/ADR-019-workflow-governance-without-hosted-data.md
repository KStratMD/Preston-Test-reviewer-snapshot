...
- NetSuite per-tenant governance API quotas could be exhausted by render bursts. Mitigated by cache + the existing Governance Pacer (`docs/01_VISION_DOCUMENT.md` "Production-Ready Features"). Open Question Q5 covers per-tenant pacing.
...
- **`docs/01_VISION_DOCUMENT.md#principle-1-erp-native-not-erp-adjacent`** — Design Principle #1 ("ERP-Native, embedded sidecar not separate app") aligns with the reference-based model: business data lives in the ERP, not in Squire.

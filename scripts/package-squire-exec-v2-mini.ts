...
const MINI_EXTRA_MEDIA_FILES = [
  ...
  'media/demos/wedge-enhance-path-video.mp4',
  'media/demos/wedge-enhance-path-video-poster.png',
  'media/demos/suitecentral-enhance-deployment-guide.mp4',
  'media/demos/suitecentral-enhance-deployment-guide-poster.png',
];
...
function getMiniExecutiveFiles(): string[] {
  ...
  return [
    ...
    // The formatted Install-and-Use Guide PDF is linked from index.html, so it
    // must be bundled or the mini-pack HTML-link validator fails (and offline
    // users would hit a dead link). Markdown links (e.g. README's .docx refs)
    // are not HTML-validated, which is why only this one needs explicit inclusion.
    'Squire-Executive-Package-v2/36-SQUIRE-INSTALL-AND-USE-GUIDE.pdf',
    ...appendixFiles.sort(),
  ];
}

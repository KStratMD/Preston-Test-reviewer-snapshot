...
// Use spawnSync with an argv array (NOT a shell string) so paths containing
...
const result = spawnSync(
  'npx',
  ['tsc', '-p', previewConfig, '--pretty', 'false'],
...
if (result.error) {
  console.error(`FAIL: could not execute 'npx tsc' — ${result.error.message}`);
  process.exit(2);
}
if (result.status === null) {
  console.error(`FAIL: 'npx tsc' terminated by signal ${result.signal ?? 'unknown'} before producing output`);
  process.exit(2);
}
...
if (result.status !== 0 && current === 0) {
  console.error(`FAIL: 'npx tsc' exited ${result.status} with no 'error TS<N>:' lines — likely a non-diagnostic failure (config parse error, bad flag, missing include), not a real strict-null run.`);
  if (output.trim()) {

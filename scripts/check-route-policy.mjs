...
function loadTsModule(tsPath, cache = new Map()) {
  ...
  const sandbox = {
    ...
    require: (id) => {
      if (id.startsWith('.')) {
        const dependencyPath = path.resolve(path.dirname(resolvedPath), `${id}.ts`);
        if (fs.existsSync(dependencyPath)) return loadTsModule(dependencyPath, cache);
      }
      throw new Error(`unexpected require('${id}') while loading ${resolvedPath}`);
    },
  };
  try {
    vm.runInNewContext(outputText, sandbox, { filename: resolvedPath, timeout: 5000 });
  } catch (err) {
    console.error(`check-route-policy: failed to evaluate ${tsPath}: ${err.message}`);
    process.exit(2);
  }
  cache.set(resolvedPath, moduleShim.exports);
...
function main() {
  ...
  // --- MATCH_MODE: shared exact-vs-prefix contract ---
  const validMatchModes = new Set(['exact', 'prefix']);
  for (const entry of manifest) {
    const mode = entry.matchMode ?? 'prefix';
    if (!validMatchModes.has(mode)) {
      errors.push(`MATCH_MODE: ROUTE_MANIFEST entry '${entry.path}' has invalid matchMode '${mode}'`);
    }
  }
  for (const policy of policies) {
    const mode = policy.match.matchMode ?? 'prefix';
    if (!validMatchModes.has(mode)) {
      errors.push(`MATCH_MODE: policy '${policy.match.pathPrefix}' has invalid matchMode '${mode}'`);
    }
  }
  ...
  for (const entry of manifest) {
    ...
    const policyMode = base.match.matchMode ?? 'prefix';
    if (manifestMode !== policyMode) {
      errors.push(
        `MATCH_MODE: '${entry.path}' is '${manifestMode}' in ROUTE_MANIFEST but '${policyMode}' in its base policy`,
      );
    }
  }

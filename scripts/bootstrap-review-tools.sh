...
# Optional tool selection: `bootstrap-review-tools.sh cloc` installs and
# version-verifies only cloc. ci-minimal.yml needs cloc for the metrics
# freshness gate and would otherwise pay for two security scanners it never
# runs. No argument keeps the previous behaviour (all three), so
# reviewer-mirror.yml is unaffected.
TOOLS=("$@")
if [[ ${#TOOLS[@]} -eq 0 ]]; then
  TOOLS=(cloc gitleaks trufflehog)
fi

for tool in "${TOOLS[@]}"; do
  case "$tool" in
    cloc|gitleaks|trufflehog) ;;
    *)
      echo "Unknown review tool: $tool (expected one or more of: cloc gitleaks trufflehog)" >&2
      exit 1
      ;;
  esac
done

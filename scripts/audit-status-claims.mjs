...
const VALID_STATUSES = new Set(CONNECTOR_STATUSES);
...
function checkRegistry(root, items, checkProofCards) {
  ...
  for (let i = 0; i < registry.length; i += 1) {
    ...
    if (isProductionTier(entry.productionStatus)) {
      if (!entry.hasFactory) {
        errors.push(
          `${ctx}: productionStatus='${entry.productionStatus}' requires a 'factory' closure ` +
      ...
      if (!entry.proofCardPath) {
        errors.push(`${ctx}: productionStatus='${entry.productionStatus}' requires a proofCardPath`);
      }
    }
  }
...
function runTenantCoverageCheck(rootDir) {
  ...
  const EXEMPT_FROM_SOURCE_REQUIREMENT = new Set([
    ...
    '/api/configurations',
    '/api/enterprise',
    // B8: same shape again — `mockDashboardAPIs` is mounted
    // `this.app.use('/', mockRouter)` (only under isDemo()) and declares its own
    // absolute `/api/...` literals with `router.get(...)`, which the
    // `app.use('<path>')` scan cannot see. These eleven were undeclared
    // entirely until B8; exempting them from the source direction is what lets
    // them be DECLARED at all, which is strictly better than the prior state of
    // being neither declared nor scanned.
    '/api/cache/demo',
    '/api/health/demo',
    '/api/optimization/demo',
    '/api/dlq/messages',
    '/api/system/status',
    '/api/test-template-endpoint',
    '/api/mapping-studio/component-status',
    '/api/mapping-studio/mappings',
    '/api/mapping-studio/share-data',
    '/api/mapping-studio/shared-templates',
    '/api/mapping-studio/validate',
  ]);
...
function main() {
  ...
  if (!fs.existsSync(metricsPath)) {
    errors.push(`${rel(root, metricsPath)}: missing — run \`npm run metrics:generate:authoring\``);
  } else {
    const metrics = JSON.parse(fs.readFileSync(metricsPath, 'utf8'));
    /** @type {Record<string, number>} */
    // Derived from the shared vocabulary: a restated literal is exactly how a
    // relabel makes this counter go NaN for the new status.
    const counts = Object.fromEntries(CONNECTOR_STATUSES.map((s) => [s, 0]));
    for (const item of items) {
    ...
    for (const key of Object.keys(counts)) {
      ...
      if (claimed !== counts[key]) {
        errors.push(
          `metrics.json connectors.${key} = ${claimed} but source-level scan found ${counts[key]}; regenerate via \`npm run metrics:generate:authoring\``,
        );
      }
    }
    ...
    if (claimedUnknown !== 0) {
      errors.push(
        `metrics.json connectors.unknown = ${claimedUnknown} but Phase 3 requires 0; regenerate via \`npm run metrics:generate:authoring\` after tagging all connectors`,
      );
    }
  }
  ...
  const partition = [...CONNECTOR_STATUSES]
    .map((k) => `${k}=${summary[k] ?? 0}`)
    .join(' ');

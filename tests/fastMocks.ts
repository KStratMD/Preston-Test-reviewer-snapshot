...
// Disable env-gated rate limiting in the unit env, matching the integration
// profile's tests/integration/setupEnv.ts. Before the env-parsing fix an
// UNSET flag accidentally parsed to false, so this dormancy was implicit;
// now it is declared (the ERP-write limiter's skip is env-gated and its
// dormancy test depends on it; per-limiter budget behavior is tested with
// injected limiters, not the env flag).

...
// this suite seeds must live there too.
const TENANT_ID = CENTRAL_DEMO_TENANT_ID;
const TENANT_B = '__central_demo_b__'; // distinct tenant for cross-tenant tests
...
describe('workflow-central activity-log durability integration (PR-OP-3b)', () => {
  ...
  beforeAll(async () => {
    ...
    // F5b: /activity is a GET read — simulate the policy gate's reads-only
  ...
  describe('GET /api/workflow-central/activity', () => {
    it('returns 200 with tenant-scoped rows after a verb has run', async () => {
      ...
      const res = await request(app).get('/api/workflow-central/activity').set('x-test-demo', '1');
    ...
    it('honors an explicit limit in range', async () => {
      ...
      const res = await request(app).get('/api/workflow-central/activity?limit=3').set('x-test-demo', '1');
      expect(res.status).toBe(200);
    ...
    it.each(['abc', '0', '-1', '101', '1.5', ''])(
      ...
      async (bad) => {
        const res = await request(app).get(`/api/workflow-central/activity?limit=${encodeURIComponent(bad)}`).set('x-test-demo', '1');
    ...
    it('returns 400 invalid_limit when limit is array-shaped', async () => {
      const res = await request(app).get('/api/workflow-central/activity?limit=1&limit=2').set('x-test-demo', '1');
      expect(res.status).toBe(400);
      expect(res.body.code).toBe('invalid_limit');
    });
    ...
    it('narrows by instanceId when supplied', async () => {
      ...
      const res = await request(app).get(`/api/workflow-central/activity?instanceId=${instA.id}`).set('x-test-demo', '1');
      expect(res.status).toBe(200);
      expect(res.body.length).toBe(1);
    ...
    it('returns 400 invalid_instance_id when instanceId is array-shaped', async () => {
      const res = await request(app).get('/api/workflow-central/activity?instanceId=a&instanceId=b').set('x-test-demo', '1');
      expect(res.status).toBe(400);
    ...
    it('returns 200 with no narrowing when instanceId is empty string (treated as no filter)', async () => {
      ...
      const service = await getService();
      await service.pauseInstance(TENANT_ID, inst.id);
      const res = await request(app).get('/api/workflow-central/activity?instanceId=').set('x-test-demo', '1');
      expect(res.status).toBe(200);
      expect(res.body.length).toBe(1);
    });
  });

...
function createApp(
  ...
  app.use('/api/help', createHelpRouter(
    ...
    { findSimilarChunksLocalOnly: jest.fn().mockReturnValue([]) } as never,
  ));
  return app;
}
...
describe('identity propagation through help chat route', () => {
  it('uses local-only retrieval and does not call processMessage when no authenticated identity exists', async () => {
...
describe('help chat context — backward compatibility', () => {
  it('anonymous request with only `message` still succeeds through local retrieval', async () => {
    const processMessage = jest.fn().mockResolvedValue(mockResponse());
...
describe('help chat context — anonymous public requests', () => {
  it('anonymous request with public context succeeds without calling processMessage', async () => {
    const processMessage = jest.fn().mockResolvedValue(mockResponse());
...
describe('help chat context — JWT with req.user but no valid tenantId is treated as anonymous', () => {
  it('internal-audience chat with a no-tenantId JWT fails closed without calling processMessage', async () => {
  ...
  it('public-audience chat with a no-tenantId JWT fails closed', async () => {
    const processMessage = jest.fn().mockResolvedValue(mockResponse());
    const app = createApp(processMessage, /* withGlobalAuth= */ true);

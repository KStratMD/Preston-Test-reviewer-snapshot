...
function makeTemplate(): FlowTemplate<ContactEvent, ContactRecord> {
  return {
    ...
    version: '1.0.0',
    source: { system: 'src', eventType: 'contact.created' },
    target: { system: 'tgt', recordType: 'Contact', operation: 'create' },
 ...
 */
function stubConnector(systemType: string = 'tgt'): jest.Mocked<IConnector> {
  const conn = {
    systemType,
    initialize: jest.fn(),

...
import { UserAssertionNonceRepository } from '../../src/services/embedded/UserAssertionNonceRepository';
...
describe('EmbeddedSessionLifecycle (PR 10a)', () => {
  ...
  let nonceRepo: UserAssertionNonceRepository;
  ...
  beforeAll(async () => {
    ...
    nonceRepo = new UserAssertionNonceRepository(dbService, logger);
    retention = new EmbeddedRetentionJob(sessionRepo, tokenRepo, nonceRepo, logger);
  });
  ...
  it('purges spent assertion nonces past the grace window and keeps recent ones', async () => {
    // Nothing else removes these rows: the ledger is a separate table from
    // embedded_sessions precisely so it survives the pagehide teardown that
    // deletes sessions, so without this query it grows once per bootstrap and
    // never shrinks.
    const now = new Date();
    const old = new Date(now.getTime() - 16 * 60 * 1000);
    const recent = new Date(now.getTime() - 5 * 60 * 1000);

    await nonceRepo.consume('nonce_stale_16m', 'erg_x', old);
    await nonceRepo.consume('nonce_fresh_5m', 'erg_x', recent);

    const result = await retention.tick(now);
    expect(result.assertionNoncesPurged).toBe(1);

    // The recent nonce must still block a replay; the purged one is beyond the
    // +/-300s skew window, so its header would be rejected as stale before the
    // ledger was ever consulted.
    expect(await nonceRepo.consume('nonce_fresh_5m', 'erg_x', now)).toBe(false);
    expect(await nonceRepo.consume('nonce_stale_16m', 'erg_x', now)).toBe(true);
  });

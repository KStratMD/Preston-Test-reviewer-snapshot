...
export async function createTestApp(initialConfigs: IntegrationConfig[] = []) {
  ...
  // Mirrors the real route's REQUIRED auth (src/index.ts mounts
  // /api/statistics behind authMiddleware) so integration tests exercise the
  // same auth posture as production instead of an open stand-in.
  expressApp.get('/api/statistics', authMiddleware, (_req, res) => {

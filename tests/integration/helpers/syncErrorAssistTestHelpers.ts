...
/**
 * Privileged roles a caller cannot simply assert. Naming one in `userRoles`
 * now means "this user HOLDS that grant", and the seeder issues a real
 * `embedded_role_grants` row plus a `squire_verified` session to back it.
 *
 * Before migration 065 a test could grant itself approval rights by writing
 * the role into `user_roles` — which is exactly what production could do too,
 * and exactly the escalation that was fixed. Keeping the seeder's surface as
 * `userRoles` lets each suite go on stating intent, while the implementation
 * proves that intent the way production now requires.
 */
const GRANTABLE_ROLES = ['viewer', 'requester', 'approver', 'admin'] as const;
type GrantableRole = (typeof GRANTABLE_ROLES)[number];

/** Admin subsumes approver, so one verified grant covers both gates. */
function pickVerifiableRole(roles: readonly string[]): GrantableRole | null {
  if (roles.includes('admin')) return 'admin';
  if (roles.includes('approver')) return 'approver';
  const first = roles.find((r): r is GrantableRole => (GRANTABLE_ROLES as readonly string[]).includes(r));
  return first ?? null;
}
...
export async function seedEmbeddedSession(args: {
  ...
  /** Skip the grant so the session stays host-asserted — for denial cases. */
  hostAssertedOnly?: boolean;
}): Promise<string> {
  const sessionId = `es_test_${randomUUID()}`;
  const now = new Date();
  const expiresAt = new Date(now.getTime() + SESSION_MAX_LIFETIME_MS);
  const roles = args.userRoles ?? ['ops'];

  const verifiableRole = args.hostAssertedOnly === true ? null : pickVerifiableRole(roles);
  let verifiedGrantId: string | null = null;

  if (verifiableRole !== null) {
    // At most one OPEN grant exists per (tenant, platform, account, user, role)
    // — the partial unique index enforces it. Suites reseed the same operator
    // across cases while beforeEach clears only sessions, so reuse the standing
    // grant instead of inserting a duplicate. Re-granting in production means
    // superseding the old row, which is not what a test reseed intends.
    const existing = await getDb()
      ...
      .where('role', '=', verifiableRole)
      .where('revoked_at', 'is', null)
      // Open is not the same as active. An expired-but-unrevoked grant still
      // occupies the unique-index slot, so reusing one would mark the session
      // squire_verified against a grant that never activates — a session that
      // looks authorized and is not, which is the most confusing possible test
      // failure. Only a grant with no expiry is reusable here; the seeder never
      // issues an expiring one, so anything else was set deliberately by a test
      // and must not be silently adopted.
      .where('expires_at', 'is', null)
      .executeTakeFirst();

    if (existing !== undefined) {
      verifiedGrantId = existing.id;
    } else {
      verifiedGrantId = `erg_test_${randomUUID()}`;
      await getDb()
        ...
        .values({
          ...
          platform: 'standalone',
          // Sessions here carry a NULL platform account; the grant column is NOT
          // NULL, and resolvePrincipal normalises both sides to '' when comparing.
          platform_account_id: '',
          user_id: args.userId,
          role: verifiableRole,
          ...
          revoked_at: null,
          revoked_by: null,
          // The assertion handshake (which consumes the secret) is covered by its
          // own suite; nothing on these paths decrypts this value.
  ...
  await getDb()
    ...
    .values({
      ...
      user_identity: verifiedGrantId === null ? 'host_asserted' : 'squire_verified',
      verified_grant_id: verifiedGrantId,
    })
    .execute();
...
/**
 * Codex R3 P1 (PR #966) — buildNetSuiteEnvAuthConfigForTenant fails closed
 * for tenants with NO embedded service-token binding row, so any integration
 * test whose flow reaches connector initialize must seed one. The account id
 * mirrors the runtime env (`setupEnv.ts` defaults NETSUITE_ACCOUNT_ID to
 * 'test-account') so the guard's match branch passes.
 */
export async function seedEmbeddedAccountBinding(tenantId: string): Promise<void> {
  await getDb()
    .insertInto('embedded_service_token_versions')
    .values({
      token_hash: `test-binding-${randomUUID()}`,
      tenant_id: tenantId,
      platform: 'netsuite',
      platform_account_id: process.env.NETSUITE_ACCOUNT_ID ?? 'test-account',
      valid_from: new Date().toISOString(),
      valid_until: '9999-01-01T00:00:00.000Z',
      retired_at: null,
    })
    .execute();
}
...
export async function clearSyncErrorAssistTestState(): Promise<void> {
  ...
  await sql`DELETE FROM embedded_service_token_versions WHERE token_hash LIKE 'test-binding-%'`.execute(db);
...
export function makeIntegrationNsConnectorStub() {
  return {
    // Fix B / Defect 2 — every call site now calls `connector.initialize(...)`
    // immediately after `ConnectorManager.getConnector()` resolves.
 ...
 */
export async function buildIntegrationApp(opts: {
  ipLimitOverride?: number;
  /** Mount the Stage 5-compatible central gate before the raw webhook parser. */
  strictCentralGate?: boolean;
} = {}): Promise<IntegrationAppKit> {
  ...
  //
  // Match the production pre-auth limiter's default IP key (/56 IPv6 in v8).
  // Tests run with `trust proxy` disabled (no X-Forwarded-For), so `req.ip`
  // is always the loopback address
  ...
  // Match production order: MiddlewareSetup mounts the IP limiter/raw parser
  // before RouteSetup mounts the central gate. The gate still runs before the
  // route handler and defers the HMAC-owned ingest path.
  if (opts.strictCentralGate) {

  ...
  describe(`SerializedAssetSyncService — durability, isolation, idempotency, restart recovery (Task 12) [${dialect}]`, () => {
    ...
    describe('tenant isolation — deferred-work store', () => {
      ...
      it('touchAttempt UPDATE-branch predicate is tenant-scoped: touching tenant B\'s row updates ONLY tenant B\'s row, even though tenant A has a row at the identical (configId, invId)', async () => {
        ...
        const result = await deferredRepo.touchAttempt(tenantB, configId, invId, 1, now);
        ...
        expect(await deferredRepo.touchAttempt(tenantB, configId, 'inv-does-not-exist', 1, now)).toBeNull();
      });

      it('touchAttempt compare-and-swap: a row REPLACED since the snapshot is left byte-for-byte unchanged, on this engine', async () => {
        // The engine, not a mock, decides whether `attempt_count = expected` in
        // the UPDATE predicate matches — so the stale-replacement invariant is
        // proven here per engine rather than inferred from the unit suite.
        const { deferredRepo } = repos();
        const tenantId = `tenant-${randomUUID()}`;
        const siblingTenant = `tenant-sib-${randomUUID()}`;
        const configId = `cfg-${randomUUID()}`;
        const siblingConfig = `cfg-sib-${randomUUID()}`;
        const invId = `inv-${randomUUID()}`;
        const t0 = new Date('2026-01-01T00:00:00.000Z');
        const replacedAt = new Date('2026-01-01T00:10:00.000Z');
        const staleTouchAt = new Date('2026-01-01T00:04:00.000Z');

        // Siblings at the same (configId, invId) / (tenantId, invId) that must
        // not move: a predicate that lost a key column would disturb one.
        await deferredRepo.upsertDeferred(deferredInput(siblingTenant, configId, invId), t0);
        await deferredRepo.upsertDeferred(deferredInput(tenantId, siblingConfig, invId), t0);

        // Run A reads the row at attempt_count = 1.
        await deferredRepo.upsertDeferred(deferredInput(tenantId, configId, invId), t0);
        const snapshot = (await deferredRepo.listForRetry(tenantId, configId, 10)).units[0];
        expect(snapshot.attemptCount).toBe(1);

        // A matching guard advances exactly once and moves the schedule out.
        const advanced = await deferredRepo.touchAttempt(tenantId, configId, invId, 1, t0);
        expect(advanced).toBe(2);
        const afterAdvance = (await deferredRepo.listForRetry(tenantId, configId, 10)).units[0];
        expect(afterAdvance.attemptCount).toBe(2);
        expect(new Date(afterAdvance.nextAttemptAt).getTime()).toBeGreaterThan(
          new Date(snapshot.nextAttemptAt).getTime(),
        );

        // Run B replaces the row: new payload, new reason, later schedule.
        await deferredRepo.upsertDeferred(
          deferredInput(tenantId, configId, invId, {
            reason: 'transient_dependency_failure',
            normalizedPayload: unit(tenantId, configId, invId, 'SN-REPLACED', 'item-replaced'),
          }),
          replacedAt,
        );
        const replacement = (await deferredRepo.listForRetry(tenantId, configId, 10)).units[0];
        expect(replacement.attemptCount).toBe(3);

        // Run A's stale touch, still carrying the count it originally read.
        expect(
          await deferredRepo.touchAttempt(tenantId, configId, invId, snapshot.attemptCount, staleTouchAt),
        ).toBeNull();

        const afterStale = (await deferredRepo.listForRetry(tenantId, configId, 10)).units[0];
        expect(afterStale).toEqual(replacement);

        // A deleted row is the other zero-match case and is equally benign.
        expect(await deferredRepo.deleteSucceeded(tenantId, configId, invId)).toBe(true);
        expect(
          await deferredRepo.touchAttempt(tenantId, configId, invId, replacement.attemptCount, staleTouchAt),
        ).toBeNull();
        expect(await deferredRepo.countForConfiguration(tenantId, configId)).toBe(0);

        // Neither sibling moved through any of the above.
        const [sibTenantRow] = (await deferredRepo.listForRetry(siblingTenant, configId, 10)).units;
        const [sibConfigRow] = (await deferredRepo.listForRetry(tenantId, siblingConfig, 10)).units;
        expect(sibTenantRow.attemptCount).toBe(1);
        expect(sibConfigRow.attemptCount).toBe(1);
      });

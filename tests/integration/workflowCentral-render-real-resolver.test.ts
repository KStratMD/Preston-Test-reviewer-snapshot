...
// F5b-3: attested anonymous demo reads resolve here, so seeded fixtures live here.
const DEMO_TENANT = CENTRAL_DEMO_TENANT_ID;
const OTHER_TENANT = 'tenant-other';
// A plain authenticated requester for cases that need TWO ordinary tenants.
// Deliberately NOT the demo tenant: no JWT is ever minted with the demo
// tenantId (demoTenant.ts), so tests must not inject it as a claim either.
const REQUESTER_TENANT = 'tenant-requester';
...
describe('workflow-central render route — real-resolver integration (T17-follow-up)', () => {
  ...
  beforeAll(async () => {
    ...
    // F5b: renders are GET reads — simulate the policy gate's reads-only
    // anonymous demo attestation (x-test-demo: 1 → system tenant, deliberate).
  ...
  async function seedTask(args: {
    ...
    const tenantId = args.tenantId ?? DEMO_TENANT;
  ...
  describe('Group A — DLP end-to-end', () => {
    it('A1: PII detected → fields redacted in response; raw PII never appears', async () => {
      ...
      const res = await request(app).get('/api/workflow-central/tasks/TASK-dlp-a1/render').set('x-test-demo', '1');
    ...
    it('A2: DLP scanFailed → outcome status=failed with PAYLOAD_REF_DLP_SCAN_FAILED', async () => {
      ...
      const res = await request(app).get('/api/workflow-central/tasks/TASK-dlp-a2/render').set('x-test-demo', '1');
    ...
    it('A3: DLP findings present but redactedData undefined → same failed outcome', async () => {
      ...
      const res = await request(app).get('/api/workflow-central/tasks/TASK-dlp-a3/render').set('x-test-demo', '1');
  ...
  describe('Group B — Cross-system compose', () => {
    it('B1: two refs (netsuite + salesforce), both succeed → resolution[] independent', async () => {
      ...
      const res = await request(app).get('/api/workflow-central/tasks/TASK-cross-b1/render').set('x-test-demo', '1');
    ...
    it('B2: two refs, one connector throws {statusCode: 503} → partial success preserved', async () => {
      ...
      const res = await request(app).get('/api/workflow-central/tasks/TASK-cross-b2/render').set('x-test-demo', '1');
    ...
    it('B3: three refs across three systems → getConnector called once per ref with correct tuples', async () => {
      ...
      const res = await request(app).get('/api/workflow-central/tasks/TASK-cross-b3/render').set('x-test-demo', '1');

      expect(res.status).toBe(200);
      expect(res.body.resolution).toHaveLength(3);

      // Resolver builds systemId as `${ref.system}_${tenantId}` (per
      // WorkflowPayloadResolver.ts:66). DEMO_TENANT = the demo-fixture tenant.
      const expectedTuples: Array<[string, string]> = [
        ['netsuite', `netsuite_${DEMO_TENANT}`],
        ['businesscentral', `businesscentral_${DEMO_TENANT}`],
        ['salesforce', `salesforce_${DEMO_TENANT}`],
      ];
  ...
  describe('Group C — Cache behavior', () => {
    it('C1: same task rendered 100 times → connector.read invoked exactly 1 time', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        const res = await request(app).get('/api/workflow-central/tasks/TASK-cache-c1/render').set('x-test-demo', '1');
        expect(res.status).toBe(200);
        expect(res.body.resolution[0].status).toBe('resolved');
    ...
    it('C2: cache miss on TTL expiry — second render after Date.now() past 30s → second read', async () => {
      ...
      const r1 = await request(app).get('/api/workflow-central/tasks/TASK-cache-c2/render').set('x-test-demo', '1');
      ...
      const r2 = await request(app).get('/api/workflow-central/tasks/TASK-cache-c2/render').set('x-test-demo', '1');
    ...
    it('C3: per-tenant cache isolation — same ref shape, two tenants → 2 read calls', async () => {
      ...
      await seedTask({
        id: 'TASK-cache-iso-requester',
        tenantId: REQUESTER_TENANT,
      ...
      const r1 = await request(app)
        .get('/api/workflow-central/tasks/TASK-cache-iso-requester/render')
        .set('X-Test-Tenant-Id', REQUESTER_TENANT);
      expect(r1.status).toBe(200);
    ...
    it('C4: failed outcome NOT cached — render fails, connector recovers, second render triggers a fresh read', async () => {
      ...
      const r1 = await request(app).get('/api/workflow-central/tasks/TASK-cache-c4/render').set('x-test-demo', '1');
      expect(r1.status).toBe(200);
      expect(r1.body.resolution[0].status).toBe('failed');

      // Second call: connector recovers → fresh read because failure wasn't cached.
      const r2 = await request(app).get('/api/workflow-central/tasks/TASK-cache-c4/render').set('x-test-demo', '1');
      expect(r2.status).toBe(200);

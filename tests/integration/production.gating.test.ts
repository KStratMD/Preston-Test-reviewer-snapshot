...
describe('Production environment gating', () => {
  ...
  it('requires auth for dashboard when enabled', async () => {
    ...
    const flaggedAppInstance = new App();
    const flaggedApp = flaggedAppInstance.getExpressApp();
    const anonymous = await request(flaggedApp).get('/api/dashboard').expect(403);
    expect(anonymous.body).toMatchObject({ code: 'TENANT_REQUIRED' });
    const token = createTestToken({ id: '1', username: 'test', tenantId: 'production-gating-tenant' });

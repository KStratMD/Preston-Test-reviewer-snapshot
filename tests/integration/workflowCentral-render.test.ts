...
// A plain authenticated requester for cross-tenant cases. Deliberately NOT the
// demo tenant: no JWT is ever minted with the demo tenantId (demoTenant.ts).
...
describe('workflow-central render route integration (T17)', () => {
  ...
  beforeAll(async () => {
    ...
    // anonymous demo attestation; anonymous renders send x-test-demo: 1 to
    // reach the system tenant DELIBERATELY (the silent fallback is gone).
  ...
  describe('200 happy paths', () => {
    it('external_reference → kind=resolved, resolver invoked with tenantId', async () => {
      ...
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-ref-1/render')
        .set('x-test-demo', '1');
      ...
      expect(spy).toHaveBeenCalledWith(refsFixture, DEMO_TENANT);
    });
    ...
    it.each([
      ...
      async (code, statusCode, system) => {
        ...
        const res = await request(app)
          .get(`/api/workflow-central/tasks/${taskId}/render`)
          .set('x-test-demo', '1');
    ...
    it('external_reference partial-success → STILL 200; failed ref carried inside resolution[i]', async () => {
      ...
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-partial-1/render')
        .set('x-test-demo', '1');
    ...
    it('ephemeral_hosted + flag enabled + unexpired → kind=ephemeral, data flows through', async () => {
      ...
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-eph-1/render')
        .set('x-test-demo', '1');
    ...
    it('legacy (no payload, data populated) → kind=legacy, legacyResolution.fields = task.data', async () => {
      ...
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-legacy-1/render')
        .set('x-test-demo', '1');
  ...
  describe('404 not found', () => {
    it('returns 404 when task does not exist in tenant', async () => {
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-missing-9/render')
        .set('x-test-demo', '1');
  ...
  describe('403 ephemeral_payload_not_allowed (env flag unset)', () => {
    it('rejects unexpired ephemeral payload when flag is not set', async () => {
      ...
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-eph-gated-1/render')
        .set('x-test-demo', '1');
  ...
  describe('410 ephemeral_payload_expired', () => {
    it('rejects ephemeral payload whose expiresAt is in the past', async () => {
      ...
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-eph-expired-1/render')
        .set('x-test-demo', '1');
  ...
  describe('audit redaction invariant', () => {
    it('ephemeral render: audit row omits ephemeral.data values', async () => {
      ...
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-audit-eph-1/render')
        .set('x-test-demo', '1');
      expect(res.status).toBe(200);
    ...
    it('external_reference render: audit row contains refs but NOT resolved field values', async () => {
      ...
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-audit-ref-1/render')
        .set('x-test-demo', '1');
      expect(res.status).toBe(200);
    ...
    it('ephemeral expired: audit row records failure + omits data', async () => {
      ...
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-audit-eph-expired-1/render')
        .set('x-test-demo', '1');
      expect(res.status).toBe(410);
    ...
    it('not_found path: no audit row emitted (the operator path throws before audit)', async () => {
      const res = await request(app)
        .get('/api/workflow-central/tasks/TASK-render-audit-missing-1/render')
        .set('x-test-demo', '1');
      expect(res.status).toBe(404);
      const audits = await fetchRenderAudits();

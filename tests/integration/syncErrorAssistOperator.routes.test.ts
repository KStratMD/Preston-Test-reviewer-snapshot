...
describe('sync-error-assist operator routes (PR 17b)', () => {
  ...
  let mockConnector: { initialize: jest.Mock; create: jest.Mock; update: jest.Mock };
  ...
  beforeEach(async () => {
    ...
    // Codex R3 P1 — the accept path's connector-initialize guard fails closed
    // without a tenant->account binding row; seed one matching the env account.
    await seedEmbeddedAccountBinding(tenantId);
  });

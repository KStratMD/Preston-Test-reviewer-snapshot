...
describe('guardedWrite — end-to-end lifecycle (PR 13b Stage C)', () => {
  ...
  it('operator approves queued row, OwnershipResumeHandler.apply() dispatches the write', async () => {
      ...
      await guardedWrite(
        {
          context: {
            ...
            correlationId,
            // This row is APPROVED later in the test, so its requester must be one the
            // policy can verify. The system identity is the honest choice here: no
            // verified human is involved. Labelling 'sf-bot' as provenance 'system'
            // would forge a trust level that requester does not have (Codex round 2).
    ...
    const decided = await approvalQueue.approve({
      tenantId: TENANT,
      id: queueId,
      // A VERIFIED approver, not a bare user id. The deprecated approverUserId
      // path carries no proof of a grant and the separation-of-duties gate
      // refuses it — which is the point of the gate, so this test states the
      // authority it is exercising rather than routing around it.
      approver: { userId: 'operator-1', verifiedRoles: ['approver'], scope: null },
    ...
    // Select by action, not by position. The approval decision now writes its
    // own audit row against this same resource id, and it lands first — so
    // resumeAudits[0] is the decision, and asserting on it would test the
    // wrong event.
    const resumeRow = resumeAudits.find((r) => r.action === 'validate_ownership');
    expect(resumeRow).toBeDefined();
    expect(resumeRow!.result).toBe('success');

    // The decision itself is audited too, attributed to the verified approver.
    const decisionRow = resumeAudits.find((r) => r.action === 'governance.approval.decided');
    expect(decisionRow).toBeDefined();
    expect(decisionRow!.user_id).toBe('operator-1');

...
const hadOriginalJwtSecret = Object.prototype.hasOwnProperty.call(process.env, 'JWT_SECRET');
...
describe('/api/configurations mount — authMiddleware + tenant kill-switch gate wiring (Task 9)', () => {
  ...
  afterAll(() => {
    if (hadOriginalJwtSecret && typeof originalJwtSecret === 'string') {
...
describe('JWT_SECRET teardown', () => {
  it('restores the exact caller environment after the integration suite', () => {
    expect(Object.prototype.hasOwnProperty.call(process.env, 'JWT_SECRET')).toBe(hadOriginalJwtSecret);

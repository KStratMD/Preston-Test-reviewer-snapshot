...
describe('workflow-central startInstance integration (T12)', () => {
  ...
  beforeAll(async () => {
    ...
    // Header-driven injector lets each test pick its actor; requests without
    // x-test-actor stay anonymous (→ 401 identity_required on writes).
  ...
  it('401s the anonymous write; body startedBy is never honored (F5b)', async () => {
    // F5b invariant: resolveCentralActor requires a verified req.user for
    // writes — the old pre-auth SYSTEM_IDENTITY body-actor fallback is gone.
    // An anonymous request with a body actor must 401 with NO audit row and
    // NO instance created.
    const workflowId = createActiveDefinition({ firstStepType: 'task' });

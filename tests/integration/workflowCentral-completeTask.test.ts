...
describe('workflow-central completeTask integration (T12)', () => {
  ...
  beforeAll(async () => {
    ...
    // Header-driven injector lets each test pick its actor.
    app.use((req, _res, next) => {
      const actor = req.headers['x-test-actor'];
      if (typeof actor === 'string' && actor.length > 0) {
        (req as any).user = { id: actor, username: actor, tenantId: TENANT_ID, roles: [], permissions: [] };
      }
      next();
    });
  ...
  describe('body validation', () => {
    ...
    it('completedBy is no longer read from the body: authenticated request without it succeeds; actor is the verified id (F5b)', async () => {
      // Old test pinned `400 invalid_request_body when completedBy is missing`.
      // F5b deleted the body-actor path — the audit actor is always the verified
      // user id, so a body WITHOUT completedBy is now the normal, valid shape.
      const { taskId } = await seedTaskWithEngine();
      const res = await request(app)
        .post(`/api/workflow-central/tasks/${taskId}/complete`)
        .set('x-test-actor', 'op_42')
      ...
      // The persisted completed_by is the VERIFIED actor, not any body field.
      const db = (await container.getAsync<DatabaseService>(TYPES.DatabaseService)).getDatabase();
      const row = await db
        .selectFrom('workflow_central_tasks')
        .select(['completed_by'])
        .where('id', '=', taskId)
        .executeTakeFirst();
      expect(row?.completed_by).toBe('op_42');
    });
    ...
    it('400 invalid_request_body when data is an array', async () => {
      ...
      const res = await request(app)
        .post(`/api/workflow-central/tasks/${taskId}/complete`)
        .set('x-test-actor', 'op_42')
        .send({ actionId: 'complete', data: [1, 2, 3] });
      expect(res.status).toBe(400);
      expect(res.body.code).toBe('invalid_request_body');
    });
  });

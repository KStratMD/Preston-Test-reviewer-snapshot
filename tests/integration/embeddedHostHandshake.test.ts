...
describe('Embedded host handshake (PR 10a)', () => {
  ...
  it('userRoles flow host-bootstrap → guest context, carrying only host-assertable roles', async () => {
    // The contract used to round-trip whatever the host sent. It no longer
    // does: this column is read by the governance role gates, so a host
    // asserting 'approver' was authorizing itself (measured 2026-09-03).
    // 'viewer' and 'requester' identify a user and still round-trip;
    // 'approver' is a privilege claim and is refused; 'finance' is simply not
    // part of this contract and is dropped.
    const { rawToken } = await tokenRepo.provisionInitialToken({
    ...
    const bootstrap = await request(app)
      ...
      .send({
        ...
        userRoles: ['finance', 'approver', 'requester'],
      });
    expect(bootstrap.status).toBe(200);
    const guest = await request(app)
      .post('/api/embedded/context')
    ...
    expect(guest.body.userRoles).toEqual(['requester']);
  });

  it('strips host-asserted admin and approver and leaves the session unverified', async () => {
    const { rawToken } = await tokenRepo.provisionInitialToken({
      tenantId: 't_roles_strip',
      platform: 'netsuite',
      platformAccountId: 'acct_roles_strip',
    });
    const boot = await request(app)
      ...
      .send({
        ...
        expectedHostOrigin: NS_ORIGIN,
        userId: 'u1',
        userRoles: ['admin', 'approver', 'requester'],
      });
    expect(boot.status).toBe(200);

    const row = await sessionRepo.findSession(boot.body.sessionId);
    expect(JSON.parse(row!.user_roles!)).toEqual(['requester']);
    // No assertion was presented, so nothing is verified and no grant activates.
    expect(row!.user_identity).toBe('host_asserted');
    expect(row!.verified_grant_id).toBeNull();
  });

  it('rejects a present-but-unverifiable assertion with 401 rather than downgrading it', async () => {
    const { rawToken } = await tokenRepo.provisionInitialToken({
      tenantId: 't_roles_forged',
      platform: 'netsuite',
      platformAccountId: 'acct_roles_forged',
    });
    const res = await request(app)
      .post('/api/embedded/host-bootstrap')
      .set('Authorization', `Bearer ${rawToken}`)
      .set('X-Embedded-Platform', 'netsuite')
      .set('X-Squire-User-Assertion', `v1.erg_nope.${Math.floor(Date.now() / 1000)}.bm9uY2UxMjM0NTY3.deadbeef`)
      .send({ platformAccountId: 'acct_roles_forged', expectedHostOrigin: NS_ORIGIN, userId: 'mallory' });

    expect(res.status).toBe(401);
    expect(res.body.error).toBe('invalid_user_assertion');
  });

  it('a tenant-A service token cannot bootstrap a tenant-B account', async () => {
    const a = await tokenRepo.provisionInitialToken({
      tenantId: 't_a_iso', platform: 'netsuite', platformAccountId: 'acct_a_iso',
    });
    await tokenRepo.provisionInitialToken({
      tenantId: 't_b_iso', platform: 'netsuite', platformAccountId: 'acct_b_iso',
    });
    const res = await request(app)
      .post('/api/embedded/host-bootstrap')
      .set('Authorization', `Bearer ${a.rawToken}`)
      .set('X-Embedded-Platform', 'netsuite')
      .send({ platformAccountId: 'acct_b_iso', expectedHostOrigin: NS_ORIGIN });

...
describe('Connector Integration Tests', () => {
  ...
  describe('Dynamics 365 Connector', () => {
    ...
    it('should reject authentication before initialization without exchanging credentials', async () => {
      const invalidConnector = new DynamicsConnector('test-invalid', mockLogger, mockAuthService);
      jest.mocked(mockAuthService.authenticateOAuth2).mockClear();

      await expect(invalidConnector.authenticate()).rejects.toThrow('Dynamics 365 credentials are not configured');
      expect(mockAuthService.authenticateOAuth2).not.toHaveBeenCalled();
    });
  });

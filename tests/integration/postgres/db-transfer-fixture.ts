...
export async function createTransferFixture(): Promise<TransferFixture> {
  ...
  try {
    ...
    const insertPrecision = raw.prepare('INSERT INTO sync_error_assist_runs (tenant_id, last_modified_at, updated_at) VALUES (?, ?, ?)');
    for (const [tenantId, value] of [
      ['precision-positive', 9007199254740993n], ['precision-negative', -9007199254740993n],
      ['precision-min', -9223372036854775808n], ['precision-max', 9223372036854775807n],
      ['precision-safe', 9007199254740991n],
    ] as const) insertPrecision.run(tenantId, value, '2026-09-12 12:34:56.123456');
  } finally {
    await db.destroy();
    raw.close();
  }

...
// F5b-3: attested anonymous reads resolve to the demo tenant, so the fixtures
// those reads assert on live there.
const TENANT_ID = CENTRAL_DEMO_TENANT_ID;
// The authenticated-write identity. Deliberately an ORDINARY tenant: an
// operator JWT claiming the demo tenant is refused (403 reserved_tenant),
// because demo-tenant rows are what anonymous visitors render.
const WRITE_TENANT = 'tenant-wc-backfill-op';
...
async function insertTaskRow(params: {
  ...
  actions?: TaskAction[];
  /**
   * Defaults to the demo tenant (where attested anonymous reads resolve). The
   * one authenticated-WRITE case passes WRITE_TENANT instead — an operator JWT
   * cannot claim the demo tenant (403 reserved_tenant).
   */
  tenantId?: string;
}): Promise<string> {
  taskSeq++;
...
describe('workflow-central backfill + unknown_recovered lifecycle (T13.2, T13.3d)', () => {
  ...
  beforeAll(async () => {
    ...
    // F5b: GET reads simulate the policy gate's reads-only anonymous demo
    // attestation (x-test-demo: 1 → system tenant, deliberate).
    app.use(attestReadsOnly());
    // F5b: workflowCentral writes require a verified user (resolveCentralActor /
    // resolveCentralTenantId). Header-driven injector lets each test pick its
    // actor; requests without x-test-actor stay anonymous (→ 401 on writes).
    app.use((req, _res, next) => {
      const actor = req.headers['x-test-actor'];
      if (typeof actor === 'string' && actor.length > 0) {
        (req as any).user = { id: actor, username: actor, tenantId: WRITE_TENANT, roles: [], permissions: [] };
      }
      next();
    });
  ...
  it('T-13: backfilled unknown_recovered instance retains status after completing a task (no auto-transition, D5)', async () => {
    ...
    const taskId = await insertTaskRow({
      ...
      status: 'pending',
      // This case COMPLETES the task as an authenticated operator, so the
      // fixture must live on the operator's ordinary tenant.
    ...
    const res = await request(app)
      ...
      .set('x-test-actor', 'lifecycle-op').send({ actionId: 'complete' });
  ...
  it('T-13c: GET /instances?status=active returns running + unknown_recovered, excludes completed', async () => {
    ...
    const res = await request(app).get('/api/workflow-central/instances?status=active').set('x-test-demo', '1');
    expect(res.status).toBe(200);
  ...
  it('T-13c-guard: GET /instances?status=running&status=waiting → 400 invalid_query_param', async () => {
    const res = await request(app).get('/api/workflow-central/instances?status=running&status=waiting').set('x-test-demo', '1');
  ...
  it('T-13c-guard: array-valued workflowId → 400 invalid_query_param', async () => {
    const res = await request(app).get('/api/workflow-central/instances?workflowId=a&workflowId=b').set('x-test-demo', '1');
    expect(res.status).toBe(400);
    expect(res.body.code).toBe('invalid_query_param');
  ...
  it('T-13c-guard: array-valued startedBy → 400 invalid_query_param', async () => {
    const res = await request(app).get('/api/workflow-central/instances?startedBy=alice&startedBy=bob').set('x-test-demo', '1');
    expect(res.status).toBe(400);
    expect(res.body.code).toBe('invalid_query_param');
  ...
  it('T-13c-guard: non-numeric limit → 400 (NaN would otherwise corrupt SQL binding)', async () => {
    const res = await request(app).get('/api/workflow-central/instances?limit=not-a-number').set('x-test-demo', '1');
    expect(res.status).toBe(400);
    expect(res.body.code).toBe('invalid_query_param');
  ...
  it('T-13c-guard: negative offset → 400 (must be non-negative integer)', async () => {
    const res = await request(app).get('/api/workflow-central/instances?offset=-5').set('x-test-demo', '1');
    expect(res.status).toBe(400);
    expect(res.body.code).toBe('invalid_query_param');
  ...
  it('T-13c-guard: array-valued limit → 400 invalid_query_param', async () => {
    const res = await request(app).get('/api/workflow-central/instances?limit=10&limit=20').set('x-test-demo', '1');
    expect(res.status).toBe(400);
    expect(res.body.code).toBe('invalid_query_param');
    expect(res.body.message).toMatch(/limit/);
  });
  ...
  it('T-13c-guard: empty limit string (`?limit=`) → treated as absent, not 0 (Copilot R7 + Codex)', async () => {
    ...
    const res = await request(app).get('/api/workflow-central/instances?limit=').set('x-test-demo', '1');
    expect(res.status).toBe(200);

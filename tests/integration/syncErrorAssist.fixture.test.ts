...
import { netsuiteConfig } from '../../src/config/env';
...
describe('Sync error AI assist — 10-scenario fixture', () => {
  ...
  beforeEach(async () => {
    ...
    // Codex P1 + R3 P1 (PR #966) — the tenant-account guard inside
    // buildNetSuiteEnvAuthConfigForTenant reads embedded_service_token_versions
    // via the REAL repository here, and FAILS CLOSED when the tenant has no
    // binding row. Seed one whose platform_account_id matches the runtime env
    // account (setupEnv.ts defaults NETSUITE_ACCOUNT_ID='test-account'), so
    // this fixture proves the provisioned-and-matching path end-to-end.
    ...
    await db.insertInto('embedded_service_token_versions').values({
      ...
      platform: 'netsuite',
      platform_account_id: netsuiteConfig.accountId ?? 'test-account',
    ...
    const mockNS = { initialize: jest.fn().mockResolvedValue(undefined), search: mockSearch, create: mockCreate };
  ...
  for (const scenario of SCENARIOS) {
    describe(`${scenario.id}: ${scenario.category}`, () => {
      it('classifies into expected category and produces fix-suggestion of expected confidence', async () => {
        ...
        // the AI-suggestion payload is wrapped in `{ fields: ... }`.
        const fields = recordData.fields;
        expect(fields.confidence).toBe(scenario.expectedConfidence);
        expect(fields.suggestion_type).toBe(scenario.expectedShapeAssertions.suggestionType);
        if (scenario.expectedShapeAssertions.mentionsTerms) {
          for (const term of scenario.expectedShapeAssertions.mentionsTerms) {
            expect(fields.suggestion_text).toContain(term);
          }
        }
      });
    });

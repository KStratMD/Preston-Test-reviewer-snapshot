...
describe('POST /api/sync-error-assist/ingest — integration', () => {
  ...
  beforeEach(async () => {
    ...
    kit = await buildIntegrationApp({ strictCentralGate: true });
    app = kit.app;
    ...
    // on plaintext rows. The feature-flag booleans stay plaintext (no secret
    // material).
    await tcRepo.upsert(TENANT, 'sync_error_assist.webhook_hmac_secret', SECRET, { isEncrypted: true });
    ...
    // Codex R3 P1 — the connector-initialize guard fails closed without a
    // tenant->account binding row; seed one matching the env account.
    await seedEmbeddedAccountBinding(TENANT);
  });
  ...
  it('C2 — webhook_hmac_secret stored plaintext (legacy is_encrypted=false) → 401 + plaintext-misconfig error log', async () => {
    // Simulates a tenant whose HMAC secret was stored before C2 enforced
    // encrypted-at-rest. Overrides the beforeEach default explicitly so the
    // test remains meaningful AFTER the beforeEach is migrated to
    // `isEncrypted: true`. The route's `getSecretString` call throws on a
    // plaintext row, the route catches + logs the misconfiguration, then
    // collapses to `secret = null` which the existing unknown_tenant branch
    // surfaces as a 401. From the operator's perspective the tenant goes
    // dark until the secret is re-set with `isEncrypted: true`.
    await tenantConfigRepoFor(kit).upsert(
      TENANT,
      'sync_error_assist.webhook_hmac_secret',
      SECRET,
      { isEncrypted: false },
    );
    ...
    const payload = JSON.stringify({
      tenantId: TENANT,
      errorRecordId: 'err-plaintext-secret',
      lastModified: new Date().toISOString(),
      errorType: 'sync',
      errorMessage: 'should never reach processing',
      sourcePayload: { foo: 'bar' },
      attemptCount: 0,
    });
    const sig = sign(payload, ts);
    ...
    // Plaintext-misconfig signal — operator's cue to re-set with isEncrypted: true.
    // Logger.error(message, error?, metadata) per the repo-wide signature.
    // R3 (Copilot R2) — assert both tenantId and settingKey are present so the
    // log shape stays consistent with the rest of the route's log lines.
    expect(kit.loggerStub.error).toHaveBeenCalledWith(
      expect.stringContaining('plaintext'),
      expect.any(Error),
      expect.objectContaining({
        tenantId: TENANT,
        settingKey: 'sync_error_assist.webhook_hmac_secret',
      }),
    );
  });
  ...
  it('R8-2 + R9-4 + R10-7 / §2.1 step 7-8 — authenticated counter fires AFTER tenant rate limiter; rate-limited request does NOT increment it', async () => {
    ...
    try {
      ...
      await tenantConfigRepoFor(noIpLimitKit).upsert(TENANT, 'sync_error_assist.webhook_hmac_secret', SECRET, { isEncrypted: true });
  ...
  it('AC #7 — post-auth tenant rate limit: 101 valid webhooks for one tenant → 101st returns 429', async () => {
    ...
    try {
      await tenantConfigRepoFor(tenantOnlyKit).upsert(TENANT, 'sync_error_assist.webhook_hmac_secret', SECRET, { isEncrypted: true });

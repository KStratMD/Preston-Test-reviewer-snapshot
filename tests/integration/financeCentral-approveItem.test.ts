 ...
 *
 * Tenant (PR-F5b): the router resolves tenants via resolveCentralTenantId —
 * anonymous requests 401 unless the demo gate attested a GET/HEAD admission.
 * Reads use attestReadsOnly() + the x-test-demo header (F5b-3: the dedicated
 * demo tenant, where the boot seeds write). Writes authenticate
 * via a header-driven req.user injector (x-test-actor); with an authenticated
 * identity, resolveActor treats the JWT userId as authoritative and ignores
...
import { attestReadsOnly } from '../unit/routes/central/centralRouterHarness';
...
// F5b-3 splits this suite's two identities apart:
//   - authenticated operator writes run as an ORDINARY tenant. They cannot run
//     as the demo tenant: resolveCentralActor refuses a claim for it (403
//     reserved_tenant), because demo-tenant rows are what anonymous visitors
//     render and no JWT is ever minted with that id.
//   - the attested ANONYMOUS reads resolve to the demo tenant, so those two
//     cases seed there explicitly.
const TENANT_ID = 'tenant-fc-operator';
const DEMO_TENANT_ID = CENTRAL_DEMO_TENANT_ID;
const DAY_MS = 86_400_000;
const TEST_ACTOR_HEADER = 'x-test-actor';

/**
 * Header-driven identity injector: a non-empty x-test-actor header
 * authenticates the request as that operator on the ordinary operator tenant.
 */
function actorInjector(): express.RequestHandler {
  return (req, _res, next) => {
    const actor = req.headers[TEST_ACTOR_HEADER];
...
interface SeedOverrides {
  ...
  submittedAt?: string;
  /** Defaults to the operator tenant; the demo-read cases pass the demo tenant. */
  tenantId?: string;
}
...
describe('finance-central operator routes (PR 6)', () => {
  ...
  describe('GET /api/finance-central/approvals', () => {
    it('returns seeded pending rows ordered by priority then daysWaiting', async () => {
      // Attested ANONYMOUS read → resolves to the demo tenant, so seed there.
      const now = Date.now();
      await seedPending({ approvalId: 'appr-low',       priority: 'low',    submittedAt: new Date(now - 5 * DAY_MS).toISOString(), tenantId: DEMO_TENANT_ID });
      await seedPending({ approvalId: 'appr-urgent',    priority: 'urgent', submittedAt: new Date(now - 1 * DAY_MS).toISOString(), tenantId: DEMO_TENANT_ID });
      await seedPending({ approvalId: 'appr-med-new',   priority: 'medium', submittedAt: new Date(now - 1 * DAY_MS).toISOString(), tenantId: DEMO_TENANT_ID });
      await seedPending({ approvalId: 'appr-med-old',   priority: 'medium', submittedAt: new Date(now - 4 * DAY_MS).toISOString(), tenantId: DEMO_TENANT_ID });
      // A row on the operator tenant must NOT appear in the demo read.
      await seedPending({ approvalId: 'appr-other-tenant', priority: 'urgent' });

      const res = await request(app).get('/api/finance-central/approvals').set('x-test-demo', '1');
    ...
    it('honors the ?priority filter', async () => {
      await seedPending({ approvalId: 'appr-h1', priority: 'high', tenantId: DEMO_TENANT_ID });
      await seedPending({ approvalId: 'appr-m1', priority: 'medium', tenantId: DEMO_TENANT_ID });

      const res = await request(app).get('/api/finance-central/approvals?priority=high').set('x-test-demo', '1');
  ...
  describe('POST /api/finance-central/approvals/:id/approve', () => {
    ...
    it('authenticated approve succeeds without a body approverId (actor comes from the verified identity)', async () => {
      // Inverted (PR-F5b): body approverId is no longer how the actor is
      // supplied — an authenticated request resolves its actor from req.user.
      const approvalId = await seedPending({ approvalId: 'appr-no-approver' });
      const res = await request(app)
        .post(`/api/finance-central/approvals/${approvalId}/approve`)
        .set(TEST_ACTOR_HEADER, 'op_test')
        .send({});
      expect(res.status).toBe(200);
      expect(mockConnector.update).toHaveBeenCalledTimes(1);
      const row = await repo.getRowByApprovalId(TENANT_ID, approvalId);
      expect(row?.operator_disposition).toBe('accepted');
      expect(row?.operator_disposition_user_id).toBe('op_test');
    });

    it('401 identity_required for anonymous approve — a body approverId no longer grants identity (PR-F5b)', async () => {
      // Inverted (PR-F5b): was "400 when approverId is a non-string"; identity
      // now fails before body-actor validation, with no side effects.
    ...
    it('authenticated approve ignores a body-supplied approverId (verified identity is authoritative)', async () => {
      // Inverted (PR-F5b): was "400 when approverId is a whitespace-only
      // string"; the body actor field is no longer read when authenticated.
      const approvalId = await seedPending({ approvalId: 'appr-whitespace' });
      const res = await request(app)
        .post(`/api/finance-central/approvals/${approvalId}/approve`)
        .set(TEST_ACTOR_HEADER, 'op_test')
        .send({ approverId: 'spoofed_op' });
      expect(res.status).toBe(200);
      const row = await repo.getRowByApprovalId(TENANT_ID, approvalId);
      expect(row?.operator_disposition_user_id).toBe('op_test');
    });
  ...
  describe('POST /api/finance-central/approvals/:id/reject', () => {
    ...
    it('authenticated reject succeeds without a body rejecterId (actor comes from the verified identity)', async () => {
      // Inverted (PR-F5b): was "400 when rejecterId is missing"; an
      // authenticated request resolves its actor from req.user.
    ...
    it('401 identity_required for anonymous reject — a body rejecterId no longer grants identity (PR-F5b)', async () => {
      // Inverted (PR-F5b): was "400 when rejecterId is a non-string"; identity
      // now fails before body-actor validation, with no side effects.

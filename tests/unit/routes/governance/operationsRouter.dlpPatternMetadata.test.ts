...
describe('operationsRouter — GET /dlp-pattern-metadata', () => {
  ...
  beforeEach(() => {
    ...
    // Snapshot BEFORE binding: taken after, restore() would put the stub back
    // rather than remove it, leaking it into other files in this Jest worker.
    container.snapshot();
    bindStubGrants('approver', { userId: 'u-dlp-1', tenantId: 't-dlp-1' });
  ...
  it('403 when the embedded session lacks an approver role', async () => {
    ...
    // A verified VIEWER grant is still not an approver — this is the denial.
    bindStubGrants('viewer', { userId: 'u-dlp-1', tenantId: 't-dlp-1' });
    const res = await request(app).get('/api/governance/dlp-pattern-metadata');

...
// F3: pass the embedded tenant kill switch through — its container-based
// TenantLifecycleService resolution is not under test here.
...
jest.mock('../../../../src/middleware/embeddedAuthMiddleware', () => ({
  validateGuestContext: (
    ...
    res.locals.embeddedSession = {
      ...
      // The gate reads a VERIFIED grant, not this column — the column is what
      // the host claimed, and a host claiming 'approver' is the escalation the
      // gate now refuses. bindStubGrants below supplies the grant.
      ...verifiedSessionFields,
...
import { bindStubGrants, verifiedSessionFields } from '../../helpers/embeddedGrantStub';

// Snapshot BEFORE binding and restore after, so the stubbed
// EmbeddedRoleGrantRepository cannot leak into other test files sharing this
// Jest worker. inversify's snapshot/restore is a stack, so this nests safely
// inside the per-describe pair further down.
beforeEach(() => {
  container.snapshot();
  bindStubGrants('approver');
});

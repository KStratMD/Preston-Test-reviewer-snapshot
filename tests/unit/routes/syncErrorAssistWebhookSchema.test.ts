...
describe('WebhookPayloadSchema (R11-1 / R11-3 / AC #17–19)', () => {
  ...
  // A5 — the local TENANT_ID_REGEX is gone; the field now validates through
  // normalizeTenantIdClaim. The rule is unchanged (same charset, same 64 bound),
  // but two things are new: a padded value is accepted and PARSED TO ITS
  // TRIMMED SPELLING rather than rejected, and the sentinel check now runs on
  // the trimmed value so a padded sentinel cannot slip past it.
  it('A5 — parses a padded tenantId to its canonical trimmed spelling', () => {
    const parsed = WebhookPayloadSchema.safeParse({ ...validBase(), tenantId: '  acme-co  ' });
    expect(parsed.success).toBe(true);
    expect(parsed.success && parsed.data.tenantId).toBe('acme-co');
  });

  it('A5 — rejects a PADDED reserved sentinel, not just the bare one', () => {
    expect(WebhookPayloadSchema.safeParse({ ...validBase(), tenantId: '  __system__  ' }).success).toBe(false);
  });

  it('A5 — 64 chars is accepted, 65 is not (boundary is inclusive)', () => {
    expect(WebhookPayloadSchema.safeParse({ ...validBase(), tenantId: 'a'.repeat(64) }).success).toBe(true);
    expect(WebhookPayloadSchema.safeParse({ ...validBase(), tenantId: 'a'.repeat(65) }).success).toBe(false);
  });

  it('A5 — the format error message states the rule without echoing the rejected value', () => {
    // The payload is attacker-controlled and this text reaches logs.
    const secret = 'tenant/../../etc/passwd';
    const parsed = WebhookPayloadSchema.safeParse({ ...validBase(), tenantId: secret });
    expect(parsed.success).toBe(false);
    const text = parsed.success ? '' : JSON.stringify(parsed.error.issues);
    expect(text).toContain('tenantId must be 1-64 characters');
  ...
  it.each([
    ...
    ['CJK character',             'acme中'],
  ])('A5 — rejects shape-illegal tenantId: %s', (_label, tenantId) => {
    expect(WebhookPayloadSchema.safeParse({ ...validBase(), tenantId }).success).toBe(false);
  });

...
describe('Dashboard Routes', () => {
  ...
  describe('GET /api/capabilities — DISABLE_REDIS normalization (#1102)', () => {
    // This endpoint read `process.env.DISABLE_REDIS` directly, which is truthy
    // for ANY non-empty string. docker-compose sets DISABLE_REDIS=0 to mean
    // "Redis enabled", so the deployed compose stack advertised lightweight
    // mode while the queue subsystem connected to the very same healthy Redis.
    // It now reads the normalized env value; these cases pin both directions.
    it.each([
      [false, 'standard'],
      [true, 'lightweight'],
    ])('reports mode=%s => %s', async (disableRedis, expectedMode) => {
      // Restored explicitly rather than leaning on `restoreMocks: true` in
      // jest.base.config.cjs. That setting does currently restore this, but a
      // test that mutates a shared module singleton should not depend on
      // ambient config for its isolation — if that flag ever changes, this
      // would start leaking DISABLE_REDIS into later tests as order-dependent
      // flake. (Raised by Copilot on #1117.)

...
describe('reconciliationCenterRoutes', () => {
  ...
  it('401s identity_required when req.user has an empty tenantId (F3 contract pin)', async () => {
    const service = {
      listOpen: jest.fn(),
      resolveException: jest.fn(),
    };
    const app = appWith(service as unknown as RoutedSvc, { tenantId: '', id: 'u_ops' });
  ...
  it('401s operator_identity_required on resolve when userId is the system sentinel (F3 contract pin)', async () => {
    const service = {
      listOpen: jest.fn(),
      resolveException: jest.fn(),
    };
    const app = appWith(service as unknown as RoutedSvc, { tenantId: 't_squire', id: '__system__' });
  ...
  /**
   * PR3 follow-up: the helper guarded the userId (above) but never the tenant,
   * so a JWT claiming the retired `__system__` sentinel as its TENANT reached
   * the durable `resolved_by` write and every tenant-scoped read.
   */
  it('401s identity_required on resolve when the TENANT claim is the system sentinel', async () => {
    const service = {
      listOpen: jest.fn(),
      resolveException: jest.fn(),
    };
    const app = appWith(service as unknown as RoutedSvc, { tenantId: '__system__', id: 'u_ops' });
  ...
  it('401s identity_required on list when the TENANT claim is the system sentinel', async () => {
    const service = {
      listOpen: jest.fn(),
      resolveException: jest.fn(),
    };
  ...
  it('401s identity_required on schedule creation when the TENANT claim is the system sentinel', async () => {
    const service = {
      listOpen: jest.fn(),
      resolveException: jest.fn(),
    ...
    const res = await request(app)
      .post('/api/reconciliation-center/schedules')
      .send({ name: 'nightly', reconcilerKey: 'k', cadence: 'daily' });
  ...
  it('rejects resolve when req.user lacks a usable user id', async () => {
    ...
    expect(res.status).toBe(401);
    // F3: a real tenant with no usable userId is an OPERATOR-identity
    // failure now (pre-F3 extractIdentityContext dropped the whole req.user
    // source when id was missing, collapsing this to identity_required).
    expect(res.body).toEqual({ error: 'operator_identity_required' });
    expect(service.resolveException).not.toHaveBeenCalled();
  });

  it('rejects resolve when only req.tenantContext is populated (identity_required)', async () => {
    const service = {
      listOpen: jest.fn(),
      resolveException: jest.fn(),
    };
    ...
    // no req.user. F3: the router reads req.user ONLY (the mount requires
    // authMiddleware), so a tenantContext-only request has no identity at
    // all — identity_required (pre-F3 extractIdentityContext surfaced the
    // bridge tenantId with a synthetic userId → operator_identity_required).
    app.use((req: Request, _res: Response, next: NextFunction) => {

...
describe('configuration routes — identity precedes every existence-dependent reply', () => {
  ...
  describe.each(HANDLERS)('$name', ({ send, serviceCall }) => {
    ...
    // B3.4: the same 401-before-lookup ordering must hold for the actor forms
    // `requireConfigurationCommandContext` rejects beyond a bare empty string —
    // whitespace-only and the case-insensitive literal 'unknown'. A cross-tenant
    // id is used here (as above) so a regression that let the lookup run would
    // surface as a 404, not just a wrong-shaped 401.
    it.each(['   ', 'unknown', 'Unknown', 'UNKNOWN', ' unknown '])(
      'returns the IDENTICAL 401 — not 404 — for a CROSS-TENANT id when the actor id is %j',
      async (id) => {
        (configService.getConfigurationForTenant as jest.Mock).mockReturnValue(undefined);
        (configService.getAllConfigurations as jest.Mock).mockReturnValue([{ id: 'other-tenants-cfg' }]);

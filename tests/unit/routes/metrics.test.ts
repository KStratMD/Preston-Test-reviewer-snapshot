...
describe('Metrics Routes', () => {
  ...
  describe('GET /', () => {
    it('refreshes the V8 heap-size-limit gauge before scraping', async () => {
      const handler = getRouteHandler('get', '/');
      if (handler) {
        await handler(mockReq as Request, mockRes as Response);
      }

      expect(mockGaugeSet).toHaveBeenCalledWith(256 * 1024 * 1024);
    });
  ...
  describe('GET /review (reviewer JSON payload)', () => {
    ...
    it('does not report metrics generation provenance as build_sha when BUILD_SHA is absent', async () => {
      const previous = process.env.BUILD_SHA;
      delete process.env.BUILD_SHA;
      __resetMetricsReviewCacheForTests();
      try {
        ...
        expect(body.metrics_git_sha).toBe(body.metrics.git_sha);
      } finally {

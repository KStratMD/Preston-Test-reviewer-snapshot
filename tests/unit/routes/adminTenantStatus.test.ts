...
// Mock the admin gate so it is a pass-through in unit tests, which exercise the
// handler bodies rather than authorization.
//
// Mocking the gate is what let the production defect survive: this suite passed
// for as long as the route gated on requireAdmin from ../middleware/rbac, whose
// module-global RBAC singleton production never initializes, because the mock
// replaced exactly the thing that was broken. The real gate is exercised in
// tests/integration/adminTenantStatus.production.routes.test.ts; keep that suite
// in step with this mock.

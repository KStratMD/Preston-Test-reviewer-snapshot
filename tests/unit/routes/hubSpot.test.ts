...
import { isPlatformAdminActor } from '../../../src/middleware/verifiedAdmin';
 ...
 *
 * `userOverride` lets a caller replace the default id/tenantId (e.g. to
 * assert a sentinel-claiming JWT is refused) without adding a parallel helper.
 */
function makeAppWithUser(
  roles: string[],
  userOverride?: { id?: string; tenantId?: string },
): express.Express {
  const app = express();
  app.use(express.json());
...
// ── F4: verified identity required on write routes ───────────────────────────

describe('F4 — writes without verified identity', () => {
  it('POST /api/hubspot/contacts anonymous → 401 identity_required (never reaches guardedWrite)', async () => {
    const resolver = makeResolverThrowing(new Error('should never be called'));
    container.rebind(TYPES.OwnershipResolver).toConstantValue(resolver);
    container.rebind(TYPES.AuditService).toConstantValue(makeAuditStub());

    // Deliberately anonymous (makeApp, no user): pins the 401 path.
    ...
    expect(res.body).toMatchObject({ error: 'identity_required' });
    expect(resolver.validateWrite).not.toHaveBeenCalled();
  });

  it('refuses a sentinel-claiming JWT on a write handler', async () => {
    const resolver = makeResolverThrowing(new Error('should never be called'));
    container.rebind(TYPES.OwnershipResolver).toConstantValue(resolver);
    container.rebind(TYPES.AuditService).toConstantValue(makeAuditStub());

    const app = makeAppWithUser(['admin'], { id: SYSTEM_IDENTITY.userId, tenantId: 'tenant-a' });
    const res = await request(app).post('/api/hubspot/contacts').send({ email: 'a@b.c' });
    expect(res.status).toBe(401);
    expect(res.body.error).toBe('identity_required');
    expect(resolver.validateWrite).not.toHaveBeenCalled();
  });

  it('requirePlatformAdmin admits the sentinel actor — the route layer must refuse it', () => {
    expect(isPlatformAdminActor({ id: SYSTEM_IDENTITY.userId, roles: ['admin'] })).toBe(true);
  });
});
...
describe('POST /api/hubspot/contacts — entity:contact, operator_action → 409 ownership_blocked', () => {
  it('returns 409 with error: ownership_blocked', async () => {
    ...
    const resolver = makeResolverThrowing(blockedErr);
    container.rebind(TYPES.OwnershipResolver).toConstantValue(resolver);
    container.rebind(TYPES.AuditService).toConstantValue(makeAuditStub());

    // F4: writes require verified identity — mount with an injected user
    // (empty roles: the block path needs no override privileges).
    const res = await request(makeAppWithUser([]))
      .post('/api/hubspot/contacts')
      .set('Content-Type', 'application/json')
      .send({ email: 'test@example.com' });
    ...
    // F4 attribution pin: the ownership decision is evaluated for the
    // verified JWT tenant, not the retired __system__ fallback.
    expect(resolver.validateWrite).toHaveBeenCalledWith(

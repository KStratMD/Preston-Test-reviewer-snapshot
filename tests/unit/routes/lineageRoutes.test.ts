...
describe('GET /api/lineage/records/:system/:entityType/:entityId', () => {
  ...
  // F3 contract pins: jwtOperatorIdentity rejects every malformed req.user
  // shape the type system cannot rule out (empty/missing tenantId, empty/
  // missing id) — 401 operator_identity_required, service never called.
  it.each([
    [{ tenantId: '', id: 'u_ops' }, 'empty tenantId'],
    [{ id: 'u_ops' }, 'missing tenantId'],
    [{ tenantId: 't_squire', id: '' }, 'empty userId'],
    [{ tenantId: 't_squire' }, 'missing userId'],
  ])('returns 401 for malformed req.user %o (%s)', async (user) => {
    const svc = {
      chainForRecord: jest.fn(),
    } as unknown as LineageQueryService;
    const app = express();

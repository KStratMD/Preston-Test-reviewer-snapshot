...
describe('requireConfigurationCommandContext', () => {
  ...
  it('sends 401 operator_identity_required when req.user.id is whitespace-only', () => {
    const req = { user: { id: '   ', tenantId: 'tenant-a' } } as unknown as Request;
    const res = makeRes();
  ...
  it.each(['unknown', 'Unknown', 'UNKNOWN', ' unknown '])(
    'sends 401 operator_identity_required when req.user.id is the literal actor %j (case/whitespace insensitive)',
    (id) => {
      const req = { user: { id, tenantId: 'tenant-a' } } as unknown as Request;
      const res = makeRes();
  ...
  it('trims a legitimate actor id with surrounding whitespace and returns the trimmed value', () => {
    const req = {
      user: { id: '  user-1  ', tenantId: 'tenant-a' },
      correlationId: 'req-correlation-1',
    } as unknown as Request;
  ...
  it('rejects the unknown actor before checking tenant mismatch (identity gate before tenant-agreement gate)', () => {
    const req = { user: { id: 'unknown', tenantId: 'tenant-b' } } as unknown as Request;
    const res = makeRes();

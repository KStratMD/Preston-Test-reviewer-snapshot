...
function createApp() {
  ...
  // F5b: invoices reads simulate the policy gate's reads-only anonymous demo
  // attestation (x-test-demo: 1 → system tenant, deliberate). Writes are never
  // attested — anonymous writes 401 (the removed SYSTEM_IDENTITY fallback).
...
describe('paymentCentral router', () => {
  ...
  describe('attribution hardening (resolveActor)', () => {
    ...
    it('approve: pre-auth 401s on missing approvedBy (identity_required, actor guard first)', async () => {
    ...
    it('createInvoice: anonymous write 401s (the "api" pre-auth fallback is gone, F5b)', async () => {
      await request(createApp())
        .post('/api/payment-central/invoices')
    ...
    it('dispute: pre-auth 401s on missing createdBy (identity_required, actor guard first)', async () => {
      await request(createApp())
        .post('/api/payment-central/invoices/inv1/dispute')
    ...
    it('credit-memo: pre-auth 401s on missing createdBy (identity_required, actor guard first)', async () => {
      await request(createApp())
        .post('/api/payment-central/credit-memos')
    ...
    it('match: anonymous write 401s (the "api" pre-auth fallback is gone, F5b)', async () => {
      await request(createApp())
        .post('/api/payment-central/invoices/inv1/match')
        .send({ poId: 'po1' })
        .expect(401);
      expect(mockMatchInvoiceToPO).not.toHaveBeenCalled();
    });

    it('dispute: anonymous write 401s — body createdBy is never honored (F5b)', async () => {
      await request(createApp())
        .post('/api/payment-central/invoices/inv1/dispute')
    ...
    it('credit-memo: anonymous write 401s — body createdBy is never honored (F5b)', async () => {
      await request(createApp())
        .post('/api/payment-central/credit-memos')
    ...
    it('gl approve-journal: pre-auth 401s — no GL write may silently use "system"', async () => {
      await request(createApp())
        .post('/api/payment-central/gl/journal-entries/je1/approve')
        .send({})
        .expect(401);
    ...
    it('gl post-journal: pre-auth 401s — no GL write may silently use "system"', async () => {
      await request(createApp())
        .post('/api/payment-central/gl/journal-entries/je1/post')
        .send({})
        .expect(401);
    ...
    it('gl batches-create: pre-auth 401s — no GL write may silently use "system"', async () => {
      await request(createApp())
        .post('/api/payment-central/gl/batches')
        .send({ name: 'B', entryIds: ['je1'] })
        .expect(401);
      expect(mockCreatePostingBatch).not.toHaveBeenCalled();
    });
    ...
    it('gl batches-process: pre-auth 401s — no GL write may silently use "system"', async () => {
      await request(createApp())
        .post('/api/payment-central/gl/batches/b1/process')
        .send({})
        .expect(401);
      expect(mockProcessPostingBatch).not.toHaveBeenCalled();
    });
    ...
    it('dispute-resolve: pre-auth 401s on missing resolvedBy (identity_required, actor guard first)', async () => {
      await request(createApp())
        .post('/api/payment-central/disputes/d1/resolve')

 ...
 *
 * Goal: lock the HTTP contract and verify every identity-bearing route
 * resolves tenant/actor only from a verified JWT (via verifiedIdentity),
 * refusing 401 identity_required when unauthenticated, so actor/tenant
 * attribution can never be spoofed via the request body.
 */
...
describe('syncCentral router', () => {
  ...
  describe('POST /alerts/:alertId/acknowledge', () => {
    ...
    it('body has no acknowledgedBy field — verified identity attributes the call anyway', async () => {
      mockAcknowledgeAlert.mockResolvedValue(undefined);
      ...
      expect(res.status).toBe(200);
      expect(mockAcknowledgeAlert).toHaveBeenCalledWith('tenant-a', 'alert1', 'u1');
    });
  });
...
describe('identity refusal (PR3)', () => {
  // All eight identity-bearing routes. /tiers and /tiers/:tierId are absent
  // deliberately — they serve the process-global catalog and take no identity.
  const cases: Array<{ path: string; send: (p: string) => request.Test }> = [
    { path: '/api/sync-central/subscriptions',                    send: (p) => request(createApp()).get(p) },
    { path: '/api/sync-central/subscriptions',                    send: (p) => request(createApp()).post(p) },
    { path: '/api/sync-central/subscriptions/sub-1/cancel',       send: (p) => request(createApp()).post(p) },
    { path: '/api/sync-central/subscriptions/sub-1/usage',        send: (p) => request(createApp()).post(p) },
    { path: '/api/sync-central/subscriptions/sub-1/limit-check',  send: (p) => request(createApp()).get(p) },
    { path: '/api/sync-central/alerts',                           send: (p) => request(createApp()).get(p) },
    { path: '/api/sync-central/alerts/alert-1/acknowledge',       send: (p) => request(createApp()).post(p) },
    { path: '/api/sync-central/analytics',                        send: (p) => request(createApp()).get(p) },
  ];

  it.each(cases)('refuses $path without a verified identity', async ({ path, send }) => {
    const res = await send(path);
    expect(res.status).toBe(401);
    expect(res.body.error).toBe('identity_required');
  });

  it('ignores a body-supplied cancelledBy and uses the verified user', async () => {
    mockCancelSubscription.mockResolvedValue({ id: 'sub-1', status: 'cancelled' });
    const app = createAuthedApp('real-user');
    await request(app)
      .post('/api/sync-central/subscriptions/sub-1/cancel')
      .send({ reason: 'x', cancelledBy: 'spoofed-actor' })
      .expect(200);
    expect(mockCancelSubscription).toHaveBeenCalledWith('tenant-a', 'sub-1', 'x', 'real-user');
  });
});

describe('cancel attribution (verified identity)', () => {
  ...
  it('unauthenticated request is refused regardless of cancelledBy', async () => {
    await request(createApp())
      .post('/api/sync-central/subscriptions/sub1/cancel')
      .send({ reason: 'churn', cancelledBy: 'spoof' })
      .expect(401);
    expect(mockSyncService.cancelSubscription).not.toHaveBeenCalled();
  });
});

describe('alert-acknowledge attribution (verified identity)', () => {
  beforeEach(() => jest.clearAllMocks());
  ...
  it('unauthenticated request is refused regardless of acknowledgedBy', async () => {
    await request(createApp())
      .post('/api/sync-central/alerts/a1/acknowledge')
      .send({ acknowledgedBy: 'spoof' })
      .expect(401);

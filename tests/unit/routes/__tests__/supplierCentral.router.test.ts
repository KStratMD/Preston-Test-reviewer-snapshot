...
describe('supplierCentral router', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    // F5b: handlers now refuse unattributable requests (resolveCentralTenantId),
    // so the HTTP-contract cases run authenticated — the contract under test is
    // status codes and response shapes, not identity semantics (those are pinned
    // in supplierCentralTenant.test.ts and the attribution block below).
    app = createAuthedApp('contract-op');
  });
...
describe('attribution hardening (resolveActor)', () => {
  ...
  it('approve: anonymous request 401s before body validation (F5b — pre-auth path retired)', async () => {
  ...
  it('approve: anonymous write 401s — body approvedBy is never honored (F5b)', async () => {
    // Pre-F5b this resolved to SYSTEM_IDENTITY and honored the body actor;
    // resolveCentralTenantId now refuses unattested anonymous requests, and
    // writes are never attested.
    await request(createApp())
      .post('/api/supplier-central/vendors/v1/approve')
  ...
  it('reject: anonymous request 401s (F5b — pre-auth path retired)', async () => {
    await request(createApp())
      .post('/api/supplier-central/vendors/v1/reject')
      .send({ reason: 'bad' })
      .expect(401);
  ...
  it('bulk-approve: anonymous request 401s (F5b — pre-auth path retired)', async () => {
    await request(createApp())
      .post('/api/supplier-central/vendors/bulk-approve')
      .send({ vendorIds: ['v1'] })
      .expect(401);
  ...
  it('createVendorProfile: anonymous write 401s — non-string createdBy never reaches the service (F5b)', async () => {
    await request(createApp())
      .post('/api/supplier-central/vendors')
  ...
  it('createVendorProfile: anonymous write 401s — body createdBy is never honored (F5b)', async () => {
    await request(createApp())
      .post('/api/supplier-central/vendors')
  ...
  it('createVendorProfile: a sentinel-user JWT with a spoofed body createdBy 401s', async () => {
    // Revert-sensitive in a way the two cases above are not: those run
    // anonymous, where resolveActor already refused before this PR. Here the
    // request IS authenticated, so only the sentinel rejection inside
    // verifiedUserId stands between the spoofed body value and the service.
    await request(createAuthedApp(SYSTEM_IDENTITY.userId))
      .post('/api/supplier-central/vendors')
      .send({ vendorName: 'Acme', createdBy: 'spoof' })
      .expect(401);
    expect(mockCreateVendorProfile).not.toHaveBeenCalled();
  ...
  // The three cases below pin the boundary that keeps every path into
  // createVendorProfile attributed. A non-plain-object body cannot carry
  // `createdBy`, and the service persists the vendor + an activity row before
  // it throws on the missing basicInfo — so the route must refuse it rather
  // than forward it.
  function createBodyOverrideApp(body: unknown) {
    const app = express();
    app.use((req, _res, next) => {
      (req as { body: unknown }).body = body;
      next();
    });
    app.use(fakeAuthMiddleware({ id: 'alice', tenantId: 'tenant-a' }));
    app.use('/api/supplier-central', supplierCentralRouter);
    return app;
  }

  it('createVendorProfile: a primitive body is refused 400 and never reaches the service', async () => {
    // Simulates a non-object req.body (e.g. non-strict JSON parsing of a
    // primitive). The old guard skipped attribution and forwarded it.
    const res = await request(createBodyOverrideApp('just-a-string'))
      .post('/api/supplier-central/vendors')
      .send();
    expect(res.status).toBe(400);
  ...
  it('createVendorProfile: an ARRAY body is refused 400 and never reaches the service', async () => {
    const res = await request(createAuthedApp('alice'))
      .post('/api/supplier-central/vendors')
      .send([]);
    expect(res.status).toBe(400);
    expect(res.body).toEqual({ error: 'Request body must be an object' });
  ...
  it('createVendorProfile: an omitted body still succeeds and carries the verified actor', async () => {
    // express.json() yields {} for an empty body — the refusal above must not
    // widen into a rejection of the ordinary no-body request.
    mockCreateVendorProfile.mockResolvedValue('v1');
    await request(createAuthedApp('alice'))
      .post('/api/supplier-central/vendors')
      .send()
  ...
  it('createPurchaseOrder: authenticated identity is used even when createdBy is omitted from the body', async () => {
    mockCreatePurchaseOrder.mockResolvedValue({ id: 'po1' });
    await request(createAuthedApp('alice'))
      .post('/api/supplier-central/purchase-orders')
      .send({ vendorName: 'Acme' })
      .expect(201);
    expect(mockCreatePurchaseOrder.mock.calls[0][1].createdBy).toBe('alice');
  });

  it('createPurchaseOrder: anonymous request 401s (F5b — pre-auth path retired)', async () => {
    await request(createApp())
      .post('/api/supplier-central/purchase-orders')
      .send({ vendorName: 'Acme', createdBy: { spoof: true } })
      .expect(401);
  ...
  it('acknowledge: anonymous request 401s (F5b — pre-auth path retired)', async () => {
    await request(createApp())
      .post('/api/supplier-central/purchase-orders/po1/acknowledge')
      .send({})
      .expect(401);
  ...
  it('approve: anonymous with NO body 401s cleanly (not 500) and pins the envelope (F5b)', async () => {
  ...
  it('refuses a sentinel actor instead of accepting a body actor', async () => {
    const res = await request(createAuthedApp(SYSTEM_IDENTITY.userId, 'tenant-a'))
      .post('/api/supplier-central/vendors/v1/approve')
      .send({ approvedBy: 'spoofed-operator' });

...
const mockSecureConfigService = {
  ...
  migrateToSecureCredentials: jest.fn().mockResolvedValue({
...
describe('POST /migrate (trusted command context, Task 8)', () => {
  function makeApp() {
    const app = express();
    app.use(express.json());
    app.use(credentialsRouter);
  ...
  it('returns 401 tenant_required when req.user is absent', async () => {
    mockAuthState.user = undefined;
    const res = await request(makeApp()).post('/migrate').send({});

    expect(res.status).toBe(401);
    expect(res.body).toEqual({ error: 'unauthorized', reason: 'tenant_required' });
    expect(mockSecureConfigService.migrateToSecureCredentials).not.toHaveBeenCalled();
  });
  ...
  it('returns 401 operator_identity_required when the caller has no actor id', async () => {
  ...
  it('threads a concrete operation, tenant, actor, and correlation id to migrateToSecureCredentials', async () => {
    mockAuthState.user = { id: 'user-1', tenantId: 'tenant-a' };
    const res = await request(makeApp())
      .post('/migrate')
      .set('x-correlation-id', 'route-correlation-1')
      .send({});

    expect(res.status).toBe(200);
    expect(mockSecureConfigService.loadConfigurations).toHaveBeenCalledTimes(1);
    expect(mockSecureConfigService.migrateToSecureCredentials).toHaveBeenCalledWith({
      tenantId: 'tenant-a',
      actorUserId: 'user-1',

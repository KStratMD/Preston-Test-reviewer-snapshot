...
jest.mock('../../../../src/inversify/inversify.config', () => ({
    container: {
        ...
        getAsync: jest.fn((type: symbol) => {
            const typeStr = type.toString();
            if (typeStr.includes('SchemaRegistryService')) return Promise.resolve(mockSchemaRegistry);
            if (typeStr.includes('SyncCentralOrchestrator')) return Promise.resolve(mockOrchestrator);
            return Promise.reject(new Error(`Unknown type: ${typeStr}`));
        }),
    },
}));
...
// This file mounts the bare router (createApp has no user-injecting
// middleware), so an authenticated call needs its own app instance.
...
describe('SyncOrchestrator Schema Routes', () => {
    ...
    describe('blocked sync API semantics', () => {
        it('should return HTTP 200 even when blocked by schema drift', async () => {
            ...
            const authedApp = appWithUser({ id: 'admin-1', roles: ['admin'] });
        ...
        it('refuses a sentinel-claiming admin with 401 identity_required', async () => {
            const sentinelApp = appWithUser({ id: SYSTEM_IDENTITY.userId, roles: ['admin'] });
            const res = await request(sentinelApp).post('/api/sync-orchestrator/operations/op-1/execute');

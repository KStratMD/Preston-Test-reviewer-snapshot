...
describe('Integration Route', () => {
  ...
  describe('POST /api/integrations/:id/run', () => {
    ...
    // Codex R4 on PR-B. `assertNoManagedCredentialReference` was upgraded from a
    // plain Error to ConflictAppError(409) so a deterministic refusal stops being
    // reported as a server fault — but the typed status never reached the wire,
    // because THIS handler is the one in the router that swallows the error into
    // a hardcoded 500 instead of rethrowing to the error boundary (testHandler
    // and syncRecordHandler both `throw error`).
    //
    // The service-level test for that upgrade asserted the thrown TYPE by calling
    // the private method directly, so it never exercised the route and could not
    // have caught this. This one goes through the wire on purpose.
    it('surfaces a typed ConflictAppError as 409, not 500', async () => {
      const { ConflictAppError } = require('../../../../src/errors/AppError');
      mockIntegrationService.executeIntegration!.mockRejectedValue(
        new ConflictAppError(
          'Configuration cfg-1 declares a managed (secret_manager) credential reference, which executeSync cannot resolve.',
        ),
      );

      const response = await request(app)
        .post('/api/integrations/cfg-1/run')
        .expect(409);

      expect(response.body.error).toContain('managed (secret_manager) credential reference');
    });

    // The same defect would have swallowed every other typed status this stack
    // introduces, so the mapping is driven by AppError.statusCode rather than a
    // ConflictAppError special case.
    it('surfaces a typed ServiceUnavailableAppError as 503, not 500', async () => {
      const { ServiceUnavailableAppError } = require('../../../../src/errors/AppError');
      mockIntegrationService.executeIntegration!.mockRejectedValue(
        new ServiceUnavailableAppError('Relationship evidence is unavailable'),
      );

      await request(app).post('/api/integrations/cfg-1/run').expect(503);
    });

    // ...and an UNCLASSIFIED failure — the genuine "server broke" case — must
    // still be a 500. This is the half that keeps the mapping honest.
    it('still returns 500 for an unclassified error', async () => {
      mockIntegrationService.executeIntegration!.mockRejectedValue(new Error('socket hang up'));

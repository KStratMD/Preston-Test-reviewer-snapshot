...
function makeApp(tenantId = 't1'): express.Express {
  ...
  app.use((req: Request, _res, next) => {
    req.user = { tenantId, id: 'u1', username: 'test-user', roles: [], permissions: [] };
    next();
  });
...
describe('costTransparencyRoutes', () => {
  ...
  it('GET /anomalies returns 401 with no identity', async () => {
    const app = express();
    app.use('/api/cost-transparency', router);
    const res = await request(app).get('/api/cost-transparency/anomalies');
    expect(res.status).toBe(401);
    expect(res.body).toEqual({ error: 'identity_required' });
  });

  it('GET /health responds without identity', async () => {
    const app = express();
    app.use('/api/cost-transparency', router);
    const res = await request(app).get('/api/cost-transparency/health');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ status: 'healthy', service: 'cost-transparency' });
  });
});

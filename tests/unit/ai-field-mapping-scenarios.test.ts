...
// OPENROUTER_API_KEY counts (C8): OpenRouter is a production provider with a
// pinned :free model, so a box configured with only that key can still run
// these live smokes at $0 instead of silently skipping.

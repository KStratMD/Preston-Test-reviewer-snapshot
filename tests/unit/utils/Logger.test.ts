 ...
 * context, and non-string non-Error values attach as `context.error` without
 * allowing raw stack/message strings into structured logs.
 */
...
describe('Logger.error second-arg handling', () => {
  ...
  it('projects primitive error strings to a safe marker', () => {
    const { logger, errorSpy } = makeLogger();

    logger.error('failed', 'string failure');

    const ctx = loggedContext(errorSpy);
    expect(ctx.error).toEqual({ name: 'NonErrorThrown' });
  });

  it.each([123456789, true])('projects primitive error values to a safe marker: %p', value => {
    const { logger, errorSpy } = makeLogger();

    logger.error('failed', value);

    expect(loggedContext(errorSpy).error).toEqual({ name: 'NonErrorThrown' });
  });

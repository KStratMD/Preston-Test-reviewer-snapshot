...
describe('OAuth2Service', () => {
  ...
  describe('validateAccessToken — jwt string-payload guard (#1033)', () => {
    const secret = 'test-secret-for-string-payload-guard';

    it('returns null for a validly-signed string-payload token without touching the DB', async () => {
      process.env.JWT_SECRET = secret;
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const jwt = require('jsonwebtoken');
      // jwt.sign with a string payload produces a validly-signed token whose
      // verified payload is a plain string — no exp claim, so the expiry
      // check below the guard would silently pass without it.
      const token = jwt.sign('not-a-json-object', secret);
      await expect(service.validateAccessToken(token)).resolves.toBeNull();
      expect(mockDatabaseService.getDatabase).not.toHaveBeenCalled();
    });

    it('lets an object-payload token through the guard to the DB revocation lookup', async () => {
      process.env.JWT_SECRET = secret;
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const jwt = require('jsonwebtoken');
      ...
      chainMock.executeTakeFirst.mockResolvedValue(undefined); // not stored → null
      await expect(service.validateAccessToken(token)).resolves.toBeNull();

...
describe('classifyNetworkAddress', () => {
  ...
  // Teredo (2001::/32) sits inside the 2001::/23 special-use denylist entry
  // but keeps its legacy `teredo` label across the ipaddr.js 2.x upgrade (the
  // carve-out in classifyNetworkAddress). Still non-public.
  it('preserves the teredo label for 2001::/32 despite the 2001::/23 denylist entry', () => {
    expect(classifyNetworkAddress('2001::1')).toBe('teredo');

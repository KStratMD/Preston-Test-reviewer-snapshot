...
describe('DocumentationKnowledgeBase', () => {
  ...
  describe('getChunksByFilePaths', () => {
    it('should return empty array before indexing', () => {
      expect(kb.getChunksByFilePaths(['docs/a.md'])).toEqual([]);
    });

    it('should return empty array when no paths match', async () => {
      const chunks = [makeChunk({ id: 'c1', filePath: 'docs/a.md' })];
      mockIndexer.indexDocumentation.mockResolvedValue(chunks);
      await kb.indexDocumentation();

      expect(kb.getChunksByFilePaths(['docs/nonexistent.md'])).toEqual([]);
    });

    it('should return empty array for an empty path list', async () => {
      const chunks = [makeChunk({ id: 'c1', filePath: 'docs/a.md' })];
      mockIndexer.indexDocumentation.mockResolvedValue(chunks);
      await kb.indexDocumentation();

      expect(kb.getChunksByFilePaths([])).toEqual([]);
    });

    it('should return matching chunks grouped by file path', async () => {
      const chunks = [
        makeChunk({ id: 'a1', filePath: 'docs/a.md' }),
        makeChunk({ id: 'b1', filePath: 'docs/b.md' }),
        makeChunk({ id: 'a2', filePath: 'docs/a.md' }),
        makeChunk({ id: 'c1', filePath: 'docs/c.md' }),
      ];
      mockIndexer.indexDocumentation.mockResolvedValue(chunks);
      await kb.indexDocumentation();

      const result = kb.getChunksByFilePaths(['docs/a.md', 'docs/b.md']);
      const ids = result.map(c => c.id).sort();
      expect(ids).toEqual(['a1', 'a2', 'b1']);
      expect(result.every(c => c.filePath === 'docs/a.md' || c.filePath === 'docs/b.md')).toBe(true);
    });

    it('should respect limitPerFile', async () => {
      const chunks = [
        makeChunk({ id: 'a1', filePath: 'docs/a.md' }),
        makeChunk({ id: 'a2', filePath: 'docs/a.md' }),
        makeChunk({ id: 'a3', filePath: 'docs/a.md' }),
      ];
      mockIndexer.indexDocumentation.mockResolvedValue(chunks);
      await kb.indexDocumentation();

      const result = kb.getChunksByFilePaths(['docs/a.md'], 2);
      expect(result.length).toBe(2);
      expect(result.every(c => c.filePath === 'docs/a.md')).toBe(true);
    });

    it('should group chunks in requested filePaths order, not global insertion order', async () => {
      // Chunks are indexed interleaved: fileA-c1, fileB-c1, fileA-c2.
      // Requesting [fileB, fileA] must return all fileB chunks BEFORE fileA chunks.
      const chunks = [
        makeChunk({ id: 'a-chunk1', filePath: 'docs/a.md' }),
        makeChunk({ id: 'b-chunk1', filePath: 'docs/b.md' }),
        makeChunk({ id: 'a-chunk2', filePath: 'docs/a.md' }),
      ];
      mockIndexer.indexDocumentation.mockResolvedValue(chunks);
      await kb.indexDocumentation();

      // Request fileB first, then fileA — output must honour this order.
      const result = kb.getChunksByFilePaths(['docs/b.md', 'docs/a.md'], 3);

      // All fileB chunks must precede all fileA chunks.
      const bIdx = result.findIndex(c => c.filePath === 'docs/b.md');
      const aIdx = result.findIndex(c => c.filePath === 'docs/a.md');
      expect(bIdx).toBeGreaterThanOrEqual(0);
      expect(aIdx).toBeGreaterThan(bIdx);

      // Correct chunks present.
      expect(result.map(c => c.id)).toContain('b-chunk1');
      expect(result.map(c => c.id)).toContain('a-chunk1');
      expect(result.map(c => c.id)).toContain('a-chunk2');
    });

    it('should deduplicate repeated paths and emit each at most once', async () => {
      const chunks = [
        makeChunk({ id: 'a1', filePath: 'docs/a.md' }),
        makeChunk({ id: 'a2', filePath: 'docs/a.md' }),
      ];
      ...
      // Requesting the same path twice must not duplicate the output.
      const result = kb.getChunksByFilePaths(['docs/a.md', 'docs/a.md'], 2);
      expect(result.length).toBe(2); // Same as a single request — no duplication.
      expect(result.every(c => c.filePath === 'docs/a.md')).toBe(true);
    });
  });

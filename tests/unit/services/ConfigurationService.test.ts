...
import { makeGateDouble, type CardinalityGateDouble } from '../../helpers/cardinalityTestDoubles';

const mockFs = fs as jest.Mocked<typeof fs>;

// A trusted command context matching validConfig's tenant, used for the active
// saves in this suite. Activation itself is covered exhaustively in
// ConfigurationService.cardinalityGate.test.ts; here the clean gate double lets
// pre-existing active-save mechanics (id-gen, timestamps, rollback) keep working
// WITHOUT loosening the production gate.
const activeCtx: ConfigurationCommandContext = {
  tenantId: 'tenant-a',
  actorUserId: 'user-1',
...
describe('ConfigurationService', () => {
  ...
  let gate: CardinalityGateDouble;
  ...
  const credentialBearingConfig = (): IntegrationConfig => ({
    ...validConfig,
    id: 'config-secret',
    isActive: false,
    sourceAuthentication: {
      type: 'api_key',
      credentials: { apiKey: 'export-source-secret-do-not-return' },
    },
    targetAuthentication: {
      type: 'basic',
      credentials: { password: 'export-target-secret-do-not-return' },
    },
    authentication: {
      source: {
        type: 'oauth2',
        credentials: { clientSecret: 'export-legacy-source-secret-do-not-return' },
      },
      target: {
        type: 'oauth1',
        credentials: { tokenSecret: 'export-legacy-target-secret-do-not-return' },
      },
    },
  });
  ...
  beforeEach(() => {
    ...
    gate = makeGateDouble();
    service = new ConfigurationService(mockLogger as any, './test-config', gate);
  });
  ...
  describe('loadConfigurations()', () => {
    ...
    it('rejects a legacy inline-credential configuration in production-strength mode', async () => {
      const originalNodeEnv = process.env.NODE_ENV;
      process.env.NODE_ENV = 'production';
      stageTenantReaddir(mockFs.readdir as unknown as jest.Mock, ['legacy.json']);
      mockFs.readFile.mockResolvedValue(JSON.stringify({
        ...
        sourceAuthentication: {
          type: 'api_key',
          credentials: { apiKey: 'SYNTHETIC_LOAD_SECRET' },
        },
      }));

      try {
        await expect(service.loadConfigurations()).rejects.toThrow();
        expect(JSON.stringify(mockLogger.warn.mock.calls)).not.toContain('SYNTHETIC_LOAD_SECRET');
      } finally {
  ...
  describe('saveConfiguration()', () => {
    it('should save valid configuration to memory and file', async () => {
      await service.saveConfiguration(validConfig, activeCtx);
    ...
    it('should generate ID if not provided', async () => {
      ...
      await service.saveConfiguration(configWithoutId, activeCtx);
    ...
    it('should add timestamps', async () => {
      ...
      await service.saveConfiguration(configWithoutDates as any, activeCtx);
    ...
    it('rejects a concurrent cross-tenant save of the same id (race-safe)', async () => {
      ...
      // Drafts here: the cross-tenant race guard lives in persistWithRollback
      // (sync memory set before the write await), which draft and active saves
      // share. Keeping these drafts isolates the concurrency guard from the
      // activation gate (covered separately).
      const first = service.saveConfiguration({ ...validConfig, id: 'shared', tenantId: 'tenant-a', isActive: false });

      await expect(
        service.saveConfiguration({ ...validConfig, id: 'shared', tenantId: 'tenant-b', isActive: false }),
    ...
    it('should restore the previous in-memory value when an update write fails', async () => {
      ...
      await service.saveConfiguration({ ...validConfig, id: 'cfg', name: 'Original' }, activeCtx);
      mockFs.writeFile.mockRejectedValueOnce(new Error('disk full'));

      await expect(
        service.saveConfiguration({ ...validConfig, id: 'cfg', name: 'Updated' }, activeCtx),
    ...
    it('should not leave the config in memory when the disk write fails', async () => {
      ...
      await expect(service.saveConfiguration(validConfig, activeCtx)).rejects.toThrow(/disk full/);
  ...
  describe('exportConfiguration()', () => {
    ...
    it('redacts current and legacy authentication from a tenant export', async () => {
      const stored = credentialBearingConfig();
      await service.saveConfiguration(stored);

      const exported = await service.exportConfigurationForTenant('tenant-a', stored.id);
      const parsed = JSON.parse(exported);

      expect(parsed).toEqual(expect.objectContaining({ id: 'config-secret', name: 'Test Config' }));
      expect(parsed).not.toHaveProperty('sourceAuthentication');
      expect(parsed).not.toHaveProperty('targetAuthentication');
      expect(parsed).not.toHaveProperty('authentication');
      expect(exported).not.toContain('do-not-return');
      expect(stored.sourceAuthentication?.credentials.apiKey)
        .toBe('export-source-secret-do-not-return');
    });

    it('redacts current and legacy authentication from every exportAll member', async () => {
      await service.saveConfiguration(credentialBearingConfig());

      const backup = await service.exportAll() as {
        configurations: Record<string, unknown>[];
      };

      const exported = backup.configurations.find((config) => config.id === 'config-secret');
      expect(exported).toEqual(expect.objectContaining({
        id: 'config-secret',
        name: 'Test Config',
      }));
      expect(exported).not.toHaveProperty('sourceAuthentication');
      expect(exported).not.toHaveProperty('targetAuthentication');
      expect(exported).not.toHaveProperty('authentication');
      expect(JSON.stringify(backup)).not.toContain('do-not-return');
    });
  });

  describe('importConfiguration()', () => {
    it('should import valid JSON configuration', async () => {
      // importConfiguration carries no command context, so only a draft can pass
      // the fail-closed activation gate (an active import needs the gated paths).
      const configJson = JSON.stringify({ ...validConfig, isActive: false });
  ...
  describe('read-accessor immutability (A4)', () => {
    // Draft save (isActive: false) so this suite exercises read-accessor
    // aliasing in isolation from the cardinality activation gate, which is
    // covered separately in ConfigurationService.cardinalityGate.test.ts.
    // Real Date instances (not a loadConfigurations JSON round-trip) so the
    // Date-instance assertions below are meaningful.
    const richConfig = (): IntegrationConfig => ({
      ...validConfig,
      id: 'rich-config',
      tenantId: 'tenant-a',
      isActive: false,
    ...
    beforeEach(async () => {
      await service.saveConfiguration(richConfig());
    });

    function mutate(config: IntegrationConfig): void {
      config.fieldMappings[0].targetField = 'MUTATED';
      (config.sourceAuthentication!.credentials as Record<string, unknown>).apiKey = 'MUTATED';
    }

    it('getConfiguration: mutating a returned record does not affect a fresh read', () => {
      mutate(service.getConfiguration('rich-config')!);

      const fresh = service.getConfiguration('rich-config')!;
      expect(fresh.fieldMappings[0].targetField).toBe('companyname');
      expect((fresh.sourceAuthentication!.credentials as Record<string, unknown>).apiKey).toBe('original-key');
    });

    it('getConfiguration: returned createdAt/updatedAt remain Date instances', () => {
      // Object.prototype.toString rather than toBeInstanceOf(Date): structuredClone
      // under jest-environment-node returns a Date from a different vm realm than
      // this file's Date global, so instanceof fails even for a correct clone. The
      // toString tag is realm-independent and still rules out a JSON stringify/parse
      // implementation, which would turn these into plain strings.
      const fresh = service.getConfiguration('rich-config')!;
      expect(Object.prototype.toString.call(fresh.createdAt)).toBe('[object Date]');
      expect(Object.prototype.toString.call(fresh.updatedAt)).toBe('[object Date]');
    });

    it('getConfigurationForTenant: mutating a returned record does not affect a fresh read', () => {
      mutate(service.getConfigurationForTenant('tenant-a', 'rich-config')!);

      const fresh = service.getConfigurationForTenant('tenant-a', 'rich-config')!;
      expect(fresh.fieldMappings[0].targetField).toBe('companyname');
      expect((fresh.sourceAuthentication!.credentials as Record<string, unknown>).apiKey).toBe('original-key');
    });

    it('getConfigurationForTenant: returned createdAt/updatedAt remain Date instances', () => {
      const fresh = service.getConfigurationForTenant('tenant-a', 'rich-config')!;
      expect(Object.prototype.toString.call(fresh.createdAt)).toBe('[object Date]');
      expect(Object.prototype.toString.call(fresh.updatedAt)).toBe('[object Date]');
    });

    it('getAllConfigurations: mutating a returned record does not affect a fresh read', () => {
      mutate(service.getAllConfigurations()[0]);

      const fresh = service.getAllConfigurations()[0];
      expect(fresh.fieldMappings[0].targetField).toBe('companyname');
      expect((fresh.sourceAuthentication!.credentials as Record<string, unknown>).apiKey).toBe('original-key');
    });

    it('getAllConfigurations: mutating the returned array does not affect internal storage', () => {
      service.getAllConfigurations().push({ ...richConfig(), id: 'injected' });

      expect(service.getAllConfigurations().length).toBe(1);
    });

    it('getAllConfigurationsForTenant: mutating a returned record does not affect a fresh read', () => {
      mutate(service.getAllConfigurationsForTenant('tenant-a')[0]);

      const fresh = service.getAllConfigurationsForTenant('tenant-a')[0];
      expect(fresh.fieldMappings[0].targetField).toBe('companyname');
      expect((fresh.sourceAuthentication!.credentials as Record<string, unknown>).apiKey).toBe('original-key');
    });

    it('exportAll: mutating an exported member does not affect a fresh export', async () => {
      const exported = await service.exportAll();
      exported.configurations[0].fieldMappings[0].targetField = 'MUTATED';

      const fresh = await service.exportAll();
      expect(fresh.configurations[0].fieldMappings[0].targetField).toBe('companyname');
    });
  });
  ...
  describe('importAll()', () => {
    it('should import all configurations from backup', async () => {
      ...
      await service.importAll(backup, activeCtx);
    ...
    it('should delete stale on-disk config files not present in the imported set', async () => {
      ...
      await service.importAll({ configurations: [{ ...validConfig, id: 'imported-1' }] }, activeCtx);
    ...
    it('should keep the prior in-memory version when a backup config write fails (memory matches preserved disk)', async () => {
      ...
      await service.importAll({ configurations: [{ ...validConfig, id: 'cfg', name: 'V2' }] }, activeCtx);
    ...
    it('should NOT delete the prior on-disk file of a backup config whose write fails (no data loss)', async () => {
      ...
      await service.importAll({ configurations: [{ ...validConfig, id: 'kept' }] }, activeCtx);
    ...
    it('should import the same id for different tenants only when distinct ids keep disk safe', async () => {
      ...
      // Bulk restore is a cross-tenant admin op: the gate attributes per-member
      // audit rows to each member's own tenant, so one command context covers both.
      await service.importAll(backup, activeCtx);

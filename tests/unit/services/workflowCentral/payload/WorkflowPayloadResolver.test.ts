...
function buildDlp(result: Partial<PIIDetectionResult> = {}): Pick<DLPService, 'scanForPII'> {
  ...
  return { scanForPII: jest.fn().mockResolvedValue(fullResult) };
}
...
function buildResolver(
  ...
  dlp: Pick<DLPService, 'scanForPII'>,
...
describe('WorkflowPayloadResolver — happy path', () => {
  ...
  it('invokes DLP scanForPII with autoRedact:true on each resolved record', async () => {
    const fakeConnector: Pick<IConnector, 'read'> = {
      read: jest.fn().mockResolvedValue({ name: 'Acme', cardNumber: '4111-1111-1111-1111' }),
    };
    const dlp = buildDlp({ detected: true, redactedData: { name: 'Acme', cardNumber: '****-****-****-1111' } });
    ...
    expect(dlp.scanForPII).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ autoRedact: true, blockOnDetection: false }),
    );
    expect(outcomes[0].status).toBe('resolved');

...
describe('WorkflowCentralRepository', () => {
  ...
  describe('insertTask / getById', () => {
    ...
    // PR-F5b-3: conflict-tolerant variants for fixed-id seed rows. The demo
    // seeds need an insert that never RAISES on a duplicate — a caught
    // constraint violation aborts the surrounding Postgres transaction and
    // fails every later row in the same pass.
    describe('insertTaskIfMissing / insertInstanceIfMissing', () => {
      it('reports true when the task row is new and false when the id exists', async () => {
        const row = makeTaskRow({ id: 'task-if-missing', description: 'original' });
        const first = await db.transaction().execute((tx) => repo.insertTaskIfMissing(tx, row));
        expect(first).toBe(true);

        const second = await db
          .transaction()
          .execute((tx) =>
            repo.insertTaskIfMissing(tx, { ...row, description: 'must not overwrite' }),
          );
        expect(second).toBe(false);
        expect((await repo.getById('tnt_A', 'task-if-missing'))?.description).toBe('original');
      });

      it('reports true when the instance row is new and false when the id exists', async () => {
        const row = makeInstanceRow({ id: 'INST-if-missing', workflowName: 'original' });
        const first = await db
          .transaction()
          .execute((tx) => repo.insertInstanceIfMissing(tx, row));
        expect(first).toBe(true);

        const second = await db
          .transaction()
          .execute((tx) =>
            repo.insertInstanceIfMissing(tx, { ...row, workflowName: 'must not overwrite' }),
          );
        expect(second).toBe(false);
      });

      it('skips a same-id row owned by a DIFFERENT tenant instead of re-keying it', async () => {
        // The ids are global primary keys, so the conflict target is `id` alone.
        // A foreign-tenant row must survive untouched.
        const foreign = makeTaskRow({ id: 'task-cross-tenant-seed', tenantId: 'tnt_B' });
        await db.transaction().execute((tx) => repo.insertTaskIfMissing(tx, foreign));

        const mine = makeTaskRow({ id: 'task-cross-tenant-seed', tenantId: 'tnt_A' });
        expect(await db.transaction().execute((tx) => repo.insertTaskIfMissing(tx, mine))).toBe(
          false,
        );
        expect(await repo.getById('tnt_A', 'task-cross-tenant-seed')).toBeNull();
        expect(await repo.getById('tnt_B', 'task-cross-tenant-seed')).not.toBeNull();
      });

      it('inserts every row of a multi-row seed pass where one id already exists', async () => {
        // The regression shape: with try/catch-inside-TX, the pre-existing id
        // poisoned the transaction and rows after it were lost.
        const existing = makeTaskRow({ id: 'task-batch-2' });
        await db.transaction().execute((tx) => repo.insertTaskIfMissing(tx, existing));

        const batch = ['task-batch-1', 'task-batch-2', 'task-batch-3'].map((id) =>
          makeTaskRow({ id }),
        );
        const results = await db.transaction().execute(async (tx) => {
          const out: boolean[] = [];
          for (const row of batch) out.push(await repo.insertTaskIfMissing(tx, row));
          return out;
        });
        expect(results).toEqual([true, false, true]);
        for (const id of ['task-batch-1', 'task-batch-2', 'task-batch-3']) {
          expect(await repo.getById('tnt_A', id)).not.toBeNull();
        }
      });
    });

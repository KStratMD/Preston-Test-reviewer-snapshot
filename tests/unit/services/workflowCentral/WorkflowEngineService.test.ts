...
describe('WorkflowEngineService', () => {
  ...
  describe('seedDemoDefinitions + getDemoInstanceRows', () => {
    ...
    it('getDemoInstanceRows stamps EVERY row with the caller-supplied tenant', () => {
      const engine = makeEngine();
      const rows = engine.getDemoInstanceRows(CENTRAL_DEMO_TENANT_ID);

      expect(rows).toHaveLength(3);
      expect(rows.every(r => r.tenantId === CENTRAL_DEMO_TENANT_ID)).toBe(true);
      expect(rows.map(r => r.id)).toEqual(['INST-1000', 'INST-1001', 'INST-1002']);
    });

    it('getDemoInstanceRows hardcodes no tenant — an arbitrary caller tenant reaches all 3 rows', () => {
      // F5b-3 blocker 1: two of the three rows once carried a hardcoded
      // SYSTEM_IDENTITY tenant, so a partial fix would still pass the check
      // above for row 1 only. Asserting an unrelated tenant catches that.
      const rows = makeEngine().getDemoInstanceRows('tenant-arbitrary');
    ...
    it('getDemoInstanceRows is pure — does not mutate the instances Map', () => {
      const engine = makeEngine();
      const { instances: before } = engine.getInstances(CENTRAL_DEMO_TENANT_ID);
      engine.getDemoInstanceRows(CENTRAL_DEMO_TENANT_ID);
      const { instances: after } = engine.getInstances(CENTRAL_DEMO_TENANT_ID);

...
import type { DLPScanMetricsRecorder, DLPScanKind, DLPScanOutcome } from '../../../../src/services/security/DLPScanMetrics';
import type { Logger } from '../../../../src/utils/Logger';

describe('DLPService', () => {
  let dlpService: DLPService;
  let mockLogger: Logger;
  let metricsRecorder: jest.Mocked<DLPScanMetricsRecorder>;
  ...
  beforeEach(() => {
    ...
    metricsRecorder = {
      recordScanOutcome: jest.fn<void, [DLPScanKind, DLPScanOutcome]>(),
    };

    dlpService = new DLPService(mockLogger, metricsRecorder);
  });
  ...
  describe('scan completion telemetry', () => {
    const completionCalls = (method: 'debug' | 'info' | 'warn' | 'error', message: string) =>
      (mockLogger[method] as jest.Mock).mock.calls.filter(([loggedMessage]) => loggedMessage === message);

    it('logs clean structured scans once at debug without exposing finding details', async () => {
      await dlpService.scanForPII({ status: 'ok' });

      expect(completionCalls('debug', 'PII scan completed')).toHaveLength(1);
      expect(completionCalls('info', 'PII scan completed')).toHaveLength(0);
      expect(completionCalls('debug', 'PII scan completed')[0][1]).toEqual({
        findingsCount: 0,
        riskLevel: 'low',
        categoryCounts: {},
      });
      expect(metricsRecorder.recordScanOutcome).toHaveBeenCalledTimes(1);
      expect(metricsRecorder.recordScanOutcome).toHaveBeenCalledWith('structured', 'clean');
    });

    it('logs structured findings at info with registry categories only', async () => {
      await dlpService.scanForPII({
        ssn: '123-45-6789',
        contact: 'person@example.com',
      });

      const completion = completionCalls('info', 'PII scan completed');
      expect(completion).toHaveLength(1);
      expect(completion[0][1]).toEqual({
        findingsCount: 2,
        riskLevel: 'critical',
        categoryCounts: { government_id: 1, contact: 1 },
      });
      expect(JSON.stringify(completion[0][1])).not.toContain('123-45-6789');
      expect(JSON.stringify(completion[0][1])).not.toContain('person@example.com');
      // Exactly one increment — a double-count in the finally path must fail here.
      expect(metricsRecorder.recordScanOutcome).toHaveBeenCalledTimes(1);
      expect(metricsRecorder.recordScanOutcome).toHaveBeenCalledWith('structured', 'findings');
    });

    it('logs structured failures with a sanitized error name and records one failed outcome', async () => {
      const sentinel = 'PII-SENTINEL-DO-NOT-LOG';
      jest.spyOn(dlpService as unknown as { scanObject: () => void }, 'scanObject')
        .mockImplementation(() => { throw new Error(sentinel); });

      const result = await dlpService.scanForPII({ value: 'input' });

      expect(result.scanFailed).toBe(true);
      const failure = completionCalls('error', 'PII scan failed');
      expect(failure).toHaveLength(1);
      expect(JSON.stringify(failure[0])).not.toContain(sentinel);
      expect(failure[0][1]).toEqual({ errorName: 'Error' });
      expect(metricsRecorder.recordScanOutcome).toHaveBeenCalledTimes(1);
      expect(metricsRecorder.recordScanOutcome).toHaveBeenCalledWith('structured', 'failed');
    });

    it('records and logs clean and finding text scans at their corresponding levels', async () => {
      await dlpService.scanText('Nothing sensitive here');
      await dlpService.scanText('Contact person@example.com');

      expect(completionCalls('debug', 'PII text scan completed')).toHaveLength(1);
      const findings = completionCalls('info', 'PII text scan completed');
      expect(findings).toHaveLength(1);
      // Text completion metadata must not leak the scanned text or matches.
      expect(JSON.stringify(findings[0])).not.toContain('person@example.com');
      // Exactly two increments total — one per scan, no double counting.
      expect(metricsRecorder.recordScanOutcome).toHaveBeenCalledTimes(2);
      expect(metricsRecorder.recordScanOutcome).toHaveBeenNthCalledWith(1, 'text', 'clean');
      expect(metricsRecorder.recordScanOutcome).toHaveBeenNthCalledWith(2, 'text', 'findings');
    });

    it('logs text scan failures without the thrown message and records one failed outcome', async () => {
      const sentinel = 'PII-TEXT-SENTINEL-DO-NOT-LOG';
      jest.spyOn(dlpService as unknown as { scanString: () => void }, 'scanString')
        .mockImplementation(() => { throw new Error(sentinel); });

      await expect(dlpService.scanText('input')).rejects.toThrow(sentinel);

      const failure = completionCalls('error', 'PII text scan failed');
      expect(failure).toHaveLength(1);
      expect(JSON.stringify(failure[0])).not.toContain(sentinel);
      expect(failure[0][1]).toEqual({ errorName: 'Error' });
      expect(metricsRecorder.recordScanOutcome).toHaveBeenCalledTimes(1);
      expect(metricsRecorder.recordScanOutcome).toHaveBeenCalledWith('text', 'failed');
    });

    it('labels a thrown non-Error as NonErrorThrown without logging the value', async () => {
      jest.spyOn(dlpService as unknown as { scanObject: () => void }, 'scanObject')
        .mockImplementation(() => { throw 'PII-STRING-SENTINEL'; });
      ...
      const failure = completionCalls('error', 'PII scan failed');
      expect(failure).toHaveLength(1);
      expect(JSON.stringify(failure[0])).not.toContain('PII-STRING-SENTINEL');
      expect(failure[0][1]).toEqual({ errorName: 'NonErrorThrown' });
    });

    it('collapses an unsafe error name to Error rather than logging it', async () => {
      const weird = new Error('PII-WEIRD-SENTINEL');
      weird.name = 'Injected Name; with PII-NAME-SENTINEL!';
      jest.spyOn(dlpService as unknown as { scanObject: () => void }, 'scanObject')
        .mockImplementation(() => { throw weird; });

      await dlpService.scanForPII({ value: 'input' });

      const failure = completionCalls('error', 'PII scan failed');
      expect(failure).toHaveLength(1);
      expect(JSON.stringify(failure[0])).not.toContain('PII-NAME-SENTINEL');
      expect(failure[0][1]).toEqual({ errorName: 'Error' });
    });

    it('collapses an identifier-shaped PII error name — allowlist, not syntax', async () => {
      // error.name is writable, and "SSN123456789" is a perfectly valid
      // identifier: any shape-based validation passes it straight into the
      // log. Only the closed allowlist may appear verbatim.
      const smuggler = new Error('boom');
      smuggler.name = 'SSN123456789';
      jest.spyOn(dlpService as unknown as { scanObject: () => void }, 'scanObject')
        .mockImplementation(() => { throw smuggler; });

      await dlpService.scanForPII({ value: 'input' });

      const failure = completionCalls('error', 'PII scan failed');
      expect(failure).toHaveLength(1);
      expect(JSON.stringify(failure[0])).not.toContain('SSN123456789');
      expect(failure[0][1]).toEqual({ errorName: 'Error' });
    });

    it('passes an allowlisted built-in error name through verbatim', async () => {
      jest.spyOn(dlpService as unknown as { scanObject: () => void }, 'scanObject')
        .mockImplementation(() => { throw new TypeError('boom'); });

      await dlpService.scanForPII({ value: 'input' });

      const failure = completionCalls('error', 'PII scan failed');
      expect(failure[0][1]).toEqual({ errorName: 'TypeError' });
    });

    it('counts findings whose type is outside the registry under the fixed unknown bucket', () => {
      // Direct unit of the private helper: registry-sourced findings cannot
      // produce an unregistered type today, so the `?? 'unknown'` arm guards
      // future custom-pattern paths. Per the B2 spec, unknown mappings count
      // under a fixed bucket and the unregistered type string is never used
      // as a key or logged.
      const grouped = (dlpService as unknown as {
        groupFindingsByCategory: (f: Array<{ type: string }>) => Record<string, number>;
      }).groupFindingsByCategory([
        { type: 'ssn' },
        { type: 'not_in_registry_type' },
        { type: 'also_unregistered' },
      ]);

      expect(grouped.unknown).toBe(2);
      // The raw type strings must not appear ANYWHERE in the grouping —
      // neither as keys nor smuggled into values.
      const serialized = JSON.stringify(grouped);
      expect(serialized).not.toContain('not_in_registry_type');
      expect(serialized).not.toContain('also_unregistered');
      expect(grouped).toEqual({ government_id: 1, unknown: 2 });
    });

    it('fails loudly when resolved through a container missing the metrics binding', () => {
      // B2.2 limits the NOOP fallback to DIRECT construction. Through DI, a
      // missing binding must throw, not silently disable telemetry — pins
      // the removal of @optional() on the metrics injection.
      const bare = new Container();
      bare.bind(TYPES.Logger).toConstantValue(mockLogger);
      bare.bind(TYPES.DLPService).to(DLPService);

      expect(() => bare.get(TYPES.DLPService)).toThrow();
    });
  });

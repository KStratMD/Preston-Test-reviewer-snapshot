...
import { GovernanceService, type TenantConfigurationProvider } from '../../../../../src/services/ai/orchestrator/GovernanceService';
...
describe('GovernanceService — DLP commit 2', () => {
  ...
  beforeEach(() => {
    ...
    // C3: commit-2 tests don't exercise per-tenant posture. `detectPII()`
    // itself never touches the posture or the provider (posture is consumed
    // only in `validateInput`/`validateOutput`, neither of which this suite
    // exercises). Stubbing the provider to throw guards against any future
    // refactor that would silently start touching the DB from a code path
    // these tests cover.
    const tcrProvider: TenantConfigurationProvider = () => {
      throw new Error('TCR provider must not be invoked in commit-2 tests (no tenantId passed)');
    };
    governanceService = new GovernanceService(mockLogger, dlpService, tcrProvider);
  });

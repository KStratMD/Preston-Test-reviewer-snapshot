...
import { GovernanceService, type PIIDetectionResult, type TenantConfigurationProvider } from '../../../../../src/services/ai/orchestrator/GovernanceService';
...
describe('GovernanceService — coverage uplift (PR 5)', () => {
  ...
  beforeEach(() => {
    ...
    // C3: coverage tests use `baseContext` without a `tenantId`. `detectPII`
    // itself never touches the provider; per-tenant posture is resolved
    // inside `validateInput`/`validateOutput` only when PII is detected
    // AND a `tenantId` is present on the context. With no tenantId here,
    // posture resolution short-circuits to DEFAULT_POSTURE without a DB
    // read on every code path this suite covers. The throwing stub guards
    // against any future refactor that would unexpectedly start touching
    // the DB from a path these tests cover.
    const tcrProvider: TenantConfigurationProvider = () => {
      throw new Error('TCR provider must not be invoked in coverage tests (no tenantId in baseContext)');
    };
    governance = new GovernanceService(mockLogger, dlpService, tcrProvider);
  ...
  describe('validateInput() PII routing matrix', () => {
    ...
    it('returns empty flags when piiResult hasPII=true but findings are empty', async () => {
      // Mock detectPII to return hasPII: true with empty arrays
      jest.spyOn(governance, 'detectPII').mockResolvedValueOnce({
        hasPII: true,
        piiTypes: [],
        confidence: 0,
        originalText: 'hello',
      ...
      const result = await governance.validateInput('hello', baseContext);
      expect(result.flags).not.toContain('pii_detected');
  ...
  describe('detectPII() shape routing', () => {
    ...
    it('uses fallback value search when finding.location.column is undefined', async () => {
      // Case A: value exists in sourceText
      jest.spyOn(dlpService, 'scanText').mockResolvedValueOnce({
        findings: [
          {
            type: 'EMAIL',
            value: 'user@example.com',
            confidence: 0.9,
            location: { path: '' } // column is undefined
          }
        ],
        piiTypes: ['EMAIL'],
        redactedData: 'contact [REDACTED]'
      });

      const result = await governance.detectPII('contact user@example.com');
      expect(result.piiTypes[0].startIndex).toBe(8); // 'contact '.length === 8

      // Case B: value does not exist in sourceText
      jest.spyOn(dlpService, 'scanText').mockResolvedValueOnce({
        findings: [
          {
            type: 'EMAIL',
      ...
      const resultNotfound = await governance.detectPII('contact user@example.com');
      expect(resultNotfound.piiTypes[0].startIndex).toBe(0); // fallback to 0
    });
  });
  ...
  describe('redactPIIFromData() fallback to legacy index-based path', () => {
    ...
    it('walks nested objects and arrays recursively via redactDataRecursive', () => {
      const data = {
        list: [
          'email: a@b.com',
          { nestedEmail: 'email: a@b.com' }
        ],
        nestedObj: {
          email: 'email: a@b.com'
        }
      ...
      const out = governance.redactPIIFromData(data, piiResult) as any;
      expect(out).not.toBe(data);
      expect(out.list[0]).toBe('email: [REDACTED]');
      expect(out.list[1].nestedEmail).toBe('email: [REDACTED]');
      expect(out.nestedObj.email).toBe('email: [REDACTED]');
    });

...
describe('SyncCentralHealthPredictor', () => {
  ...
  it('uses fixture data even when the retired live-data flag is true', async () => {
    const originalFlag = process.env.USE_REAL_MODULE_APIS;
    ...
    try {
      const result = await service.getAllPredictions();
      ...
      if (originalFlag === undefined) {
        ...
        process.env.USE_REAL_MODULE_APIS = originalFlag;
      }
    }
  });

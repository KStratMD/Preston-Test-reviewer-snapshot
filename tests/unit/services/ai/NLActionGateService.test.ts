...
import { NLActionGateService, ParsedIntent, PER_TENANT_MAX_PENDING, ActionCapacityExceededError } from '../../../../src/services/ai/NLActionGateService';
...
// F4: the store is tenant-scoped — one constant tenant for the pre-F4
// behavior tests ('tnt_T' matches the identity-propagation cases that pass
// an explicit ctx), plus a matching execute ctx for calls that previously
// ran ctx-less (missing ctx now fails closed as not_found).
const F4_TENANT = 'tnt_T';
const F4_EXEC_CTX = { tenantId: F4_TENANT, userId: 'user-1' };
...
describe('NLActionGateService', () => {
    ...
    describe('proposeAction', () => {
        it('should create a proposed action with correct risk level for refund', () => {
            ...
            const proposed = service.proposeAction(intent, F4_TENANT);
    ...
    describe('approveAction', () => {
        it('should approve a pending action', () => {
            ...
            const approved = service.approveAction(proposed.id, 'user-1', F4_TENANT);
            expect(approved).not.toBeNull();
            expect(approved!.status).toBe('approved');
        });

        it('should return null for unknown action ID', () => {
            const result = service.approveAction('nla-nonexistent', 'user-1', F4_TENANT);
            expect(result).toBeNull();
        });
        ...
        it('should return null for already-approved action', () => {
            const intent = service.parseIntent('Refund customer for $50')!;
            const proposed = service.proposeAction(intent, F4_TENANT);
            service.approveAction(proposed.id, 'user-1', F4_TENANT);

            const second = service.approveAction(proposed.id, 'user-1', F4_TENANT);
        ...
        it('should mark expired action as expired', () => {
            ...
            const result = service.approveAction(proposed.id, 'user-1', F4_TENANT);
            expect(result).not.toBeNull();
    ...
    describe('rejectAction', () => {
        it('should reject a pending action', () => {
            ...
            const rejected = service.rejectAction(proposed.id, F4_TENANT, 'Changed my mind');
            expect(rejected).not.toBeNull();
            expect(rejected!.status).toBe('rejected');
        });

        it('should return null for unknown action ID', () => {
            expect(service.rejectAction('nla-nonexistent', F4_TENANT)).toBeNull();
        });

        it('should return null for an already-approved action (no retroactive invalidation)', () => {
            const intent = service.parseIntent('Refund customer for $50')!;
            const proposed = service.proposeAction(intent, F4_TENANT);
            service.approveAction(proposed.id, 'user-1', F4_TENANT);

            expect(service.rejectAction(proposed.id, F4_TENANT, 'too late')).toBeNull();
            expect(proposed.status).toBe('approved');
        });

        it('should return null for an already-rejected action', () => {
            const intent = service.parseIntent('Refund customer for $50')!;
            const proposed = service.proposeAction(intent, F4_TENANT);
            service.rejectAction(proposed.id, F4_TENANT);

            expect(service.rejectAction(proposed.id, F4_TENANT)).toBeNull();
        });
    });
    ...
    describe('executeAction', () => {
        function proposeAndApprove(input: string, paramOverrides?: Record<string, unknown>) {
            ...
            const proposed = service.proposeAction(intent, F4_TENANT);
            service.approveAction(proposed.id, 'test-user', F4_TENANT);
            return proposed;
        }

        it('should return error for non-existent action', async () => {
            const result = await service.executeAction('nla-does-not-exist', F4_EXEC_CTX);
        ...
        it('should return error for non-approved action', async () => {
            ...
            const result = await service.executeAction(proposed.id, F4_EXEC_CTX);
            expect(result.success).toBe(false);
        ...
        it('should dispatch refund to PaymentCentralService.createCreditMemo using ctx.userId', async () => {
            ...
            // the prior hardcoded 'nl-action-gate'. F4: ctx is required — the
            // no-ctx SYSTEM_IDENTITY fallback is retired (fails closed).
        ...
        it('executeAction forwards the provided ctx to dispatch (F4: ctx required, SYSTEM_IDENTITY fallback retired)', async () => {
            ...
            expect(mockFinanceOperator.approveItem).toHaveBeenCalledWith(
                expect.objectContaining({
                    approvalId: 'appr-002',
                    approverId: F4_EXEC_CTX.userId,
                    tenantId: F4_EXEC_CTX.tenantId,
                })
            );
        });
        ...
        it('should dispatch cancel action to SyncCentralService.cancelSubscription', async () => {
            ...
            expect(mockSync.cancelSubscription).toHaveBeenCalledWith(
                F4_TENANT, // identity.tenantId from the required ctx (F4)
                'SUB-999',
                undefined,
                'nl-action-gate'
            );
        ...
        it('should handle missing service gracefully', async () => {
            ...
            const proposed = noServices.proposeAction(intent, F4_TENANT);
            noServices.approveAction(proposed.id, 'user-1', F4_TENANT);
            const result = await noServices.executeAction(proposed.id, F4_EXEC_CTX);
            expect(result.success).toBe(false);
    ...
    describe('getPendingActions', () => {
        it('should return only pending actions', () => {
            ...
            service.proposeAction(intent1, F4_TENANT);
            const intent2 = service.parseIntent('Approve the invoice #DOC1')!;
            const proposed2 = service.proposeAction(intent2, F4_TENANT);
            service.approveAction(proposed2.id, 'user-1', F4_TENANT);

            const pending = service.getPendingActions(F4_TENANT);
        ...
        it('should mark expired actions', () => {
            const intent = service.parseIntent('Refund customer for $50')!;
            service.proposeAction(intent, F4_TENANT);
...
describe('tenant isolation (F4)', () => {
    const A = 'tenant-a';
    const B = 'tenant-b';
    let svc: NLActionGateService;
    let intent: ParsedIntent;

    beforeEach(() => {
        svc = new NLActionGateService(mockLogger);
        // The intent VALUE does not matter for isolation (only the store keys) —
        // a literal avoids coupling these tests to the regex parser.
        intent = { action: 'refund', targetSystem: 'payment', operation: 'POST', parameters: { amount: 50 }, confidence: 1, rawInput: 'refund $50' };
    });
    ...
    it('stamps tenantId at propose', () => {
        const action = svc.proposeAction(intent, A);
        expect(action.tenantId).toBe(A);
    });

    it('cross-tenant list is empty; own-tenant list contains the action', () => {
        const action = svc.proposeAction(intent, A);
        expect(svc.getPendingActions(B).map((a) => a.id)).not.toContain(action.id);
        expect(svc.getPendingActions(A).map((a) => a.id)).toContain(action.id);
    });

    it('cross-tenant approve/reject → null; own-tenant approve retains approvedBy', () => {
        const action = svc.proposeAction(intent, A);
        expect(svc.approveAction(action.id, 'user-b', B)).toBeNull();
        expect(svc.rejectAction(action.id, B)).toBeNull();
        const approved = svc.approveAction(action.id, 'user-a', A);
        expect(approved?.status).toBe('approved');
        expect(approved?.approvedBy).toBe('user-a');
    });

    it('cross-tenant execute → not_found; missing ctx → not_found (fail-closed)', async () => {
        const action = svc.proposeAction(intent, A);
        svc.approveAction(action.id, 'user-a', A);
        const crossTenant = await svc.executeAction(action.id, { tenantId: B, userId: 'user-b' });
        expect(crossTenant).toMatchObject({ success: false, errorCode: 'not_found' });
        const noCtx = await svc.executeAction(action.id);
        expect(noCtx).toMatchObject({ success: false, errorCode: 'not_found' });
    });

    // Codex R2 (#1055): capacity is per-tenant with SELF-eviction only — one
    // tenant's flood must never evict another tenant's actions.
    describe('per-tenant capacity', () => {
        it("tenant A's flood past the cap evicts only A's own oldest; B's action survives", () => {
            const bAction = svc.proposeAction(intent, B);
            const aFirst = svc.proposeAction(intent, A);

            for (let i = 0; i < PER_TENANT_MAX_PENDING + 50; i++) {
                svc.proposeAction(intent, A);
            }

            // A capped at PER_TENANT_MAX_PENDING; its oldest was self-evicted.
            const aPending = svc.getPendingActions(A);
            expect(aPending.length).toBe(PER_TENANT_MAX_PENDING);
            expect(aPending.map((a) => a.id)).not.toContain(aFirst.id);

            // B's single action is untouched and still fully operable.
            expect(svc.getPendingActions(B).map((a) => a.id)).toContain(bAction.id);
            expect(svc.approveAction(bAction.id, 'user-b', B)?.status).toBe('approved');
        });

        // Codex R3 (#1055): eviction is status-aware — an approved action
        // awaiting execution is never removed by the tenant's own flood.
        it('an approved action survives its own tenant flood; oldest PENDING evicted instead', async () => {
            const approved = svc.proposeAction(intent, A);
            svc.approveAction(approved.id, 'user-a', A);
            const oldestPending = svc.proposeAction(intent, A);

            for (let i = 0; i < PER_TENANT_MAX_PENDING + 10; i++) {
                svc.proposeAction(intent, A);
            }

            // The pending entry was evicted; the approved one was not — it is
            // still addressable (anything but not_found; execution itself may
            // fail on missing backend services in this fixture).
            expect(svc.getPendingActions(A).map((a) => a.id)).not.toContain(oldestPending.id);
            const result = await svc.executeAction(approved.id, { tenantId: A, userId: 'user-a' });
            expect(result.errorCode).not.toBe('not_found');
        });

        it('cap held entirely by approved actions → new proposal refused with typed error', () => {
            for (let i = 0; i < PER_TENANT_MAX_PENDING; i++) {
                const a = svc.proposeAction(intent, A);
                svc.approveAction(a.id, 'user-a', A);
            }

            expect(() => svc.proposeAction(intent, A)).toThrow(ActionCapacityExceededError);
            // Other tenants are unaffected by A's saturation.
            expect(svc.proposeAction(intent, B).tenantId).toBe(B);
        });
    });
});

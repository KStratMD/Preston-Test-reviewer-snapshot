...
export function makeMockRepo() {
  return {
    ...
    // Codex P1 (PR #966) — tenant-account guard lookup. Default null =
    // "no embedded provisioning on file", which the guard passes through.
    getEmbeddedPlatformAccountId: jest.fn().mockResolvedValue(null),
  };
}
...
export function makeNsConnectorStub(opts: { createId?: string } = {}) {
  return {
    ...
    // immediately after `ConnectorManager.getConnector()` resolves. Default to
    // a no-op resolved promise; tests exercising the initialize contract
    // directly can still override via `.initialize.mockResolvedValueOnce(...)`.

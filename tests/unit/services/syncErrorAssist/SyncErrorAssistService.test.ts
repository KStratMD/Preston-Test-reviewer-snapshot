...
// fixture AuthConfig — tests that care assert `initialize` was called with it.
...
// PR-C3.1a — default-posture GovernanceService stub for inline constructor calls
// in this test file. Returns DEFAULT_POSTURE (regression-equivalent with pre-C3.1
// behavior). Tests that exercise allowPII / piiTypes / autoRedact override the
// resolved posture via `getPostureForTenant.mockResolvedValueOnce(...)`.
...
describe('SyncErrorAssistService.runCycle', () => {
  ...
  it('Codex P1 (PR #966) — tenant-account guard rejection aborts the tenant cycle ' +
     'before initialize() or any NetSuite read', async () => {
    ...
    const mockNS = { initialize: jest.fn(), search: jest.fn(), create: jest.fn() };
    const mockRepo = {
      getActiveTenants: jest.fn(), claim: jest.fn(),
      ...
      getEmbeddedPlatformAccountId: jest.fn().mockResolvedValue('OTHER_TENANT_ACCT'),
    };
    const service = new SyncErrorAssistService(
      makeMockLogger() as any,
    ...
    buildNetSuiteEnvAuthConfigForTenant.mockRejectedValueOnce(
      new NetSuiteTenantAccountMismatchError('t1', 'OTHER_TENANT_ACCT', 'ENV_ACCT'),
    );

    const providerInfo = { provider: { mode: 'cloud-api' } as any, providerId: 'claude' };
    await expect(service.runCycle('t1', { tenantId: 't1', userId: 'u' }, providerInfo))
      .rejects.toThrow(NetSuiteTenantAccountMismatchError);

    // The guard is the tenant-checked variant, fed by THIS tenant's id + the
    // repo (which owns the embedded-token account lookup). No connector I/O
    // may happen once it rejects.
    expect(buildNetSuiteEnvAuthConfigForTenant).toHaveBeenCalledWith('t1', mockRepo);
    expect(mockNS.initialize).not.toHaveBeenCalled();
    expect(mockNS.search).not.toHaveBeenCalled();
  });
  ...
  it('clamps unrecognized confidence_threshold to "mid" before interpolating into the system prompt', async () => {
    ...
    const mockNS = { initialize: jest.fn().mockResolvedValue(undefined), search: jest.fn().mockResolvedValue([]), create: jest.fn() };
    const mockRepo = {
  ...
  describe('runCycle — per-error happy path', () => {
    it('processes a single error end-to-end: claim, AI, audit, NS write, update', async () => {
      ...
      expect(mockChat).toHaveBeenCalledTimes(1);
      // Defect 1 — NetSuiteConnector.formatDataForNetSuite reads `data.fields ?? {}`;
      // the AI-suggestion payload MUST be wrapped in `{ fields: ... }` or the
      // connector silently writes an empty record.
      expect(mockNS.create).toHaveBeenCalledWith('customrecord_suitecentral_fix_suggestion', {
      ...
      // Defect 2 — connector must be initialized (with the env-derived AuthConfig)
      // before the create() write, or NetSuiteConnector.ensureAuthenticated() would
      // throw TokenError before any network I/O.
      expect(mockNS.initialize).toHaveBeenCalledWith({
        type: 'oauth1',
        credentials: expect.objectContaining({ accountId: 'test-ns-account' }),
      });
      expect(mockNS.initialize.mock.invocationCallOrder[0]).toBeLessThan(mockNS.create.mock.invocationCallOrder[0]);
      // Fix D1 (reviewer follow-up) — initialize() must also precede the FIRST
      // search() call (the polling-path read), not just the later create() write.
      // ns.initialize() gates ensureAuthenticated() for every connector call,
      // including search(); pinning only the create-order left the search-order
      // half of the contract unverified.
      expect(mockNS.initialize.mock.invocationCallOrder[0]).toBeLessThan(mockNS.search.mock.invocationCallOrder[0]);
    ...
    it('skips records with missing id without attempting claim/create', async () => {
      ...
      const mockNS = { initialize: jest.fn().mockResolvedValue(undefined), search: jest.fn().mockResolvedValueOnce([errorRecord]).mockResolvedValueOnce([]), create: jest.fn() };
  ...
  describe('runAllEnabledTenants', () => {
    ...
    it('Fix D1 — RUNCYCLE polling path: connector.initialize() throw (e.g. missing NetSuite env credentials) ' +
       'is isolated per-tenant as aborted, does NOT take down other tenants', async () => {
      // Mirrors the "isolates per-tenant runCycle errors" test above, but the
      // failure originates from ns.initialize() throwing (the env-credentials
      // degrade path — SyncErrorAssistService.ts:421 calls this uncaught-by-design
      // immediately after getConnector() resolves) rather than getConnector()
      // itself throwing. Confirms the per-tenant catch in runAllEnabledTenants
      // treats an initialize() failure the same as a getConnector() failure.
      const order: string[] = [];
      const mockRepo = {
        getActiveTenants: jest.fn().mockResolvedValue(['t1', 't2', 't3']),
        reapStuckProcessing: jest.fn().mockResolvedValue({ reaped: 0, recoveries: [] }),
      ...
      const okNS = { initialize: jest.fn().mockResolvedValue(undefined), search: jest.fn().mockResolvedValue([]), create: jest.fn() };
      // t2's connector resolves fine, but initialize() rejects — simulating
      // NetSuiteEnvCredentialsMissingError (or any other init-time failure).
      const brokenInitNS = {
        initialize: jest.fn().mockRejectedValue(new Error('NetSuite env credentials missing')),
      ...
      const mockConnectorManager = {
        getConnector: jest.fn().mockImplementation(async (_sys: string, key: string) => {
          order.push(key);
          if (key === 'netsuite_t2') return brokenInitNS;
          return okNS;
        }),
      };
      const mockTenantConfig = { getBoolean: jest.fn().mockResolvedValue(true), getString: jest.fn().mockResolvedValue('mid'), getInt: jest.fn() };
      ...
      expect(results.find(r => r.tenantId === 't2')?.aborted).toBe(true);
      expect(mockMetrics.recordCycleOutcome).toHaveBeenCalledWith('t2', 'aborted');
      // t2's broken connector never reached search() — initialize() failed first.
      expect(brokenInitNS.search).not.toHaveBeenCalled();
      // All 3 tenants attempted; t1/t3 unaffected by t2's initialize() failure.
      expect(order).toContain('netsuite_t1');
      expect(order).toContain('netsuite_t2');
      expect(order).toContain('netsuite_t3');
      expect(results.find(r => r.tenantId === 't1')?.aborted).toBeUndefined();
      expect(results.find(r => r.tenantId === 't3')?.aborted).toBeUndefined();
    });
    ...
    it('propagates fatal infrastructure errors (ServiceUnavailableAppError) instead of producing a partial CycleResult[]', async () => {
      ...
      const mockConnectorManager = {
        getConnector: jest.fn().mockImplementation(async (_sys: string, key: string) => {
          ...
          return { initialize: jest.fn().mockResolvedValue(undefined), search: jest.fn().mockResolvedValue([]), create: jest.fn() };
        }),
      };
      const mockTenantConfig = {
        getBoolean: jest.fn().mockResolvedValue(true),
...
describe('ingestWebhook (PR 17c)', () => {
  ...
  it('R23-1 + PR-C3.1a R1 — fire-and-forget log falls back to [redaction-unavailable] when narrowed redaction returns non-string (fail-safe)', async () => {
    // R23-1 — original semantic: DLP detects PII but downstream redaction
    // fails to deliver a usable substituted value → must placeholder, never
    // leak the raw message. Spec §7.1 requires the placeholder.
    //
    // PR-C3.1a R1 — the fail-closed condition shifted from "scan.redactedData
    // missing" (pre-R1: code consumed scan.redactedData) to "narrowed
    // redactData returned non-string" (R1+: code calls dlpService.redactData
    // directly with the filtered findings). Both protect the same invariant.
    stubs.repo.claim.mockResolvedValueOnce(makeClaim({ id: 'c-1' }));
    jest.spyOn(service, 'processClaimedRecord').mockRejectedValueOnce(new Error('contains 123-45-6789'));
    stubs.dlpService.scanText.mockResolvedValueOnce({
      detected: true, piiTypes: ['ssn'],
    ...
    // Force the narrowed redaction to return a non-string to exercise the
    // fail-closed branch in the fire-and-forget log.
    stubs.dlpService.redactData.mockReturnValueOnce(undefined);
  ...
  /* ==================================================================
   * PR-C3.1a — site #4: fire-and-forget catch posture migration.
   * The fire-and-forget log redaction now consumes per-tenant posture
   * at the DECISION layer.
   * ================================================================== */

  it('PR-C3.1a site #4 — posture.allowPII=true: fire-and-forget log records RAW errorMessage (no DLP scan)', async () => {
    stubs.repo.claim.mockResolvedValueOnce(makeClaim({ id: 'c-allow' }));
    jest.spyOn(service, 'processClaimedRecord').mockRejectedValueOnce(new Error('raw error with SSN 123-45-6789'));
    ...
    // scanText was NOT called for the fire-and-forget redaction (allowPII short-circuit).
    expect(stubs.dlpService.scanText).not.toHaveBeenCalled();
    // Log records the RAW message (no redaction).
    const errArg = (stubs.logger.error as jest.Mock).mock.calls.find(
      (c) => c[0] === 'webhook fire-and-forget failed',
    )![1] as Error;
    expect(errArg.message).toBe('raw error with SSN 123-45-6789');
  });

  it('PR-C3.1a site #4 — posture.piiTypes=["ssn"] but scan finds only EMAIL: raw message logged (EMAIL filtered out)', async () => {
    stubs.repo.claim.mockResolvedValueOnce(makeClaim({ id: 'c-filter' }));
    jest.spyOn(service, 'processClaimedRecord').mockRejectedValueOnce(new Error('contact: a@b.com'));
    stubs.governanceService.getPostureForTenant.mockResolvedValue({
      allowPII: false, blockOnDetection: false, autoRedact: true, piiTypes: ['ssn'],
    });
    stubs.dlpService.scanText.mockResolvedValueOnce({
      detected: true, piiTypes: ['email'],
      findings: [{
        type: 'email', value: 'a@b.com', confidence: 0.99,
        location: { path: '' }, severity: 'medium', redactedValue: '[REDACTED:email]',
      }],
      riskLevel: 'medium', recommendation: 'redact',
      redactedData: 'contact: [REDACTED:email]',
    });
    ...
    // EMAIL filtered out by posture.piiTypes=['ssn'] → no relevant findings → raw kept.
    expect(errArg.message).toBe('contact: a@b.com');
  });

  it('PR-C3.1a site #4 — posture.autoRedact=false with findings: placeholder used (no raw, no substituted)', async () => {
    stubs.repo.claim.mockResolvedValueOnce(makeClaim({ id: 'c-noredact' }));
    jest.spyOn(service, 'processClaimedRecord').mockRejectedValueOnce(new Error('SSN 123-45-6789 leaked'));
    stubs.governanceService.getPostureForTenant.mockResolvedValue({
  ...
  it('PR-C3.1a site #4 — DEFAULT_POSTURE: pre-C3.1 redaction behavior preserved (SSN substituted)', async () => {
    stubs.repo.claim.mockResolvedValueOnce(makeClaim({ id: 'c-default' }));
    jest.spyOn(service, 'processClaimedRecord').mockRejectedValueOnce(new Error('SSN 123-45-6789 leaked'));
    // Default posture (testHelpers builder default).
    stubs.dlpService.scanText.mockResolvedValueOnce({
      detected: true, piiTypes: ['ssn'],
      findings: [{
        type: 'ssn', value: '123-45-6789', confidence: 0.99,
    ...
    expect(errArg.message).toBe('SSN [REDACTED:ssn] leaked');
  });
});

...
import { migration as createEmbeddedTokenVersions } from '../../../../src/database/migrations/033-create-embedded-service-token-versions-table';
...
describe('SyncErrorAssistRepository', () => {
  ...
  beforeEach(async () => {
    ...
    await createEmbeddedTokenVersions.run(db, 'sqlite');
    repo = makeRepo(db);
  });
  ...
  describe('getEmbeddedPlatformAccountId (PR #966 Codex P1 tenant-account guard)', () => {
    async function seedTokenVersion(row: {
      tenantId: string;
      platform?: string;
      ...
      await db.insertInto('embedded_service_token_versions').values({
        token_hash: randomUUID(),
        tenant_id: row.tenantId,
        platform: row.platform ?? 'netsuite',
        platform_account_id: row.platformAccountId,
        valid_from: row.validFrom,
        valid_until: '9999-01-01T00:00:00.000Z',
        retired_at: row.retiredAt ?? null,
      }).execute();
    }

    it('returns null when the tenant has no embedded service-token rows', async () => {
      expect(await repo.getEmbeddedPlatformAccountId('tnt_A', 'netsuite')).toBeNull();
    });

    it('returns the platform_account_id of the newest non-retired row (rotation overlap: newest wins)', async () => {
      await seedTokenVersion({ tenantId: 'tnt_A', platformAccountId: 'OLD_ACCT', validFrom: '2026-01-01T00:00:00.000Z' });
      await seedTokenVersion({ tenantId: 'tnt_A', platformAccountId: 'NEW_ACCT', validFrom: '2026-06-01T00:00:00.000Z' });

      expect(await repo.getEmbeddedPlatformAccountId('tnt_A', 'netsuite')).toBe('NEW_ACCT');
    });

    it('ignores retired rows', async () => {
      await seedTokenVersion({
        tenantId: 'tnt_A', platformAccountId: 'RETIRED_ACCT',
        validFrom: '2026-06-01T00:00:00.000Z', retiredAt: '2026-06-02T00:00:00.000Z',
      });
      await seedTokenVersion({ tenantId: 'tnt_A', platformAccountId: 'LIVE_ACCT', validFrom: '2026-01-01T00:00:00.000Z' });

      expect(await repo.getEmbeddedPlatformAccountId('tnt_A', 'netsuite')).toBe('LIVE_ACCT');
    });

    it('scopes by tenant and platform', async () => {
      await seedTokenVersion({ tenantId: 'tnt_B', platformAccountId: 'OTHER_TENANT', validFrom: '2026-06-01T00:00:00.000Z' });
      await seedTokenVersion({
        tenantId: 'tnt_A', platform: 'business_central',
        platformAccountId: 'BC_ACCT', validFrom: '2026-06-01T00:00:00.000Z',
      });

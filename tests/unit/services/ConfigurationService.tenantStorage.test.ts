...
function makeIntegrationConfig(overrides: Partial<{ id: string; tenantId: string; name: string }> = {}) {
  ...
  return {
    ...
    syncMode: 'batch',
    // Draft (isActive: false): this suite exercises segment-safety, tenant-isolated
    // keying, and flat-file layout — not the activation gate — so it deliberately
    // bypasses the gate rather than threading command context through every save.
    isActive: false,
...
describe('storage isolation is deep, not just top-level', () => {
  // saveConfiguration documents "never mutates the caller / caller can never
  // mutate storage ... in both directions". A shallow copy made that true only
  // for top-level scalars: persistWithRollback stores the sanitized candidate
  // itself, so every nested value stayed aliased to the caller's object.
  let tmpDir: string;
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'config-deep-'));
  });
  afterEach(async () => {
    await fs.rm(tmpDir, { recursive: true, force: true });
  });

  it('does not let a caller mutate stored state through the RETURNED config', async () => {
    const svc = new ConfigurationService(silentLogger, tmpDir);
    const returned = await svc.saveConfiguration(makeIntegrationConfig());

    returned.fieldMappings[0].targetField = 'HIJACKED';
    returned.fieldMappings.push({
      sourceField: 'x', targetField: 'y', transformationType: 'direct', isRequired: false,
    } as never);

    const stored = svc.getConfigurationForTenant('tenant-a', 'cfg-1');
    expect(stored?.fieldMappings).toHaveLength(1);
    expect(stored?.fieldMappings[0].targetField).toBe('companyname');
  });

  it('does not let a caller mutate stored state through the config it PASSED IN', async () => {
    const svc = new ConfigurationService(silentLogger, tmpDir);
    const input = makeIntegrationConfig();
    await svc.saveConfiguration(input);

    input.fieldMappings[0].targetField = 'HIJACKED';

    const stored = svc.getConfigurationForTenant('tenant-a', 'cfg-1');
    expect(stored?.fieldMappings[0].targetField).toBe('companyname');
  });

  // The clone is `structuredClone` specifically BECAUSE a JSON round-trip would
  // flatten `createdAt`/`updatedAt` to strings. Without these assertions that
  // stated reason is unproven — a regression to JSON.parse(JSON.stringify(...))
  // would still pass both aliasing tests above.
  // NOTE: asserted via the toString tag, not `toBeInstanceOf(Date)`. Under jest's
  // vm realm `structuredClone` returns a Date whose prototype is the HOST realm's
  // Date, so `instanceof Date` is false here while being true in production
  // (single realm). Asserting instanceof would test the harness; the guarantee
  // that matters is "a live Date, not a string", which the tag + getTime() prove.
  const isDate = (v: unknown) => Object.prototype.toString.call(v) === '[object Date]';

  it('preserves Dates through the clone rather than flattening them to strings', async () => {
    const svc = new ConfigurationService(silentLogger, tmpDir);
    const returned = await svc.saveConfiguration(makeIntegrationConfig());

    expect(isDate(returned.createdAt)).toBe(true);
    expect(isDate(returned.updatedAt)).toBe(true);

    const stored = svc.getConfigurationForTenant('tenant-a', 'cfg-1');
    expect(isDate(stored?.createdAt)).toBe(true);
    expect(isDate(stored?.updatedAt)).toBe(true);
    // Deep copy, not the same object: the returned Date must not be the stored one.
    expect(returned.updatedAt).not.toBe(stored?.updatedAt);
    expect(returned.updatedAt.getTime()).toBe(stored?.updatedAt.getTime());
  });

  // `structuredClone` throws DataCloneError where the previous shallow copy could
  // not. An unguarded throw would leave saveConfiguration with an unclassified
  // 500 for a bad in-process input instead of a rejected one.
  it('rejects a non-serializable value as a ValidationError rather than throwing DataCloneError', async () => {
    const svc = new ConfigurationService(silentLogger, tmpDir);
    const input = makeIntegrationConfig();
    input.fieldMappings[0].defaultValue = () => 'not cloneable';

    await expect(svc.saveConfiguration(input)).rejects.toThrow(ValidationError);

import {
  ...
  InvalidCardinalityFingerprintError,
...
describe('computeCombinedFingerprint', () => {
  const fpA = sha256Hex('fingerprint-a');
  const fpB = sha256Hex('fingerprint-b');
  const fpC = sha256Hex('fingerprint-c');

  it('is order-independent — combining is computed from sorted direction/fingerprint pairs', () => {
    const forward = { direction: 'source_to_target' as const, fingerprint: fpA };
    const backward = { direction: 'target_to_source' as const, fingerprint: fpB };
  ...
  it('changes when a direction fingerprint changes', () => {
    ...
    const b = computeCombinedFingerprint([forward, { ...backward, fingerprint: fpC }]);
    expect(a).not.toBe(b);
  });
...
/**
 * Both hex64 gates (sampleDigest and directional fingerprint) share the same
 * shape rule, so they share the same rejection matrix: empty, short, one
 * character short of 64, one character long, valid-length-but-uppercase, and
 * valid-length-but-non-hex. Each also keeps a real lowercase-64-hex passing
 * control alongside the rejections.
 */
const HEX64_REJECTION_CASES: Array<[string, string]> = [
  ['empty string', ''],
  ['short (10 characters)', 'a'.repeat(10)],
  ['63 characters (one short of 64)', 'a'.repeat(63)],
  ['65 characters (one long)', 'a'.repeat(65)],
  ['uppercase hex (case-sensitive, lowercase only)', 'A'.repeat(64)],
  ['non-hex characters at the correct length', 'g'.repeat(64)],
];

describe('computeReportFingerprint — sampleDigest hex64 validation matrix', () => {
  const validDigest = sha256Hex('valid-sample-digest');
  const base = {
    analyzerVersion: '1.0.0',
    direction: 'source_to_target' as const,
    plan: { sourceEntity: 'Contact', targetEntity: 'Customer' },
  ...
  it('accepts a well-formed lowercase 64-character hex digest', () => {
    expect(() => computeReportFingerprint({ ...base, sampleDigest: validDigest })).not.toThrow();
  });

  it.each(HEX64_REJECTION_CASES)('rejects sampleDigest: %s', (_label, value) => {
    expect(() => computeReportFingerprint({ ...base, sampleDigest: value })).toThrow(
      InvalidCardinalityFingerprintError,
    );
  });
});

describe('computeCombinedFingerprint — directional fingerprint hex64 validation matrix', () => {
  const validFingerprint = sha256Hex('valid-directional-fingerprint');

  it('accepts a well-formed lowercase 64-character hex digest', () => {
    const forward = { direction: 'source_to_target' as const, fingerprint: validFingerprint };
    expect(() => computeCombinedFingerprint([forward])).not.toThrow();
  });

  it.each(HEX64_REJECTION_CASES)('rejects directional fingerprint: %s', (_label, value) => {
    const forward = { direction: 'source_to_target' as const, fingerprint: validFingerprint };
    const malformed = { direction: 'target_to_source' as const, fingerprint: value };
    expect(() => computeCombinedFingerprint([forward, malformed])).toThrow(
      InvalidCardinalityFingerprintError,
    );
  });
});

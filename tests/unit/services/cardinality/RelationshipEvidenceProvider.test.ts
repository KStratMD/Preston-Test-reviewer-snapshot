...
// B3.5: real Salesforce relationship discovery is gated ONLY by
// salesforceRelationshipDiscovery, never by enableSalesforce.
const LIVE_SALESFORCE_DISCOVERY = {
...
describe('RelationshipEvidenceProvider', () => {
  ...
  describe('Salesforce', () => {
    it('treats a manual (real discovery disabled) schema as unavailable, never available with empty edges', async () => {
      const schemaDiscoveryService = new SchemaDiscoveryService({ salesforceRelationshipDiscovery: { mode: 'disabled' } });
    ...
    it('orients an API-discovered child relationship from the parent relationship name to the child foreign key', async () => {
      ...
      try {
        const schemaDiscoveryService = new SchemaDiscoveryService(LIVE_SALESFORCE_DISCOVERY);
        const provider = makeProvider(schemaDiscoveryService);
    ...
    it('throws on a transport failure instead of returning unavailable evidence, with the raw transport text sanitized (B3.5)', async () => {
      const originalFetch = global.fetch;
      ...
      try {
        ...
        await expect(provider.getEvidence('salesforce', 'Account')).rejects.not.toThrow('ECONNREFUSED');
      } finally {
        global.fetch = originalFetch;
      }
    });

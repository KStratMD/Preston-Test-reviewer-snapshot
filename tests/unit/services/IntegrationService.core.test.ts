...
describe('IntegrationService executes managed credential references through ConnectorManager', () => {
  ...
  describe('standard run path (executeStandardSync)', () => {
    ...
    it('counts a record whose transformation fails closed as failed and never writes it (Task A2)', async () => {
      // TransformationEngine.transform() now reports a declared-only mapping as
      // success:false with a per-field error; this path already throws on that
      // and its per-record catch counts the failure. No create/update may run.
      const h = createManagedHarness();
      h.createConnectorSpy.mockImplementation((...args: unknown[]) => {
        const systemType = args[0] as string;
        const connector = makeFakeConnector(systemType);
        (connector.list as jest.Mock).mockResolvedValue([{ id: 'r1', fields: { name: 'Ada' } }]);
        h.connectors.set(systemType, connector);
        return connector;
      });
      h.transformationEngine.transform.mockResolvedValue({
        success: false,
        transformedData: { id: 'r1', fields: { name: 'Ada' } },
        errors: [{ field: 'parts', message: "transformationType 'split' is declared but not executable (mapping name)", severity: 'error' }],
        warnings: [],
      });
      ...
      expect(h.targetConnector().create).not.toHaveBeenCalled();
      expect(h.targetConnector().update).not.toHaveBeenCalled();
    });
  ...
  describe('test path (testIntegration)', () => {
    ...
    it('reports a fail-closed transformation as invalid with the engine error attached (Task A2)', async () => {
      const h = createManagedHarness();
      h.transformationEngine.transform.mockResolvedValue({
        success: false,
        transformedData: { id: 'sample', fields: {} },
        errors: [{ field: 'parts', message: "transformationType 'split' is declared but not executable (mapping name)", severity: 'error' }],
        warnings: [],
      });
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toEqual([expect.stringMatching(/Transformation Error: .*declared but not executable/)]);
    });
  });
  ...
  describe('single-record path (syncSingleRecord)', () => {
    ...
    it('reports a fail-closed transformation as a failed sync and never writes (Task A2)', async () => {
      const h = createManagedHarness();
      h.createConnectorSpy.mockImplementation((...args: unknown[]) => {
        const systemType = args[0] as string;
        const connector = makeFakeConnector(systemType);
        (connector.read as jest.Mock).mockResolvedValue({ id: 'rec-1', fields: { name: 'Ada' } });
        h.connectors.set(systemType, connector);
        return connector;
      });
      h.transformationEngine.transform.mockResolvedValue({
        success: false,
        transformedData: { id: 'rec-1', fields: { name: 'Ada' } },
        errors: [{ field: 'parts', message: "transformationType 'split' is declared but not executable (mapping name)", severity: 'error' }],
        warnings: [],
      });

      const result = await h.service.syncSingleRecordForTenant('tenant-a', 'cfg-managed', 'rec-1');

      expect(result.status).toBe('failed');
      expect(result.recordsFailed).toBe(1);
      expect(result.errors).toEqual([expect.stringMatching(/Transformation failed: .*declared but not executable/)]);
      expect(h.targetConnector().create).not.toHaveBeenCalled();
      expect(h.targetConnector().update).not.toHaveBeenCalled();
    });

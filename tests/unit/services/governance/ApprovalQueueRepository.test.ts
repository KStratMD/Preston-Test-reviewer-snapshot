...
import { migration as addRequestFingerprint } from '../../../../src/database/migrations/062-add-governance-approval-request-fingerprint';
import { migration as createEmbeddedSessions } from '../../../../src/database/migrations/032-create-embedded-sessions-table';
import { migration as createRoleGrants } from '../../../../src/database/migrations/065-create-embedded-role-grants';
import {
...
function makePendingRow(
  ...
  return {
    ...
    // A verified requester by default. The CAS refuses to APPROVE a row whose
    // requester was never verified (Task B4b), so a fixture without this would
    // model a caller that no longer exists in production — every enqueue path
    // now derives provenance. Tests that want the refusal pass it explicitly;
    // see 'refuses to approve an unverified requester' below.
...
describe('ApprovalQueueRepository', () => {
  ...
  beforeEach(async () => {
    ...
    // A8 (062): request_fingerprint + the pending-ownership dedupe index. Same
    // reason 050 is here per the note above — `insertPending` now always
    // references the column, so a fixture missing it tests a schema that no
    // longer exists in production. This enumerated list is fragile by
    // construction; every migration touching this table has to be added here.
    await addRequestFingerprint.run(db, 'sqlite');
    // 065 adds requester_provenance and the requester scope columns to
    // governance_approvals. It also alters embedded_sessions, so that table is
    // created first — the migration is deliberately not tolerant of a missing
    // table, because in a real run a missing embedded_sessions would be an
    // ordering bug and skipping it would leave the columns silently absent.
    await createEmbeddedSessions.run(db, 'sqlite');
    await createRoleGrants.run(db, 'sqlite');
    repo = makeRepo(db);
  });
  ...
  describe('listPendingForTenant + countPendingForTenant', () => {
    ...
    // Copilot round 3: the repository comment claims this list agrees with the
    // service's sodWeak expression, and until now only BEHAVIOUR was asserted.
    // This reads both sources and compares them, so the claim is measured.
    it("UNVERIFIED_PROVENANCE agrees with the service's sodWeak expression", () => {
      const repoSrc = readFileSync(
        join(__dirname, '../../../../src/services/governance/ApprovalQueueRepository.ts'),
        'utf8',
      );
      const svcSrc = readFileSync(
        join(__dirname, '../../../../src/services/governance/ApprovalQueueService.ts'),
        'utf8',
      );
      const repoList = repoSrc
        .match(/UNVERIFIED_PROVENANCE = \[([^\]]*)\]/)?.[1] ?? '';
      const fromRepo = [...repoList.matchAll(/'([a-z_]+)'/g)].map((m) => m[1]).sort();
      const sodWeak = svcSrc.match(/const sodWeak =([\s\S]*?);/)?.[1] ?? '';
      const fromService = [...sodWeak.matchAll(/'([a-z_]+)'/g)].map((m) => m[1]).sort();
      expect(fromRepo).toEqual(['host_asserted', 'unknown']);
      expect(fromService).toEqual(fromRepo);
    });

    // Task B4b at the SINK. The service refuses this too, but the predicate
    // lives in the CAS so a caller holding the repository cannot get round it.
    it.each(['host_asserted', 'unknown'] as const)(
      'refuses to APPROVE a %s requester, and leaves the row pending',
      async (provenance) => {
        const row = makePendingRow({ id: `apr_weak_${provenance}`, tenantId: 'tnt_W', requesterProvenance: provenance });
        await repo.insertPending(row);
        ...
        expect((await repo.getById('tnt_W', row.id))?.status).toBe('pending');
      },
    );

    it('refuses an unsupported decision value rather than silently rejecting', async () => {
      // The snapshot validation added after Codex found a getter TOCTOU. Without
      // it an unexpected value would fall through to the non-approve path and
      // write whatever it said.
      const row = makePendingRow({ id: 'apr_bad_decision', tenantId: 'tnt_W' });
      await repo.insertPending(row);
      await expect(
        repo.decide({
          tenantId: 'tnt_W',
          id: row.id,
          decidedByUserId: 'admin',
          decision: 'approved_maybe',
          decisionReason: null,
          decidedAt: new Date().toISOString(),
        } as never),
      ).rejects.toThrow(/unsupported decision/i);
    });

    it('still allows REJECTING an unverified requester at the sink', async () => {
      // D-SoD prohibits approval, not refusal. Without this, unverifiable rows
      // would be impossible to clear.
      const row = makePendingRow({ id: 'apr_weak_reject', tenantId: 'tnt_W', requesterProvenance: 'host_asserted' });
      await repo.insertPending(row);
      const outcome = await repo.decide({
        tenantId: 'tnt_W',
        id: row.id,
        decidedByUserId: 'admin',
        decision: 'rejected',
        decisionReason: 'unverifiable origin',
        decidedAt: new Date().toISOString(),
      } as never);
  ...
  describe('writeDescriptor round-trip (PR 13b migration 050)', () => {
    ...
    beforeEach(async () => {
      ...
      await addRequestFingerprint.run(db050, 'sqlite');
      await createEmbeddedSessions.run(db050, 'sqlite');
      await createRoleGrants.run(db050, 'sqlite');

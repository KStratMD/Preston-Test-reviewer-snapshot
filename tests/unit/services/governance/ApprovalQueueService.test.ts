...
/**
 * A8 dedupe key. Deterministic so fingerprints are reproducible across runs,
 * and deliberately DIFFERENT from TEST_ENCRYPTION_KEY — reusing one key for
 * both is exactly what `validateApprovalFingerprintKey` refuses in production,
 * and a test fixture that models the forbidden arrangement teaches the wrong
 * shape.
 */
const TEST_FINGERPRINT_KEY = parseApprovalFingerprintKey('1a'.repeat(32));
...
/**
 * The persisted audit writer. Decisions and refusals both go through it, so a
 * test that never asserts on it still has to supply it — an approval service
 * that cannot record what it did is not the service under test.
 */
function mockAuditLogs() {
  return { create: jest.fn().mockResolvedValue({}), insertWithin: jest.fn().mockResolvedValue(undefined) };
}

/** A verified approver, distinct from the requester on the fixture rows. */
const APPROVER = { userId: 'op-1', verifiedRoles: ['approver'], scope: null };
...
function makePersistedApproval(overrides: Partial<PersistedApproval> = {}): PersistedApproval {
  return {
    ...
    tenantId: 't1',
    requesterUserId: 'u1',
    // Migration 065 fields. Defaulting the scope to null keeps the fixture a
    // JWT-shaped request: approvable by any verified approver in the tenant.
    requesterProvenance: 'jwt',
    requesterPlatform: null,
    requesterPlatformAccountId: null,
    operationType: 'ai_call',
    resourceType: 'openai_chat',
    resourceId: 'res-1',
    riskLevel: 'high',
...
describe('ApprovalQueueService.enqueue', () => {
  it('throws InvalidDecisionError when decision.approvalRequired is false', async () => {
    const repo = mockRepo();
    const svc = new ApprovalQueueService(repo as unknown as ApprovalQueueRepository, mockLogger() as any, testEncryption, TEST_FINGERPRINT_KEY, mockAuditLogs() as never);
  ...
  it('logger.info called on success with tenantId + approvalId + operationType', async () => {
    ...
    const svc = new ApprovalQueueService(repo as unknown as ApprovalQueueRepository, logger as any, testEncryption, TEST_FINGERPRINT_KEY, mockAuditLogs() as never);
...
describe('ApprovalQueueService.approve', () => {
  it('calls repo.decide with decision="approved" and returns repo row on outcome="updated"', async () => {
    ...
    repo.getById.mockResolvedValue(makePersistedApproval({ requesterUserId: 'requester-1' }));
    repo.decideWithAudit.mockResolvedValue(outcome);
    const svc = new ApprovalQueueService(repo as unknown as ApprovalQueueRepository, mockLogger() as any, testEncryption, TEST_FINGERPRINT_KEY, mockAuditLogs() as never);

    const result = await svc.approve({ tenantId: 't1', id: 'approval-1', approver: APPROVER });

    expect(repo.decideWithAudit).toHaveBeenCalledTimes(1);
    const args = repo.decideWithAudit.mock.calls[0][0];
  ...
  it('throws ApprovalNotFoundError on repo outcome="not_found"', async () => {
    const repo = mockRepo();
    // The pre-read is what finds a missing row now — the decision never runs.
    repo.getById.mockResolvedValue(null);
    repo.decideWithAudit.mockResolvedValue({ outcome: 'not_found' });
    const svc = new ApprovalQueueService(repo as unknown as ApprovalQueueRepository, mockLogger() as any, testEncryption, TEST_FINGERPRINT_KEY, mockAuditLogs() as never);

    await expect(
      svc.approve({ tenantId: 't1', id: 'missing-id', approver: APPROVER }),
  ...
  it('throws AlreadyDecidedError on repo outcome="already_decided" carrying the persisted row status', async () => {
    const repo = mockRepo();
    const row = makePersistedApproval({ status: 'rejected', requesterUserId: 'requester-1' });
    repo.getById.mockResolvedValue(row);
    repo.decideWithAudit.mockResolvedValue({ outcome: 'already_decided', row });
    const svc = new ApprovalQueueService(repo as unknown as ApprovalQueueRepository, mockLogger() as any, testEncryption, TEST_FINGERPRINT_KEY, mockAuditLogs() as never);

    let captured: unknown;
    try {
      await svc.approve({ tenantId: 't1', id: 'approval-1', approver: APPROVER });
    } catch (e) {
      captured = e;
    }
  ...
  it('throws ApprovalExpiredError on repo outcome="expired"', async () => {
    const repo = mockRepo();
    const row = makePersistedApproval({ status: 'pending', requesterUserId: 'requester-1' });
    repo.getById.mockResolvedValue(row);
    repo.decideWithAudit.mockResolvedValue({ outcome: 'expired', row });
    const svc = new ApprovalQueueService(repo as unknown as ApprovalQueueRepository, mockLogger() as any, testEncryption, TEST_FINGERPRINT_KEY, mockAuditLogs() as never);

    await expect(
      svc.approve({ tenantId: 't1', id: 'approval-1', approver: APPROVER }),
...
describe('ApprovalQueueService.reject', () => {
  ...
  it('calls repo.decide with decision="rejected" + reason on success', async () => {
    const repo = mockRepo();
    const row = makePersistedApproval({ status: 'rejected', decisionReason: 'bad data', requesterUserId: 'requester-1' });
    repo.getById.mockResolvedValue(makePersistedApproval({ requesterUserId: 'requester-1' }));
    repo.decideWithAudit.mockResolvedValue({ outcome: 'updated', row });
    const svc = new ApprovalQueueService(repo as unknown as ApprovalQueueRepository, mockLogger() as any, testEncryption, TEST_FINGERPRINT_KEY, mockAuditLogs() as never);
    ...
    expect(repo.decideWithAudit).toHaveBeenCalledTimes(1);
    expect(repo.decideWithAudit.mock.calls[0][0]).toEqual(expect.objectContaining({
      tenantId: 't1',
      id: 'approval-1',
      decidedByUserId: 'op-1',
      decision: 'rejected',

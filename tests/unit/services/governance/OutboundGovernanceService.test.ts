...
import { OutboundGovernanceService, type OutboundContext } from '../../../../src/services/governance/OutboundGovernanceService';
...
function mockDLP(result: PIIDetectionResult): DLPService {
  return { scanForPII: jest.fn().mockResolvedValue(result), scanText: jest.fn(), getRegisteredPatterns: jest.fn().mockReturnValue([]) } as unknown as DLPService;
}
...
describe('OutboundGovernanceService', () => {
  ...
  it('approves payload with no PII', async () => {
    const svc = new OutboundGovernanceService(mockDLP(makeClean()), mockLogger() as any);
  ...
  it('approves and redacts low-risk PII (email)', async () => {
    ...
    const svc = new OutboundGovernanceService(mockDLP(makeResult([f], red)), mockLogger() as any);
  ...
  it('approves and redacts medium-risk PII (email + phone)', async () => {
    ...
    const svc = new OutboundGovernanceService(mockDLP(makeResult(fs, red)), mockLogger() as any);
  ...
  it('sets approvalRequired for high-risk PII under default queue mode (PR 3B default)', async () => {
    ...
    const svc = new OutboundGovernanceService(mockDLP(makeResult([f], {})), mockLogger() as any);
  ...
  it('blocks any PII in strict mode', async () => {
    const f = makeFinding({ type: 'email' });
    const svc = new OutboundGovernanceService(mockDLP(makeResult([f], {})), mockLogger() as any, { strictMode: true });
  ...
  it('blocks oversized payloads without calling DLP', async () => {
    ...
    const svc = new OutboundGovernanceService(dlp, mockLogger() as any, { maxPayloadBytes: 50 });
  ...
  it('fail-safe blocks when DLP scanner throws', async () => {
    ...
    const svc = new OutboundGovernanceService(dlp, logger as any);
  ...
  it('fail-safe blocks when DLP scanner resolves with scanFailed:true', async () => {
    ...
    const logger = mockLogger();
    const svc = new OutboundGovernanceService(mockDLP(silentFailure), logger as any);
    const d = await svc.validateAIProviderRequest({ msg: 'x' }, ctx());
    expect(d.approved).toBe(false);
    expect(d.approvalRequired).toBe(false);
    expect(d.riskLevel).toBe('high');
  ...
  describe('risk classification matrix', () => {
    ...
    it.each(cases)('%s at %s → %s risk', async (piiType, dest, expected) => {
      ...
      const svc = new OutboundGovernanceService(mockDLP(makeResult([f], {})), mockLogger() as any, { approvalMode: 'queue' });

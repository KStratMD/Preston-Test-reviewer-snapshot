...
describe('BatchProcessingService', () => {
  ...
  beforeEach(() => {
    ...
    mockQueueService = {
      ...
      getAvailability: jest.fn().mockReturnValue('ready'),
  ...
  describe('initialize', () => {
    it('should initialize queue with correct name', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Batch processing service initialized', {
        queueAvailability: 'ready',
      });
    });

    it('should boot and degrade if queue initialization fails', async () => {
      mockQueueService.initializeQueue.mockRejectedValue(new Error('Queue init failed'));

      await expect(service.initialize()).resolves.toBeUndefined();
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Failed to initialize batch processing service; batch processing unavailable',
  ...
  describe('availability gate', () => {
    it('refuses a small batch before the direct-processing path when unavailable', async () => {
      mockQueueService.isReady.mockReturnValue(false);
      mockQueueService.getAvailability.mockReturnValue('disabled');

      const records: DataRecord[] = [{ id: '1', fields: { name: 'Test' } }];

      await expect(service.submitBatch('integration-1', records)).rejects.toThrow(
        'Batch processing is unavailable (queue subsystem is disabled)',
      );
      expect(mockIntegrationService.syncSingleRecord).not.toHaveBeenCalled();

...
describe('HealthCheckService', () => {
  ...
  describe('setFeatureAvailability() — runtime feature truth (#1102)', () => {
    // A feature can be configured on and still be unusable: the batch queue is
    // enabled whenever DISABLE_REDIS is unset, but a Redis that refuses
    // connections leaves it unavailable. Startup calls this after
    // initialization so health reports what is actually true rather than what
    // was requested. Before this PR the payload hardcoded batchProcessing:true
    // for a subsystem nothing could reach.
    it('overrides a configured feature in both directions', () => {
      const service = new HealthCheckService(testConfig);
      // advancedCaching is configured true, so this proves the override really
      // wins over config rather than merely filling an absent key.
      expect((service.getDeploymentInfo().config as any).features.advancedCaching).toBe(true);

      service.setFeatureAvailability('advancedCaching', false);
      expect((service.getDeploymentInfo().config as any).features.advancedCaching).toBe(false);

      service.setFeatureAvailability('advancedCaching', true);
      expect((service.getDeploymentInfo().config as any).features.advancedCaching).toBe(true);
    });

    it('records a feature the config never declared', () => {
      // The real batchProcessing path: startup reports actual availability for
      // a key a given deployment config may not list at all.
      const service = new HealthCheckService(testConfig);
      expect((service.getDeploymentInfo().config as any).features.batchProcessing).toBeUndefined();

      service.setFeatureAvailability('batchProcessing', false);
      expect((service.getDeploymentInfo().config as any).features.batchProcessing).toBe(false);
    });

    it('leaves features without an override falling through to config', () => {
      const service = new HealthCheckService(testConfig);

      service.setFeatureAvailability('advancedCaching', false);

      const features = (service.getDeploymentInfo().config as any).features;
      expect(features.requestOptimization).toBe(true);
    });

    it('applies the override to the /health/detailed features check too', async () => {
      // Regression for the gap Fizz caught on bb5ad6d8b8: the override
      // originally reached getDeploymentInfo() only, so checkFeatureFlags()
      // still reported a feature as enabled from raw config while the
      // subsystem behind it was unreachable. Features are surfaced on two
      // separate paths and both must tell the same truth.
      const service = new HealthCheckService(testConfig);
      service.setFeatureAvailability('advancedCaching', false);

      const req = { path: '/health/detailed' } as Request;
      const json = jest.fn();
      const res = {
      ...
      await service.createHealthCheckMiddleware()(req, res, jest.fn());

      const payload = json.mock.calls[0][0] as HealthCheckResult;
      const enabled = (payload.checks.features.details as { enabled: string[] }).enabled;

      expect(enabled).not.toContain('advancedCaching');
      expect(enabled).toContain('requestOptimization');
    });

    it('does not leak an override between instances', () => {
      const a = new HealthCheckService(testConfig);
      const b = new HealthCheckService(testConfig);

      a.setFeatureAvailability('advancedCaching', false);

      // Instance-level, not module-global — the distinction Honey drew when
      // accepting this mechanism in review.
      expect((b.getDeploymentInfo().config as any).features.advancedCaching).toBe(
        (testConfig.features as Record<string, boolean>).advancedCaching,
      );
    });
  });

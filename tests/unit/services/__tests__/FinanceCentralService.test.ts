...
describe('FinanceCentralService', () => {
  ...
  describe('initialization', () => {
    it('should initialize with demo data', () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        'FinanceCentralService demo data initialized',
        expect.objectContaining({
          glAccounts: expect.any(Number),
          documents: expect.any(Number),
          bankAccounts: expect.any(Number),
  ...
  describe('getCashFlowForecast', () => {
    ...
    it('should have inflow and outflow breakdowns', async () => {
      const forecast = await service.getCashFlowForecast(2);
  ...
  describe('getGLAccounts', () => {
    it('should return all GL accounts', async () => {
      const accounts = await service.getGLAccounts();
    ...
    it('should filter by type', async () => {
      const assetAccounts = await service.getGLAccounts({ type: 'asset' });
    ...
    it('should filter by active status', async () => {
      const activeAccounts = await service.getGLAccounts({ isActive: true });
  ...
  describe('getFinancialDocuments', () => {
    it('should return all documents', async () => {
      const documents = await service.getFinancialDocuments();

      expect(documents.length).toBeGreaterThan(0);
    });

    it('should filter by type', async () => {
      const invoices = await service.getFinancialDocuments({ type: 'invoice' });
    ...
    it('should filter by status', async () => {
      const overdueDocuments = await service.getFinancialDocuments({ status: 'overdue' });
    ...
    it('should filter by amount range', async () => {
      const documents = await service.getFinancialDocuments({ minAmount: 50000, maxAmount: 100000 });
  ...
  describe('recordPayment', () => {
    it('should record payment on existing document', async () => {
      const documents = await service.getFinancialDocuments({ type: 'invoice', status: 'open' });

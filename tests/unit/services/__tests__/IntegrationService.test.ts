...
describe('IntegrationService', () => {
  ...
  beforeEach(() => {
    ...
    service = new IntegrationService(
      ...
      // A2: the standard paths go through ConnectorManager. No connector
      // override here — this suite already jest.mock()s the connector modules,
      // so the manager's registry factory builds exactly those doubles, which
      // is the pre-A2 behavior preserved rather than re-created.
      createStandardPathConnectorManager().manager as any, // 10th: ConnectorManager (Task 8)

...
describe('SyncCentralService', () => {
  ...
  describe('Subscription Management', () => {
    it('should create subscription', async () => {
      const tiers = await service.getPricingTiers();
      if (tiers.length > 0) {
        const subscriptionId = await service.createSubscription(T, {
          customerId: 'cust-123',
          ...
          support: { supportTier: 'standard', incidentsThisMonth: 0, avgResponseTimeHours: 0 },
        });
    ...
    it('should filter subscriptions by status', async () => {
      const result = await service.getSubscriptions(T, { status: ['active'] });
      expect(result).toHaveProperty('subscriptions');
      expect(Array.isArray(result.subscriptions)).toBe(true);
    });
    ...
    it('should filter subscriptions by tier', async () => {
      const tiers = await service.getPricingTiers();
      if (tiers.length > 0) {
        const result = await service.getSubscriptions(T, { tierId: tiers[0].id });
        expect(result).toHaveProperty('subscriptions');
        expect(Array.isArray(result.subscriptions)).toBe(true);
      }
    });
    ...
    it('should filter subscriptions by customer', async () => {
      const result = await service.getSubscriptions(T, { customerId: 'cust-test' });
      expect(result).toHaveProperty('subscriptions');
      expect(Array.isArray(result.subscriptions)).toBe(true);
    });

    it('should support subscription pagination', async () => {
      const result = await service.getSubscriptions(T, { limit: 5, offset: 0 });
  ...
  describe('Usage Tracking', () => {
    ...
    beforeEach(async () => {
      const tiers = await service.getPricingTiers();
      if (tiers.length > 0) {
        subscriptionId = await service.createSubscription(T, {
    ...
    it('should update usage', async () => {
      await service.updateUsage(T, subscriptionId, {
        integrations: 5,
        transactionsThisMonth: 1000,
        apiCallsThisMonth: 5000,
      });

      const result = await service.getSubscriptions(T, { customerId: 'cust-usage' });
    ...
    it('should throw error updating non-existent subscription', async () => {
      await expect(
        service.updateUsage(T, 'non-existent', { integrations: 5 })
      ).rejects.toThrow('Subscription not found');
    });

    it('should check limits', async () => {
      const result = await service.checkLimit(T, subscriptionId, 'apiCallsPerMonth', 100);
    ...
    it('should allow usage within limits', async () => {
      const result = await service.checkLimit(T, subscriptionId, 'users', 1);
    ...
    it('should throw error checking limits for non-existent subscription', async () => {
      await expect(
        service.checkLimit(T, 'non-existent', 'users', 1)
      ).rejects.toThrow('Subscription not found');
    });
  });
  ...
  describe('Usage Alerts', () => {
    ...
    it('should filter alerts by subscription', async () => {
      ...
      if (tiers.length > 0) {
        ...
        const alerts = await service.getUsageAlerts(T, subscriptionId);
        expect(Array.isArray(alerts)).toBe(true);
      }
    });

    it('should acknowledge alert', async () => {
      // Seed deterministically (PR-L / C3): the old version guarded on
      // pre-existing alerts and asserted expect(true) inside the if — on a
      // clean tenant the body never ran and the test could not fail. Drive
      // usage past a finite tier limit so an alert MUST exist, then verify
      // the acknowledgement is actually recorded.
      const tiers = await service.getPricingTiers();
      const tier = tiers.find(
        (t) => Number.isFinite(t.limits.apiCallsPerMonth) && t.limits.apiCallsPerMonth > 0,
      );
      ...
      const subscriptionId = await service.createSubscription(T, {
        ...
        status: 'active',
        billingCycle: 'monthly',
        pricing: { basePrice: 100, addOns: [], totalMonthly: 100, totalAnnual: 1200, currency: 'USD' },
        usage: {
          integrations: 0,
          transactionsThisMonth: 0,
          apiCallsThisMonth: 0,
        ...
        billing: {
          nextBillingDate: Date.now() + 30 * 24 * 60 * 60 * 1000,
          paymentMethod: 'credit_card',
          billingContact: { name: 'Test', email: 'test@example.com', company: 'Test' },
          invoicePreferences: { format: 'pdf', delivery: 'email', terms: 30 },
        },
        contract: {
          startDate: Date.now(),
      ...
      await service.updateUsage(T, subscriptionId, {
        apiCallsThisMonth: tier!.limits.apiCallsPerMonth + 1,
      });

      const alerts = await service.getUsageAlerts(T, subscriptionId);
      expect(alerts.length).toBeGreaterThan(0);

      await service.acknowledgeAlert(T, alerts[0].id, 'admin@example.com');

      const afterAck = await service.getUsageAlerts(T, subscriptionId);
      const acked = afterAck.find((a) => a.id === alerts[0].id);
    ...
    it('should throw error acknowledging non-existent alert', async () => {
      await expect(
        service.acknowledgeAlert(T, 'non-existent-alert', 'admin@example.com')

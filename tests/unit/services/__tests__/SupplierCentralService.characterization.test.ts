...
async function createDocumentUploadVendor(service, tenantId = 'tenant-test') {
  return service.createVendorProfile(tenantId, {
    basicInfo: {
...
describe('SupplierCentralService — characterization (pre-refactor behavior lock)', () => {
  describe('per-tenant isolation', () => {
    it('seeds each tenant with a populated vendor directory', async () => {
      const { service } = createSupplierCentralService();
      const a = await service.getVendorProfiles('tenant-a');
      expect(a.vendors.length).toBeGreaterThan(0);
    });

    it('does not leak a created purchase order across tenants', async () => {
      const { service } = createSupplierCentralService();
      // Seed is now deterministic (vendor IDs are index-based, no wall-clock time), so
      // both tenants get the SAME canonical vendor ids — isolation is proven by the created
      // PO id, with the shared vendor present in both.
      const vendorA = (await service.getVendorProfiles('tenant-a')).vendors[0];
      const vendorBIds = (await service.getVendorProfiles('tenant-b')).vendors.map(v => v.id);
      expect(vendorBIds).toContain(vendorA.id); // deterministic clone: same vendor id in both tenants
      const po = await service.createPurchaseOrder('tenant-a', {
        vendorId: vendorA.id,
        lines: [{ itemName: 'Widget', quantity: 1, unitPrice: 10 }],
      });
      const aIds = (await service.getPurchaseOrdersForVendor('tenant-a', vendorA.id, { limit: 100, offset: 0 })).orders.map(o => o.id);
      const bIds = (await service.getPurchaseOrdersForVendor('tenant-b', vendorA.id, { limit: 100, offset: 0 })).orders.map(o => o.id);
      expect(aIds).toContain(po.id);       // created under tenant-a
      expect(bIds).not.toContain(po.id);   // isolated: not visible to tenant-b (which shares the vendor id)
    });
  });
  ...
  describe('getVendorProfiles', () => {
    ...
    it('filters by stage (only returns matching stage — service:835-837)', async () => {
      ...
      const result = await service.getVendorProfiles('tenant-test', { stage: ['approved'] });
    ...
    it('respects limit while totalCount reflects unfiltered total (service:869-877)', async () => {
      const { service } = createSupplierCentralService();
      const unfiltered = await service.getVendorProfiles('tenant-test', {});
      const limited = await service.getVendorProfiles('tenant-test', { limit: 1 });
  ...
  describe('uploadDocument', () => {
    it('returns a fresh vendor snapshot after upload updates', async () => {
      ...
      const vendorBeforeUpload = await service.getVendorProfile('tenant-test', vendorId);
      ...
      const vendorAfterUpload = await service.getVendorProfile('tenant-test', vendorId);
    ...
    it('strips path traversal and unsafe characters from uploadUrl', async () => {
      ...
      const result = await service.uploadDocument('tenant-test', vendorId, 'w9', {
        fileName: '../../etc/passwd',
        fileSize: 1024,
        mimeType: 'application/pdf',
      });
      ...
      const resultWithSpaces = await service.uploadDocument('tenant-test', vendorId, 'w9', {
  ...
  describe('AI-absent fallbacks', () => {
    ...
    it('assessVendorForApproval returns exact fallback when vendorOnboardingAgent absent (service:947-950)', async () => {
      const { service } = createSupplierCentralService();
      const seeded = await service.getVendorProfiles('tenant-test', {});
      const vendorId = seeded.vendors[0].id;

      const result = await service.assessVendorForApproval('tenant-test', vendorId);
  ...
  describe('approveVendor', () => {
    it('flips stage to approved OR active because BC sync runs inline (service:885-931, 1163-1227)', async () => {
      ...
      await service.approveVendor('tenant-test', vendorId, 'approver@test.com');
      const vendor = await service.getVendorProfile('tenant-test', vendorId);
  ...
  describe('rejectVendor', () => {
    it('flips stage to rejected and stores reason (service:1127-1158)', async () => {
      ...
      await service.rejectVendor('tenant-test', vendorId, 'reviewer@test.com', 'failed compliance');
      const vendor = await service.getVendorProfile('tenant-test', vendorId);
  ...
  describe('acknowledgePurchaseOrder', () => {
    it('returns PO with status acknowledged (service:2022-2097)', async () => {
      ...
      const ack = await service.acknowledgePurchaseOrder('tenant-test', po.id, { acknowledgedBy: 'tester' });
  ...
  describe('createAdvancedShippingNotice', () => {
    it('returns ASN with status created (service:2102-2156)', async () => {
      ...
      await service.acknowledgePurchaseOrder('tenant-test', po.id, { acknowledgedBy: 'tester' });

      const asn = await service.createAdvancedShippingNotice('tenant-test', {
  ...
  describe('getNetSuiteSyncStatus', () => {
    it('returns {summary, recentSyncs, failedSyncs, governance} (service:2511-2581)', async () => {
      const { service } = createSupplierCentralService();
      const status = await service.getNetSuiteSyncStatus('tenant-test');
  ...
  describe('getGovernanceMetrics', () => {
    it('returns {requestsInLastMinute, activeRequests, config, healthStatus} (service:2634-2658)', async () => {
      const { service } = createSupplierCentralService();
      const metrics = service.getGovernanceMetrics('tenant-test');

...
import { createMockOwnershipResolver, createMockAuditService, createMockApprovalQueueService } from '../../../governanceTestUtils';
...
describe('SyncCentralOrchestrator - Schema Drift', () => {
    ...
    describe('executeSync - schema drift block path', () => {
        it('should pass through when schemaRegistry not injected', async () => {
            ...
            const orchestrator = new SyncCentralOrchestrator(logger, connectorManager, undefined, createMockOwnershipResolver() as any, createMockAuditService() as any, createMockApprovalQueueService() as any);
            ...
            const result = await orchestrator.executeSync(op.id, 'test-admin');
        ...
        it('should pass through when no schema registered', async () => {
            ...
            const orchestrator = new SyncCentralOrchestrator(logger, connectorManager, schemaRegistry, createMockOwnershipResolver() as any, createMockAuditService() as any, createMockApprovalQueueService() as any);
            const op = await orchestrator.createOperation({
                name: 'Test Sync',
                sourceSystem: 'hubspot',
                targetSystem: 'netsuite',
        ...
        it('should set operation.status to "error" on block', async () => {
            ...
            await orchestrator.executeSync(op.id, 'test-admin');
        ...
        it('should store blocked result in syncHistory', async () => {
            ...
            const result2 = await orchestrator.executeSync(op.id, 'test-admin');
        ...
        it('should remove operationId from runningOperations after block (finally block runs)', async () => {
            ...
            connectorManager.getConnector
                .mockResolvedValueOnce(sourceConnector as any)
                .mockResolvedValueOnce(targetConnector as any);
            await expect(orchestrator.executeSync(op.id, 'test-admin')).resolves.toBeDefined();
        });
    });
});

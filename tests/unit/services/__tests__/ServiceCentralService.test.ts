  ...
  describe('Technician Management', () => {
    ...
    describe('updateTechnicianStatus', () => {
      it('should update technician status', async () => {
        // Deterministic: demo seed always has available technicians (TECH-001/004).
    ...
    describe('findBestTechnician', () => {
      it('should return technician recommendations for a work order', async () => {
        // Deterministic: demo seed always has open work orders (WO-001/004)
        // and available technicians, so recommendations must be non-empty.
        const orders = await service.getWorkOrders({ status: 'open' });
        expect(orders.length).toBeGreaterThan(0);
      ...
      it('should score technicians based on multiple factors', async () => {
        ...
        // Deterministic: the two available demo technicians are always
        // recommended (base score 50, no cut-off filter).
  ...
  describe('Dispatch Management', () => {
    describe('dispatchTechnician', () => {
      it('should dispatch a technician to a work order', async () => {
        ...
        // Get an available technician (demo seed guarantees at least one)
      ...
      it('should fail for non-existent work order', async () => {
        // Deterministic: demo seed always has available technicians.
        const techs = await service.getTechnicians({ available: true });
        expect(techs.length).toBeGreaterThan(0);
        const result = await service.dispatchTechnician({
      ...
      it('should fail for non-existent technician', async () => {
        // Deterministic: demo seed always has open work orders.
        const orders = await service.getWorkOrders({ status: 'open' });
        expect(orders.length).toBeGreaterThan(0);
        const result = await service.dispatchTechnician({
      ...
      it('should fail for unavailable technician', async () => {
        ...
        // Get a busy technician (demo seed guarantees TECH-002 is on_job)
    ...
    describe('updateDispatchStatus', () => {
      it('should update dispatch status', async () => {
        // Deterministic: demo seed always has active dispatches (DSP-001 working,
        // DSP-002 en_route).
  ...
  describe('SLA Management', () => {
    ...
    describe('getSLAComplianceReport', () => {
      ...
      it('should calculate compliance rate correctly', async () => {
        // Seed deterministically: the demo work orders are never resolved/closed,
        // so totalTickets was 0 and the guarded assertion never ran. Complete a
        // fresh work order so the report MUST include at least one ticket.
        const order = await service.createWorkOrder({
          customerId: 'CUST-1',
          customerName: 'Test',
          title: 'SLA Report Seed',
          description: 'Test',
          priority: 'medium',
          type: 'repair',
          location: { address: '1', city: 'C', state: 'S', zipCode: '1' },
        });
        await service.completeWorkOrder(order.id, { laborHours: 1 });

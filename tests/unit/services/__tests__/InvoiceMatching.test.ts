...
describe('PaymentCentralService - Invoice Matching', () => {
  ...
  describe('createInvoice', () => {
    it('should create an invoice with valid data', async () => {
      ...
      const invoice = await service.createInvoice(TENANT, 'VENDOR-001', invoiceData);
    ...
    it('should allow multiple invoices for same vendor', async () => {
      const invoice1 = await service.createInvoice(TENANT, 'VENDOR-001', {
        vendorId: 'VENDOR-001',
      ...
      const invoice2 = await service.createInvoice(TENANT, 'VENDOR-001', {
        vendorId: 'VENDOR-001',
  ...
  describe('getInvoice', () => {
    it('should retrieve an existing invoice', async () => {
      ...
      const created = await service.createInvoice(TENANT, 'VENDOR-GET', invoiceData);
      const retrieved = await service.getInvoice(TENANT, created.id);
    ...
    it('should return null for non-existent invoice', async () => {
      const invoice = await service.getInvoice(TENANT, 'non-existent-id');
  ...
  describe('getInvoices', () => {
    beforeEach(async () => {
      ...
      await service.createInvoice(TENANT, 'VENDOR-LIST', {
    ...
    it('should list invoices with filters', async () => {
      const result = await service.getInvoices(TENANT, { vendorId: 'VENDOR-LIST' });
    ...
    it('should filter by match status', async () => {
      const result = await service.getInvoices(TENANT, { matchStatus: ['pending'] });

      expect(result.invoices.every((inv: Invoice) => inv.matchStatus === 'pending')).toBe(true);
    });

    it('should respect limit parameter', async () => {
      const result = await service.getInvoices(TENANT, { limit: 1 });
  ...
  describe('autoMatchInvoice', () => {
    it('should attempt auto-matching for pending invoice', async () => {
      ...
      const invoice = await service.createInvoice(TENANT, 'VENDOR-MATCH', invoiceData);
      const result = await service.autoMatchInvoice(TENANT, invoice.id);
    ...
    it('should return error for non-existent invoice', async () => {
      const result = await service.autoMatchInvoice(TENANT, 'non-existent');
  ...
  describe('matchInvoiceToPO', () => {
    it('should match invoice to purchase order', async () => {
      ...
      const invoice = await service.createInvoice(TENANT, 'VENDOR-PO', invoiceData);
      const result = await service.matchInvoiceToPO(TENANT, invoice.id, 'PO-001');
      ...
      const updatedInvoice = await service.getInvoice(TENANT, invoice.id);
  ...
  describe('approveInvoice', () => {
    it('should approve a matched invoice', async () => {
      ...
      const invoice = await service.createInvoice(TENANT, 'VENDOR-APPROVE', invoiceData);

      // First match to PO
      await service.matchInvoiceToPO(TENANT, invoice.id, 'PO-APPROVE-001');

      // Then approve
      const result = await service.approveInvoice(TENANT, invoice.id, 'test-approver');

      expect(result.success).toBe(true);

      // Check the invoice was updated
      const approved = await service.getInvoice(TENANT, invoice.id);
    ...
    it('should return error for pending invoice without match', async () => {
      ...
      const invoice = await service.createInvoice(TENANT, 'VENDOR-REJECT', invoiceData);

      const result = await service.approveInvoice(TENANT, invoice.id, 'test-user');
      expect(result.success).toBe(false);
  ...
  describe('createInvoiceDispute', () => {
    ...
    beforeEach(async () => {
      testInvoice = await service.createInvoice(TENANT, 'VENDOR-DISPUTE', {
    ...
    it('should update invoice status to disputed', async () => {
      ...
      const updatedInvoice = await service.getInvoice(TENANT, testInvoice.id);
      expect(updatedInvoice?.matchStatus).toBe('disputed');
    });

    it('should return error for non-existent invoice', async () => {
      const result = await service.createInvoiceDispute(TENANT, 'non-existent', 'other', 'Test', 'user');
  ...
  describe('resolveDispute', () => {
    ...
    beforeEach(async () => {
      testInvoice = await service.createInvoice(TENANT, 'VENDOR-RESOLVE', {
    ...
    it('should resolve a dispute with credit memo', async () => {
      const result = await service.resolveDispute(TENANT, testDispute.id, {
        type: 'credit_memo',
      ...
      const disputes = await service.getDisputes(TENANT, { status: ['resolved'] });
    ...
    it('should resolve dispute with invoice approval', async () => {
      ...
      const invoice = await service.getInvoice(TENANT, testInvoice.id);
      expect(invoice?.matchStatus).toBe('approved');
    });

    it('should return error for non-existent dispute', async () => {
      const result = await service.resolveDispute(TENANT, 'non-existent', {
        type: 'other',
        description: 'Test',
      }, 'user');
  ...
  describe('createCreditMemo', () => {
    ...
    beforeEach(async () => {
      testInvoice = await service.createInvoice(TENANT, 'VENDOR-CREDIT', {
    ...
    it('should reject credit memo exceeding invoice amount', async () => {
      await expect(
        service.createCreditMemo(TENANT, testInvoice.id, 2000, 'Too much', 'user')
      ).rejects.toThrow('exceeds');
    });

    it('should reject credit memo for non-existent invoice', async () => {
      await expect(
        service.createCreditMemo(TENANT, 'non-existent', 100, 'Test', 'user')
  ...
  describe('getInvoiceStatistics', () => {
    beforeEach(async () => {
      ...
      await service.createInvoice(TENANT, 'VENDOR-STATS', {
    ...
    it('should return invoice statistics', async () => {
      const stats = await service.getInvoiceStatistics(TENANT);
  ...
  describe('getDisputes', () => {
    beforeEach(async () => {
      const invoice = await service.createInvoice(TENANT, 'VENDOR-LIST-DISPUTES', {
      ...
      await service.createInvoiceDispute(TENANT, invoice.id, 'other', 'Test dispute', 'user');
    });

    it('should list disputes', async () => {
      const result = await service.getDisputes(TENANT);
    ...
    it('should filter disputes by status', async () => {
      const result = await service.getDisputes(TENANT, { status: ['open'] });
  ...
  describe('getCreditMemos', () => {
    beforeEach(async () => {
      const invoice = await service.createInvoice(TENANT, 'VENDOR-LIST-CREDITS', {
      ...
      await service.createCreditMemo(TENANT, invoice.id, 100, 'Test credit', 'user');
    });

    it('should list credit memos', async () => {
      const result = await service.getCreditMemos(TENANT);
    ...
    it('should filter by vendor', async () => {
      const result = await service.getCreditMemos(TENANT, { vendorId: 'VENDOR-LIST-CREDITS' });

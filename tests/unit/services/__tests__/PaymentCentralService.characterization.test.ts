...
describe('PaymentCentralService — characterization suite', () => {
  ...
  describe('invoice creation', () => {
    it('createInvoice returns an invoice with an id', async () => {
      const { service } = createPaymentCentralService();
      const invoice = await service.createInvoice('VENDOR-CHAR-001', {
    ...
    it('getInvoices returns {invoices, totalCount} shape', async () => {
      ...
      await service.createInvoice('VENDOR-CHAR-002', {
      ...
      const result = await service.getInvoices({ vendorId: 'VENDOR-CHAR-002' });
    ...
    it('autoMatchInvoice returns {success, candidates} shape (pins return contract)', async () => {
      const { service } = createPaymentCentralService();
      const invoice = await service.createInvoice('VENDOR-CHAR-MATCH', {
    ...
    it('createInvoiceDispute sets invoice paymentStatus to held', async () => {
      const { service } = createPaymentCentralService();
      const invoice = await service.createInvoice('VENDOR-CHAR-DISP', {
      ...
      await service.createInvoiceDispute(invoice.id, 'price_mismatch', 'Price does not match', 'test-user');
    ...
    it('createCreditMemo throws when amount exceeds remaining invoice balance', async () => {
      const { service } = createPaymentCentralService();
      const invoice = await service.createInvoice('VENDOR-CHAR-CREDIT', {
      ...
      await expect(
        service.createCreditMemo(invoice.id, 400, 'Over-credit attempt', 'test-user')

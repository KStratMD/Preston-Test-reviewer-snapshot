...
describe('SupplierCentralService', () => {
  ...
  describe('Vendor Profile Management', () => {
    ...
    it('should return null for non-existent vendor', async () => {
      const profile = await service.getVendorProfile('non-existent-id');
    ...
    it('should update vendor profile', async () => {
      ...
      await service.updateVendorProfile(vendorId, {
        basicInfo: {
      ...
      const updated = await service.getVendorProfile(vendorId);
      expect(updated?.basicInfo.companyName).toBe('Updated Name');
    });

    it('should throw error updating non-existent vendor', async () => {
      await expect(
        service.updateVendorProfile('non-existent', { basicInfo: { companyName: 'Test', taxId: '12-3456789', industry: 'Tech', companySize: 'small' } })
  ...
  describe('Vendor Listing and Filtering', () => {
    beforeEach(async () => {
      // Create test vendors
      await service.createVendorProfile({
    ...
    it('should filter vendors by onboarding stage', async () => {
      const result = await service.getVendorProfiles({ stage: ['profile_complete'] });
      expect(result).toHaveProperty('vendors');
      expect(Array.isArray(result.vendors)).toBe(true);
    ...
    it('should filter vendors by industry', async () => {
      const result = await service.getVendorProfiles({ industry: ['Tech'] });
      expect(result).toHaveProperty('vendors');
      expect(Array.isArray(result.vendors)).toBe(true);
    ...
    it('should filter vendors by company size', async () => {
      const result = await service.getVendorProfiles({ companySize: ['small', 'medium'] });
      expect(result).toHaveProperty('vendors');
      expect(Array.isArray(result.vendors)).toBe(true);
    ...
    it('should filter vendors by source', async () => {
      const result = await service.getVendorProfiles({ source: ['portal'] });
      expect(result).toHaveProperty('vendors');
      expect(Array.isArray(result.vendors)).toBe(true);
    ...
    it('should support pagination', async () => {
      const result = await service.getVendorProfiles({ limit: 1, offset: 0 });
  ...
  describe('Document Upload', () => {
    ...
    beforeEach(async () => {
      vendorId = await service.createVendorProfile({
    ...
    it('should upload insurance certificate', async () => {
      await service.uploadDocument(vendorId, 'insurance_gl', {
    ...
    it('should upload workers comp certificate', async () => {
      await service.uploadDocument(vendorId, 'insurance_wc', {
    ...
    it('should throw error uploading to non-existent vendor', async () => {
      await expect(
        service.uploadDocument('non-existent', 'w9', {
          fileName: 'w9.pdf',
          fileSize: 102400,
          mimeType: 'application/pdf',
        })
  ...
  describe('Vendor Approval Workflow', () => {
    ...
    it('should approve vendor', async () => {
      await service.approveVendor(vendorId, 'approver@company.com', 'All documents verified');
      const vendor = await service.getVendorProfile(vendorId);
    ...
    it('should reject vendor', async () => {
      await service.rejectVendor(vendorId, 'reviewer@company.com', 'Missing required documents');
      const vendor = await service.getVendorProfile(vendorId);
    ...
    it('should throw error approving non-existent vendor', async () => {
      await expect(
        service.approveVendor('non-existent', 'approver@company.com')
      ).rejects.toThrow('Vendor not found');
    });

    it('should throw error rejecting non-existent vendor', async () => {
      await expect(
        service.rejectVendor('non-existent', 'reviewer@company.com', 'Reason')
      ).rejects.toThrow('Vendor not found');
    });
  });
  ...
  describe('Business Central Sync', () => {
    ...
    beforeEach(async () => {
      ...
      await service.approveVendor(vendorId, 'approver@company.com');
    });

    it('should sync approved vendor to Business Central', async () => {
      const result = await service.syncVendorToBusinessCentral(vendorId);
    ...
    it('should handle sync errors gracefully', async () => {
      ...
      const unapprovedId = await service.createVendorProfile({
      ...
      const result = await service.syncVendorToBusinessCentral(unapprovedId);
    ...
    it('should throw error syncing non-existent vendor', async () => {
      await expect(
        service.syncVendorToBusinessCentral('non-existent')
      ).rejects.toThrow('Vendor not found');
    });
  });
  ...
  describe('Portal Activity', () => {
    ...
    it('should filter activity by vendor', async () => {
      ...
      const result = await service.getPortalActivity(vendorId);
      expect(result).toHaveProperty('activities');
      expect(Array.isArray(result.activities)).toBe(true);
    });

    it('should support activity pagination', async () => {
      const result = await service.getPortalActivity(undefined, 5, 0);

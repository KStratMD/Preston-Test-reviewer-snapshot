...
const T = 'demo-tenant'; // tenant used throughout this test suite
...
describe('SyncCentralServiceExtended', () => {
  ...
  describe('Constructor and Initialization', () => {
    ...
    it('should initialize demo subscriptions', async () => {
      const result = await service.getSubscriptions(T);
    ...
    it('should seed each tenant independently on its first access', async () => {
      // A second, never-before-seen tenant gets its OWN freshly-seeded demo set —
      // proving per-tenant lazy seeding, not a shared/global store.
      const first = await service.getSubscriptions(T);
      const other = await service.getSubscriptions('tenant-sync-other');
      expect(first.totalCount).toBe(5);
      expect(other.totalCount).toBe(5);
      // A write in one tenant must not affect the other's seeded set.
      await service.createSubscription('tenant-sync-other', {
        customerId: 'c-iso', customerName: 'Iso', tierId: 'tier_starter', status: 'active',
      } as never);
      expect((await service.getSubscriptions('tenant-sync-other')).totalCount).toBe(6);
      expect((await service.getSubscriptions(T)).totalCount).toBe(5);
    });
  });
  ...
  describe('createSubscription', () => {
    it('should create a subscription and return an ID starting with sub_', async () => {
      const id = await service.createSubscription(T, buildSubscriptionData());
      expect(id).toMatch(/^sub_/);
    });

    it('should store the subscription so it can be retrieved', async () => {
      const id = await service.createSubscription(T,
        buildSubscriptionData({ customerId: 'cust_retrieve_test' }),
      );
      const result = await service.getSubscriptions(T, { customerId: 'cust_retrieve_test' });
    ...
    it('should auto-generate metadata with createdAt and updatedAt timestamps', async () => {
      ...
      const result = await service.getSubscriptions(T, { customerId: 'cust_meta_test' });
    ...
    it('should log subscription creation with relevant details', async () => {
      await service.createSubscription(T, buildSubscriptionData({ customerId: 'cust_log_test' }));
      expect(mockLogger.info).toHaveBeenCalledWith(
    ...
    it('should create multiple subscriptions with unique IDs', async () => {
      const id1 = await service.createSubscription(T, buildSubscriptionData());
      const id2 = await service.createSubscription(T, buildSubscriptionData());
  ...
  describe('getSubscriptions', () => {
    beforeEach(async () => {
      ...
      await service.createSubscription(T,
    ...
    it('should filter by customerId', async () => {
      const result = await service.getSubscriptions(T, { customerId: 'cust_filter_A' });
      expect(result.totalCount).toBe(1);
      expect(result.subscriptions[0].customerName).toBe('Filter A');
    });

    it('should filter by tierId', async () => {
      const result = await service.getSubscriptions(T, { tierId: 'tier_enterprise' });
    ...
    it('should filter by status array', async () => {
      const result = await service.getSubscriptions(T, { status: ['suspended', 'cancelled'] });
      result.subscriptions.forEach(s => {
    ...
    it('should return empty array for no matches', async () => {
      const result = await service.getSubscriptions(T, { customerId: 'nonexistent' });
    ...
    it('should apply custom limit', async () => {
      const result = await service.getSubscriptions(T, { limit: 2 });
    ...
    it('should apply offset for pagination', async () => {
      const allResult = await service.getSubscriptions(T);
      const offsetResult = await service.getSubscriptions(T, { limit: 2, offset: 2 });
      // Deterministic: the demo seed creates 5 subscriptions, so an offset of 2
      // must skip the first 2 items.
      expect(allResult.totalCount).toBeGreaterThan(2);
    ...
    it('should return correct totalCount even with limit/offset', async () => {
      const full = await service.getSubscriptions(T);
      const paginated = await service.getSubscriptions(T, { limit: 1, offset: 0 });
      expect(paginated.totalCount).toBe(full.totalCount);
    });

    it('should return empty subscriptions when offset exceeds total', async () => {
      const result = await service.getSubscriptions(T, { limit: 10, offset: 1000 });
      expect(result.subscriptions).toEqual([]);
      expect(result.totalCount).toBeGreaterThan(0);
    });

    it('should handle empty status array as no filter', async () => {
      const withEmpty = await service.getSubscriptions(T, { status: [] });
      const withNone = await service.getSubscriptions(T, {});
  ...
  describe('updateUsage', () => {
    ...
    beforeEach(async () => {
      subId = await service.createSubscription(T, buildSubscriptionData());
    });

    it('should update individual usage fields', async () => {
      await service.updateUsage(T, subId, { integrations: 2 });
      const result = await service.getSubscriptions(T, { customerId: 'cust_test_001' });
    ...
    it('should update multiple usage fields simultaneously', async () => {
      await service.updateUsage(T, subId, {
        integrations: 3,
    ...
    it('should auto-set lastUsageUpdate timestamp', async () => {
      const before = Date.now();
      await service.updateUsage(T, subId, { integrations: 1 });
      const after = Date.now();
    ...
    it('should update metadata.updatedAt on usage change', async () => {
      const result1 = await service.getSubscriptions(T, { customerId: 'cust_test_001' });
      ...
      const result2 = await service.getSubscriptions(T, { customerId: 'cust_test_001' });
    ...
    it('should trigger approaching_limit alert at 80% usage', async () => {
      ...
      // 80% of 50000 = 40000
      await service.updateUsage(T, subId, { apiCallsThisMonth: 42000 });
      const alerts = await service.getUsageAlerts(T, subId);
    ...
    it('should trigger limit_exceeded alert at 100% usage', async () => {
      // Starter tier has apiCallsPerMonth: 50000
      await service.updateUsage(T, subId, { apiCallsThisMonth: 55000 });
      const alerts = await service.getUsageAlerts(T, subId);
    ...
    it('should not create duplicate alerts for the same metric/type', async () => {
      await service.updateUsage(T, subId, { apiCallsThisMonth: 55000 });
      const alerts1 = await service.getUsageAlerts(T, subId);
      ...
      await service.updateUsage(T, subId, { apiCallsThisMonth: 56000 });
      const alerts2 = await service.getUsageAlerts(T, subId);
  ...
  describe('checkLimit', () => {
    ...
    beforeEach(async () => {
      subId = await service.createSubscription(T,
        buildSubscriptionData({
          tierId: 'tier_starter',
          usage: {
            integrations: 2,
    ...
    it('should allow request within limits', async () => {
      const result = await service.checkLimit(T, subId, 'apiCallsPerMonth', 100);
    ...
    it('should deny request exceeding limit', async () => {
      const result = await service.checkLimit(T, subId, 'apiCallsPerMonth', 25000);
      expect(result.allowed).toBe(false);
    });

    it('should correctly map environments to integrations usage', async () => {
      const result = await service.checkLimit(T, subId, 'environments', 1);
    ...
    it('should correctly map concurrentConnections usage', async () => {
      const result = await service.checkLimit(T, subId, 'concurrentConnections', 1);
    ...
    it('should throw when subscription tier is not found', async () => {
      ...
      const badSubId = await service.createSubscription(T,
        buildSubscriptionData({ tierId: 'tier_nonexistent' }),
      );
      await expect(
        service.checkLimit(T, badSubId, 'apiCallsPerMonth'),
  ...
  describe('getUsageAlerts', () => {
    it('should return empty array when no alerts exist', async () => {
      const subId = await service.createSubscription(T, buildSubscriptionData());
      const alerts = await service.getUsageAlerts(T, subId);
      expect(alerts).toEqual([]);
    });

    it('should return all unresolved alerts when no subscriptionId filter', async () => {
      const alerts = await service.getUsageAlerts(T);
    ...
    it('should sort alerts by severity (critical first) then by timestamp', async () => {
      ...
      const alerts = await service.getUsageAlerts(T, subId);
      // Deterministic: starter limits are 50k apiCalls and 10GB transfer, so the
      // usage above MUST create exactly two alerts — apiCalls limit_exceeded
      // (critical) and dataTransfer approaching_limit (warning) — sorted
      // critical-first.
      expect(alerts.length).toBe(2);
      expect(alerts[0].severity).toBe('critical');
      expect(alerts[0].metric).toBe('apiCalls');
      expect(alerts[1].severity).toBe('warning');
      expect(alerts[1].metric).toBe('dataTransfer');
    });
    ...
    it('should not return resolved alerts', async () => {
      ...
      const alerts = await service.getUsageAlerts(T, subId);
      // Deterministic: 55000/50000 apiCalls MUST create exactly one critical alert.
      expect(alerts.length).toBe(1);
      await service.acknowledgeAlert(T, alerts[0].id, 'admin');
      // Even after acknowledge, alert should still show (acknowledged != resolved)
      const stillShowing = await service.getUsageAlerts(T, subId);
      expect(stillShowing.length).toBe(1);
      expect(stillShowing[0].id).toBe(alerts[0].id);
      expect(stillShowing[0].acknowledgedBy).toBe('admin');
      expect(stillShowing[0].resolvedAt).toBeUndefined();
    });
  });
  ...
  describe('acknowledgeAlert', () => {
    it('should set acknowledgedAt and acknowledgedBy on the alert', async () => {
      ...
      const before = Date.now();
      await service.acknowledgeAlert(T, alertId, 'admin@test.com');

      // Get the alert again - it should still be unresolved but acknowledged
      const allAlerts = await service.getUsageAlerts(T, subId);
    ...
    it('should throw for non-existent alert', async () => {
      await expect(
        service.acknowledgeAlert(T, 'alert_nonexistent', 'admin'),
    ...
    it('should log acknowledgment', async () => {
      ...
      // Deterministic: 55000/50000 apiCalls MUST create an alert.
      expect(alerts.length).toBeGreaterThan(0);
      await service.acknowledgeAlert(T, alerts[0].id, 'ops-team');
      expect(mockLogger.info).toHaveBeenCalledWith(
  ...
  describe('getAnalytics', () => {
    it('should return complete analytics structure', async () => {
      const analytics = await service.getAnalytics(T);
    ...
    it('should have revenue by tier percentages that sum to approximately 100', async () => {
      const analytics = await service.getAnalytics(T);
      // Deterministic: the demo seed guarantees active subscriptions (asserted
      // above in 'should count total and active customers'), so byTier is
      // non-empty and its percentages must sum to 100.
      expect(analytics.revenue.byTier.length).toBeGreaterThan(0);
  ...
  describe('Alert message content', () => {
    ...
    it('should generate suggested action for enterprise tier contact AM', async () => {
      const entSubId = await service.createSubscription(T,
        buildSubscriptionData({ tierId: 'tier_enterprise' }),
      );
      // Enterprise apiCallsPerMonth is 5,000,000
      await service.updateUsage(T, entSubId, { apiCallsThisMonth: 5500000 });
      const alerts = await service.getUsageAlerts(T, entSubId);
      const alert = alerts.find(a => a.type === 'limit_exceeded');
      expect(alert).toBeDefined();

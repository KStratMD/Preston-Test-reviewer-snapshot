...
describe('IntegrationExecutor', () => {
  ...
  describe('executeSync', () => {
    ...
    it('counts a record whose transformation fails closed as failed and never writes it (Task A2)', async () => {
      ...
      // instead of returning partial fields. syncRecord's existing per-record
      // catch must turn that into recordsFailed with NO target write.
      sourceConnector.list.mockResolvedValue([
        { id: 'r1', fields: { name: 'Test' } },
        { id: 'r2', fields: { name: 'Test2' } },
      ]);
      targetConnector.read.mockResolvedValue(null);
      targetConnector.create.mockResolvedValue({ id: 'r1' });
      mockTransformationEngine.transformRecord
        .mockImplementationOnce(async (record: any) => ({ ...record, transformed: true }))
        .mockRejectedValueOnce(
          new TransformationFailedError([{ field: 'parts', message: "transformationType 'split' is declared but not executable (mapping first)", severity: 'error' }]),
        );
    ...
    it('should handle object-type source/target system', async () => {
      ...
      expect(result.status).toBe('success');
      // Prereq B fix: IntegrationExecutor now derives the connector key via
      // connectorKeyForSystem() (the same registry-key projection
      // ConnectorManager itself uses), not the raw getSystemType() spelling
      // — 'Salesforce' -> 'salesforce'. See
      // IntegrationExecutor.connectorKeyConsistency.test.ts for the
      // real-ConnectorManager regression proof.

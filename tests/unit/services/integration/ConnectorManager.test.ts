...
describe('ConnectorManager (registry-driven)', () => {
  function build(): ConnectorManager {
    ...
    // These tests exercise getConnector()/createConnector() directly and never
    // call initializeConnectorsForConfig(), so the resolver double is never
    // invoked — a stub satisfying the constructor signature is sufficient.
    const credentialResolver: ConnectorCredentialResolver = { resolve: jest.fn() };

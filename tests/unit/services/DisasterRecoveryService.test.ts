...
// Trusted command context (Task 8, 'bulk_restore'): restoreFromBackup /
// performFailover now REQUIRE it — there is no HTTP request behind this
// admin/background operation to derive one from, so every call site in this
// suite supplies an explicit, verified-shaped context.
const restoreContext = makeCommandContext({
...
describe('DisasterRecoveryService', () => {
  ...
  describe('restoreFromBackup', () => {
    ...
    it('should throw error if recovery is already in progress', async () => {
      ...
      const restore1 = service.restoreFromBackup(mockBackupId, restoreContext);

      // Try to start second restore immediately
      await expect(service.restoreFromBackup(mockBackupId, restoreContext)).rejects.toThrow('Recovery already in progress');
    ...
    it('should throw error if backup metadata not found', async () => {
      ...
      await expect(service.restoreFromBackup('nonexistent', restoreContext)).rejects.toThrow('Backup metadata not found');
    });
    ...
    it('should throw error if backup file not found', async () => {
      ...
      await expect(service.restoreFromBackup(mockBackupId, restoreContext)).rejects.toThrow('Backup file not found');
    });
    ...
    it('should emit restore:started event', async () => {
      ...
      await simpleService.restoreFromBackup(mockBackupId, restoreContext);

      expect(startedHandler).toHaveBeenCalledWith({ backupId: mockBackupId });
      // Trusted command context (Task 8, 'bulk_restore') threads through to
      // ConfigurationService.importAll exactly as the caller supplied it.
      expect(mockConfigService.importAll).toHaveBeenCalledWith({ test: 'data' }, restoreContext);
    ...
    it('should emit restore:failed event on error', async () => {
      ...
      await expect(service.restoreFromBackup(mockBackupId, restoreContext)).rejects.toThrow();
      expect(failedHandler).toHaveBeenCalled();
    });
  });
  ...
  describe('performFailover', () => {
    it('should emit failover:started event', async () => {
      ...
      try {
        await service.performFailover('staging', restoreContext);
      } catch {
    ...
    it('should call stopServices during failover', async () => {
      ...
      try {
        await service.performFailover('production', restoreContext);
      } catch {
    ...
    it('should emit failover:failed on error', async () => {
      ...
      await expect(service.performFailover('staging', restoreContext)).rejects.toThrow();
      expect(failedHandler).toHaveBeenCalled();
    });
  });

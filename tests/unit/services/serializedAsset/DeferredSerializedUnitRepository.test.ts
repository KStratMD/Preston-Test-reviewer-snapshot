...
describe('DeferredSerializedUnitRepository', () => {
  ...
  describe('touchAttempt', () => {
    it('advances attempt_count and next_attempt_at without changing reason or the payload', async () => {
      ...
      const attemptCount = await repo.touchAttempt('tenant-a', 'cfg-1', 'inv-1', 1, later);
    ...
    it('backs off further on each successive touch, then saturates', async () => {
      ...
      for (let i = 0; i < 14; i += 1) {
        ...
        const attemptCount = await repo.touchAttempt('tenant-a', 'cfg-1', 'inv-1', i + 1, at);
    ...
    it('returns null when no row matches, without creating one', async () => {
      expect(await repo.touchAttempt('tenant-a', 'cfg-1', 'inv-absent', 1, NOW)).toBeNull();
      expect(await repo.countForConfiguration('tenant-a', 'cfg-1')).toBe(0);
    });
    ...
    it('never crosses a tenant or configuration boundary', async () => {
      ...
      expect(await repo.touchAttempt('tenant-b', 'cfg-1', 'inv-1', 1, later)).toBe(2);

      const untouched = (await repo.listForRetry('tenant-a', 'cfg-1', 10)).units[0];
      expect(untouched.attemptCount).toBe(1);
      expect(await repo.touchAttempt('tenant-a', 'cfg-2', 'inv-1', 1, later)).toBeNull();
    });

    it('leaves a row replaced since the snapshot completely unchanged, and reports no advance', async () => {
      // Run A reads the backlog and holds a snapshot at attempt_count = 1.
      await repo.upsertDeferred(input({ reason: 'parent_missing' }), NOW);
      const snapshot = (await repo.listForRetry('tenant-a', 'cfg-1', 10)).units[0];
      expect(snapshot.attemptCount).toBe(1);

      // Run B re-defers the SAME key in the meantime: new payload, new reason,
      // new schedule. The row Run A read no longer exists.
      const replacedAt = new Date(NOW.getTime() + 10 * 60_000);
      await repo.upsertDeferred(
        input({
          reason: 'transient_dependency_failure',
          normalizedPayload: unit({ itemId: 'item-replaced' }),
        }),
        replacedAt,
      );
      const replacement = (await repo.listForRetry('tenant-a', 'cfg-1', 10)).units[0];
      expect(replacement.attemptCount).toBe(2);

      // Run A's touch lands late, still carrying the count it actually read.
      // Key-only matching double-increments the REPLACEMENT to 3 and, because
      // this `now` predates the replacement, drags its schedule backwards —
      // near MAX_DEFERRAL_ATTEMPTS that tips a legitimately rescheduled unit
      // into attempts_exhausted quarantine.
      const staleTouchAt = new Date(NOW.getTime() + 4 * 60_000);
      const result = await repo.touchAttempt(
        'tenant-a',
        'cfg-1',
        'inv-1',
      ...
      expect(new Date(after.nextAttemptAt).getTime()).toBeGreaterThan(staleTouchAt.getTime());
    });

    it('raises instead of guessing when the driver reports no updated-row count', async () => {
      // Coalescing an absent count to zero would report a successful UPDATE as
      // a no-op — indistinguishable from a genuine CAS miss, and silently
      // skipping the back-off. "Zero rows" and "unknown rows" are different
      // facts, so the unknown one is raised.
      const chain = {
        set: () => chain,
        where: () => chain,
        executeTakeFirst: async () => ({}),
      };
      const blindRepo = makeRepo({ updateTable: () => chain } as never);

      await expect(
        blindRepo.touchAttempt('tenant-a', 'cfg-1', 'inv-1', 1, NOW),
      ).rejects.toThrow(/no updated-row count/);
    });

    it('advances exactly once when the snapshot count still matches, and never rereads', async () => {
      await repo.upsertDeferred(input(), NOW);
      const later = new Date(NOW.getTime() + 60_000);

      expect(await repo.touchAttempt('tenant-a', 'cfg-1', 'inv-1', 1, later)).toBe(2);
      // Replaying the SAME expected count is the stale case, not a retry: the
      // row is already at 2, so the second touch must not advance it to 3.
      expect(await repo.touchAttempt('tenant-a', 'cfg-1', 'inv-1', 1, later)).toBeNull();

      const row = (await repo.listForRetry('tenant-a', 'cfg-1', 10)).units[0];
      expect(row.attemptCount).toBe(2);
      expect(new Date(row.nextAttemptAt).getTime()).toBe(
        later.getTime() + computeDeferredBackoffMs(2),
      );
    });
  });

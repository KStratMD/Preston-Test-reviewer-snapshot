...
describe('SerializedAssetSyncService.run — deferred retry-state advancement', () => {
  it('REVIEWER PROBE: an ambiguous parent on a DEFERRED unit advances its backoff', async () => {
    ...
    expect(store.touchAttempt).toHaveBeenCalledWith(TENANT, CONFIG_ID, 'inv-1', 1, RUN_NOW);
  ...
  it('a row at the attempt ceiling is abandoned: never dispatched, backed off once, and reported', async () => {
    ...
    // The 25 is the compare-and-swap guard, and it is THIS row's listed count —
    // not the default 1 and not the 2 the store double returns, so the
    // assertion discriminates the snapshot from any later read.
    expect(store.touchAttempt).toHaveBeenCalledWith(TENANT, CONFIG_ID, 'inv-1', 25, RUN_NOW);
    expect(store.deleteSucceeded).not.toHaveBeenCalled();
  ...
  it('guards the back-off with the count the LISTING returned, never a later read', async () => {
    // The point of the compare-and-swap is that the guard is a value read
    // BEFORE the work happened. A guard re-read at back-off time would always
    // match, making the predicate vacuous — the exact failure this pins.
    const upsert = jest.fn().mockRejectedValue(new Error('Salesforce 503'));
    const store = makeDeferredStore();
    const listed = { ...deferredRow('inv-1', 'SN-1', 'item-1'), attemptCount: 9 };
    const moved = { ...listed, attemptCount: 17 };
    // First listing is what the run acts on; any SUBSEQUENT read sees a row a
    // concurrent run has already advanced.
    store.listDue.mockResolvedValueOnce(deferredPage([listed])).mockResolvedValue(deferredPage([moved]));
    store.listForRetry.mockResolvedValueOnce(deferredPage([listed])).mockResolvedValue(deferredPage([moved]));

    const h = makeHarness({ records: [], store, products: { 'item-1': [{ Id: 'p1' }] }, upsertImpl: upsert });
    await h.run();

    expect(store.touchAttempt).toHaveBeenCalledWith(TENANT, CONFIG_ID, 'inv-1', 9, RUN_NOW);
    expect(store.touchAttempt).not.toHaveBeenCalledWith(TENANT, CONFIG_ID, 'inv-1', 17, RUN_NOW);
  });

  it('carries the deferred row\'s count onto a unit the FRESH listing also returned', async () => {
    // The fresh values win for the payload (they are newer), but the durable
    // row is still what the back-off must compare against — this is the merge
    // branch that mutates an existing work item rather than creating one, and
    // it is the one that would silently keep an undefined guard.
    const upsert = jest.fn().mockRejectedValue(new Error('Salesforce 503'));
    const backlogged = { ...deferredRow('inv-1', 'SN-STALE', 'item-1'), attemptCount: 6 };
    const store = makeDeferredStore([backlogged]);
    const h = makeHarness({
      records: [sourceRecord('inv-1', 'SN-FRESH', 'item-1')],
      store,
      products: { 'item-1': [{ Id: 'p1' }] },
      upsertImpl: upsert,
    });
    ...
    expect(store.touchAttempt).toHaveBeenCalledTimes(1);
    expect(store.touchAttempt).toHaveBeenCalledWith(TENANT, CONFIG_ID, 'inv-1', 6, RUN_NOW);
    // The fresh payload is what was dispatched, so this really is the merged item.
    expect(upsert).toHaveBeenCalledWith(
      'Asset',
      ASSET_EXTERNAL_ID_FIELD,
      'inv-1',
      expect.objectContaining({ SerialNumber: 'SN-FRESH' }),
    );
  });
  ...
  describe('undecodable deferred rows', () => {
    ...
    it('quarantines the row, backs it off, and never dispatches it', async () => {
      ...
      // The 3 is the corrupt row's OWN listed count: an undecodable row has no
      // work item to carry a snapshot, so the identity-only projection has to
      // supply the compare-and-swap guard itself.
      expect(store.touchAttempt).toHaveBeenCalledWith(TENANT, CONFIG_ID, 'inv-CORRUPT', 3, RUN_NOW);
      expect(store.deleteSucceeded).not.toHaveBeenCalled();

...
describe('FileUploadService', () => {
  ...
  describe('exportFieldMappingsToCSV()', () => {
    it('should export mappings to CSV format', () => {
      ...
      expect(csv).toBe(
        'sourceField,targetField,transformationType,transformationValue,isRequired,validation,condition\r\n' +
        'source1,target1,direct,,true,,\r\n' +
        'source2,target2,lookup,,false,,'
      );
    });
    ...
    it('should include transformation rules in export', () => {
      const mappings: FieldMapping[] = [
        {
          sourceField: 'source,雪',
          targetField: 'target"name',
          transformationType: 'calculation',
          isRequired: true,
          transformationConfig: { type: 'calculation', expression: 'line1\nline2' }
        },
      ];
      ...
      const rules: TransformationRule[] = [
        {
          ...
          action: 'transform',
          condition: 'ready\r\nnow',
          parameters: { sourceField: 'source,雪', targetField: 'target"name' }
        },
      ];
      ...
      expect(csv).toBe(
        'sourceField,targetField,transformationType,transformationValue,isRequired,validation,condition\r\n' +
        '"source,雪","target""name",calculation,"line1\nline2",true,"line1\nline2","ready\r\nnow"'
      );
    });
    ...
    it('preserves row count, order and values in a bounded large export', async () => {
      const mappings: FieldMapping[] = Array.from({ length: 1000 }, (_, index) => ({
        sourceField: `source-${index}`,
        targetField: `target-${index}`,
        transformationType: 'direct',
        isRequired: index % 2 === 0,
      }));
      const csv = service.exportFieldMappingsToCSV(mappings);
      // The independent csv-parser import path checks every serialized row.
      const parsed = await service.parseFile({
        originalname: 'large-export.csv',
        buffer: Buffer.from(csv),
      } as Express.Multer.File);

      expect(parsed.totalRows).toBe(1000);
      expect(parsed.rows).toEqual(mappings.map(mapping => ({

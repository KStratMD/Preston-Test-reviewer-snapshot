...
describe('SecureConfigurationService', () => {
  ...
  describe('getSecureIntegration', () => {
    it('should throw for nonexistent integration', async () => {
      await expect(service.getSecureIntegration('nonexistent'))
        .rejects.toThrow('Integration nonexistent not found');
    });

    it('should resolve credentials for secret_manager source', async () => {
      // First save a config with secret_manager credential source
      ...
      const result = await service.getSecureIntegration('int-resolve');
    ...
    it('should log and throw on credential resolution failure', async () => {
      ...
      await expect(service.getSecureIntegration('int-resolve-fail'))
  ...
  describe('updateIntegrationCredentials', () => {
    it('should throw for nonexistent integration', async () => {
      await expect(service.updateIntegrationCredentials('nonexistent', 'source', {}))
        .rejects.toThrow('Integration nonexistent not found');
    });
    ...
    it('should rotate credentials for secret_manager source', async () => {
      ...
      // createSecureIntegration overwrites systemId from sourceSystemConfig
      await service.createSecureIntegration(
        config, makeSystemConfig({ systemId: 'src-1' }), makeSystemConfig({ systemType: 'netsuite' }),
      );

      await service.updateIntegrationCredentials('int-update', 'source', { newKey: 'updated' });
    ...
    it('should throw when system is a plain string (no credential source)', async () => {
      // Directly save a config with string targetSystem (bypassing createSecureIntegration)
      // by using getConfiguration on a config saved through parent
      const config = makeIntegrationConfig({ id: 'int-no-sm' });
      // Save through parent's saveConfiguration (which stores string sourceSystem/targetSystem)
      await (service as any).saveConfiguration(config);

      await expect(service.updateIntegrationCredentials('int-no-sm', 'target', {}))

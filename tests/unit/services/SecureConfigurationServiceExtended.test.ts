...
import type { PreflightRunResult } from '../../../src/types/cardinality';
...
// Mock config validation to always pass. The canonical Zod schema DOES accept
// object sourceSystem/targetSystem since Prerequisite PR B (2026-07-27
// serialized-asset plan), but most fixtures in this file are legacy plain
// strings with no sourceAuthentication supplied — this suite exercises
// credential storage/resolution behavior, not schema validation, so the mock
// keeps it isolated from the credential-pair cross-field rules (covered by
// tests/unit/schemas/configurationSchemas.credentials.test.ts instead).
...
function makeIntegrationConfig(overrides: Record<string, any> = {}) {
  return {
    ...
    syncMode: 'batch',
    // Draft: SecureConfigurationService.createSecureIntegration saves without
    // command context, so an active secure save fails the activation gate closed.
    // This suite covers credential storage/resolution, not activation, so it uses
    // drafts. Threading command context through the secure-save path is separate
    // caller-plumbing work (design: 'secure_save' operation kind).
    isActive: false,
    fieldMappings: [
...
// createSecureIntegration now REQUIRES trusted command context (Task 8, 'secure_save'
// operation kind) — it can persist active configuration. Every fixture in this suite
// is a draft (isActive: false; see makeIntegrationConfig), so the gate is bypassed
// regardless of context content; this constant exists purely to satisfy the
// (now-required) 4th positional parameter. Threading a REAL, verified context is
// exercised in SecureConfigurationService.commandContext.test.ts.
const draftContext = makeCommandContext({ operation: 'secure_save' });
...
describe('SecureConfigurationService', () => {
  ...
  describe('createSecureIntegration', () => {
    it('should store source and target credentials', async () => {
      ...
      await service.createSecureIntegration(
        config, sourceSystem, targetSystem, draftContext,
        { apiKey: 'source-key' },
        { apiKey: 'target-key' },
      );
    ...
    it('should skip credential storage when neither provided', async () => {
      ...
      await service.createSecureIntegration(config, sourceSystem, targetSystem, draftContext);
    ...
    it('should log success', async () => {
      ...
      await service.createSecureIntegration(
        config, makeSystemConfig(), makeSystemConfig({ systemType: 'netsuite' }), draftContext,
      );
    ...
    it('should set credential rotation from source system metadata', async () => {
      ...
      await service.createSecureIntegration(
        config, sourceSystem, makeSystemConfig({ systemType: 'netsuite' }), draftContext,
      );
  ...
  describe('updateIntegrationCredentials', () => {
    ...
    it('should rotate credentials for secret_manager source', async () => {
      ...
      await service.createSecureIntegration(
        config, makeSystemConfig({ systemId: 'src-1' }), makeSystemConfig({ systemType: 'netsuite' }), draftContext,
      );
  ...
  describe('migrateToSecureCredentials', () => {
    it('should call credential manager migration', async () => {
      ...
      const result = await service.migrateToSecureCredentials(draftContext);
    ...
    it('should throw on critical migration failure', async () => {
      ...
      await expect(service.migrateToSecureCredentials(draftContext))
    ...
    it('does not corrupt the stored record when a read-derived migration candidate is mutated and the save fails (A4)', async () => {
      // Mirrors what migrateToSecureCredentials does with its per-integration
      // read: obtain a live config via a read accessor, then mutate/delete
      // nested authentication on it while building a migration candidate.
      // Pre-fix, getConfiguration returns the SAME object stored in the map,
      // so the delete below corrupts live storage immediately, and
      // persistWithRollback's failure-path "restore previous" is a no-op
      // because `previous` IS that already-corrupted object.
      const config = makeIntegrationConfig({
        id: 'int-migrate-fail',
        sourceAuthentication: { type: 'api_key', credentials: { apiKey: 'original-key' } },
      });
      await service.saveConfiguration(config, draftContext);

      const candidate = service.getConfiguration('int-migrate-fail')!;
      delete candidate.sourceAuthentication;

      const mockWriteFile = require('fs').promises.writeFile as jest.Mock;
      mockWriteFile.mockRejectedValueOnce(new Error('disk full'));

      await expect(
        service.saveConfiguration({ ...candidate, name: 'Migrated' }, draftContext),
      ).rejects.toThrow('disk full');

      const fresh = service.getConfiguration('int-migrate-fail');
      expect(fresh?.sourceAuthentication).toEqual({
        type: 'api_key',
...
/**
 * Cardinality activation gate thread-through (review fix, 2026-07-26).
 *
 * `SecureConfigurationService`'s constructor previously called `super(logger,
 * configDirectory)` without the optional `CardinalityActivationGate`, so an
 * ACTIVE secure save could never reach an injected gate — it always hit the
 * inherited fail-closed branch (no gate wired -> ServiceUnavailableAppError),
 * silently defeating the DI-bound gate for `createSecureIntegration` and
 * `migrateToSecureCredentials`. These tests construct the service WITH a
 * gate double (mirroring `ConfigurationService.cardinalityGate.test.ts`) to
 * prove the gate is now actually reachable through the secure-save path, on
 * both the clean-activation and blocking-preflight branches.
 */
describe('SecureConfigurationService cardinality activation gate thread-through', () => {
  const gateLogger = {
    debug: jest.fn(),
    info: jest.fn(),
    warn: jest.fn(),
  ...
  function makeActiveConfig(overrides: Record<string, any> = {}) {
    return {
      id: `int-active-${Math.random().toString(36).substr(2, 5)}`,
      tenantId: 'gate-tenant',
      name: 'Active Gate Integration',
      sourceSystem: 'Salesforce',
      targetSystem: 'NetSuite',
      sourceEntity: 'Account',
      targetEntity: 'Customer',
  ...
  const trustedContext = makeCommandContext({
    tenantId: 'gate-tenant',
    operation: 'secure_save',
  });

  function blockingResult(fp = 'fp-secure-1'): PreflightRunResult {
    return makePreflightRunResult({
      blocking: true,
      combinedFingerprint: fp,
      reports: [makeReport({ findings: [makeFinding({ key: 'k1', overrideable: true, severity: 'blocking' })] })],
    });
  }
  ...
  it('activates an ACTIVE secure save through a clean preflight (gate reachable, not fail-closed)', async () => {
    const gate = makeGateDouble(makeCleanPreflight());
    const service = new SecureConfigurationService(gateLogger, './test-config-gate', gateCredentialManager, gate);

    await service.createSecureIntegration(
      makeActiveConfig({ id: 'int-gate-clean' }),
      { systemId: 'sys-1', systemType: 'salesforce', name: 'Salesforce Prod', config: {}, credentialSource: 'secret_manager' },
      { systemId: 'sys-2', systemType: 'netsuite', name: 'NetSuite Prod', config: {}, credentialSource: 'secret_manager' },
      trustedContext,
    );

    expect(gate.preflight.runForConfig).toHaveBeenCalled();
    expect(gate.audit.logCardinalityDecision).toHaveBeenCalledWith(
      expect.objectContaining({ decision: 'allowed', configurationId: 'int-gate-clean' }),
    );
    const result = await service.getSecureIntegration('gate-tenant', 'int-gate-clean');
    expect(result).toBeDefined();
  });

  it('propagates a 422 CardinalityViolationError when preflight blocks an ACTIVE secure save', async () => {
    ...
    try {
      await service.createSecureIntegration(
        makeActiveConfig({ id: 'int-gate-blocked' }),
        { systemId: 'sys-1', systemType: 'salesforce', name: 'Salesforce Prod', config: {}, credentialSource: 'secret_manager' },
        { systemId: 'sys-2', systemType: 'netsuite', name: 'NetSuite Prod', config: {}, credentialSource: 'secret_manager' },
        trustedContext,
      );
    ...
    expect(caught).toBeInstanceOf(CardinalityViolationError);
    expect(caught).toMatchObject({ statusCode: 422, errorCode: 'CARDINALITY_VIOLATION' });

    // Nothing persisted — the blocked save never reaches saveConfiguration's commit path.
    await expect(service.getSecureIntegration('gate-tenant', 'int-gate-blocked')).rejects.toThrow(
      'Integration int-gate-blocked not found',
    );
  });

  it('fails closed (503) on an ACTIVE secure save when no gate is wired at all', async () => {
    const service = new SecureConfigurationService(gateLogger, './test-config-gate', gateCredentialManager);

    await expect(
      service.createSecureIntegration(
        makeActiveConfig({ id: 'int-gate-none' }),
        { systemId: 'sys-1', systemType: 'salesforce', name: 'Salesforce Prod', config: {}, credentialSource: 'secret_manager' },
        { systemId: 'sys-2', systemType: 'netsuite', name: 'NetSuite Prod', config: {}, credentialSource: 'secret_manager' },
        trustedContext,
      ),

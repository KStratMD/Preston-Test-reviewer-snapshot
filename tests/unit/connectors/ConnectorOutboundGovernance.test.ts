import { authenticateBusinessFixture } from '../../authenticationTestUtils';
...
import { createMockOutboundGovernanceService, createMockOwnershipResolver, createMockAuditService, createMockConnectorManager, createMockSerializedAssetSyncService } from '../../governanceTestUtils';
...
/** Preserve real URI normalization while stubbing dispatch with a DataRecord fixture. */
function makeHttpClient() {
  return Object.assign(axios.create({ baseURL: 'https://fixture.invalid' }), {
...
async function makeNetSuite(governance = createMockOutboundGovernanceService()) {
  const c = new NetSuiteConnector('ns', makeLogger(), makeAuthService(), governance);
  (c as any).httpClient = makeHttpClient();
  await authenticateBusinessFixture(c);
  (c as any).accountId = 'ACCT';
  (c as any).baseUrl = 'https://acct.api.netsuite.com';
  return { c, governance };
}

async function makeSalesforce(governance = createMockOutboundGovernanceService()) {
...
async function makeBusinessCentral(governance = createMockOutboundGovernanceService()) {
...
async function makeHubSpot(governance = createMockOutboundGovernanceService()) {
...
async function makeShipStation(governance = createMockOutboundGovernanceService()) {
...
describe('validateConnectorWrite() is called for each operation', () => {
  describe('NetSuiteConnector', () => {
    it('calls validateConnectorWrite on create', async () => {
      const { c, governance } = await makeNetSuite();
  ...
  describe('SalesforceConnector', () => {
    it('calls validateConnectorWrite on create', async () => {
      const { c, governance } = await makeSalesforce();
  ...
  describe('BusinessCentralConnector', () => {
    it('calls validateConnectorWrite on create', async () => {
      const { c, governance } = await makeBusinessCentral();
      await c.create('customer', { id: '1', externalId: 'e1', fields: { name: 'Acme' } });
      expect(governance.validateConnectorWrite).toHaveBeenCalledTimes(1);
    });
  ...
  describe('HubSpotConnector', () => {
    it('calls validateConnectorWrite on create', async () => {
      const { c, governance } = await makeHubSpot();
  ...
  describe('ShipStationConnector', () => {
    it('calls validateConnectorWrite on create', async () => {
      const { c, governance } = await makeShipStation();
...
describe('Approved redacted payload is sent to makeRequest when guard redacts', () => {
  it('sends redacted payload to makeRequest on NetSuite create', async () => {
    ...
    const { c } = await makeNetSuite(governance);
...
describe('Block path — GovernanceBlockedError thrown, makeRequest not called', () => {
  ...
  it('throws GovernanceBlockedError on Salesforce create when blocked', async () => {
    const governance = createMockOutboundGovernanceService();
    governance.validateConnectorWrite.mockResolvedValueOnce(makeBlockedDecision());
    const { c } = await makeSalesforce(governance);
    await expect(
  ...
  it('throws GovernanceBlockedError on BusinessCentral create when blocked', async () => {
    const governance = createMockOutboundGovernanceService();
    governance.validateConnectorWrite.mockResolvedValueOnce(makeBlockedDecision());
    const { c } = await makeBusinessCentral(governance);
    await expect(
      c.create('customer', { id: '1', externalId: 'e1', fields: { ssn: '123-45-6789' } }),
    ).rejects.toBeInstanceOf(GovernanceBlockedError);
    expect((c as any).httpClient.request).not.toHaveBeenCalled();
  ...
  it('throws GovernanceBlockedError on HubSpot create when blocked', async () => {
    const governance = createMockOutboundGovernanceService();
    governance.validateConnectorWrite.mockResolvedValueOnce(makeBlockedDecision());
    const { c } = await makeHubSpot(governance);
    await expect(
  ...
  it('throws GovernanceBlockedError on ShipStation create when blocked', async () => {
    const governance = createMockOutboundGovernanceService();
    governance.validateConnectorWrite.mockResolvedValueOnce(makeBlockedDecision());
    const { c } = await makeShipStation(governance);
    await expect(
...
describe('ConnectorManager produces protected connectors', () => {
  it('ConnectorManager passes governance to NetSuiteConnector', async () => {
    const governance = createMockOutboundGovernanceService();
    // Never calls initializeConnectorsForConfig() — a stub resolver satisfies
    // the constructor signature (Prerequisite PR B).
    const credentialResolver = { resolve: jest.fn() };
    const manager = new ConnectorManager(makeLogger(), makeAuthService(), governance, credentialResolver as any);
    ...
    await authenticateBusinessFixture(connector);
...
describe('IntegrationService factory path', () => {
  it('passes governance to connectors created via getConnector (NetSuite)', async () => {
    ...
    const mockOwnershipResolver = createMockOwnershipResolver();
    const mockAuditService = createMockAuditService();
    const mockApprovalQueueService = { enqueue: jest.fn().mockResolvedValue('q-id') };

    // ObservabilityService is optional (5th); outboundGovernance 6th;
    // ownershipResolver 7th; auditService 8th; approvalQueueService 9th
    // (added in PR 13b Stage A2.5 — required because guardedWrite needs it
    // for the queue_for_human path even if this fixture never hits it).
    const service = new IntegrationService(
      ...
      undefined,
      governance,
      mockOwnershipResolver as any,
      mockAuditService as any,
      mockApprovalQueueService as any,
      createMockConnectorManager() as any,
...
// ---------------------------------------------------------------------------
// 10. A2 standard execution path — parity with the pre-A2 factory path
// ---------------------------------------------------------------------------

/**
 * Section 9 above proves the LEGACY `IntegrationService.getConnector` factory
 * wires governance. That path is no longer how a sync reaches a connector: PR
 * A2 (deployment-readiness Tranche A) routes run/test/single-record/initialize
 * through `ConnectorManager` instead, and the manager builds connectors with
 * ITS OWN injected `OutboundGovernanceService`, not the service's.
 *
 * Section 9 is still worth keeping — `getSystemHealth()` remains a caller of
 * the legacy factory — but on its own it would now be proving governance for a
 * path no sync takes. This section closes that gap end to end: a real
 * `runIntegrationForTenant()` over a real `ConnectorManager` building a real
 * `SalesforceConnector`, asserting the write actually crossed
 * `validateConnectorWrite`.
 *
 * Deliberately NOT asserted by spying on `validateOutboundWrite`: that would
 * pin the mechanism rather than the guarantee. The guarantee is that the
 * governance service the MANAGER holds sees the write, which is exactly what
 * changes hands in A2.
 */
describe('A2 standard execution path crosses the same outbound guard', () => {
  const A2_CONFIG = {
    id: 'cfg-a2-governance',
    tenantId: 'tenant-a',
    name: 'A2 governance parity',
    ...
    fieldMappings: [],
    transformationRules: [],
    sourceAuthentication: { type: 'api_key', credentials: { apiKey: 'inline-source' } },
    targetAuthentication: { type: 'api_key', credentials: { apiKey: 'inline-target' } },
  };

  const buildStandardPath = async () => {
    const governance = createMockOutboundGovernanceService();
    const credentialResolver = {
      resolve: jest.fn(async (config: any, side: 'source' | 'target') =>
        side === 'source' ? config.sourceAuthentication : config.targetAuthentication,
      ),
    };
    ...
    // Pre-create through the manager so these are the SAME cached instances the
    // sync will retrieve (`${connectorKey}_${config.id}`), then give them the
    // transport and auth state a real connector needs. Nothing about the
    // governance wiring is stubbed.
    const source = await manager.getConnector('netsuite', `netsuite_${A2_CONFIG.id}`);
    // `initialize()` is stubbed on BOTH sides only because these are real
    // connectors that reject a credential shape they cannot use (NetSuite
    // demands oauth1), and this section is about the WRITE guard, not the
    // credential handshake. That the manager resolves and initializes each side
    // — and refuses before creating a connector when it cannot — is proven
    // against the real resolver and registry in
    // `tests/unit/services/IntegrationService.core.test.ts`.
    jest.spyOn(source, 'initialize').mockResolvedValue(undefined);
    (source as any).httpClient = makeHttpClient();
    await authenticateBusinessFixture(source);
    (source as any).accountId = 'ACCT';
    (source as any).baseUrl = 'https://acct.api.netsuite.com';
    jest
      .spyOn(source, 'list')
      .mockResolvedValue([{ id: 'src-1', externalId: 'e1', fields: { Name: 'Acme' } } as any]);

    const target = await manager.getConnector('salesforce', `salesforce_${A2_CONFIG.id}`);
    jest.spyOn(target, 'initialize').mockResolvedValue(undefined);
    const targetHttp = makeHttpClient();
    targetHttp.request
      .mockResolvedValueOnce({ data: { id: 'r1', success: true, errors: [] } })
      .mockResolvedValue({ data: { Id: 'r1', Name: 'Acme', externalId: 'e1', fields: {} } });
    (target as any).httpClient = targetHttp;
    await authenticateBusinessFixture(target);
    (target as any).demoMode = false;
    (target as any).accessToken = 'tok';
    (target as any).instanceUrl = 'https://sf.example.com';
    // First read: no existing target record, so the sync takes the create()
    // path — the one the outbound guard protects. Subsequent reads are the
    // connector's own post-write read-back of what it just created.
    jest
      .spyOn(target, 'read')
      .mockResolvedValueOnce(null)
      .mockResolvedValue({ id: 'r1', externalId: 'e1', fields: { Name: 'Acme' } } as any);
    ...
    const service = new IntegrationService(
      makeLogger(),
      {
        transform: jest.fn().mockResolvedValue({
          success: true,
          transformedData: { id: 'src-1', externalId: 'e1', fields: { Name: 'Acme' } },
          errors: [],
          warnings: [],
        }),
      } as any,
      {
        loadConfigurations: jest.fn().mockResolvedValue(undefined),
        getConfiguration: jest.fn().mockReturnValue(A2_CONFIG),
        getConfigurationForTenant: jest.fn().mockReturnValue(A2_CONFIG),
        getAllConfigurations: jest.fn().mockReturnValue([]),
      } as any,
      makeAuthService(),
      {
      ...
      // The SERVICE's governance is a different instance on purpose: if the
      // sync were still reaching connectors through the legacy factory, this
      // one would be the one invoked and the manager's would stay untouched.
      createMockOutboundGovernanceService(),
      createMockOwnershipResolver() as any,
      createMockAuditService() as any,
      { enqueue: jest.fn().mockResolvedValue('q-id') } as any,
      manager as any,
      createMockSerializedAssetSyncService() as any,
    );

    return { service, governance, target };
  };

  it('invokes the manager-wired outbound guard on the record write', async () => {
    const { service, governance } = await buildStandardPath();

    const result = await service.runIntegrationForTenant('tenant-a', A2_CONFIG.id, {});
  ...
  it('blocks the write when that guard refuses, so the record fails instead of landing', async () => {
    const { service, governance, target } = await buildStandardPath();
    (governance.validateConnectorWrite as jest.Mock).mockResolvedValue(makeBlockedDecision());
    const targetRequest = (target as any).httpClient.request as jest.Mock;
    targetRequest.mockClear();
    ...
    // The guard is not advisory on this path: nothing reached the wire.

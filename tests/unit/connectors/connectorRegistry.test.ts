...
import { CONNECTOR_STATUSES, isProductionTier } from '../../../src/connectors/connectorStatuses';

// Sourced from the shared vocabulary, not restated: restating it is how a
// relabel escapes a test that was written to catch exactly that.
const VALID_STATUSES: ReadonlySet<ProductionStatus> = new Set(CONNECTOR_STATUSES);
...
describe('CONNECTOR_REGISTRY (PR 6A)', () => {
  ...
  it('every production-TIER entry (production or production_ready) has a factory closure and a proofCardPath', () => {
    // The tier, not the literal: a relabel must not empty this assertion.
    const productionEntries = CONNECTOR_REGISTRY.filter((e) => isProductionTier(e.productionStatus));
  ...
  it('the partition matches metrics.json: production=5 beta=1 demo_only=11 stub=1', () => {
    ...
    // Option B (D-Demote, approved 2026-09-02): the five readiness-complete
    // connectors are production_ready -- production TIER, no live evidence.
    expect(partition).toEqual({ production: 0, production_ready: 5, beta: 1, demo_only: 11, stub: 1 });
  });
});
...
describe('connector registry helpers', () => {
  it('getConnectorRegistration returns the entry for a known key', () => {
    ...
    expect(ns?.productionStatus).toBe('production_ready');
  });
  ...
  it('listProductionConnectors returns only production-tier entries', () => {
    ...
    for (const entry of list) {
      // The function returns the production TIER; today every member is production_ready.
      expect(entry.productionStatus).toBe('production_ready');
    }
  });

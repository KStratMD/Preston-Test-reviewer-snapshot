...
// Real hooks and transport dispatch: probe failures terminate and a later
// attempt can run again without a retained in-flight attempt.
...
describe.each(cases)('%s authentication termination', (_name, make, expectedPath) => {
  it('executes the real auth probe and terminates under permanent 401', async () => {
    ...
    try {
      ...
      if (connector instanceof ShopifyConnector) await connector.initialize(configs.Shopify);
      ...
      await expect(connector['makeRequest']({ method: 'GET', url: '/probe' })).rejects.toThrow();
      expect(request.mock.calls.length).toBeGreaterThan(previousCalls);
    } finally {

...
class AuthenticationHarness extends StripeConnector {
...
describe('StripeConnector', () => {
  ...
  beforeEach(async () => {
    ...
    connector = new AuthenticationHarness('Stripe', 'stripe-test', mockLogger);
    adapter = jest.fn(async (config: InternalAxiosRequestConfig) => ({ status: 200, statusText: 'OK', headers: {}, config, data: { id: 'acct_123' } }));
    connector.setAdapter(adapter);
    await connector.initialize({ type: 'api_key', credentials: { apiKey: 'sk_live_123' } });
    adapter.mockClear();
  ...
  describe('authentication', () => {
    ...
    it('should authenticate successfully and set headers', async () => {
      ...
      expect(adapter).toHaveBeenCalledWith(expect.objectContaining({ method: 'get', url: '/account', headers: expect.objectContaining({ Authorization: 'Bearer sk_live_123' }) }));

...
describe('DemoConnectorDecorator', () => {
  ...
  describe('CRUD operations', () => {
    ...
    // Codex review finding 2 on the Prerequisite PR A commit: unlike the
    // other CRUD methods above (which use an in-memory demo store), upsert
    // has no demo-mode emulation — so it must FAIL CLOSED in demo mode
    // rather than delegate. Once a real connector implements upsert, an
    // unconditional delegation here would let a demo-mode run issue a real
    // external mutation.
    it('should throw a typed ValidationError for upsert in demo mode (never delegates)', async () => {
      await expect(
        decorator.upsert('Asset', 'External_Id__c', 'inv-1', { name: 'X' }),
      ).rejects.toMatchObject({
        name: 'ValidationError',
        message: 'upsert is not supported in demo mode',
      });
      expect(mockInner.upsert).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Non-demo mode delegation', () => {
    ...
    // Prerequisite PR A (2026-07-27 serialized-asset plan): this block runs
    // with demo mode OFF, which is the ONLY state in which upsert delegates.
    // Like every other CRUD method, the delegation IS gated by isDemoMode() —
    // there is no demo-mode in-memory emulation for upsert, so demo mode fails
    // closed rather than reaching the wrapped connector. The refusal is pinned
    // separately by 'should throw a typed ValidationError for upsert in demo
    // mode (never delegates)' above.
    it('should delegate upsert to inner connector with all four arguments', async () => {
      mockInner.upsert.mockResolvedValue({ outcome: 'created', id: 'sf-1' });

      const result = await decorator.upsert('Asset', 'External_Id__c', 'inv-1', { name: 'X' });

      expect(mockInner.upsert).toHaveBeenCalledWith('Asset', 'External_Id__c', 'inv-1', { name: 'X' });
      expect(result).toEqual({ outcome: 'created', id: 'sf-1' });
    });

...
describe('SuiteCentralConnectorProd — pinned transport', () => {
  it('redacts credential transport failures from authentication, connection status and logs, then recovers', async () => {
    const canary = 'pinned-credential-error-canary';
    const client = makeClient();
    client.request.mockRejectedValue(Object.assign(new Error(canary), { cause: new Error('secret-xyz'), response: { data: canary } }));
    ...
    const error = await connector.authenticate().catch((failure: unknown) => failure);
    const connection = await connector.testConnection();
    expect(error).toBeInstanceOf(Error);
    expect(connection.isConnected).toBe(false);
    const serialized = inspect([error, connection, ...['debug', 'info', 'warn', 'error'].map(level => (logger[level as keyof Logger] as jest.Mock).mock.calls)], { depth: 20, showHidden: true });
    expect(serialized).not.toContain(canary);
    expect(serialized).not.toContain('secret-xyz');
    expect(error).not.toHaveProperty('cause');
    expect(client.request).toHaveBeenCalledTimes(2);
    client.request.mockResolvedValueOnce(tokenResponse('recovered')).mockResolvedValueOnce(resp(200, { customerId: 'c1' }));
    await connector.read('customers', 'c1');
    expect(client.request.mock.calls[3][0].headers?.Authorization).toBe('Bearer recovered');
  });
  it.each([false, true])('a cached approval does not protect the same token from a late 401 (pending=%s)', async pendingApproval => {
    const client = makeClient();
    let release: (response: PinnedResponse) => void = () => {};
    let entered: () => void = () => {};
    const blocked = new Promise<PinnedResponse>(resolve => { release = resolve; });
    const started = new Promise<void>(resolve => { entered = resolve; });
    let exchanges = 0;
    client.request.mockImplementation(async request => {
      if (request.path === '/oauth/token') return tokenResponse(++exchanges === 1 ? 'old' : 'new');
      if (request.path.endsWith('/c1')) { entered(); return blocked; }
      return resp(200, { customerId: 'c2' });
    });
    const connector = new SuiteCentralConnectorProd('suitecentral-prod', logger, undefined, client);
    await connector.initialize(authConfig); await connector.authenticate();
    const pending = connector.read('customers', 'c1').catch((error: unknown) => error);
    await started;
    const approval = connector.authenticate();
    if (!pendingApproval) await approval;
    expect(client.request).toHaveBeenCalledTimes(2);
    release(resp(401, {})); await approval; expect(await pending).toBeInstanceOf(Error);
    await connector.read('customers', 'c2');
    expect(client.request).toHaveBeenCalledTimes(4);
    expect(client.request.mock.calls[3][0].headers?.Authorization).toBe('Bearer new');
  });

  it('waits for authentication started while a business request is rate-limited', async () => {
    const client = makeClient();
    client.request.mockResolvedValue(tokenResponse());
    const connector = new SuiteCentralConnectorProd('suitecentral-prod', logger, undefined, client);
    await connector.initialize(authConfig); await connector.authenticate();
    // Consume the rate-limiter capacity without sleeps, then control its timer.
    const limiter = connector as unknown as { rateLimiter: { tokens: number; lastRefill: number } };
    limiter.rateLimiter.tokens = 0; limiter.rateLimiter.lastRefill = Date.now();
    jest.useFakeTimers();
    let release: () => void = () => {};
    let entered: () => void = () => {};
    const blocked = new Promise<void>(resolve => { release = resolve; });
    const started = new Promise<void>(resolve => { entered = resolve; });
    const fixture = connector as unknown as { canReuseAuthentication(): boolean; performAuthentication(): Promise<boolean> };
    // A controlled hook failure isolates the pending-at-dispatch contract.
    // The production pinned exchange is exercised independently above.
    const cache = jest.spyOn(fixture, 'canReuseAuthentication').mockReturnValue(false);
    const hook = jest.spyOn(fixture, 'performAuthentication').mockImplementation(async () => { entered(); await blocked; return false; });
    try {
      const business = connector.read('customers', 'c1').catch((error: unknown) => error);
      await Promise.resolve(); await Promise.resolve();
      const auth = connector.authenticate(); await started;
      await jest.advanceTimersByTimeAsync(1000);
      const callsBeforeRelease = client.request.mock.calls.length;
      release(); await auth; await business;
      expect(callsBeforeRelease).toBe(1);
      expect(client.request).toHaveBeenCalledTimes(1);
    } finally { release(); hook.mockRestore(); cache.mockRestore(); jest.useRealTimers(); }
  });

  it('a late old-token 401 cannot clear a newer token and is never replayed', async () => {
    const client = makeClient();
    let release: (response: PinnedResponse) => void = () => {};
    let entered: () => void = () => {};
    const blocked = new Promise<PinnedResponse>(resolve => { release = resolve; });
    const started = new Promise<void>(resolve => { entered = resolve; });
    client.request.mockResolvedValueOnce(tokenResponse('old'))
      .mockImplementationOnce(async () => { entered(); return blocked; })
      .mockResolvedValueOnce(tokenResponse('new'))
      .mockResolvedValueOnce(resp(200, { customerId: 'c2' }));
    const connector = new SuiteCentralConnectorProd('suitecentral-prod', logger, undefined, client);
    await connector.initialize(authConfig); await connector.authenticate();
    const pending = connector.read('customers', 'c1').catch((error: unknown) => error);
    await started;
    // Advance the clock so the next direct attempt must exchange the cached token.
    const now = Date.now();
    jest.useFakeTimers({ now: now + 600_000 });
    try {

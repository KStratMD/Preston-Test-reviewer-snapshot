...
class AuthenticationHarness extends PayPalConnector {
...
describe('PayPalConnector', () => {
  ...
  beforeEach(async () => {
    ...
    connector = new AuthenticationHarness('PayPal', 'paypal-test', mockLogger);
    adapter = jest.fn(async (config: InternalAxiosRequestConfig) => ({ status: 200, statusText: 'OK', headers: {}, config, data: { access_token: 'live-token', expires_in: 3600 } }));
    connector.setAdapter(adapter);
    await connector.initialize({ type: 'oauth2', credentials: { clientId: 'live-client-id', clientSecret: 'live-secret' } });
    adapter.mockClear();

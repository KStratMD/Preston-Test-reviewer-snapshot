...
async function startServer(env: Record<string,string|undefined>){
  ...
  (authService.verifyJWT as jest.Mock).mockReturnValue({
    id: 'dashboard-parity-user',
    tenantId: 'dashboard-parity-tenant'
  });
...
async function runSuite(disableRedis: boolean){
  ...
  const authorization = 'Bearer dashboard-parity-token';
  try {
    for (const ep of endpoints){
      const res = await request(base).get(ep.path).set('Authorization', authorization).expect(200);

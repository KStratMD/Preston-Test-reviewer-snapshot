...
it.each(['reviews', 'files'] as const)('accepts GitHub canonical repository-id pagination URLs for %s', async resource => {
  const fetcher = jest.spyOn(global, 'fetch')
    .mockResolvedValueOnce(reply([], `<https://api.github.com/repositories/1023330483/pulls/1308/${resource}?per_page=100&page=2>; rel="next"`))
    .mockResolvedValueOnce(reply([]));

  const api = githubApi('test-token', 'KStratMD', 'Preston-Test', 1308);
  await expect(resource === 'reviews' ? api.listReviews() : api.listFiles()).resolves.toEqual([]);
  expect(fetcher).toHaveBeenCalledTimes(2);
});
it('accepts a canonical repository-id path under a GitHub Enterprise API prefix', async () => {
  process.env.GITHUB_API_URL = 'https://github.example/api/v3';
  jest.spyOn(global, 'fetch')
    .mockResolvedValueOnce(reply([], '<https://github.example/api/v3/repositories/42/pulls/5/files?page=2>; rel="next"'))
    .mockResolvedValueOnce(reply([]));
  await expect(githubApi('test-token', 'o', 'r', 5).listFiles()).resolves.toEqual([]);
});
it.each(['reviews', 'files'] as const)('pins the canonical repository id for %s pagination', async resource => {
  const root = `https://api.github.com/repositories/42/pulls/5/${resource}`;
  jest.spyOn(global, 'fetch')
    .mockResolvedValueOnce(reply([], `<${root}?page=2>; rel="next"`))
    .mockResolvedValueOnce(reply([], `<https://api.github.com/repositories/43/pulls/5/${resource}?page=3>; rel="next"`));
  const api = githubApi('test-token', 'o', 'r', 5);

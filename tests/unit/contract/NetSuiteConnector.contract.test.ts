...
describe('NetSuiteConnector Contract Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockHttpClient = Object.assign(axios.create(), { request: jest.fn() });
  ...
  describe('Record CRUD Contract', () => {
    ...
    describe('Read Operation', () => {
      it.each([null, 'not-a-record'])('rejects a malformed vendor record response (%p)', async payload => {
        mockHttpClient.request.mockResolvedValue({ data: payload });
        await expect(connector.read('customer', '12345')).rejects.toThrow('Invalid data format from NetSuite');
      });

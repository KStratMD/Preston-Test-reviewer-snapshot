...
describe('IntegrationService concurrency', () => {
  ...
  const createService = (sourceConnector: IConnector, targetConnector: IConnector) => {
    ...
    // A2 routes the standard sync through ConnectorManager. These two doubles
    // are what this suite measures — the source paces record delivery and the
    // target records overlap — so they are handed to the REAL manager rather
    // than injected by overwriting the service's now-unused private
    // getConnector. Connector keys are the manager's own projection.
    const { manager } = createStandardPathConnectorManager({
      connectorFor: (connectorKey) =>
        connectorKey === 'salesforce' ? sourceConnector : targetConnector,
    });

...
describe('DynamicsConnector', () => {
  ...
  beforeEach(async () => {
    // Mock AuthService
    authService = {
      authenticateOAuth2: jest.fn().mockResolvedValue({ accessToken: 'token', expiresAt: new Date(Date.now() + 3_600_000), issued: new Date() }),
    } as any;
    // Mock Logger
    logger = {
      info: jest.fn(),
    ...
    await connector.initialize({ type: 'oauth2', credentials: {
      clientId: 'cid', clientSecret: 'cs', tenantId: 'tid', resourceUrl: 'https://dynamics.example.com',
    } });
  ...
  describe('authenticate', () => {
    it('authenticates and sets headers', async () => {
      ...
      // Mock token
      (authService.authenticateOAuth2 as jest.Mock).mockResolvedValue({ accessToken: 'abc', expiresAt: new Date(Date.now() + 3_600_000), issued: new Date() });

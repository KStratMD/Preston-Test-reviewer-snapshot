...
class TestConnector extends BaseConnector {
  ...
  performAuthentication = jest.fn().mockResolvedValue(true);
  ...
  public async setAuthenticated(isAuthenticated: boolean): Promise<void> {
    if (isAuthenticated) await this.authenticate();
    else this.resetAuthentication();
    this.performAuthentication.mockClear();
  }
}
...
describe.each(['ordinary', 'sensitive'] as const)('%s transport retry policy', transport => {
  ...
  beforeEach(async () => {
    ...
    await connector.setAuthenticated(true);
  ...
  it('reauthenticates once through the guard and then succeeds', async () => {
    connector.performAuthentication.mockImplementation(async () => {
      return true;
    });
    mockAxiosInstance.request.mockRejectedValueOnce(failure(401)).mockResolvedValueOnce(success);
    await send();
    expect(connector.performAuthentication).toHaveBeenCalledTimes(1);
    expect(mockAxiosInstance.request).toHaveBeenCalledTimes(2);
  });

  it('a second 401 is terminal', async () => {
    connector.performAuthentication.mockImplementation(async () => { return true; });
  ...
  it('a nested authentication 401 is terminal', async () => {
    await connector.setAuthenticated(false);
    connector.performAuthentication.mockImplementation(async (probe: import('../../../src/core/BaseConnector').AuthenticationProbe) => { await probe.request({ method: 'POST', url: '/token' }); return true; });
    mockAxiosInstance.request.mockRejectedValue(failure(401));
    await expect(send()).rejects.toBeInstanceOf(UnauthorizedAppError);
    expect(connector.performAuthentication).toHaveBeenCalledTimes(1);
    expect(mockAxiosInstance.request).toHaveBeenCalledTimes(1);
  ...
  it.each([false, true])('fails closed when authenticate returns false (already authenticated: %s)', async alreadyAuthenticated => {
    await connector.setAuthenticated(alreadyAuthenticated);
    connector.performAuthentication.mockResolvedValue(false);
  ...
  if (transport === 'sensitive') {
    it.each(['retry', 'reauth', 'authentication'])('keeps secrets out of errors and logs after %s failure', async path => {
      ...
      if (path === 'authentication') {
        await connector.setAuthenticated(false);
        connector.performAuthentication.mockRejectedValue(raw);
...
describe('BaseConnector', () => {
  ...
  describe('Authentication', () => {
    it('should call authenticate if not already authenticated', async () => {
      ...
      expect(connector.performAuthentication).toHaveBeenCalled();
    });
    ...
    it('should not call authenticate if already authenticated', async () => {
      ...
      mockAxiosInstance.request.mockResolvedValueOnce(mockResponse);
      await connector.testMakeRequest({});
      expect(connector.performAuthentication).not.toHaveBeenCalled();
    });
  });

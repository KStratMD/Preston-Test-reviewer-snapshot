...
describe('Integration Performance Benchmarks', () => {
  ...
  const routeConnectorPair = (source: IConnector, target: IConnector): void => {
    connectorRoute = (connectorKey) => {
      if (connectorKey === 'salesforce') return source;
      if (connectorKey === 'netsuite') return target;
  ...
  beforeEach(() => {
    ...
    // A2 routes the standard run path through ConnectorManager. This suite
    // keeps its OWN deterministic connector doubles: it benchmarks the
    // per-record loop, so building registry connectors here would benchmark
    // different work and move numbers the suite exists to pin. Only creation
    // is replaced; resolution stays the cheap synchronous inline default. Do
    // not read real-manager cache coverage out of this file — that is proven
    // in the dedicated cache/concurrency regression.
    const { manager } = createStandardPathConnectorManager({
      connectorFor: (connectorKey) => {
        if (!connectorRoute) throw new Error(`Unknown system type: ${connectorKey}`);
  ...
  describe('Record Processing Performance', () => {
    it('should process 100 records within acceptable time', async () => {
      ...
      routeConnectorPair(sourceConnector, targetConnector);

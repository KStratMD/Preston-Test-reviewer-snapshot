...
describe('MCPAggregatorService', () => {
  ...
  beforeEach(() => {
    ...
    dlpService = {
      ...
      redactData: jest.fn().mockImplementation((val) => val),
    };
    auditService = {
  ...
  it('emits one DLP completion event for one MCP result without a duplicate caller event', async () => {
    const adapter = createAdapter({
      systemName: 'netsuite',
      tools: [{ name: 'ns_getRecord', description: 'Read', inputSchema: {} }],
    });
    const realDlpService = new DLPService(logger);
    const service = new MCPAggregatorService(
      logger,
      governanceService as unknown as GovernanceService,
      realDlpService,
    ...
    await service.callTool('netsuite.ns_getRecord', {}, { tenantId: 'tenant-a' });

    expect(logger.debug.mock.calls.filter(([message]) => message === 'PII scan completed')).toHaveLength(1);
    expect(logger.info.mock.calls.filter(([message]) => message === 'PII scan completed')).toHaveLength(0);
  });
});

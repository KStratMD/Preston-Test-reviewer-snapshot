...
describe('SecureCredentialManager', () => {
  ...
  it('does not expose provider text when secure storage fails', async () => {
    jest.spyOn(secretManager, 'setSecret')
      .mockRejectedValueOnce(new Error('Invalid credentials from provider: SYNTHETIC_BACKEND_SECRET'));
    const mgr = new SecureCredentialManager(logger, secretManager, metadataStore);

    await expect(mgr.storeCredentials('Salesforce', 'default', { apiKey: 'value' } as any))
      .rejects.toThrow('Failed to store credentials for Salesforce:default: credential operation failed');
  });
  ...
  it('deletes the provider secret before removing metadata', async () => {
    const mgr = new SecureCredentialManager(logger, secretManager, metadataStore);
    await mgr.storeCredentials('Salesforce', 'delete-me', { apiKey: 'secret-to-delete' } as any);

    await mgr.deleteCredentials('Salesforce', 'delete-me');

    await expect(mgr.getCredentials('Salesforce', 'delete-me')).rejects.toThrow(/Failed to retrieve credentials/);
    expect(await mgr.getCredentialMetadata('Salesforce', 'delete-me')).toBeUndefined();
  });

  it('preserves in-memory metadata when persisted metadata deletion fails', async () => {
    const mgr = new SecureCredentialManager(logger, secretManager, metadataStore);
    await mgr.storeCredentials('Salesforce', 'delete-failure', { apiKey: 'secret-to-delete' } as any);
    jest.spyOn(metadataStore, 'delete').mockRejectedValueOnce(new Error('metadata unavailable'));

    await expect(mgr.deleteCredentials('Salesforce', 'delete-failure')).rejects.toThrow('metadata unavailable');
    expect(await mgr.getCredentialMetadata('Salesforce', 'delete-failure')).toEqual(
      expect.objectContaining({ systemType: 'Salesforce', systemId: 'delete-failure' }),
    );
  });
});

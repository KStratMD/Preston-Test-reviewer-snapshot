...
describe('SAPConnector', () => {
  ...
  describe('syncMaterials uses one HTTP retry layer', () => {
    beforeEach(async () => {
      jest.useFakeTimers();
      await sapConnector.initialize(mockBasicAuthConfig);
      await authenticateBusinessFixture(sapConnector);
      mockAxiosInstance.request.mockReset();
      mockedAxios.isAxiosError.mockImplementation((error: unknown): error is import('axios').AxiosError => Boolean(error && typeof error === 'object' && 'isAxiosError' in error));
    });
    afterEach(() => jest.useRealTimers());

    it.each([[404, 1, false], [429, 3, true], [500, 2, true], [503, 3, false]] as const)('%i produces exactly %i HTTP calls', async (status, calls, succeeds) => {
      const failure = { isAxiosError: true, response: { status, headers: {}, data: {} } };
      const response = { data: { d: { results: [{ Product: 'material-1' }] } }, status: 200, headers: {} };
      if (succeeds) {
        for (let i = 1; i < calls; i++) mockAxiosInstance.request.mockRejectedValueOnce(failure);
        mockAxiosInstance.request.mockResolvedValue(response);
      } else mockAxiosInstance.request.mockRejectedValue(failure);
      const promise = sapConnector.syncMaterials({ batchSize: 10 });
      const checked = succeeds
        ? expect(promise).resolves.toMatchObject({ recordsProcessed: 1, recordsSuccessful: 1 })
        : expect(promise).rejects.toThrow();
      await jest.runAllTimersAsync();
      await checked;
      expect(mockAxiosInstance.request).toHaveBeenCalledTimes(calls);
    });

    it('preserves pagination and accumulated counts', async () => {
      mockAxiosInstance.request
        .mockResolvedValueOnce({ data: { d: { results: [{ Product: 'a' }, { Product: 'b' }] } } })
        .mockResolvedValueOnce({ data: { d: { results: [{ Product: 'c' }] } } });
      const promise = sapConnector.syncMaterials({ batchSize: 2, maxRetries: 1 });
      const checked = expect(promise).resolves.toMatchObject({ recordsProcessed: 3, recordsSuccessful: 3 });
      await jest.runAllTimersAsync();
      await checked;
      expect(mockAxiosInstance.request).toHaveBeenCalledTimes(2);
      expect(mockAxiosInstance.request.mock.calls[0][0].url).toContain('$skip=0');
      expect(mockAxiosInstance.request.mock.calls[1][0].url).toContain('$skip=2');
    });
  });

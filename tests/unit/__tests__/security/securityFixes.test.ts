...
describe('Security Fixes Validation', () => {
  ...
  describe('JWT Secret Strength Validation', () => {
    ...
    beforeEach(() => {
      ...
      // A8: the approval-fingerprint key is a production/hosted boot
      // requirement, so a suite that re-requires env.ts under
      // NODE_ENV=production must supply one like the two guards above.
      process.env.APPROVAL_FINGERPRINT_HMAC_KEY = '7f'.repeat(32);
    });

...
describe('SchemaDiscoveryService', () => {
  ...
  describe('discoverSalesforceRelationshipSchema (relationship-evidence path)', () => {
    ...
    it('returns null when Salesforce discovery is not configured', async () => {
      const service = new SchemaDiscoveryService({ enableSalesforce: false });
    ...
    it('throws (does not fall back to a mock schema) when the transport rejects', async () => {
      global.fetch = jest.fn().mockRejectedValue(new Error('network down')) as unknown as typeof fetch;

      const service = new SchemaDiscoveryService({ enableSalesforce: true });

      await expect(service.discoverSalesforceRelationshipSchema('Account')).rejects.toThrow('network down');
    });

    it('throws (does not fall back to a mock schema) on a non-ok response', async () => {
      global.fetch = jest.fn().mockResolvedValue({
        ok: false,
        status: 500,
      ...
      await expect(service.discoverSalesforceRelationshipSchema('Account')).rejects.toThrow(
        'Salesforce API error: 500 Internal Server Error',
      );
    });
  });

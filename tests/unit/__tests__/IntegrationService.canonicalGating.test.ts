...
describe('IntegrationService — squire-fallback canonical-entity gating (C1)', () => {
  ...
  const makeService = (config: IntegrationConfig) => {
    ...
    // A2 routes the standard sync through ConnectorManager, so these doubles
    // are supplied to the REAL manager (keyed by its own projection —
    // 'Dynamics365' projects to 'dynamics') instead of by overwriting the
    // service's now-unused private getConnector. This suite's whole point is
    // asserting on targetConnector.create(), so the doubles have to survive.
    const connectorsByKey = new Map<string, IConnector>([
      [connectorKeyForSystem(config.sourceSystem), sourceConnector],
      [connectorKeyForSystem(config.targetSystem), targetConnector],
    ]);
    const { manager } = createStandardPathConnectorManager({
      connectorFor: (connectorKey) => {
        const connector = connectorsByKey.get(connectorKey);
        if (!connector) throw new Error(`canonicalGating: no connector double for '${connectorKey}'`);
        return connector;
      },
    });

...
describe('IntegrationService - New Connectors Support', () => {
  ...
  let connectorRoute: ((connectorKey: string) => unknown) | undefined;

  /**
   * Routes this test's connector doubles by RAW system spelling. The manager
   * asks by its PROJECTED key ('BusinessCentral' -> 'businesscentral'), so the
   * match is on the lowercased raw name — every system in this suite projects
   * that way. An unrouted key throws, preserving the loud failure the
   * per-test `getConnector` spies used to produce.
   */
  const routeConnectorsByType = (byRawType: Record<string, unknown>): void => {
    const entries = Object.entries(byRawType);
    connectorRoute = (connectorKey) => {
      const hit = entries.find(([rawType]) => rawType.toLowerCase() === connectorKey);
      if (!hit) throw new Error(`Unknown system type: ${connectorKey}`);
      return hit[1];
    };
  };
  ...
  beforeEach(() => {
    ...
    // A2 routes run/test/single-record/initialize through ConnectorManager, so
    // the scenario tests below hand their doubles to the REAL manager instead
    // of spying on the service's private getConnector. The `Connector Creation`
    // describe above still exercises that private path directly and still
    // should: `getSystemHealth()` remains its caller.
  ...
  describe('Connector Creation', () => {
    ...
    it('should create Oracle connector', async () => {
      const _mockConnector = new MockedOracleConnector('test', mockLogger, mockAuthService, createMockOutboundGovernanceService());

      const getConnector = (integrationService as any).getConnector.bind(integrationService);
      const connector = await getConnector('Oracle', 'test-system-id');

      expect(MockedOracleConnector).toHaveBeenCalledWith('test-system-id', mockLogger, mockAuthService, expect.anything());
  ...
  describe('Integration Scenarios', () => {
    ...
    it('should test Salesforce to SAP integration', async () => {
      ...
      routeConnectorsByType({ Salesforce: mockSalesforceConnector, SAP: mockSAPConnector });
    ...
    it('should test Oracle to Business Central integration', async () => {
      ...
      routeConnectorsByType({ Oracle: mockOracleConnector, BusinessCentral: mockBCConnector });
  ...
  describe('Multi-System Integration Patterns', () => {
    ...
    it('should support bidirectional sync between new systems', async () => {
      ...
      routeConnectorsByType({ SAP: mockSAPConnector, BusinessCentral: mockBCConnector });
    ...
    it('should handle complex transformation scenarios between different system types', async () => {
      ...
      routeConnectorsByType({ Oracle: mockOracleConnector, Salesforce: mockSalesforceConnector });

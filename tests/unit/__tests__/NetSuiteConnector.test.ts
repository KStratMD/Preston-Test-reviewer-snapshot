...
describe('NetSuiteConnector', () => {
  ...
  beforeEach(() => {
    ...
    // Preserve Axios URI normalization while stubbing physical dispatch.
    (connector as any).httpClient = Object.assign(axios.create(), { request: jest.fn() });
  });
  ...
  describe('sandbox account ID base URL derivation', () => {
    it('DNS-normalizes a sandbox account ID (underscore/uppercase) in the derived base URL', async () => {
      const sandboxConfig: AuthConfig = {
        type: 'oauth1',
        credentials: {
          ...
          tokenId: 'tid',
          tokenSecret: 'ts',
          // no base_url / baseUrl — must be derived from accountId
        },
      };

      await connector.initialize(sandboxConfig);
    ...
    it('leaves a bare production account ID unchanged in the derived base URL', async () => {
      ...
      await connector.initialize(productionConfig);
    ...
    it('uses an explicit base_url credential verbatim even for a sandbox-shaped account ID', async () => {
      const explicitBaseUrlConfig: AuthConfig = {
        type: 'oauth1',
        credentials: {
          accountId: '1234567_SB1',
          consumerKey: 'ck',
      ...
      await connector.initialize(explicitBaseUrlConfig);

      expect((connector as any).httpClient.defaults.baseURL).toBe('https://custom.example.com');
    });

    it('keeps the OAuth1 realm as the original, un-normalized account ID', async () => {
      const sandboxConfig: AuthConfig = {
        type: 'oauth1',
        credentials: {
          accountId: '1234567_SB1',
      ...
      await connector.initialize(sandboxConfig);
      // The account ID field itself must be untouched — it doubles as the
      // OAuth1 realm, which NetSuite requires in its original form.
      expect((connector as any).accountId).toBe('1234567_SB1');

      await authenticateBusinessFixture(connector);
      (connector as any).httpClient.request.mockResolvedValueOnce({ data: { internalid: '1' } });
      await connector.read('customer', '1');

      const mockedGetOAuth1AuthorizationHeader = getOAuth1AuthorizationHeader as jest.Mock;
      const lastCallOptions = mockedGetOAuth1AuthorizationHeader.mock.calls[
        mockedGetOAuth1AuthorizationHeader.mock.calls.length - 1
      ][2];
      expect(lastCallOptions.realm).toBe('1234567_SB1');
    });
  });
});

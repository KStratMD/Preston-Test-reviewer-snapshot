...
describe('Connector Unit Tests', () => {
  ...
  describe('NetSuiteConnector authentication', () => {
    it('rejects an exchange that returns no tokens', async () => {
      const service = { authenticateOAuth1: jest.fn().mockResolvedValue({}) } as unknown as AuthService;
      const connector = new NetSuiteConnector('ns-empty', mockLogger, service, createMockOutboundGovernanceService());
      await connector.initialize({ type: 'oauth1', credentials: { accountId: 'acct', consumerKey: 'key', consumerSecret: 'secret', tokenId: 'tid', tokenSecret: 'tsec' } });
      await expect(connector.authenticate()).rejects.toThrow('Failed to obtain NetSuite OAuth1 tokens');
  ...
  describe('BusinessCentralConnector', () => {
    it('does not discover companies when OAuth returns no access token', async () => {
      const service = { authenticateOAuth2: jest.fn().mockResolvedValue({}) } as unknown as AuthService;
      const connector = new BusinessCentralConnector('bc-empty', mockLogger, service, createMockOutboundGovernanceService());
      await connector.initialize({ type: 'oauth2', credentials: { tenantId: 'tid', clientId: 'cid', clientSecret: 'sec' } });
      const adapter = jest.fn(); connector['httpClient'].defaults.adapter = adapter;
      await expect(connector.authenticate()).rejects.toThrow('Failed to obtain Business Central access token');
      expect(adapter).not.toHaveBeenCalled(); expect(connector['isAuthenticated']).toBe(false);
    });
    it('authenticates via OAuth2 and fetches company ID', async () => {
      ...
      connector['httpClient'].defaults.adapter = async config => ({ data: { value: [{ id: 'comp-1' }] }, status: 200, statusText: 'OK', headers: {}, config });
  ...
  describe('OracleConnector', () => {
    it('preserves read error presentation after authentication succeeds', async () => {
      const connector = new OracleConnector('oracle-read-error', mockLogger, mockAuthService, createMockOutboundGovernanceService());
      await connector.initialize({ type: 'basic', credentials: { username: 'u', password: 'p', host: 'oracle.real.local' } });
      await authenticateBusinessFixture(connector);
      jest.spyOn(connector as any, 'makeRequest').mockRejectedValue(new Error('read refused'));
      await expect(connector.read('customer', '1')).rejects.toThrow('read refused');
    });
    it('authenticates using basic auth', async () => {
      const connector = new OracleConnector('oracle-test', mockLogger, mockAuthService, createMockOutboundGovernanceService());
      await connector.initialize({
        type: 'basic',
      ...
      const reqSpy = jest.fn(async (config: import('axios').InternalAxiosRequestConfig) => ({ data: {}, status: 200, statusText: 'OK', headers: {}, config }));
      connector['httpClient'].defaults.adapter = reqSpy;
      await connector.authenticate();
    ...
    it('maps data to and from Oracle format', () => {
      const connector = new OracleConnector('oracle-map', mockLogger, mockAuthService, createMockOutboundGovernanceService());
      const access = connector as any;
    ...
    it('handles API errors gracefully', async () => {
      const connector = new OracleConnector('oracle-err', mockLogger, mockAuthService, createMockOutboundGovernanceService());
      await connector.initialize({
        type: 'basic',
        // Use a host that does not trigger demo mode in tests
        credentials: { username: 'u', password: 'p', host: 'oracle.real.local' },

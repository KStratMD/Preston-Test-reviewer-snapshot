...
describe('FlowExecutor — governance context plumbing', () => {
  ...
  // Task B4b: the executor DERIVES requester provenance on the enqueued row
  // rather than letting it default to 'unknown', which would make the approval
  // un-approvable. A system-originated write and a user-triggered one are not
  // the same claim, so both branches are asserted.
  it("enqueues requesterProvenance 'system' when no caller userId reached the executor", async () => {
    const queue = mockApprovalQueue();
    const exec = new FlowExecutor(
      mockLogger() as any,
      mockGovernance(makeDecision({ approved: false, approvalRequired: true })),
      queue,
      mockOwnershipResolver() as unknown as OwnershipResolver,
      mockAuditService() as unknown as AuditService,
    );
    await exec.execute(makeTemplate(), { id: 'e1', name: 'x' }, makeCtx({ userId: undefined }, mockConnector()));
    expect(queue.enqueue).toHaveBeenCalledWith(
      expect.objectContaining({ requesterProvenance: 'system' }),
    );
  });

  it('enqueues what the caller proved when a userId is present, and nothing when they could not say', async () => {
    const withProof = mockApprovalQueue();
    const execA = new FlowExecutor(
      mockLogger() as any,
      mockGovernance(makeDecision({ approved: false, approvalRequired: true })),
      withProof,
      mockOwnershipResolver() as unknown as OwnershipResolver,
    ...
    await execA.execute(
      makeTemplate(),
      { id: 'e1', name: 'x' },
      { ...makeCtx({ userId: 'u-1' }, mockConnector()), requesterProvenance: 'jwt' },
    );
    expect(withProof.enqueue).toHaveBeenCalledWith(
      expect.objectContaining({ requesterUserId: 'u-1', requesterProvenance: 'jwt' }),
    );

    // A caller that named a user but cannot say how it was established must not
    // have 'system' invented for it — the row stays unverifiable, by design.
    const noProof = mockApprovalQueue();
    const execB = new FlowExecutor(
      mockLogger() as any,
      mockGovernance(makeDecision({ approved: false, approvalRequired: true })),
      noProof,
      mockOwnershipResolver() as unknown as OwnershipResolver,
    ...
    await execB.execute(makeTemplate(), { id: 'e2', name: 'x' }, makeCtx({ userId: 'u-2' }, mockConnector()));
    expect(noProof.enqueue).toHaveBeenCalledWith(
      expect.objectContaining({ requesterUserId: 'u-2', requesterProvenance: undefined }),
    );
  });

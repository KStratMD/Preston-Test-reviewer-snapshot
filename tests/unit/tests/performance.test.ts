...
const { PerformanceMonitor: RealPerformanceMonitor } = jest.requireActual(
  '../../../src/services/PerformanceMonitor',
) as typeof import('../../../src/services/PerformanceMonitor');
...
function createServiceSampler(snapshots: ProcessResourceSnapshot[]): ProcessResourceSampler {
  let lastSnapshot = snapshots[snapshots.length - 1];

  return {
    sample: jest.fn(() => {
      const snapshot = snapshots.shift() ?? lastSnapshot;
      if (!snapshot) {
        throw new Error('createServiceSampler requires at least one snapshot');
      }
...
async function collectServiceMetrics(monitor: InstanceType<typeof RealPerformanceMonitor>): Promise<void> {
...
describe('Real service PerformanceMonitor CPU sampling', () => {
  afterEach(() => {
    jest.useRealTimers();
  });

  it('warms up on the first collection and uses a deterministic second delta', async () => {
    jest.useFakeTimers();
    const monitor = new RealPerformanceMonitor(createServiceSampler([
      createServiceSnapshot(1_000_000_000n, 0),
      createServiceSnapshot(2_000_000_000n, 0),
      createServiceSnapshot(3_000_000_000n, 500_000),
    ]));

    monitor.startMonitoring(1000);
    await collectServiceMetrics(monitor);
    expect(monitor.getLatestMetrics()?.cpu.usage).toBe(0);
    expect(monitor.getAlerts().some(alert => alert.type === 'cpu')).toBe(false);

    await collectServiceMetrics(monitor);
    expect(monitor.getLatestMetrics()?.cpu.usage).toBe(50);
    expect(monitor.getAlerts().some(alert => alert.type === 'cpu')).toBe(false);

    monitor.shutdown();
  });

  it('repeats the last fixture snapshot when a test collects beyond its fixture length', async () => {
    jest.useFakeTimers();
    const monitor = new RealPerformanceMonitor(createServiceSampler([
      createServiceSnapshot(1_000_000_000n, 0),
    ]));
  ...
  it('raises a CPU alert only when the deterministic delta exceeds the threshold', async () => {
    jest.useFakeTimers();
    const monitor = new RealPerformanceMonitor(createServiceSampler([
      createServiceSnapshot(1_000_000_000n, 0),
      createServiceSnapshot(2_000_000_000n, 0),
      createServiceSnapshot(3_000_000_000n, 900_000),
    ]));
    ...
    await collectServiceMetrics(monitor);
    expect(monitor.getLatestMetrics()?.cpu.usage).toBe(90);
    expect(monitor.getAlerts().filter(alert => alert.type === 'cpu')).toHaveLength(1);

    monitor.shutdown();
  });

  it('warms up again after monitoring restarts', async () => {
    jest.useFakeTimers();
    const monitor = new RealPerformanceMonitor(createServiceSampler([
      createServiceSnapshot(1_000_000_000n, 0),
      createServiceSnapshot(2_000_000_000n, 0),
      createServiceSnapshot(3_000_000_000n, 900_000),
      createServiceSnapshot(4_000_000_000n, 1_800_000),
      createServiceSnapshot(5_000_000_000n, 1_800_000),
    ]));

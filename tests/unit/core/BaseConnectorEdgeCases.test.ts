...
describe('BaseConnector - Edge Cases and Error Handling', () => {
  ...
  // Prerequisite PR A (2026-07-27 serialized-asset plan): BaseConnector.upsert
  // is a concrete unsupported-by-default so every IConnector implementor
  // stays type-correct without touching every connector subclass.
  describe('upsert (concrete unsupported default)', () => {
    it('throws a typed ValidationError without issuing I/O or interpolating arguments', async () => {
      await expect(
        connector.upsert('Asset', 'External_Id__c', 'secret-serial-value', { name: 'X' }),
      ).rejects.toMatchObject({
        name: 'ValidationError',
        message: 'Connector does not support upsert',
      });
    });

    it('never leaks entityType/externalIdValue into the error message', async () => {
      await expect(
        connector.upsert('Asset', 'External_Id__c', 'secret-serial-value', {}),
      ).rejects.not.toMatchObject({
        message: expect.stringContaining('secret-serial-value'),
      });
    });
  });

  describe('Retry Logic', () => {
    it('does not retry an unclassified setup error', async () => {
      const error = new Error('Setup failed');
      const operation = jest.fn().mockRejectedValue(error);
      await expect(connector.testRetry(operation, 3, 50, true)).rejects.toBe(error);
    ...
    it('should throw after max attempts', async () => {
      const operation = jest.fn(async () => {
        throw Object.assign(new Error('Persistent failure'), { response: { status: 503 } });
      });

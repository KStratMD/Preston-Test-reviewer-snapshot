...
describe('BaseConnector interceptor redaction', () => {
  ...
  it('logs only status when an interceptor error has no tagged config', async () => {
    const [, onRejected] = lastResponseInterceptorHandlers();
    const plainError = new Error(CANARY_VALUE);

    await expect(onRejected(plainError)).rejects.toBe(plainError);

    expect(logger.error).toHaveBeenCalledWith('Response interceptor error', { status: undefined });
    expect(JSON.stringify((logger.error as jest.Mock).mock.calls)).not.toContain(CANARY_VALUE);
  });
});

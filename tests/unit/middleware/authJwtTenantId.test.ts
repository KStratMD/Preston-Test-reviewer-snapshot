...
describe('authMiddleware tenantId-claim propagation', () => {
  ...
  it('uses decoded.id as the required user identity when sub is absent', () => {
    verifyJWT.mockReturnValue({ id: 'legacy-user-1', tenantId: 'acme-co', roles: ['user'] });
    const req = mkReq(mintJwt({})) as Request;
    authMiddleware(req, mkRes() as Response, next as unknown as NextFunction);
    expect((req as Request & { user?: { id: string } }).user?.id).toBe('legacy-user-1');
    expect(next).toHaveBeenCalledWith();
  });

  it('rejects a signature-valid JWT with no sub/id instead of fabricating unknown', () => {
    verifyJWT.mockReturnValue({ tenantId: 'acme-co', roles: ['user'] });
    const req = mkReq(mintJwt({})) as Request;
    const res = mkRes() as Response;
  ...
  it('rejects blank sub/id claims instead of treating them as usable identity', () => {
    verifyJWT.mockReturnValue({ sub: '   ', id: '', tenantId: 'acme-co', roles: ['user'] });
    const req = mkReq(mintJwt({})) as Request;
    const res = mkRes() as Response;
    authMiddleware(req, res, next as unknown as NextFunction);
    expect(res.status).toHaveBeenCalledWith(401);
  ...
  it('falls back to decoded.id when sub is blank but id is valid (claims normalized independently)', () => {
    verifyJWT.mockReturnValue({ sub: '   ', id: 'legacy-user-2', tenantId: 'acme-co', roles: ['user'] });
    const req = mkReq(mintJwt({})) as Request;
    authMiddleware(req, mkRes() as Response, next as unknown as NextFunction);
    expect((req as Request & { user?: { id: string } }).user?.id).toBe('legacy-user-2');
    expect(next).toHaveBeenCalledWith();
  });

  it('falls back to email when username is blank but email is valid', () => {
    verifyJWT.mockReturnValue({ sub: 'u1', username: '   ', email: 'u1@example.com', tenantId: 'acme-co', roles: ['user'] });
    const req = mkReq(mintJwt({})) as Request;
    authMiddleware(req, mkRes() as Response, next as unknown as NextFunction);
    expect((req as Request & { user?: { username: string } }).user?.username).toBe('u1@example.com');
    expect(next).toHaveBeenCalledWith();
  });
  ...
  // A5: the canonical tenant-identity boundary is 64 characters, unified with
  // tenantIsolation's `^[a-zA-Z0-9_-]{1,64}$` and the sync-error-assist webhook
  // schema. Previously this middleware admitted up to 255, so a 65..255-char
  // claim passed auth and reached tenantStatusGate's `readTenantClaim` — which
  // applies no format check at all — and auto-registered a tenant row through
  // `requireActive` → `ensureExists`. The wider `tenants.id` column
  // (VARCHAR(255) on Postgres) is deliberately NOT narrowed; the ingress bound
  // is the security boundary, the column is just storage.
  it('accepts a tenantId of exactly 64 characters (the boundary)', () => {
  ...
  it.each([
    ['65 chars (one past the boundary)', 65],
    ['200 chars (previously admitted under the 255 bound)', 200],
    ['255 chars (the old boundary value itself)', 255],
    ['300 chars (well oversized)', 300],
  ])('rejects an over-length tenantId claim — %s — by normalizing to undefined', (_label, len) => {
    verifyJWT.mockReturnValue({ sub: 'u1', tenantId: 'a'.repeat(len), roles: ['user'] });
    const req = mkReq(mintJwt({})) as Request;
    authMiddleware(req, mkRes() as Response, next as unknown as NextFunction);
    expect((req as Request & { user?: { tenantId?: string } }).user?.tenantId).toBeUndefined();
  });

  // A5.1 checkbox 3: the shipped sentinel and the configured demo tenant must
  // both survive the narrowed bound. Imported from their real source of truth
  // rather than restated, so a rename cannot leave this test asserting a value
  // the system no longer uses. normalizeTenantIdClaim deliberately does NOT
  // reject the sentinel — that rejection belongs to the consumers
  // (tenantIsolation, tenantStatusGate), which is asserted in their own suites.
  it.each([
    ['system sentinel',        SYSTEM_IDENTITY.tenantId],
    ['configured demo tenant', CENTRAL_DEMO_TENANT_ID],
  ])('passes the reserved identity through unchanged: %s', (_label, value) => {
  ...
  it('trims a padded claim to its canonical spelling before the length check', () => {
    // 64 content chars plus padding is still valid: trim happens first.
    const content = 'b'.repeat(64);
    verifyJWT.mockReturnValue({ sub: 'u1', tenantId: `   ${content}   `, roles: ['user'] });
    const req = mkReq(mintJwt({})) as Request;
    authMiddleware(req, mkRes() as Response, next as unknown as NextFunction);
    expect((req as Request & { user?: { tenantId?: string } }).user?.tenantId).toBe(content);
  });
...
describe('optionalAuthMiddleware tenantId-claim propagation', () => {
  ...
  it('uses decoded.id as the optional JWT user identity when sub is absent', () => {
    verifyJWT.mockReturnValue({ id: 'legacy-user-1', tenantId: 'acme-opt', roles: ['user'] });
    const req = mkReq(mintJwt({})) as Request;
    optionalAuthMiddleware(req, mkRes() as Response, next as unknown as NextFunction);
    expect((req as Request & { user?: { id: string } }).user?.id).toBe('legacy-user-1');
    expect(next).toHaveBeenCalledWith();
  ...
  it('rejects a signature-valid JWT with no sub/id instead of continuing as unknown', () => {
    verifyJWT.mockReturnValue({ tenantId: 'acme-opt', roles: ['user'] });
    const req = mkReq(mintJwt({})) as Request;
    const res = mkRes() as Response;
    optionalAuthMiddleware(req, res, next as unknown as NextFunction);
    expect(res.status).toHaveBeenCalledWith(401);
    expect(res.json).toHaveBeenCalledWith({ success: false, error: 'Invalid or expired token' });
    expect((req as Request & { user?: unknown }).user).toBeUndefined();
    expect(next).not.toHaveBeenCalled();

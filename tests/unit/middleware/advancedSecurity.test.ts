...
describe('AdvancedSecurityMiddleware', () => {
  ...
  describe('cleanup timer', () => {
    it('should clean up old suspicious IPs', () => {
      ...
      // The suspicious request was tracked before any cleanup ran.
      const beforeCleanup = middleware.getSecurityReport() as { suspicious: { ipCount: number } };
      expect(beforeCleanup.suspicious.ipCount).toBe(1);

      // Advance time past cleanup interval (5 minutes) and past max age (24 hours)
      jest.advanceTimersByTime(25 * 60 * 60 * 1000); // 25 hours

      // The cleanup timer should have evicted the stale suspicious IP entry
      const afterCleanup = middleware.getSecurityReport() as { suspicious: { ipCount: number } };
      expect(afterCleanup.suspicious.ipCount).toBe(0);
    });
  });
});

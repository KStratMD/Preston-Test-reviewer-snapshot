...
describe('requireIntegrationTenantAdmin', () => {
  const runIntegrationAdmin = (user: unknown) => runWith(requireIntegrationTenantAdmin, user);

  it('rejects an anonymous request with 401', () => {
    const r = runIntegrationAdmin(undefined);
    expect(r.status).toBe(401);
    expect(r.nextCalled).toBe(false);
  });

  it('allows the tenant_admin role', () => {
    const r = runIntegrationAdmin({ id: 'u1', tenantId: 't1', roles: ['tenant_admin'], permissions: [] });
    expect(r.nextCalled).toBe(true);
    expect(r.status).toBeUndefined();
  });

  it('rejects an authenticated tenant user without an admin claim', () => {
    const r = runIntegrationAdmin({ id: 'u1', tenantId: 't1', roles: ['user'], permissions: [] });
    expect(r.status).toBe(403);
    expect(r.nextCalled).toBe(false);
  });

  // Task 9's governing concern: `suitecentral:admin` is unrelated authority
  // for an ERP retry command. A caller holding it but NOT `tenant_admin` must
  // be refused, so this guard cannot be quietly satisfied by SuiteCentral's
  // admin permission (unlike requireSuiteCentralTenantAdmin, which honors it
  // on its own namespace).
  it('rejects a SuiteCentral-only admin (suitecentral:admin permission, no tenant_admin role)', () => {
    const r = runIntegrationAdmin({ id: 'u1', tenantId: 't1', roles: [], permissions: ['suitecentral:admin'] });
    expect(r.status).toBe(403);
    expect(r.nextCalled).toBe(false);
  });

  // Tenant identity is the target of every operation here. Without it there
  // is no scope to authorize against, so this is 401 (incomplete identity)
  // rather than 403 (identity understood, access denied).
  it('rejects an admin claim with no tenantId as unauthenticated', () => {
    const r = runIntegrationAdmin({ id: 'u1', roles: ['tenant_admin'], permissions: [] });
    expect(r.status).toBe(401);
    expect(r.nextCalled).toBe(false);
  });
  ...
  it.each([[''], ['   '], [null], [42], [{}]])(
    'rejects a non-usable tenantId (%p) as unauthenticated',
    (tenantId) => {
      const r = runIntegrationAdmin({ id: 'u1', tenantId, roles: ['tenant_admin'], permissions: [] });
      expect(r.status).toBe(401);
      expect(r.nextCalled).toBe(false);
    },
  );
  ...
  it('rejects an empty-string id as unauthenticated', () => {
    const r = runIntegrationAdmin({ id: '', tenantId: 't1', roles: ['tenant_admin'] });
    expect(r.status).toBe(401);
  });
  ...
  // The system sentinel is a fallback attribution for unauthenticated work,
  // not a tenant anyone administers.
  it('rejects the system identity sentinel as a target tenant', () => {
    const r = runIntegrationAdmin({
      id: 'u1',
      tenantId: SYSTEM_IDENTITY.tenantId,
      roles: ['tenant_admin'],
      permissions: [],
  ...
  // A platform admin is not implicitly an integration tenant admin: the
  // wildcard would let a platform actor bypass the deferred-retry ceiling on
  // any tenant's configuration without going through a tenant-scoped path.
  it('does not accept the platform wildcard permission', () => {
    const r = runIntegrationAdmin({ id: 'admin-1', tenantId: 't1', roles: ['admin'], permissions: ['*'] });
    expect(r.status).toBe(403);
    expect(r.nextCalled).toBe(false);
  });

  it('ignores non-string entries in roles/permissions', () => {
    const r = runIntegrationAdmin({ id: 'u1', tenantId: 't1', roles: [123, null], permissions: [{}] });
    expect(r.status).toBe(403);
  });
});

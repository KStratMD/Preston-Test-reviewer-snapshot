...
describe('tenantIsolation middleware', () => {
  ...
  describe('JWT-based tenant extraction', () => {
    it('prefers the normalized verified req.user tenant over raw bearer parsing', async () => {
      (jwt.verify as jest.Mock).mockReturnValue({ tenant_id: 'tenant-from-bearer' });
      ...
      expect(next).toHaveBeenCalled();
      expect(req.tenantContext).toEqual(expect.objectContaining({
        tenantId: 'tenant-from-user',
        permissions: ['write'],
      }));
      expect(req.tenantContext?.metadata?.source).toBe('verified-user');
      expect(jwt.verify).not.toHaveBeenCalled();
    });

    it('forwards the verified organizationId, which the displaced bearer path also carried', async () => {
      // The verified-user path outranks raw bearer parsing for every
      // authenticated request, so any TenantContext field a consumer reads
      // (MappingRouter cost rows, MCPRouter tenant fallback) must survive it.
      const middleware = tenantIsolation({ jwtSecret: 'test-secret' });
      const req = createMockRequest({
        user: {
          id: 'user-1',
      ...
      expect(next).toHaveBeenCalled();
      expect(req.tenantContext?.organizationId).toBe('org-42');
    });

    it('drops a non-string organizationId rather than propagating it', async () => {
      const middleware = tenantIsolation({ jwtSecret: 'test-secret' });
      const req = createMockRequest({
        user: {
          id: 'user-1',
          tenantId: 'tenant-from-user',
          organizationId: { spoofed: true },
          permissions: [],
        },
      });
      const res = createMockResponse();
      ...
      expect(next).toHaveBeenCalled();
      expect(req.tenantContext?.tenantId).toBe('tenant-from-user');
      expect(req.tenantContext?.organizationId).toBeUndefined();
    });

    it('rejects a sentinel-tenant req.user and falls through to the 403 tenant-required response', async () => {
      // Pins the strict-mode OUTCOME when a sentinel user is the only identity
      // on the request. Measured mutation coverage (PR4 Stage 1): deleting the
      // `=== SYSTEM_IDENTITY.tenantId` clause in extractTenantFromVerifiedUser
      // DOES fail this test — `__system__` satisfies isValidTenantId, so the
      // context populates and next() runs instead of the 403. Disabling the
      // whole verified-user step does NOT fail it; that regression is caught by
      // the three `source: 'verified-user'` / organizationId cases above.
      const middleware = tenantIsolation({
        strictMode: true,
    ...
    it('a sentinel-tenant req.user never outranks a valid bearer — the JWT tenant wins', async () => {
      // The PRECEDENCE half of the sentinel contract, which the 403 case above
      // cannot express: there, refusal and "no context at all" are the same
      // observable. Here a sentinel user is paired with a bearer carrying a
      // DIFFERENT, valid tenant, so the two candidate sources are
      // distinguishable in the result — the sentinel is skipped at step 2 and
      // step 3 (JWT) supplies the context. Deleting the sentinel clause makes
      // the context {tenantId: '__system__', source: 'verified-user'} and
      // fails both assertions (verified by mutation).
      //
      // This also documents the fallthrough itself: a sentinel `req.user` does
      // NOT short-circuit to a refusal, it defers to the next source. Anything
      // depending on "sentinel user ⇒ 403" is depending on there being no
      // bearer.
      (jwt.verify as jest.Mock).mockReturnValue({
        tenant_id: 'tenant-from-bearer',
        sub: 'user-1',
        permissions: [],
      });
      const middleware = tenantIsolation({
      ...
      expect(next).toHaveBeenCalled();
      expect(req.tenantContext?.tenantId).toBe('tenant-from-bearer');
    ...
    it('should use an empty JWT secret fallback when no secret is configured', async () => {
      ...
      try {
        const middleware = tenantIsolation();
        const req = createMockRequest({
          headers: { authorization: 'Bearer ignored-without-secret' },
        });
        const res = createMockResponse();
        const next = createMockNext();
  ...
  describe('header-based tenant extraction', () => {
    ...
    // A5 — this module's private `^[a-zA-Z0-9_-]{1,64}$` regex is gone; the
    // header path now runs through normalizeTenantIdClaim and stores its RETURN
    // VALUE. Deliberate behavioral change: a merely-padded header used to be a
    // 400 (the regex disallowed spaces) and is now accepted as its trimmed
    // spelling — so `' t1 '` and `'t1'` can never become two tenant identities.
    it('A5 — accepts a padded header and stores the TRIMMED spelling', async () => {
      const middleware = tenantIsolation();
      const req = createMockRequest({ headers: { 'x-tenant-id': '   padded-tenant   ' } });
      const res = createMockResponse();
      const next = createMockNext();
      ...
      expect(next).toHaveBeenCalled();
      expect(req.tenantContext?.tenantId).toBe('padded-tenant');
      expect(res.status).not.toHaveBeenCalledWith(400);
    });

    it('A5 — 64-char header accepted, 65-char rejected with 400', async () => {
      const ok = tenantIsolation();
      const reqOk = createMockRequest({ headers: { 'x-tenant-id': 'a'.repeat(64) } });
      const resOk = createMockResponse();
      await ok(reqOk, resOk, createMockNext());
      expect(reqOk.tenantContext?.tenantId).toBe('a'.repeat(64));

      const bad = tenantIsolation();
      const reqBad = createMockRequest({ headers: { 'x-tenant-id': 'a'.repeat(65) } });
      const resBad = createMockResponse();
      await bad(reqBad, resBad, createMockNext());
      expect(resBad.status).toHaveBeenCalledWith(400);
      expect(reqBad.tenantContext).toBeUndefined();
    });
    ...
    it('rejects an invalid tenant ID from a custom resolver at the context-validation gate', async () => {
      // Custom resolvers (and trusted-tenant contexts) bypass the per-source
      // validation, so the shared context-level isValidTenantId gate is the
      // only defense — this test pins that branch.
      const middleware = tenantIsolation({
        resolveTenant: async () => ({ tenantId: 'invalid resolver id!@#' }),
      });
      const req = createMockRequest({});
      const res = createMockResponse();
      const next = createMockNext();
  ...
  describe('trusted tenants', () => {
    ...
    it('should fall through to header extraction for a non-trusted tenant', async () => {
      const middleware = tenantIsolation({ trustedTenants: ['system-service'] });
      const req = createMockRequest({
        headers: { 'x-tenant-id': 'ordinary-tenant' },
      });
      const res = createMockResponse();
      const next = createMockNext();
      ...
      expect(next).toHaveBeenCalled();
      expect(req.tenantContext?.tenantId).toBe('ordinary-tenant');
      expect(req.tenantContext?.metadata?.source).toBe('header');
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('should use safe error handling for non-Error thrown values', async () => {
      const customResolver = jest.fn().mockRejectedValue('Resolver error');

...
describe('tenantStatusGate', () => {
  ...
  // ---- A5: canonical-format enforcement at the auto-registering seam ------
  // Before A5 this gate applied NO format check — only trim + empty + sentinel.
  // authMiddleware admitted up to 255 chars, tenantIsolation refused to build a
  // tenantContext above 64, but this gate still handed the claim to
  // requireActive, which auto-creates the row via ensureExists. Each case below
  // reached requireActive before the fix; all must now fail closed.

  it.each([
    ['65 chars — one past the boundary', 'a'.repeat(65)],
    ['200 chars — inside the old 255 bound', 'a'.repeat(200)],
    ['255 chars — the old boundary value', 'a'.repeat(255)],
  ])('fails closed on an over-length tenant claim (%s) instead of auto-registering it', async (_label, tenantId) => {
    const svc = { requireActive: jest.fn(async () => {}) } as any;
    const gate = makeTenantStatusGate(svc);
    await gate(mkReq(tenantId) as Request, res, next);
    expect(svc.requireActive).not.toHaveBeenCalled();
    expect(res.status).toHaveBeenCalledWith(403);
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: 'tenant_id_missing' }));
    expect(next).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    ['RTL override (U+202E)',     'acme‮co'],
    ['period',                    'acme.co'],
    ['slash',                     'acme/co'],
    ['CJK character',             'acme中'],
  ])('fails closed on a shape-illegal tenant claim (%s) instead of auto-registering it', async (_label, tenantId) => {
    const svc = { requireActive: jest.fn(async () => {}) } as any;
    const gate = makeTenantStatusGate(svc);
    await gate(mkReq(tenantId) as Request, res, next);
    expect(svc.requireActive).not.toHaveBeenCalled();
  ...
  it('accepts a claim of exactly 64 characters (the boundary is inclusive)', async () => {
    const boundary = 'a'.repeat(64);
    const svc = { requireActive: jest.fn(async () => {}) } as any;
    const gate = makeTenantStatusGate(svc);
    await gate(mkReq(boundary) as Request, res, next);
    expect(svc.requireActive).toHaveBeenCalledWith(boundary);
    expect(next).toHaveBeenCalledWith();
  });

  it('hands requireActive the TRIMMED spelling of a padded claim, never the padded one', async () => {
    // Pins the auto-registration invariant: the padded spelling must never be
    // the string that reaches ensureExists, or it would create a second row
    // rendering identically to an existing tenant.
    const svc = { requireActive: jest.fn(async () => {}) } as any;
    const gate = makeTenantStatusGate(svc);
    await gate(mkReq('   tenant-a   ') as Request, res, next);
    expect(svc.requireActive).toHaveBeenCalledWith('tenant-a');
    expect(svc.requireActive).not.toHaveBeenCalledWith('   tenant-a   ');
  });

  it('still rejects a non-string claim (the typeof guard is not subsumed by the normalizer)', async () => {
    // normalizeTenantIdClaim accepts `unknown` and coerces via String(), so
    // routing through it without the typeof guard would newly admit numerics.
    const svc = { requireActive: jest.fn(async () => {}) } as any;
    const gate = makeTenantStatusGate(svc);
    const req: Partial<Request> = {
      auth: { tenantId: 12345, userId: 'u1' },
      path: '/api/anything', originalUrl: '/api/anything',
    } as any;
    await gate(req as Request, res, next);
    expect(svc.requireActive).not.toHaveBeenCalled();
    expect(svc.requireActive).not.toHaveBeenCalledWith('12345');
  });

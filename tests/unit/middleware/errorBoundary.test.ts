...
describe('ErrorBoundary Middleware', () => {
  ...
  describe('globalErrorHandler()', () => {
    ...
    it('redacts all credential-route request bodies for non-validation failures', () => {
      mockRequest.originalUrl = '/API/credentials';
      mockRequest.body = { username: 'operator@example.com', clientId: 'credential-client-id' };
      const error = new ServiceUnavailableAppError('Credential store unavailable');
      errorHandler(error, mockRequest as Request, mockResponse as Response, mockNext);

      expect(mockLoggerError).toHaveBeenCalledWith('Unhandled error', expect.objectContaining({ body: '[REDACTED]' }));
      expect(mockLoggerError).toHaveBeenCalledWith('Request context', expect.objectContaining({ body: '[REDACTED]' }));
    });

    it('redacts credential-route bodies when path is mount-relative but originalUrl is full', () => {
      mockRequest.path = '/abc123';
      mockRequest.originalUrl = '/api/credentials/abc123';
      mockRequest.body = { clientSecret: 'should-be-redacted' };
      const error = new ServiceUnavailableAppError('Credential store unavailable');
      errorHandler(error, mockRequest as Request, mockResponse as Response, mockNext);
    ...
    it('redacts sibling connector-credential route bodies', () => {
      mockRequest.originalUrl = '/api/connector-credentials/123';
      mockRequest.body = { certificate: 'credential-certificate-sentinel' };
      const error = new ServiceUnavailableAppError('Credential store unavailable');
      errorHandler(error, mockRequest as Request, mockResponse as Response, mockNext);
    ...
    it('should handle CardinalityViolationError with a sanitized 422 body', () => {
      function makeResult(overrides: Partial<PreflightRunResult> = {}): PreflightRunResult {
        return {
          reports: [makeReport({ unavailableChecks: ['key_uniqueness'] })],
          blocking: true,
      ...
      const finding = makeFinding({ key: 'relationship_flatten|source_to_target|contacts|0' });
      ...
      expect(jsonMock).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          reportFingerprint: 'combined-fp-1',
          unavailableChecks: ['key_uniqueness'],
          findings: [expect.objectContaining({ key: 'relationship_flatten|source_to_target|contacts|0' })],
        }),
      );
    });

    it('should never leak sample values, raw keys, or credentials through the CardinalityViolationError 422 body', () => {
      const result: PreflightRunResult = {
        reports: [makeReport()],
        blocking: true,
        combinedFingerprint: 'combined-fp-2',
      };
      const hostileFinding = makeFinding() as CardinalityFinding & Record<string, unknown>;
      hostileFinding.sampleValue = 'SECRET_SAMPLE_VALUE';
      hostileFinding.credentials = { apiKey: 'sk-hostile-credential' };

      const error = new CardinalityViolationError(result, [hostileFinding]);

      errorHandler(error, mockRequest as Request, mockResponse as Response, mockNext);

      expect(statusMock).toHaveBeenCalledWith(422);
      const body = jsonMock.mock.calls[jsonMock.mock.calls.length - 1][0];

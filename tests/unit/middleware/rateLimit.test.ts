...
describe('createAiDemoRateLimit (F2)', () => {
  it('limits anonymous demo traffic to 30 requests per window per IP, JSON error on limit', async () => {
    const app = express();
    app.use(createAiDemoRateLimit());
    ...
    expect(res.body.error).toBe('demo_rate_limited');
  });
});

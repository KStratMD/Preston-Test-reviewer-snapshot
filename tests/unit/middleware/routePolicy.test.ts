...
describe('routePolicy — registry + resolver (PR-F0)', () => {
  ...
  it('supports exact policy entries with shared path normalization', () => {
    const policies: RoutePolicy[] = [
      {
        match: { pathPrefix: '/API//health/demo/', matchMode: 'exact' },
        auth: 'public',
        lifecycle: 'not_applicable',
        rateProfile: 'anonymous_read',
      },
    ];
    expect(resolveRoutePolicy('/api/health/demo', 'GET', policies)?.auth).toBe('public');
    expect(resolveRoutePolicy('/api/health/demo/details', 'GET', policies)).toBeNull();
  });

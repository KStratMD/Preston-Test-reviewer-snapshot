...
describe('guardedWrite — Stage B (PR 13b)', () => {
  ...
  // Prerequisite PR A (2026-07-27 serialized-asset plan): 'upsert' joins the
  // WriteOperation union. Exercised here through the same derivation path as
  // the business_central test above, with a salesforce target so a later
  // feature PR's Asset upsert queue path is provably supported end-to-end.
  it('queue_required + upsert operation + salesforce target: persisted descriptor carries operation=upsert and the derived registry key', async () => {
    mockResolver.validateWrite.mockResolvedValue({
      allowed: false as const,
      reason: 'queue_required' as const,
      declaredOwner: 'salesforce' as const,
    });
    const ctxUpsert = {
      ...baseContext(),
    ...
    const resumeUpsert = {
      ...
      operation: 'upsert' as const,
      entityType: 'Asset',
      args: { externalIdField: 'External_Id__c', externalIdValue: 'inv-1', data: { name: 'X' } },
    };

    await expect(
      guardedWrite(
        { context: ctxUpsert, do: async () => ({}), resume: resumeUpsert },
        baseDeps(),
      ),
    ).rejects.toBeInstanceOf(OwnershipPendingApprovalError);

    const enqueueArg = mockApprovalQueueService.enqueue.mock.calls[0][0];
    expect(enqueueArg.reason.writeDescriptor.targetSystemId).toBe('salesforce');
    expect(enqueueArg.reason.writeDescriptor.operation).toBe('upsert');
    expect(enqueueArg.reason.writeDescriptor.entityType).toBe('Asset');
  });

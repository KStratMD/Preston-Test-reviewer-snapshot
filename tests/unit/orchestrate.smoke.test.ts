...
// F2: /api/ai/proxy requires auth outside the exact demo allowlist. fastMocks'
// AuthService stub ignores tokens (no subject → 401), so restore the real
// HS256 verification path for these smoke tests.
jest.unmock('../../src/services/AuthService');

import { Server } from '../../src/index';

const smokeToken = () => jwt.sign(
  { id: 'smoke-test-user', username: 'smoke-test-user', tenantId: 'tenant-smoke', roles: ['user'] },
...
describe('Multi-Agent Orchestrator smoke test (in-process)', () => {
  ...
  (hasAIProvider ? it : it.skip)('POST orchestrate then fetch status and trace (may be ephemeral)', async () => {
    ...
    const statusRes = await request(app).get(`/api/ai/proxy/orchestrate/${encodeURIComponent(sessionId)}/status`).set('Authorization', `Bearer ${smokeToken()}`);
    expect([200, 404]).toContain(statusRes.status);

    const traceRes = await request(app).get(`/api/ai/proxy/orchestrate/${encodeURIComponent(sessionId)}/trace`).set('Authorization', `Bearer ${smokeToken()}`);

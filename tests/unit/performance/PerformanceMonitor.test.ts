...
function createSnapshot(monotonicNs: bigint, user: number, system = 0): ProcessResourceSnapshot {
  return {
    monotonicNs,
    cpu: { user, system },
    memory: {
...
function createIncrementingSampler(): ProcessResourceSampler {
  let monotonicNs = 0n;

  return {
    sample: jest.fn(() => {
      monotonicNs += 1_000_000_000n;
      return createSnapshot(monotonicNs, 0);
    }),
  };
}
...
describe('PerformanceMonitor', () => {
  ...
  beforeEach(() => {
    ...
    performanceMonitor = new PerformanceMonitor(mockLogger, createIncrementingSampler());
  });
  ...
  describe('CPU sampling', () => {
    it('uses one on-demand sample for a generated report', () => {
      const sampler = createIncrementingSampler();
      const monitor = new PerformanceMonitor(mockLogger, sampler);
    ...
    it('keeps on-demand and periodic CPU delta streams independent', () => {
      const sampler: ProcessResourceSampler = {
        sample: jest.fn()
          .mockReturnValueOnce(createSnapshot(1_000_000_000n, 0))
          .mockReturnValueOnce(createSnapshot(2_000_000_000n, 900_000))
          .mockReturnValueOnce(createSnapshot(3_000_000_000n, 1_000_000)),
      };
      const monitor = new PerformanceMonitor(mockLogger, sampler);
      const collectResourceUsage = (monitor as unknown as { collectResourceUsage: () => void }).collectResourceUsage;

      expect(monitor.getStatistics().resourceUsage.cpuUsage).toBe(0);
      collectResourceUsage.call(monitor);

      expect(monitor.getStatistics().resourceUsage.cpuUsage).toBe(50);
    });

    it('warms up without a CPU decision, then reports deterministic low CPU', () => {
      const sampler: ProcessResourceSampler = {
        sample: jest.fn()
          .mockReturnValueOnce(createSnapshot(1_000_000_000n, 0))
          .mockReturnValueOnce(createSnapshot(2_000_000_000n, 500_000))
          .mockReturnValueOnce(createSnapshot(3_000_000_000n, 1_000_000)),
      };
      const monitor = new PerformanceMonitor(mockLogger, sampler);
      ...
      expect(monitor.getStatistics().resourceUsage.cpuUsage).toBe(50);
      expect(stress.reasons).not.toContain('High CPU usage: 50.0%');
    });

    it('reports high CPU and adds a stress reason only after the second sample', () => {
      const sampler: ProcessResourceSampler = {
        sample: jest.fn()
          .mockReturnValueOnce(createSnapshot(1_000_000_000n, 0))
          .mockReturnValueOnce(createSnapshot(2_000_000_000n, 900_000))
          .mockReturnValueOnce(createSnapshot(3_000_000_000n, 1_800_000)),
      };
      const monitor = new PerformanceMonitor(mockLogger, sampler);

      expect(monitor.isSystemUnderStress().reasons).toEqual([]);
      const stats = monitor.getStatistics();

      expect(stats.resourceUsage.cpuUsage).toBe(90);
      expect(monitor.isSystemUnderStress().reasons).toContain('High CPU usage: 90.0%');
    });
  });

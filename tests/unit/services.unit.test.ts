...
describe('Service Unit Tests (Fixed)', () => {
  ...
  describe('ConfigurationService', () => {
    ...
    it('should save and retrieve configuration', async () => {
      const testConfig: IntegrationConfig = {
        ...
        syncMode: 'realtime',
        // Draft: this test only checks save+retrieve roundtrip, not the activation
        // gate, so it bypasses the gate rather than wiring command context.
        isActive: false,
        sourceEntity: 'TestEntity',
        targetEntity: 'TestEntity',
        createdAt: new Date(),
  ...
  describe('TransformationEngine', () => {
    ...
    it('fails closed on a calculation that cannot execute (was: handle errors gracefully)', async () => {
      const sourceRecord: DataRecord = {
        externalId: 'test-ext-id',
        metadata: { source: 'test', lastModified: new Date(), version: '1' },
        fields: {
      ...
      // Tranche 2 Workstream A: the engine fails closed. This test used to accept
      // partial output ('skip the field or provide a default'); a calculation that
      // cannot execute is now an error on its target field, whatever isRequired
      // says, and transformRecord() throws rather than hand back a partial record.
      await expect(transformationEngine.transformRecord(sourceRecord, fieldMappings, [])).rejects.toMatchObject({
        name: 'TransformationFailedError',
        message: expect.stringMatching(/target_field: Calculation failed/),
      });
    });

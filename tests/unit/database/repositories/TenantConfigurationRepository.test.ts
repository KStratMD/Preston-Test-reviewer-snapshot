...
import { TenantConfigurationRepository, PlaintextSecretError } from '../../../../src/database/repositories/TenantConfigurationRepository';
...
describe('TenantConfigurationRepository', () => {
  ...
  describe('getStringStrict (plaintext-only, fail-closed)', () => {
    it('returns null for a missing row (absent setting = not configured)', async () => {
      expect(await repo.getStringStrict('t1', 'missing.key')).toBeNull();
    });

    it('returns the plaintext value verbatim', async () => {
      await repo.upsert('t1', 'strict.list', '["sf-prod"]');
      expect(await repo.getStringStrict('t1', 'strict.list')).toBe('["sf-prod"]');
    });

    it('throws when the row is encrypted, rather than collapsing an outage to null', async () => {
      // getString returns null when SecretManager fails, so an encrypted row
      // would turn an infra outage into an indistinguishable "not configured".
      // Callers that must tell those apart use this method.
      await repo.upsert('t1', 'strict.enc', '["sf-prod"]', { isEncrypted: true });
      await expect(repo.getStringStrict('t1', 'strict.enc')).rejects.toThrow(
        /must be stored as plaintext/,
      );
      expect(secretManager.getSecret).not.toHaveBeenCalled();
    });
    ...
    it('does not consult SecretManager on the plaintext happy path', async () => {
      await repo.upsert('t1', 'strict.plain', 'value');
      jest.clearAllMocks();
      await repo.getStringStrict('t1', 'strict.plain');
      expect(secretManager.getSecret).not.toHaveBeenCalled();
    });
  });
  ...
  describe('getSecretString (encrypted-only, fail-closed for secrets)', () => {
    it('returns null for missing row (absent setting = no secret configured)', async () => {
      expect(await repo.getSecretString('t1', 'missing.key')).toBeNull();
      expect(secretManager.getSecret).not.toHaveBeenCalled();
    });

    it('throws PlaintextSecretError when row is plaintext — secret-bearing reads refuse cleartext', async () => {
      await repo.upsert('t1', 'sec.plain', 'plaintext-secret');
      // Assert on the TYPED error class so callers can discriminate via
      // `instanceof` rather than regex-matching the message string. The
      // Error subclass carries tenantId + settingKey for structured log
      // metadata at the call site. Copilot R1 on PR #831: an `instanceof`-
      // gated catch lets the route differentiate plaintext-misconfig (→ 401)
      // from infra errors like DB outages (→ 500) without misclassifying
      // every thrown error as a plaintext misconfig.
      await expect(repo.getSecretString('t1', 'sec.plain')).rejects.toBeInstanceOf(PlaintextSecretError);
      await expect(repo.getSecretString('t1', 'sec.plain')).rejects.toMatchObject({
        tenantId: 't1',
        settingKey: 'sec.plain',
        name: 'PlaintextSecretError',
      });
      // SecretManager MUST NOT be consulted on the throw path — symmetric to
      // getBooleanStrict's "do not consult SecretManager" invariant. The throw
      // tells the caller "this row's at-rest posture is wrong"; we don't want
      // the backend lookup to mask the misconfiguration with a successful read.
      expect(secretManager.getSecret).not.toHaveBeenCalled();
    });

    it('propagates non-PlaintextSecretError errors (e.g. DB read throws) so callers can return 500', async () => {
      // Stub the DB to throw on the SELECT — getSecretString must NOT
      // swallow this as a plaintext misconfig. Copilot R1 on PR #831:
      // collapsing every error to plaintext-misconfig would mask a real
      // infra outage as a 401 unknown_tenant.
      //
      // R4 (Copilot): wrap the monkey-patch + assertion in try/finally so
      // the restore step runs even when the assertion throws — otherwise a
      // failed expect would leave the patched `db.getDatabase` in place
      // and later tests in this file would inherit the broken stub
      // non-deterministically (jest doesn't reset module-scoped patches
      // between `it()` blocks).
      const dbHolder = repo as unknown as { db: { getDatabase: () => unknown } };
      const realDbGet = dbHolder.db.getDatabase;
      const dbErr = new Error('DB connection lost');
      dbHolder.db.getDatabase = () => {
        throw dbErr;
      };
      try {
        await expect(repo.getSecretString('t1', 'sec.any')).rejects.toBe(dbErr);
      } finally {
        dbHolder.db.getDatabase = realDbGet;
      }
    });

    it('round-trips plaintext through SecretManager when is_encrypted=true', async () => {
      await repo.upsert('t1', 'sec.enc', 'encrypted-secret-value', { isEncrypted: true });
      expect(await repo.getSecretString('t1', 'sec.enc')).toBe('encrypted-secret-value');
      expect(secretManager.getSecret).toHaveBeenCalledWith(
        expect.stringMatching(SECRET_NAME_RE),
      );
    });
    ...
    it('returns null and logs error when SecretManager.getSecret throws', async () => {
      await repo.upsert('t1', 'sec.fail', 'plaintext', { isEncrypted: true });
      const thrownError = new Error('backend unavailable');
      secretManager.getSecret.mockRejectedValueOnce(thrownError);
      expect(await repo.getSecretString('t1', 'sec.fail')).toBeNull();
      // Same Logger.error contract as getString — (message, error?, metadata),
      // with the Error in 2nd position.
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('decrypt'),
        thrownError,
        expect.objectContaining({ tenantId: 't1', settingKey: 'sec.fail' }),
      );
    });

    it('rejects rows whose setting_value mismatches the deterministic secret name (anti-exfiltration)', async () => {
      // Direct DB insert simulating a tampered/corrupted row: is_encrypted=true
      // but setting_value points at an attacker-chosen name. Without the guard,
      // SecretManager's env-fallback could resolve `PATH` to process.env.PATH.
      await db.insertInto('tenant_configurations').values({
        id: 'tampered-sec',
        tenant_id: 't1',
        setting_key: 'sec.tampered',
        setting_value: 'PATH',
        is_encrypted: true,
        created_at: new Date(),
        updated_at: new Date(),
      }).execute();
      expect(await repo.getSecretString('t1', 'sec.tampered')).toBeNull();
      // Mismatch guard kicks in BEFORE any backend lookup.
      expect(secretManager.getSecret).not.toHaveBeenCalled();
      // Same mismatch log shape as getString — no Error on the mismatch path,
      // so `undefined` is the 2nd arg and metadata is the 3rd.
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('mismatches deterministic secret name'),
        undefined,
        expect.objectContaining({ tenantId: 't1', settingKey: 'sec.tampered' }),
      );
    });
  });
});

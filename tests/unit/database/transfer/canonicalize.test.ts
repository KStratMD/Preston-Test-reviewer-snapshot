...
describe('canonicalize transfer values', () => {
  it.each([2 ** 53, -(2 ** 53), Number(9007199254740993n), Number(-9007199254740993n)])('rejects unsafe integer Number %s', (value) => {
    expect(() => canonicalize(value, 'integer')).toThrow(/Unsafe integer number/);
  });

  it('accepts safe numbers and exact large integer representations', () => {
    for (const [value, expected] of [
      [Number.MIN_SAFE_INTEGER, '-9007199254740991'], [Number.MAX_SAFE_INTEGER, '9007199254740991'],
      [-0, '0'], [9007199254740993n, '9007199254740993'], [-9007199254740993n, '-9007199254740993'],
      ['9007199254740993', '9007199254740993'], ['-9007199254740993', '-9007199254740993'],
    ] as const) expect(canonicalize(value, 'integer')).toEqual({ type: 'integer', value: expected });
  });

  it('retains fraction/nonfinite rejection and decimal/float behavior', () => {
    expect(() => canonicalize(1.25, 'integer')).toThrow('Invalid integer');
    for (const value of [NaN, Infinity, -Infinity]) {
      expect(() => canonicalize(value, 'integer')).toThrow('Non-finite number');
    }
    expect(canonicalize(1.25, 'decimal')).toEqual({ type: 'decimal', value: '1.25' });
    expect(canonicalize(2 ** 53, 'float64')).toEqual({ type: 'float64', value: '9007199254740992' });
    expect(canonicalize(2 ** 53, 'decimal')).toEqual({ type: 'decimal', value: '9007199254740992' });
  });

...
# 1b. Invalid manifest match mode → MATCH_MODE
make_fixture "$TMP/invalid-mode"
mutate_file "$TMP/invalid-mode/src/middleware/setup/routeManifest.ts" \
  "fs.writeFileSync(f, fs.readFileSync(f,'utf8').replace(\"{ path: '/api/alpha', classification: 'tenant_required' }\", \"{ path: '/api/alpha', matchMode: 'wildcard', classification: 'tenant_required' }\"));"
run_case "invalid-match-mode" 1 "MATCH_MODE" "$TMP/invalid-mode"

# 1c. Manifest/policy mode drift → MATCH_MODE
make_fixture "$TMP/mode-mismatch"
mutate_file "$TMP/mode-mismatch/src/middleware/setup/routeManifest.ts" \
  "fs.writeFileSync(f, fs.readFileSync(f,'utf8').replace(\"{ path: '/api/alpha', classification: 'tenant_required' }\", \"{ path: '/api/alpha', matchMode: 'exact', classification: 'tenant_required' }\"));"
run_case "manifest-policy-mode-mismatch" 1 "MATCH_MODE" "$TMP/mode-mismatch"

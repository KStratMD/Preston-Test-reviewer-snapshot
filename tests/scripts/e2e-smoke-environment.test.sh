...
# E2E_SMOKE_LOCK_FILE substitutes a lockfile fixture (repo-relative) for the
# self-test at the bottom of this file; normal runs read package-lock.json.
run_guard() {
node - <<'NODE'
const fs = require('fs');
...
const LOCK_FILE = process.env.E2E_SMOKE_LOCK_FILE || 'package-lock.json';
...
//    Every playwright / playwright-core install (top-level or nested) must match
//    too: #1325 bumped bare `playwright` alone, which left @playwright/test
//    matching the image while a second playwright-core needed other browsers.
for (const [key, entry] of Object.entries(lock.packages)) {
  if (!/(^|\/)node_modules\/(playwright|playwright-core)$/.test(key)) continue;
  if (!entry || entry.version !== imageVersion) {
    fail(`${key} ${entry && entry.version} in ${LOCK_FILE} does not match container image playwright v${imageVersion}`);
  }
}
...
# Self-test: the version-match rule must reject the #1325 shape (a root
# `playwright` off the image version) and a nested playwright-core, not only a
# mismatched @playwright/test. Each fixture is the real lockfile with one entry
# changed, so a failure for any other reason is reported, not counted.
if [ -z "${E2E_SMOKE_LOCK_FILE:-}" ]; then
  fixture_dir="$(mktemp -d "$ROOT_DIR/.e2e-guard-fixture.XXXXXX")"
  trap 'rm -rf "$fixture_dir"' EXIT
  rel_dir="$(basename "$fixture_dir")"
  node -e '
    const fs = require("fs");
    const [dir] = process.argv.slice(1);
    const base = JSON.parse(fs.readFileSync("package-lock.json", "utf8"));
    const rootSplit = structuredClone(base);
    rootSplit.packages["node_modules/playwright"].version = "0.0.0-split";
    fs.writeFileSync(`${dir}/split-root.json`, JSON.stringify(rootSplit));
    const nestedSplit = structuredClone(base);
    nestedSplit.packages["node_modules/@playwright/test/node_modules/playwright-core"] = { version: "0.0.0-split" };
    fs.writeFileSync(`${dir}/split-nested.json`, JSON.stringify(nestedSplit));
  ' "$rel_dir"
  for fixture in split-root split-nested; do
    if out="$(E2E_SMOKE_LOCK_FILE="$rel_dir/$fixture.json" run_guard 2>&1)"; then
      echo "FAIL: version guard accepted the $fixture lockfile fixture"
      exit 1
    fi
    if ! grep -q "0.0.0-split in .* does not match container image" <<<"$out"; then
      echo "FAIL: $fixture fixture failed for another reason: $out"
      exit 1
    fi
  done
  echo "PASS: version guard rejects root and nested Playwright installs off the image version"
fi

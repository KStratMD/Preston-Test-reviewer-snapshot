...
grep -Eq "audit-status-claims: OK \([0-9]+ connectors; production=[0-9]+ production_ready=[0-9]+ beta=[0-9]+ demo_only=[0-9]+ stub=[0-9]+\)" "$REAL_OUT" \
...
write_metrics() {
  local prod="$1" beta="$2" demo="$3" stub="$4" unknown="$5" ready="${6:-0}"
...
[[ "$OUT" == *"audit-status-claims: OK (1 connectors; production=1 production_ready=0 beta=0 demo_only=0 stub=0)"* ]] \
...
# D4 (option B): production_ready is the production TIER. It must carry every
# production obligation, or the relabel has loosened the gate.
rm -f "$TMP_DIR/src/connectors/"*.ts
write_metrics 0 0 0 0 0 1
write_connector "ReadyConnector.ts" "  static readonly productionStatus = 'production_ready' as const;
  static readonly statusEvidence = 'readiness-complete, no live evidence';"
if run_audit OUT ERR; then
  fail "production_ready connector without a proofCard must be rejected like production"
fi
[[ "$ERR" == *"requires a static proofCard"* ]] \
  || fail "production_ready proofCard obligation not enforced -- err='$ERR'"
echo "PASS: production_ready connector is held to the proof-card obligation"

...
An auto-generated architecture wiki + "chat with this codebase" interface for this snapshot is available at **https://deepwiki.com/KStratMD/Preston-Test-reviewer-snapshot** (Cognition's DeepWiki; free for public repos). Useful as a fast orientation layer before running the verification recipes above.

Two caveats:

- DeepWiki is **auto-generated third-party analysis, not maintainer-authored**. Where it disagrees with the proof cards, audit scripts, or `metrics.json`, those remain the evidence of record.
- This mirror is an orphan force-pushed snapshot, so DeepWiki's index **may lag the latest push**. Cross-check the `SOURCE_COMMIT` sentinel against the commit DeepWiki reports having indexed.

...
Mirror copy of Part 1 lives at [`docs/CODEX-WORKING-NOTES.md`](docs/CODEX-WORKING-NOTES.md) and is the canonical source if the two ever drift. The sync guard at `scripts/check-cli-agent-notes-sync.mjs` enforces this.
...
- TypeScript Node app. Main branch: `main`. Latest stable SHA at last update: `41d33449`.
- Unit suite: ~9,868 passing, 100% on the unit profile (`jest.ci.config.cjs`).
...
- `.core-coverage-budget.json` — per-file floor for 15 load-bearing files. **Both directions are tracked**: regressions fail CI; improvements without a re-stamp also fail. Re-stamp via the script's `--write` in the same PR.
...
The runtime AI agent orchestrator (FieldMappingAgent, DataQualityAgent, etc.) is documented in `src/services/ai/` and the proof cards at `docs/review/proof-cards/`. See `CLAUDE.md` for the production AI provider setup (OpenAI, Anthropic, OpenRouter, LMStudio).

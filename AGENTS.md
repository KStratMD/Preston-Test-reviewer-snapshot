...
Use the nearest `AGENTS.md` first. This root file is only the repo-wide baseline.

- Before starting any task, use the applicable installed superpowers/skills. If a requested or clearly relevant superpower is unavailable in the current agent environment, say so and continue with the closest repo-compatible workflow.
- Prefer `rg` / `rg --files` for search and `apply_patch` for edits.
- Keep changes surgical and do not revert user work or use destructive git commands.
- Use git worktrees for parallel work when needed.
- Verify with the relevant tests before claiming success.
- Check `CLAUDE.md`, `README.md`, and `docs/CODEX-WORKING-NOTES.md` for fuller repo context.
- **Current shared session handoff:** [`docs/SESSION-HANDOFF-2026-09-23.md`](docs/SESSION-HANDOFF-2026-09-23.md). Read this before starting a new session; it is the repository-canonical starting point for all agents.
...
`AGENTS.md` Part 1 is the canonical shared workflow for every CLI agent. [`docs/CODEX-WORKING-NOTES.md`](docs/CODEX-WORKING-NOTES.md) is its exact guarded mirror; if they drift, repair the mirror from Part 1. `CLAUDE.md` loads this workflow and adds Claude-specific discovery and technical references. The sync guard at `scripts/check-cli-agent-notes-sync.mjs` enforces mirror equality.

# Part 1 — CLI Agent Working Notes
...
- Karpathy principles apply: think-before-coding, simplicity first, surgical changes, goal-driven execution. Spelled out, with their source, in `.claude/commands/karpathy-check.md` (readable by any agent; only the slash command itself is Claude-specific).
...
## Task effort protocol

Use the owner's task-based defaults for Codex and Claude. Choose effort for the
current phase from its risk and uncertainty, not its label or line count.
Explicit owner settings and mandatory tool/agent constraints take precedence.

| Task or phase | Typical effort |
| --- | --- |
| Architecture, design, planning, security, ambiguous requirements | High |
| Debugging, moderate implementation, test design | Medium |
| Small edits, straightforward implementation of a settled plan, routine commands | Low |
| Independent review of consequential changes | High |

- **Route by phase.** A task can use high effort to settle its design, low or
  medium for bounded implementation, and high for consequential independent
  review. Straightforward implementation qualifies for low only when the plan,
  acceptance criteria and relevant behavior are understood. Running an existing
  test command is routine; designing a test or diagnosing its failure is not.
- **Escalate when evidence warrants it.** Move low work to medium when new logic,
  unclear behavior or unexplained failures appear. Use high for security,
  authorization/approval boundaries, destructive data operations, substantial
  contract changes, architectural uncertainty, or repeated failures without a
  clear cause. A tiny permissions change can need high effort. Return to the
  lower default when the remaining steps are bounded and understood.
- **Keep verification and review intact.** Low effort does not waive tests,
  independent review, Copilot, CI, hooks or owner approval. Review effort is
  chosen independently of implementation effort. Effort routing does not
  authorize new tasks, delegation, model/provider changes, expanded scope or
  additional spending outside existing authorization.
- **Use real controls where available.** When an authorized tool or reviewer
  launch exposes an effort setting, request the selected supported level while
  preserving the assigned model and reviewer. Claude CLI launches may use
  `--effort low`, `--effort medium` or `--effort high` when supported; verify
  capability rather than assuming every model/tool accepts the same levels.
  Routine effort selection within these defaults needs no extra confirmation.
- **Be honest about session limits.** A routing decision or deeper analysis does
  not change a backend setting. If the active session's effort cannot be changed,
  continue with appropriate analysis using the available session and disclose
  that limitation briefly. Do not claim an actual setting changed, edit global
  configuration, or start another task solely to simulate a switch. An explicit
  owner requirement for a particular unavailable setting must be surfaced.
- **Remind the owner at phase transitions.** This applies to every executor and
  reviewer using the shared workflow. When an agent cannot change its own effort
  setting, briefly recommend the appropriate effort setting before a material
  phase transition (for example, high for design or consequential review, medium
  for debugging, or low for settled implementation). Do not repeat the reminder
  for every command or claim the owner changed a setting without confirmation.
  Optional changes need not block progress; if the owner explicitly requires a
  particular setting before proceeding, wait for confirmation. Include any pending
  setting request in the handoff so the next agent can honor it.
- **Keep the record lightweight.** At the start or handoff of substantial work,
  give one brief line with the requested effort for relevant phases and the
  reason for any escalation. Distinguish requested effort, the control actually
  supplied, and effective effort when the tool reports it; otherwise say the
  effective setting is not exposed. Do not infer it from elapsed time, token
  counts or prose quality, and do not narrate an effort decision for every command.

## Dependency maintenance

For recurring dependency triage and implementation, use the focused
[dependency-maintenance runbook](docs/operations/DEPENDENCY-MAINTENANCE.md).
It defines evidence collection, risk classification, current model defaults,
review/CI requirements, and the distinction between a candidate update and merge
approval. Agent entry points link to that runbook; do not create competing copies
of its procedure or current inventory in agent memory.
...
1. **Create the connector class** — `src/connectors/<Name>Connector.ts`
   ...
   ```ts
   static readonly productionStatus = 'demo_only' as const;  // or 'production_ready' / 'beta' / 'production' / 'stub'
...
# Part 2 — AI Agents System Documentation

The runtime AI agent orchestrator (FieldMappingAgent, DataQualityAgent, etc.) is documented in `src/services/ai/` and the proof cards at `docs/review/proof-cards/`. See `docs/tutorials/ai-provider-configuration-guide.md` for the production AI provider setup (OpenAI, Anthropic, OpenRouter, LMStudio).

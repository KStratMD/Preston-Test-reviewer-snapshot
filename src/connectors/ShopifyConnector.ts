...
export class ShopifyConnector extends BaseConnector implements IConnector {
    ...
    protected override async performAuthentication(probe: AuthenticationProbe): Promise<boolean> {
        if (!this.shopName || !this.accessToken) throw new Error('Shopify connector requires shopName and accessToken');
        try {
    ...
    protected override resetAuthenticationCredentials(): void {
        ...
        delete this.httpClient.defaults.headers.common['X-Shopify-Access-Token'];
    }
    ...
    private async probeAuthentication(probe: AuthenticationProbe): Promise<void> {
        this.httpClient.defaults.headers.common['X-Shopify-Access-Token'] = this.accessToken;
        await probe.request({ method: 'GET', url: '/shop.json' });
    }

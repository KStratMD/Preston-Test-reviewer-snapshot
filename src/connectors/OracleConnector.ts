...
export class OracleConnector extends BaseConnector implements IConnector {
  ...
  constructor(
    ...
    if (!outboundGovernance) {
      // "gated" = productionStatus production OR beta (Oracle is beta) — the
      // statuses the outbound-DLP CI gate requires this wiring for.
      throw new Error('OutboundGovernanceService is required for gated (production/beta) connector outbound protection');
    }
  ...
  protected override async performAuthentication(probe: AuthenticationProbe): Promise<boolean> {
    if (this.authConfig?.type !== 'basic' && this.authConfig?.type !== 'api_key') {
      throw new Error('Oracle connector requires Basic or API Key authentication');
    }
    if (this.demoMode) {
  ...
  async create(entityType: string, data: DataRecord): Promise<DataRecord> {
    ...
    const validatedData = await this.validateOutboundWrite(this.outboundGovernance, 'create', entityType, data);

    if (this.demoMode) {
      return this.demoCreate(entityType, validatedData);
    }

    const tableName = this.getTableName(entityType);
    const payload = this.formatDataForOracle(validatedData);
  ...
  async update(entityType: string, id: string, data: Partial<DataRecord>): Promise<DataRecord> {
    ...
    const validatedData = await this.validateOutboundWrite(this.outboundGovernance, 'update', entityType, data, { resourceId: id });

    if (this.demoMode) {
      return this.demoUpdate(entityType, id, validatedData);
    }

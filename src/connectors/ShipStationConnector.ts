...
export class ShipStationConnector extends BaseConnector implements IConnector {
  ...
  protected override async performAuthentication(probe: AuthenticationProbe): Promise<boolean> {
    if (!this.apiKey || !this.apiSecret) throw new Error('ShipStation connector requires apiKey and apiSecret');
    try {
      await this.probeAuthentication(probe);
      return true;
    } catch (error) {
  ...
  private async probeAuthentication(probe: AuthenticationProbe): Promise<void> {
    this.httpClient.defaults.headers.common.Authorization =
      `Basic ${Buffer.from(`${this.apiKey}:${this.apiSecret}`).toString('base64')}`;
    await probe.request({ method: 'GET', url: '/carriers' });
  }

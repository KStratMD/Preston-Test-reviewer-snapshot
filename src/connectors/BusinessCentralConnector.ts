import { BaseConnector, type AuthenticationProbe } from '../core/BaseConnector';
...
export class BusinessCentralConnector extends BaseConnector implements IConnector {
  ...
  async initialize(config: AuthConfig): Promise<void> {
    ...
    this.initializeMetadataClient(baseUrl, demoMode);
  ...
  private initializeMetadataClient(baseUrl: string, demoMode: boolean): void {
  ...
  protected override async performAuthentication(probe: AuthenticationProbe): Promise<boolean> {
    if (this.authConfig?.type !== 'oauth2') throw new Error('Business Central connector requires OAuth2 authentication');
    try {
      ...
      const tokenInfo = await this.authService.authenticateOAuth2(authCredentials, { cache: 'bypass' });
      ...
      if (!this.companyId) {
        await this.fetchCompanyId(probe);
        this.initializeMetadataClient(
          `https://api.businesscentral.dynamics.com/${this.apiVersion}/${this.tenantId}/${this.environment}`,
          isDemoMode() || isTestEnvironment(),
        );
      }
  ...
  private async fetchCompanyId(probe: AuthenticationProbe): Promise<void> {
    try {
      const response = await probe.request<BusinessCentralResponse<{ id: string }>>({
        method: 'GET',
        url: '/companies',
      });

...
export class SuiteCentralProductionConnector extends BaseConnector implements IConnector {
  ...
  protected override async performAuthentication(probe: AuthenticationProbe): Promise<boolean> {
    if (this.authConfig?.type !== 'api_key') throw new Error('SuiteCentral connector requires API key authentication');
    if (this.apiKey) this.httpClient.defaults.headers.common.Authorization = `Bearer ${this.apiKey}`;
    if (!this.isProductionMode) {
      // Demo mode - always authenticate successfully
      this.logger.info('SuiteCentral demo authentication successful');
      return true;
    }
    if (!this.apiKey) throw new Error('SuiteCentral API key is not configured');
    
    try {
      // Production mode - validate API key with SuiteCentral
      const authResponse = await probe.request<{ valid: boolean; permissions?: string[] }>({
        method: 'POST',
        url: '/auth/validate',
        data: {
          apiKey: this.apiKey,
  ...
  protected override onReconfigurationReset(): void {
    ...
    delete this.httpClient.defaults.headers.common['X-SuiteCentral-Tenant'];
  }

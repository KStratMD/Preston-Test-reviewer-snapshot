...
export class DemoConnectorDecorator implements IConnector {
  ...
  /**
   * Native upsert-by-external-id. No demo-mode in-memory emulation exists
   * yet (unlike create/read/update/delete/list/search above), so — matching
   * every sibling CRUD method's demo-guard-first structure — demo mode MUST
   * NOT delegate: once a real connector (e.g. Salesforce) implements upsert,
   * an unconditional delegation here would let a demo-mode run issue a real
   * external mutation. Fail closed with the decorator's typed error instead;
   * delegate only when demo mode is off.
   */
  async upsert(
    entityType: string,
    ...
    if (isDemoMode()) {
      throw new ValidationError('upsert is not supported in demo mode', []);
    }
    return this.inner.upsert(entityType, externalIdField, externalIdValue, data);
  }

 ...
 */
// The vocabulary lives in ./connectorStatuses (mirrored by scripts/lib/connectorStatuses.mjs
// for the CI gates; a test asserts the two agree). Re-exported so existing imports keep working.
export type { ProductionStatus } from './connectorStatuses';
import { isProductionTier } from './connectorStatuses';
import type { ProductionStatus } from './connectorStatuses';
...
export interface ConnectorRegistration {
   ...
   * Required for every production-TIER status ('production' or 'production_ready'; the same gate enforced
...
/**
 * Returns the production TIER: entries whose status is `production` or
 * `production_ready`. Both are readiness-complete and carry every production
 * obligation; the difference is only whether live evidence is on record.
 * Nothing in src/ may treat `production_ready` as non-production-tier.
 */
export function listProductionConnectors(): ConnectorRegistration[] {
  return CONNECTOR_REGISTRY.filter((entry) => isProductionTier(entry.productionStatus));
}

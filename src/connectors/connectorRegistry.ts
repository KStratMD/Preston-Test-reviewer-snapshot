 ...
 *     `inversify.config.ts`; consumed by `src/routes/suiteCentralProd.ts`.
...
export const CONNECTOR_REGISTRY: readonly ConnectorRegistration[] = [
  ...
  },
  {
    ...
    factory: (systemId, deps) => new OracleConnector(systemId, deps.logger, deps.authService),
    diBindingAvailable: false,
    bulkRollbackStrategy: 'unsupported',
    notes: 'ORDS REST scaffolding; basic CRUD only — needs broader API depth before promotion to production.',
  },
  {
    key: 'paypal',
  ...
  },
  {
    ...
    diBindingAvailable: true,
    bulkRollbackStrategy: 'unsupported',
    notes: 'Legacy *ConnectorProd.ts naming exception; bound at inversify.config.ts and used by src/routes/suiteCentralProd.ts. DI-only — no factory closure (not reachable through ConnectorManager.createConnector by design). PR 6B will consolidate the three SuiteCentral variants.',
  },
  {

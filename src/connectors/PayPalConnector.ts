...
export class PayPalConnector extends BaseConnector {
  ...
  protected override async performAuthentication(probe: AuthenticationProbe): Promise<boolean> {
    ...
    try {
      ...
      const response = await probe.request<{ access_token: string; expires_in: number }>({
        method: 'POST',
        url: '/v1/oauth2/token',
  ...
  protected override isTokenExpired(): boolean {
    return !this.demoMode && (!this.accessToken || !this.tokenExpiry || Date.now() >= this.tokenExpiry);
  }

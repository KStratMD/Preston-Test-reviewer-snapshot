...
export class SAPConnector extends BaseConnector implements IConnector {
  ...
  protected override onReconfigurationReset(): void {
    ...
    delete this.httpClient.defaults.headers.common['sap-client'];
  }
   ...
   */
  // maxRetries is retained for call-site compatibility; BaseConnector owns retries.
  async syncMaterials(options: { batchSize?: number; maxRetries?: number } = {}): Promise<SyncResult> {
    ...
    try {
      await this.ensureAuthenticated();
      this.logger.info('Starting optimized SAP material sync', { batchSize });
      ...
      while (hasMore) {
        ...
        this.logger.debug('Fetching material batch', { skip, batchSize });

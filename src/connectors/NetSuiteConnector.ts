...
export class NetSuiteConnector extends BaseConnector implements IConnector {
  // production_ready: readiness-complete (governance, rate limiting, DLP, unit +
  // contract suites, docs) with NO live-credential evidence on record. Same
  // obligations as 'production'; see the proof card's Known Gaps.
  static readonly productionStatus = 'production_ready' as const;
  ...
  protected override async performAuthentication(): Promise<boolean> {
    if (!this.accountId || !this.consumerKey || !this.consumerSecret || !this.tokenId || !this.tokenSecret) {
      throw new Error('NetSuite OAuth1 credentials are not configured');
    }
    try {
      const credentials = {
        type: 'token' as const,
  ...
  protected override onReconfigurationReset(): void {
    this.accountId = '';
    this.consumerKey = '';
    this.consumerSecret = '';
    this.tokenId = '';
  ...
  protected override prepareAuthenticatedRequest(config: AxiosRequestConfig): AxiosRequestConfig {
    // All production callsites encode query parameters in url. Refuse a
    // separate params bag rather than dispatch a signature missing parameters.
    if (config.params !== undefined) throw new Error('NetSuite query parameters must be encoded in the request URL');
    // Request config permits optional entries; AxiosHeaders normalizes their
    // undefined values, although its input type omits those optional fields.
    const headers = AxiosHeaders.concat({ 'Content-Type': 'application/json', Accept: 'application/json' }, config.headers as RawAxiosHeaders | undefined);
    headers.set('Authorization', this.getAuthHeaders(config.method ?? 'GET', this.httpClient.getUri(config)).Authorization, true);
    return {
      ...config,
      headers,
    };

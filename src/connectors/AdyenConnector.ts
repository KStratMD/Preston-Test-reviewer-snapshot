...
export class AdyenConnector extends BaseConnector {
  ...
  async initialize(config: AuthConfig): Promise<void> {
    ...
    this.httpClient.defaults.baseURL = this.getDefaultBaseUrl();
  ...
  private async probeAuthentication(probe: AuthenticationProbe): Promise<void> {
    ...
    await probe.request({ method: 'POST', url: '/v1/me', data: { merchantAccount: this.merchantAccount } });
  }
  ...
  protected override onReconfigurationReset(): void {
    ...
    this.merchantAccount = undefined;
    this.demoMode = false;
  }

...
export class SuiteCentralConnectorProd extends BaseConnector {
  ...
  protected override async performAuthentication(): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error('SuiteCentral authentication failed');
  ...
  /**
   * Clear connector-local credentials with the base lifecycle. Freshness below
   * retains this pinned client's exact-expiry policy, without the base buffer.
   */
  protected override resetAuthenticationCredentials(): void {
    super.resetAuthenticationCredentials();
    this.accessToken = undefined;
    this.tokenExpiresAt = undefined;
  }

  protected override canReuseAuthentication(): boolean { return this.isTokenValid(); }

  protected override isTokenExpired(): boolean { return !this.isTokenValid(); }
  ...
  private async pinnedRequest(
    ...
    if (options?.auth !== false) await this.awaitPendingAuthentication();
    ...
    const generation = this.captureAuthenticationGeneration();
    ...
    if (response.status === 401) {
      // A pending replacement owns its token fields. After it settles, only
      // invalidate if it reused this same generation (or failed to replace it).
      if (options?.auth !== false) {
        try { await this.awaitPendingAuthentication(); } catch { /* Report the original 401 below. */ }
      }

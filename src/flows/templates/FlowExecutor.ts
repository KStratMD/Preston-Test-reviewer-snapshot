    ...
    if (decision.approvalRequired) {
      ...
      try {
        const approvalId = await this.approvalQueue.enqueue({
          ...
          // Derived, not defaulted (Task B4b). With no caller userId the
          // executor boundary is the requester and 'system' is the truth; with
          // one, only the caller knows how that identity was established, so we
          // forward exactly what they proved. Anything else reads as 'unknown'
          // and cannot be approved, which is the intended direction.
          requesterProvenance: ctx.userId === undefined ? 'system' : ctx.requesterProvenance,
      ...
      const targetRecordId = await guardedWrite(
        {
          context: {
            ...
            correlationId: ctx.correlationId,
            requesterUserId: ctx.userId ?? SYSTEM_IDENTITY.userId,
            // Same derivation as the enqueue above: no caller userId means the
            // executor boundary is the requester ('system'); with one, only the
            // caller can say how it was established (Task B4b).
            requesterProvenance: ctx.userId === undefined ? 'system' : ctx.requesterProvenance,
            operation: template.target.operation,
          },
          do: async () => {

...
/**
 * What the executor passes into template hooks. PR 12 ships `lineageRecorder`
 * as an optional FlowContext field — opt-in lineage instrumentation for the
 * transform / governance-decision / target-write events. The PR 12 follow-up
 * adds `sourceRecord` — when both `lineageRecorder` and `sourceRecord` are
 * provided, the executor also emits a `source_read` event as the first row of
 * the chain. PR 13 will add `ownershipResolver`. The context stays intentionally
 ...
 * call. The executor normalizes `template.target.system` through
 * `SOURCE_SYSTEM_TO_CONNECTOR_KEY` (so `business_central` →
 * `businesscentral`) and compares case-insensitively to
 * `connector.systemType`, rejecting with `{status: 'failed'}` on mismatch.
 */
...
import type { RequesterProvenance } from '../../services/governance/ApprovalQueueRepository';
import type { LineageRecorder } from '../../services/lineage/LineageRecorder';
...
export interface FlowContext {
  ...
  userId?: string;
  /**
   * How `userId` was established, when the caller can say. Only a caller that
   * actually verified the identity may set this — a flow triggered behind JWT
   * auth passes 'jwt'; one that merely received a user id from elsewhere passes
   * nothing, and the request is treated as unverified.
   *
   * Omitting it is the safe direction and the reason it is optional: an
   * approval whose requester cannot be vouched for must not be approvable
   * (decision D-SoD). Do not set it to make an approval go through.
   */
  ...
  /**
   * Optional record-level lineage recorder (PR 12). When provided, the
   * executor starts a lineage chain at the top of `execute()` and emits
   * `transform`, `governance_decision`, and `target_write` events at the
   * corresponding pipeline steps. When undefined, the executor is a no-op
   * w.r.t. lineage — instrumentation is fully opt-in so existing callers
   * remain unchanged. `source_read` emission additionally requires
   * `sourceRecord` (below).
   */
  lineageRecorder?: LineageRecorder;
  /**
   * Optional source-record identifier (PR 12 follow-up). When provided AND
   * `lineageRecorder` is also present, FlowExecutor emits a `source_read`
   * event as the FIRST row of the chain — before the transform step. Callers
   * that ingest events at a connector poll / webhook boundary should
   * resolve the triple from the inbound event metadata and pass it through.
   * Omitting `sourceRecord` keeps the executor's pre-emitter behaviour
   * (transform → governance → target_write only).
   */
  sourceRecord?: { system: string; entityType: string; entityId: string };
}
...
export interface FlowTemplate<TSourceEvent = unknown, TTargetRecord extends Record<string, unknown> = Record<string, unknown>> {
  ...
  source: { system: SourceSystem; eventType: string };
  target: FlowTarget;

...
export const businessCentralExtensionAdapter: EmbeddedPlatformAdapter = {
  ...
  requiredConfigKeys: [
    ...
    // Not sent in the bootstrap body — the server takes the tenant from the
    // service-token row — but it IS part of the signed user-assertion material,
    // so the producer must know the same value. A mismatch signs the wrong
    // material and fails verification indistinguishably from a wrong secret.
    'SuiteCentralTenantId',
  ],
};

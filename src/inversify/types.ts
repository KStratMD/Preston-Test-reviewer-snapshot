const TYPES = {
  ...
  // SuiteCentral control-plane (PR-A1..A5): tenant-scoped persistence,
  // write-only secret store, outbound allowlist/DNS policy, pinned transport,
  // sanitized audit trail, application service, and tenant-keyed monitoring.
  SuiteCentralControlPlaneRepository: Symbol.for('SuiteCentralControlPlaneRepository'),
  SuiteCentralSecretStore: Symbol.for('SuiteCentralSecretStore'),
  SuiteCentralOutboundPolicy: Symbol.for('SuiteCentralOutboundPolicy'),
  SuiteCentralPinnedHttpsTransport: Symbol.for('SuiteCentralPinnedHttpsTransport'),
  SuiteCentralAuditWriter: Symbol.for('SuiteCentralAuditWriter'),
  SuiteCentralControlPlaneService: Symbol.for('SuiteCentralControlPlaneService'),
  SuiteCentralMonitoringRuntime: Symbol.for('SuiteCentralMonitoringRuntime'),
  // New integration services
  ConnectorManager: Symbol.for('ConnectorManager'),
  ConnectorCredentialResolver: Symbol.for('ConnectorCredentialResolver'),
  // Tenant-ownership registry for managed (`secret_manager`) system references.
  // SecureCredentialManager's secret namespace is platform-global, so this is
  // what stops a tenant-A configuration from resolving tenant B's credentials.
  TenantSystemCredentialRegistry: Symbol.for('TenantSystemCredentialRegistry'),
  // Lazy provider so ConnectorManager (sync-bound) can inject
  // ConnectorCredentialResolver without cascading the async-bound
  // SecureCredentialManager -> SecretManager chain through it (see
  // TenantConfigurationRepositoryProvider for the established pattern).
  SecureCredentialManagerProvider: Symbol.for('SecureCredentialManagerProvider'),
  ...
  // A8: raw 32-byte HMAC key for the ownership-approval dedupe fingerprint.
  ApprovalFingerprintKey: Symbol.for('ApprovalFingerprintKey'),
  // A9: durable forced serialized-asset retry — persistence, the narrow
  // route-facing service, and the worker that drains claimed operations.
  SerializedAssetRetryOperationRepository: Symbol.for('SerializedAssetRetryOperationRepository'),
  SerializedAssetRetryOperationService: Symbol.for('SerializedAssetRetryOperationService'),
  SerializedAssetRetryJob: Symbol.for('SerializedAssetRetryJob'),
  /**
   * The narrow seam between the retry job and the sync engine. Bound
   * separately from IntegrationService so the job receives COUNTERS rather
   * than a SyncResult — see ForcedRetryExecutor's note on why.
   */
  SerializedAssetForcedRetryExecutor: Symbol.for('SerializedAssetForcedRetryExecutor'),
  ...
  EmbeddedRoleGrantRepository: Symbol.for('EmbeddedRoleGrantRepository'),
  UserAssertionNonceRepository: Symbol.for('UserAssertionNonceRepository'),
  ...
  // Cardinality preflight and activation gate: relationship-evidence
  // normalizer (Task 4) + trusted preflight coordinator (Task 5).
  RelationshipEvidenceProvider: Symbol.for('RelationshipEvidenceProvider'),
  CardinalityPreflightService: Symbol.for('CardinalityPreflightService'),
  // The activation gate bundle (coordinator + audit) that ConfigurationService
  // optionally injects to authorize active saves (Task 7).
  CardinalityActivationGate: Symbol.for('CardinalityActivationGate'),
  // Generic pre-activation guard extension point (Prerequisite C). Unbound by
  // default — ConfigurationService's @optional() injection resolves to
  // undefined, so standard configurations see no behavior change. A later,
  // specialized readiness guard binds this symbol without touching callers.
  ConfigurationActivationGuard: Symbol.for('ConfigurationActivationGuard'),
  // Task 6 (NetSuite serialized-asset sync): the live activation-readiness
  // evaluator behind both the specialized ConfigurationActivationGuard and
  // POST /api/configurations/:id/serialized-asset-readiness.
  SerializedAssetReadinessService: Symbol.for('SerializedAssetReadinessService'),
  // Task 2 (NetSuite serialized-asset sync): durable, tenant-scoped
  // deferred-work store (migration 058). Task 2 deliberately shipped the
  // repository unbound because Task 7 is its first consumer; the binding lands
  // here with that consumer. Async-bound (transitive DatabaseService) —
  // consumers must resolve via `getAsync`.
  DeferredSerializedUnitRepository: Symbol.for('DeferredSerializedUnitRepository'),
  // Task 7 review round 2: durable per-(tenant, configuration) source-sweep
  // position (migration 059). Async-bound; resolve via `getAsync`.
  SerializedAssetSweepCursorRepository: Symbol.for('SerializedAssetSweepCursorRepository'),
  // Task 7 (NetSuite serialized-asset sync): the eight fixed Prometheus
  // counters, and the bounded orchestrator that owns the governed Asset write.
  SerializedAssetMetrics: Symbol.for('SerializedAssetMetrics'),
  SerializedAssetSyncService: Symbol.for('SerializedAssetSyncService'),

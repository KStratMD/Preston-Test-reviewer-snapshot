...
container.bind<IntegrationService>(TYPES.IntegrationService).to(IntegrationService).inSingletonScope();
...
container.bind<SyncCentralOrchestrator>(TYPES.SyncCentralOrchestrator).toDynamicValue((context: interfaces.Context) => {
  const logger = context.container.get<Logger>(TYPES.Logger);
  ...
  const schemaRegistry = context.container.isBound(TYPES.SchemaRegistryService)
    ...
    : undefined;
  return new SyncCentralOrchestrator(logger, connectorManager, schemaRegistry);
}).inSingletonScope();
...
if (process.env.OPENAI_API_KEY) {
  container.bind<OpenAIProvider>(TYPES.OpenAIProvider).toDynamicValue((context: interfaces.Context) => {
    ...
    return new OpenAIProvider(logger, {
      ...
      model: (process.env.OPENAI_MODEL as 'gpt-4o' | 'gpt-4o-mini') || 'gpt-4o',
...
if (process.env.ANTHROPIC_API_KEY) {
  container.bind<ClaudeProvider>(TYPES.ClaudeProvider).toDynamicValue((context: interfaces.Context) => {
    ...
    return new ClaudeProvider(logger, {
      ...
      model: (process.env.ANTHROPIC_MODEL as 'claude-3-5-sonnet-20241022') || 'claude-3-5-sonnet-20241022',
...
// LMStudio Provider - bind if explicit base URL or development mode
...
container.bind<IntegrationExecutor>(TYPES.IntegrationExecutor).toDynamicValue((context: interfaces.Context) => {
  const logger = context.container.get<Logger>(TYPES.Logger);
  ...
  return new IntegrationExecutor(logger, transformationEngine, connectorManager, statusManager, observabilityService);
}).inSingletonScope();

container.bind<IntegrationOrchestrator>(TYPES.IntegrationOrchestrator).toDynamicValue((context: interfaces.Context) => {
  const logger = context.container.get<Logger>(TYPES.Logger);
  const configService = context.container.get<ConfigurationService>(TYPES.ConfigurationService);
  const connectorManager = context.container.get<ConnectorManager>(TYPES.ConnectorManager);
  const statusManager = context.container.get<IntegrationStatusManager>(TYPES.IntegrationStatusManager);
  const executor = context.container.get<IntegrationExecutor>(TYPES.IntegrationExecutor);
  const observabilityService = context.container.get<ObservabilityService>(TYPES.ObservabilityService);
...
container
  ...
  .toDynamicValue(async (context: interfaces.Context) => {
    ...
    const registry = context.container.get<ApprovalResumeRegistry>(TYPES.ApprovalResumeRegistry);
...
container
  ...
  .toDynamicValue(async (context: interfaces.Context) => {
    const logger = context.container.get<Logger>(TYPES.Logger);
    const repo = await context.container.getAsync<ApprovalQueueRepository>(TYPES.ApprovalQueueRepository);
    const service = new ApprovalQueueService(repo, logger);
...
container.bind<SyncErrorAssistService>(TYPES.SyncErrorAssistService)
  .to(SyncErrorAssistService).inSingletonScope();
...
container.bind<FinanceCentralOperatorService>(TYPES.FinanceCentralOperatorService)
  .toDynamicValue(async (context: interfaces.Context) => {
    ...
    return new FinanceCentralOperatorService(logger, repo, connectorManager, auditLog);
  })
  .inSingletonScope();
...
container.bind<WorkflowCentralService>(TYPES.WorkflowCentralService)
  .toDynamicValue(async (context: interfaces.Context) => {
    ...
    const auditLog = context.container.get<AuditLogRepository>(TYPES.AuditLogRepository);
    const dlpService = context.container.get<DLPService>(TYPES.DLPService);
    return new WorkflowCentralService(logger, engine, repo, db, auditLog, dlpService);
  })
  .inSingletonScope();

// PR 14: FlowExecutor — single runtime for governed flow templates.
// Constructor-injected dependencies: Logger, OutboundGovernanceService,
// ApprovalQueueService. The target connector is NOT injected — callers
// supply it via FlowContext.connector after resolving + initializing it
// themselves (Codex 5.5 HIGH restructure on PR #825).

...
import {
  ...
  type SerializedAssetReadinessEvaluator,
} from '../services/serializedAsset/SerializedAssetReadinessService';
import { SerializedAssetActivationGuard } from '../services/serializedAsset/SerializedAssetActivationGuard';
import { DeferredSerializedUnitRepository } from '../services/serializedAsset/DeferredSerializedUnitRepository';
import { SerializedAssetSweepCursorRepository } from '../services/serializedAsset/SerializedAssetSweepCursorRepository';
import { SerializedAssetMetrics } from '../services/serializedAsset/SerializedAssetMetrics';
import { SerializedAssetSyncService } from '../services/serializedAsset/SerializedAssetSyncService';
...
} from '../services/integration/ConnectorCredentialResolver';
import {
  TenantSettingSystemCredentialRegistry,
  type TenantSystemCredentialRegistry,
} from '../services/integration/TenantSystemCredentialRegistry';
...
// Cardinality preflight and activation gate (Tasks 4 + 5): the relationship
// evidence normalizer and the trusted preflight coordinator. Both are bound
// explicitly below — no default/mock discovery service is constructed in DI.
import { RelationshipEvidenceProvider } from '../services/cardinality/RelationshipEvidenceProvider';
import { CardinalityPreflightService } from '../services/cardinality/CardinalityPreflightService';
import { SchemaDiscoveryService } from '../services/ai/validation/SchemaDiscoveryService';
import { NetSuiteSchemaIntelligence } from '../services/ai/NetSuiteSchemaIntelligence';
import {
  type ConnectorDeps,
...
import { SuiteCentralControlPlaneRepository } from '../services/suitecentral/controlPlane/SuiteCentralControlPlaneRepository';
import { SuiteCentralSecretStore } from '../services/suitecentral/controlPlane/SuiteCentralSecretStore';
import { SuiteCentralOutboundPolicy } from '../services/suitecentral/controlPlane/SuiteCentralOutboundPolicy';
import { PinnedHttpsTransport } from '../services/suitecentral/controlPlane/PinnedHttpsTransport';
import { SuiteCentralConnectorFactory } from '../services/suitecentral/controlPlane/SuiteCentralConnectorFactory';
import { SuiteCentralAuditWriter } from '../services/suitecentral/controlPlane/SuiteCentralAuditWriter';
import { SuiteCentralMonitoringRuntime } from '../services/suitecentral/controlPlane/SuiteCentralMonitoringRuntime';
import { SuiteCentralControlPlaneService } from '../services/suitecentral/controlPlane/SuiteCentralControlPlaneService';
...
import { AdminDemoModeService } from '../services/settings/AdminDemoModeService';
...
import { DLPScanMetrics } from '../services/security/DLPScanMetrics';
...
container.bind<AdminDemoModeService>(TYPES.AdminDemoModeService).to(AdminDemoModeService).inSingletonScope();
// Security Services (DLP/NER)
container.bind<DLPScanMetrics>(TYPES.DLPScanMetrics).to(DLPScanMetrics).inSingletonScope();
...
container.bind<IntegrationService>(TYPES.IntegrationService).toDynamicValue(async (context: interfaces.Context) => {
  ...
  // Task 8 (2026-07-27 NetSuite serialized-asset sync plan): the specialized
  // profile dispatch's own dependencies. ConnectorManager is sync-bound;
  // SerializedAssetSyncService is async-bound end-to-end (its own transitive
  // DatabaseService/OwnershipResolver chain), so it must be resolved with
  // `getAsync` here too — this factory is already async.
  const connectorManager = context.container.get<ConnectorManager>(TYPES.ConnectorManager);
  const serializedAssetSyncService = await context.container.getAsync<SerializedAssetSyncService>(
    TYPES.SerializedAssetSyncService,
  );
  return new IntegrationService(logger, transformationEngine, configService, authService, observabilityService, outboundGovernance, ownershipResolver, auditService, approvalQueueService, connectorManager, serializedAssetSyncService);
}).inSingletonScope();
...
// Prerequisite PR B (2026-07-27 NetSuite serialized-asset sync plan): resolves
// managed (`secret_manager`/`environment`) system-reference credentials for
// ConnectorManager, so a config referencing a secret-manager-backed system is
// actually executable — not silently ignored.
//
// SecureCredentialManager injects SecretManager, which is ASYNC-bound
// (factory above). ConnectorManager — and everything that resolves it
// synchronously (e.g. `ReconcilerRegistry`, explicitly documented below as
// "sync-bound... no async resolution needed") — must stay sync-constructible.
// A lazy `SecureCredentialManagerProvider` (mirrors
// `TenantConfigurationRepositoryProvider`) lets
// `DefaultConnectorCredentialResolver` itself be bound and constructed
// synchronously; the async SecretManager chain is only awaited inside
// `resolve()`, which was already async.
container
  .bind<SecureCredentialManagerProvider>(TYPES.SecureCredentialManagerProvider)
  .toProvider<SecureCredentialManager>((context: interfaces.Context) => () =>
    context.container.getAsync<SecureCredentialManager>(TYPES.SecureCredentialManager),
  );
// Tenant-ownership registry consulted before every managed credential
// resolution. Backed by the existing tenant-settings table (no new migration)
// through a LAZY provider, so DefaultConnectorCredentialResolver — and
// therefore ConnectorManager — stays synchronously constructible while the
// async TenantConfigurationRepository chain is awaited only inside the check.
container
  .bind<TenantSystemCredentialRegistry>(TYPES.TenantSystemCredentialRegistry)
  .toDynamicValue((context: interfaces.Context) => {
    const registryLogger = context.container.get<Logger>(TYPES.Logger);
    return new TenantSettingSystemCredentialRegistry(
      () => context.container.getAsync<TenantConfigurationRepository>(TYPES.TenantConfigurationRepository),
      registryLogger,
    );
  })
  .inSingletonScope();
container
  .bind<ConnectorCredentialResolver>(TYPES.ConnectorCredentialResolver)
  .to(DefaultConnectorCredentialResolver)
  .inSingletonScope();
container
...
// SuiteCentral control-plane bindings (PR-A1..A4). All lazy: nothing resolves
// these at boot — the redesigned routes are mounted in PR-A6 — so this
// registration is inert. The connector factory constructs a FRESH, address-
// pinned connector per operation; it is never used to build a singleton
// connector.
container
  .bind<SuiteCentralControlPlaneRepository>(TYPES.SuiteCentralControlPlaneRepository)
  .toDynamicValue(async (context: interfaces.Context) => {
    ...
    return new SuiteCentralControlPlaneRepository(databaseService);
  })
  .inSingletonScope();

container
  .bind<SuiteCentralSecretStore>(TYPES.SuiteCentralSecretStore)
  .toDynamicValue(async (context: interfaces.Context) => {
    const secretManager = await context.container.getAsync<SecretManager>(TYPES.SecretManager);
    return new SuiteCentralSecretStore(secretManager);
  })
  .inSingletonScope();

container
  .bind<SuiteCentralOutboundPolicy>(TYPES.SuiteCentralOutboundPolicy)
  .toDynamicValue(async (context: interfaces.Context) => {
    const repository = await context.container.getAsync<SuiteCentralControlPlaneRepository>(
      TYPES.SuiteCentralControlPlaneRepository,
    );
    // Production DNS resolver: return EVERY A/AAAA answer (all: true) in
    // resolver order (verbatim: true) so the policy can enforce that all of
    // them classify as public before pinning.
    ...
    return new SuiteCentralOutboundPolicy(repository, resolveAll);
  })
  .inSingletonScope();

container
  .bind<PinnedHttpsTransport>(TYPES.SuiteCentralPinnedHttpsTransport)
  .toDynamicValue(() => new PinnedHttpsTransport())
  .inSingletonScope();

container
  .bind<SuiteCentralConnectorFactory>(TYPES.SuiteCentralConnectorFactory)
  .toDynamicValue(async (context: interfaces.Context) => {
    const repository = await context.container.getAsync<SuiteCentralControlPlaneRepository>(
      TYPES.SuiteCentralControlPlaneRepository,
    );
    const outboundPolicy = await context.container.getAsync<SuiteCentralOutboundPolicy>(
      TYPES.SuiteCentralOutboundPolicy,
    );
    const secretStore = await context.container.getAsync<SuiteCentralSecretStore>(TYPES.SuiteCentralSecretStore);
    const transport = context.container.get<PinnedHttpsTransport>(TYPES.SuiteCentralPinnedHttpsTransport);
    const logger = context.container.get<Logger>(TYPES.Logger);
    return new SuiteCentralConnectorFactory(repository, outboundPolicy, secretStore, transport, logger);
  })
  .inSingletonScope();

// SuiteCentral control-plane audit writer, monitoring runtime, and application
// service (PR-A5). All INERT until PR-A6 mounts the redesigned routes: nothing
// resolves the service yet, and the runtime reconciles zero enabled configs on
// existing databases.
container
  .bind<SuiteCentralAuditWriter>(TYPES.SuiteCentralAuditWriter)
  .toDynamicValue((context: interfaces.Context) => {
    const auditLogRepository = context.container.get<AuditLogRepository>(TYPES.AuditLogRepository);
    const outboundGovernance = context.container.get<OutboundGovernanceService>(
      TYPES.OutboundGovernanceService,
    );
    const logger = context.container.get<Logger>(TYPES.Logger);
    return new SuiteCentralAuditWriter(auditLogRepository, outboundGovernance, logger);
  })
  .inSingletonScope();

container
  .bind<SuiteCentralMonitoringRuntime>(TYPES.SuiteCentralMonitoringRuntime)
  .toDynamicValue(async (context: interfaces.Context) => {
    const repository = await context.container.getAsync<SuiteCentralControlPlaneRepository>(
      TYPES.SuiteCentralControlPlaneRepository,
    );
    const connectorFactory = await context.container.getAsync<SuiteCentralConnectorFactory>(
      TYPES.SuiteCentralConnectorFactory,
    );
    const audit = await context.container.getAsync<SuiteCentralAuditWriter>(TYPES.SuiteCentralAuditWriter);
    const logger = context.container.get<Logger>(TYPES.Logger);
    return new SuiteCentralMonitoringRuntime(repository, connectorFactory, audit, logger);
  })
  .inSingletonScope();

container
  .bind<SuiteCentralControlPlaneService>(TYPES.SuiteCentralControlPlaneService)
  .toDynamicValue(async (context: interfaces.Context) => {
    const repository = await context.container.getAsync<SuiteCentralControlPlaneRepository>(
      TYPES.SuiteCentralControlPlaneRepository,
    );
    const secretStore = await context.container.getAsync<SuiteCentralSecretStore>(
      TYPES.SuiteCentralSecretStore,
    );
    const outboundPolicy = await context.container.getAsync<SuiteCentralOutboundPolicy>(
      TYPES.SuiteCentralOutboundPolicy,
    );
    ...
    const monitoring = await context.container.getAsync<SuiteCentralMonitoringRuntime>(
      TYPES.SuiteCentralMonitoringRuntime,
    );
    const audit = await context.container.getAsync<SuiteCentralAuditWriter>(TYPES.SuiteCentralAuditWriter);
    const logger = context.container.get<Logger>(TYPES.Logger);
    return new SuiteCentralControlPlaneService(
      repository,
      secretStore,
      outboundPolicy,
      connectorFactory,
...
container.bind<ConnectorManager>(TYPES.ConnectorManager).toDynamicValue((context: interfaces.Context) => {
  ...
  const authService = context.container.get<AuthService>(TYPES.AuthService);
  const outboundGovernance = context.container.get<OutboundGovernanceService>(TYPES.OutboundGovernanceService);
  const credentialResolver = context.container.get<ConnectorCredentialResolver>(TYPES.ConnectorCredentialResolver);
...
// SuiteCentralConnectorFactory is bound above (PR-A4 control-plane bindings).
...
container
  ...
  .toDynamicValue(async (context: interfaces.Context) => {
    ...
    return new ReasoningTraceEngine(logger, dlpService, repo);
  })
  .inSingletonScope();
...
import { parseApprovalFingerprintKey } from '../services/governance/approvalFingerprint';
import {
...
/**
 * A8 — the ownership-approval dedupe key, resolved once at composition.
 *
 * Production and hosted demo are already refused at boot by `env.ts` unless
 * `APPROVAL_FINGERPRINT_HMAC_KEY` is present and well-formed, so reaching this
 * fallback means development or test.
 *
 * DERIVED from a fixed public label rather than written as a hex literal.
 * Three properties had to hold at once and this is the only shape that gets
 * all of them: deterministic across restarts (fingerprints must be stable or
 * the dedupe fails outright), independent of JWT_SECRET and the encryption
 * keys (deriving from those would defeat the separation the boot guard
 * enforces), and not a credential-shaped constant. The literal this replaced
 * was indistinguishable from a real leaked key — gitleaks flagged it on
 * #1131, correctly: a 64-hex string sitting in production source IS the thing
 * a secret scanner exists to find, and allowlisting it would have taught the
 * scanner to ignore the next one that mattered.
 */
const DEVELOPMENT_APPROVAL_FINGERPRINT_KEY = createHash('sha256')
  .update('preston-test:development-only:approval-fingerprint-key')
  .digest('hex');

container
  .bind<Buffer>(TYPES.ApprovalFingerprintKey)
  .toDynamicValue(() =>
    parseApprovalFingerprintKey(
      process.env.APPROVAL_FINGERPRINT_HMAC_KEY ?? DEVELOPMENT_APPROVAL_FINGERPRINT_KEY,
    ),
  )
  .inSingletonScope();
...
container
  ...
  .toDynamicValue(async (context: interfaces.Context) => {
    ...
    const fingerprintKey = context.container.get<Buffer>(TYPES.ApprovalFingerprintKey);
    // The persisted audit writer: decisions and refusals are both recorded
    // through it, and an accepted decision commits its audit row in the same
    // transaction as the decision itself.
    const auditLogs = await context.container.getAsync<AuditLogRepository>(TYPES.AuditLogRepository);
    const service = new ApprovalQueueService(repo, logger, encryption, fingerprintKey, auditLogs);
...
import { EmbeddedRoleGrantRepository } from '../services/embedded/EmbeddedRoleGrantRepository';
import { UserAssertionNonceRepository } from '../services/embedded/UserAssertionNonceRepository';
...
container.bind<EmbeddedRoleGrantRepository>(TYPES.EmbeddedRoleGrantRepository).to(EmbeddedRoleGrantRepository).inSingletonScope();
container.bind<UserAssertionNonceRepository>(TYPES.UserAssertionNonceRepository).to(UserAssertionNonceRepository).inSingletonScope();
...
// A9: durable forced serialized-asset retry.
import { SerializedAssetRetryOperationRepository } from '../services/serializedAsset/SerializedAssetRetryOperationRepository';
import { SerializedAssetRetryOperationService } from '../services/serializedAsset/SerializedAssetRetryOperationService';
import {
  SerializedAssetRetryJob,
  toRetryCounters,
  type ForcedRetryExecutor,
} from '../services/serializedAsset/SerializedAssetRetryJob';

container
  .bind<SerializedAssetRetryOperationRepository>(TYPES.SerializedAssetRetryOperationRepository)
  .toDynamicValue(async (context: interfaces.Context) => {
    // getAsync: DatabaseService is async-bound, and a sync .get() against an
    // async dependency chain yields a Promise instead of the instance.
    const dbService = await context.container.getAsync<DatabaseService>(TYPES.DatabaseService);
    return new SerializedAssetRetryOperationRepository(dbService);
  })
  .inSingletonScope();

/**
 * The executor adapter, and the ONLY place a `SyncResult` from the forced
 * retry is converted into what the durable row may hold.
 *
 * Bound separately from IntegrationService so the worker depends on this
 * one-method seam rather than the service: it receives bounded counters and
 * never sees `errors` or `metadata`, which carry failure categories and the
 * whole serialized-asset run respectively.
 */
container
  .bind<ForcedRetryExecutor>(TYPES.SerializedAssetForcedRetryExecutor)
  .toDynamicValue(async (context: interfaces.Context) => {
    const integrationService = await context.container.getAsync<IntegrationService>(TYPES.IntegrationService);
    return {
      async run(args) {
        const result = await integrationService.retryDeferredSerializedAssetsForTenant(
          args.tenantId,
          args.configurationId,
          { userId: args.requesterUserId, correlationId: args.correlationId },
        );
        return toRetryCounters(result);
      },
    };
  })
  .inSingletonScope();
...
container
  .bind<SerializedAssetRetryOperationService>(TYPES.SerializedAssetRetryOperationService)
  .toDynamicValue(async (context: interfaces.Context) => {
    const repo = await context.container.getAsync<SerializedAssetRetryOperationRepository>(
      TYPES.SerializedAssetRetryOperationRepository,
    );
    const integrationService = await context.container.getAsync<IntegrationService>(TYPES.IntegrationService);
    return new SerializedAssetRetryOperationService(repo, integrationService);
  })
  .inSingletonScope();

container
  .bind<SerializedAssetRetryJob>(TYPES.SerializedAssetRetryJob)
  .toDynamicValue(async (context: interfaces.Context) => {
    const repo = await context.container.getAsync<SerializedAssetRetryOperationRepository>(
      TYPES.SerializedAssetRetryOperationRepository,
    );
    const executor = await context.container.getAsync<ForcedRetryExecutor>(
      TYPES.SerializedAssetForcedRetryExecutor,
    );
    const logger = context.container.get<Logger>(TYPES.Logger);
    return new SerializedAssetRetryJob(repo, executor, logger);
  })
  .inSingletonScope();
...
// Cardinality preflight and activation gate (Tasks 4 + 5).
//
// RelationshipEvidenceProvider takes a single deps OBJECT (schema discovery +
// NetSuite intelligence), so it is bound via toDynamicValue rather than `.to()`
// (Inversify cannot resolve a bare Object constructor parameter). It is wired
// with the REAL SchemaDiscoveryService (discovery config-gated off by default —
// unconfigured Salesforce discovery yields trustworthy `unavailable` evidence,
// NOT mock data) and the REAL NetSuiteSchemaIntelligence (its relationship
// catalog is hardcoded, performs no I/O). No `new MockX()` fallback is
// constructed here, per the Task-5 binding rule.
container.bind<RelationshipEvidenceProvider>(TYPES.RelationshipEvidenceProvider)
  .toDynamicValue((context: interfaces.Context) => {
    const logger = context.container.get<Logger>(TYPES.Logger);
    // Built once from validated env (B3.5) — SchemaDiscoveryService itself
    // must not read process.env for this path. env.ts already refuses to
    // boot with mode 'live' and a missing credential, so the non-null
    // assertions below are backed by that fail-closed guard, not by chance.
    const salesforceRelationshipDiscovery =
      env.CARDINALITY_SALESFORCE_RELATIONSHIP_MODE === 'live'
        ? {
            mode: 'live' as const,
    ...
    return new RelationshipEvidenceProvider({
      schemaDiscoveryService: new SchemaDiscoveryService({ salesforceRelationshipDiscovery }),
      netSuiteSchemaIntelligence: new NetSuiteSchemaIntelligence(logger),
    });
  })
  .inSingletonScope();

// The trusted preflight coordinator (@injectable, @inject the provider above).
container.bind<CardinalityPreflightService>(TYPES.CardinalityPreflightService)
  .to(CardinalityPreflightService)
  .inSingletonScope();

// The activation gate bundle ConfigurationService optionally injects: the
// coordinator plus the database-backed cardinality audit methods. Kept as a
// dynamic bundle so the pinned `this.cardinality.preflight` / `.audit` shape is
// satisfied from one optional binding. Neither dependency imports
// ConfigurationService, so this introduces no DI cycle.
container.bind<CardinalityActivationGate>(TYPES.CardinalityActivationGate)
  .toDynamicValue((context: interfaces.Context) => {
    const preflight = context.container.get<CardinalityPreflightService>(TYPES.CardinalityPreflightService);
    // AuditService reaches the async DatabaseService through AuditLogRepository.
    // Keep the activation bundle synchronously constructible for ConfigurationService,
    // and resolve the audit writer only when an active save actually needs it.
    const audit: CardinalityActivationGate['audit'] = {
      async logCardinalityDecision(data) {
        const service = await context.container.getAsync<AuditService>(TYPES.AuditService);
        return service.logCardinalityDecision(data);
      },
      async logCardinalityOutcome(data) {
        const service = await context.container.getAsync<AuditService>(TYPES.AuditService);
        return service.logCardinalityOutcome(data);
      },
    };
...
// --- Task 6: NetSuite serialized-asset activation readiness ---------------
//
// Both bindings must be SYNCHRONOUSLY constructible: ConfigurationService is
// resolved with a plain `container.get`, so an async-bound guard would hand it
// a Promise and silently disable the gate. The async dependencies are deferred
// exactly like the cardinality bundle above — the tenant-settings repository is
// reached through a lazy provider that is only awaited inside `evaluate()`.
//
// ConnectorManager is the prerequisite-B credential-resolving provisioner: a
// managed (`secret_manager`) system reference becomes real credentials there,
// and a lookup failure propagates rather than initializing with nothing.
container
  .bind<SerializedAssetReadinessEvaluator>(TYPES.SerializedAssetReadinessService)
  .toDynamicValue((context: interfaces.Context) => {
    const logger = context.container.get<Logger>(TYPES.Logger);
    const connectorManager = context.container.get<ConnectorManager>(TYPES.ConnectorManager);
    return new SerializedAssetReadinessService(
      () => context.container.getAsync<TenantConfigurationRepository>(TYPES.TenantConfigurationRepository),
      connectorManager,
      logger,
    );
...
// Binds prerequisite C's previously-unbound extension point. Standard
// configurations are unaffected (the guard returns immediately for anything
// that is not a `netsuite_serialized_asset` config); specialized configs can
// only reach an active state through POST /api/configurations/:id/activate.
container
  .bind<ConfigurationActivationGuard>(TYPES.ConfigurationActivationGuard)
  .toDynamicValue((context: interfaces.Context) =>
    new SerializedAssetActivationGuard(
      context.container.get<SerializedAssetReadinessEvaluator>(TYPES.SerializedAssetReadinessService),
    ),
  )
  .inSingletonScope();

// --- Tasks 2 + 7: deferred-work store, metrics, and the sync orchestrator ---
//
// Async end-to-end (the ApprovalQueueRepository pattern at L928): the deferred
// store's transitive DatabaseService and the OwnershipResolver /
// ApprovalQueueService guardedWrite deps are all async-bound, so resolving any
// of these synchronously would hand the consumer a Promise. Consumers
// (IntegrationService dispatch, Task 8) must use `getAsync`.
container
  .bind<DeferredSerializedUnitRepository>(TYPES.DeferredSerializedUnitRepository)
  .toDynamicValue(async (context: interfaces.Context) => {
    const dbService = await context.container.getAsync<DatabaseService>(TYPES.DatabaseService);
    return new DeferredSerializedUnitRepository(dbService);
  })
  .inSingletonScope();

// Migration 059: durable per-(tenant, configuration) source-sweep position, so
// successive runs resume where the last one stopped instead of re-sweeping the
// same first window forever.
container
  .bind<SerializedAssetSweepCursorRepository>(TYPES.SerializedAssetSweepCursorRepository)
  .toDynamicValue(async (context: interfaces.Context) => {
    const dbService = await context.container.getAsync<DatabaseService>(TYPES.DatabaseService);
    return new SerializedAssetSweepCursorRepository(dbService);
  })
  .inSingletonScope();

// Singleton by necessity, not just convention: the constructor registers eight
// counters on prom-client's global registry, and a second instance would throw
// on duplicate registration.
container
  .bind<SerializedAssetMetrics>(TYPES.SerializedAssetMetrics)
  .toDynamicValue(() => new SerializedAssetMetrics())
  .inSingletonScope();

container
  .bind<SerializedAssetSyncService>(TYPES.SerializedAssetSyncService)
  .toDynamicValue(async (context: interfaces.Context) => {
    const deferredStore = await context.container.getAsync<DeferredSerializedUnitRepository>(
      TYPES.DeferredSerializedUnitRepository,
    );
    const cursorStore = await context.container.getAsync<SerializedAssetSweepCursorRepository>(
      TYPES.SerializedAssetSweepCursorRepository,
    );
    const readiness = context.container.get<SerializedAssetReadinessEvaluator>(
      TYPES.SerializedAssetReadinessService,
    );
    const metrics = context.container.get<SerializedAssetMetrics>(TYPES.SerializedAssetMetrics);
    const ownershipResolver = await context.container.getAsync<OwnershipResolver>(TYPES.OwnershipResolver);
    const auditService = context.container.get<AuditService>(TYPES.AuditService);
    const approvalQueueService = await context.container.getAsync<ApprovalQueueService>(
      TYPES.ApprovalQueueService,
    );
    const logger = context.container.get<Logger>(TYPES.Logger);
    return new SerializedAssetSyncService(
      deferredStore,
      cursorStore,
      readiness,
      metrics,
      { ownershipResolver, auditService, approvalQueueService },
      auditService,
      logger,
    );
  })
...
// Batch processing queue subsystem (#1102 activation).
//
// These two symbols have existed in types.ts since the subsystem was written
// but were never bound, so the services were unreachable through the
// container. Binding them is the activation.
//
// QueueService is a plain singleton — it takes only the logger and decides its
// own availability from env.DISABLE_REDIS.
//
// BatchProcessingService must be an ASYNC singleton because IntegrationService
// is async-bound (toDynamicValue(async …) above), so it can only be obtained
// via getAsync. Resolving it synchronously would throw at container level.
//
// Neither binding initializes anything: construction is side-effect free and
// Server.start() drives initialize() so startup ordering stays explicit.
...
container
  .bind<BatchProcessingService>(TYPES.BatchProcessingService)
  .toDynamicValue(async (context: interfaces.Context) => {
    const logger = context.container.get<Logger>(TYPES.Logger);
    const queueService = context.container.get<QueueService>(TYPES.QueueService);
    const integrationService = await context.container.getAsync<IntegrationService>(
      TYPES.IntegrationService,
    );
    return new BatchProcessingService(logger, queueService, integrationService);
  })
  .inSingletonScope();

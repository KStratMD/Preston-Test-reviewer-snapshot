import { z } from 'zod';
import { FieldMappingSchema as CanonicalFieldMappingSchema, validateMappingSet } from '../domain/mapping/MappingContract';
...
// Field Mapping Schema — the canonical contract (tranche 2, Task A1). Codex on
// PR #1253 found this file still carried its own, looser copy (type optional,
// no per-type rules), so config validation accepted mappings the engine would
// refuse at run time. One schema now: src/domain/mapping/MappingContract.ts.
export const FieldMappingSchema = CanonicalFieldMappingSchema;
...
export const IntegrationConfigSchema = z.object({
  ...
  // Codex round 8 on PR #1253: each mapping validated on its own, so an ACTIVE
  // configuration could carry a declared-only type or two spellings of one
  // lodash target slot. The set is validated for an active configuration; an
  // inactive one is a draft and keeps its draft freedom (duplicate targets and
  // declared-only types are allowed until activation — pinned by
  // configurationSchemas.cardinality.test.ts).
  if (config.isActive) {
    for (const issue of validateMappingSet(config.fieldMappings).issues) {
      ctx.addIssue({ code: 'custom', path: ['fieldMappings'], message: issue });
    }
  }

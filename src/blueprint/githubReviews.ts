...
export function githubApi(token: string, owner: string, repo: string, pullNumber: number): ReviewsApi {
  ...
  async function pages<T>(path: string, schema: z.ZodType<T>): Promise<T[]> {
    ...
    const expected = new URL(path);
    const reposSegment = expected.pathname.indexOf('/repos/');
    const apiPathPrefix = reposSegment >= 0 ? expected.pathname.slice(0, reposSegment) : '';
    const expectedResource = expected.pathname.match(/(\/pulls\/\d+\/(?:reviews|files))$/)?.[1];
    let canonicalPathname: string | undefined;
    while (url) {
      const candidate = new URL(url);
      candidate.searchParams.sort(); candidate.hash = '';
      const key = candidate.href;
      const canonicalMatch = candidate.pathname.match(/^(.*)\/repositories\/\d+(\/pulls\/\d+\/(?:reviews|files))$/);
      const sameResourceViaCanonicalPath = canonicalMatch?.[1] === apiPathPrefix
        && canonicalMatch[2] === expectedResource
        && (!canonicalPathname || canonicalPathname === candidate.pathname);
      if (candidate.pathname !== expected.pathname && sameResourceViaCanonicalPath) canonicalPathname ??= candidate.pathname;
      if (candidate.origin !== expected.origin
        || (candidate.pathname !== expected.pathname && !sameResourceViaCanonicalPath)
        || visited.has(key)) throw new Error('Unsafe or cyclic GitHub pagination');

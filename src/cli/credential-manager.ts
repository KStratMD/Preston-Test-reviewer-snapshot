...
program
  ...
  .action(async (options: MigrateCommandOptions) => {
    try {
      ...
      // Task 8: migrateToSecureCredentials now requires trusted command
      // context (it can persist active configuration). This CLI runs with the
      // OS shell's own access control, not an HTTP-verified JWT, so it
      // attributes to the local operator account rather than the request-path
      // SYSTEM_IDENTITY sentinel (reserved for the boot-owned startup
      // migration only). The context's tenantId is a placeholder:
      // migrateToSecureCredentials re-scopes it per member to that member's
      // own tenant before any active save.
      const migrationContext: ConfigurationCommandContext = {
        tenantId: 'cli-migration',
        actorUserId: process.env.USER || process.env.USERNAME || 'cli-operator',
        correlationId: uuidv4(),
        operation: 'secure_save',
      };
      await secureConfigService.loadConfigurations();
      const results = await secureConfigService.migrateToSecureCredentials(migrationContext);

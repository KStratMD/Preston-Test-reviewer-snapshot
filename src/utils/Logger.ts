...
export class Logger {
   ...
   * the context (explicit `metadata` keys and the correlationId win); primitive
   * values are normalized to a safe `NonErrorThrown` marker, while other
   * defined objects remain available to LoggingService's sanitizer. Previously
   * only Error instances were honored and everything else was silently
   * dropped — an AST sweep found 497 callsites relying on the `{ error }`
   * object shape.
   */
  error(message: string, error?: unknown, metadata: LogMetadata = {}): void {
    ...
    if (error instanceof Error) {
      ...
      // Do not attach primitive error values verbatim: callers sometimes pass
      // an Error.stack or driver message here, and those values can contain
      // credentials. Error instances are projected safely by LoggingService;
      // object metadata is sanitized during structured serialization.
      context.error = typeof error === 'string'
        || typeof error === 'number'

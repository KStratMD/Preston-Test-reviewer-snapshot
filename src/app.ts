...
import { shouldEnableGlobalRateLimit } from "./middleware/setup/globalRateLimitPolicy";
...
import { installRouteRegistrationRecorder } from "./middleware/setup/routeRegistrationRecorder";
...
export class App {
  ...
  constructor(options: AppOptions = {}) {
    this.app = express();
    // Record literal route registrations before any middleware or router setup
    // can occur. The recorder is observational and preserves Express return
    // values while giving the boot audit an authoritative registration view.
    installRouteRegistrationRecorder(this.app);
  ...
  private async initializeServices(): Promise<void> {
    ...
    const middlewareConfig: MiddlewareConfig = {
      ...
      // A6 — rate limiting is decoupled from lightweight mode.
      //
      // Dockerfile.hosted sets DEMO_MODE=1 and DISABLE_REDIS=1, which selects
      // lightweight mode (index.ts), which previously passed `false` here and
      // made setupRateLimit() return before mounting anything. The hosted
      // service was therefore globally unthrottled — not merely skipping
      // paths, but running with no global limiter at all. Fixing the skip list
      // alone would have left Railway exactly as exposed.
      //
      // Lightweight mode exists to avoid loading heavy optional dependencies;
      // a rate limiter is neither heavy nor optional. Any hosted or
      // production-strength runtime mounts it. Purely local lightweight/demo
      // runs still bypass it via the isDemo() && !HOSTED_DEMO short-circuit
      // inside setupRateLimit.
      enableRateLimit: shouldEnableGlobalRateLimit({
        lightweight: this.options.lightweight ?? true,

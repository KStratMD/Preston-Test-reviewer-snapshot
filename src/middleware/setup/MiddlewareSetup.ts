...
import { denyUnlistedHtml, denyStaticHtmlOnAliasMount } from '../staticHtmlPolicy';
import { HTML_WHITELIST } from './htmlWhitelist';
import { resolvePublicDir } from './publicDir';
import { resolveGlobalRateLimitPolicy, requiresStrictRateLimitPolicy } from './globalRateLimitPolicy';
import { createClientIpKeyGenerator } from './hostedClientIp';
...
function createIpPreAuthLimiter() {
  return rateLimit({
    ...
    legacyHeaders: false,
    // Uses express-rate-limit v8's default IP key, including /56 IPv6 grouping.
    // The separate hosted global resolver retains its established /64 policy.
...
const DEFAULT_CONFIG: Required<MiddlewareConfig> = {
  ...
  rateLimitOptions: {
    ...
    // A6: the exemption table lives in globalRateLimitPolicy.ts, which owns it
    // and validates it at construction. There is deliberately no caller
    // override: an arbitrary skipPaths list could widen the signed-off policy
    // without the policy changing, and a rejected value would have thrown
    // inside the limiter try/catch and disabled the limiter outright.
  },
};
...
export class MiddlewareSetup {
  ...
  private setupStaticFiles(): void {
    ...
    // index/redirect are disabled so a directory request cannot serve or redirect to
    // that directory's index.html, which bypassed the HTML policy entirely.
    this.app.use(
      '/public',
      denyUnlistedHtml(
        express.static(staticPath, { ...staticOptions, index: false, redirect: false }),
        HTML_WHITELIST,
      ),
    );
    // These alias mounts precede the guarded root mount and strip their own
    // prefix, so the whitelist keys (relative to public/) cannot match here. They
    // deny HTML outright and let it fall through to the root mount, where the
    // whitelist applies to the full path. Asset directories serve no pages.
    this.app.use('/vendor', denyStaticHtmlOnAliasMount(express.static(path.join(staticPath, 'vendor'), staticOptions)));
    this.app.use('/webfonts', denyStaticHtmlOnAliasMount(express.static(path.join(staticPath, 'webfonts'), staticOptions)));
    ...
    this.app.use(denyUnlistedHtml(skipEmbeddedHtml(express.static(staticPath, {
      ...staticOptions,
  ...
  private async setupRateLimit(): Promise<void> {
    ...
    // A6 — resolve the policy ONCE, and deliberately OUTSIDE the try/catch
    // below. That catch exists to tolerate a missing optional dependency
    // ("rate limiting not available, skipping"). An invalid POLICY is a
    // different failure: swallowing it would silently leave the server
    // unthrottled, so a construction error must propagate and stop startup
    // rather than degrade into the very no-op A6 exists to remove.
    const limiterIsRequired = requiresStrictRateLimitPolicy({
      hostedDemo: env.HOSTED_DEMO,
      nodeEnv: env.NODE_ENV,
    });
    const policy = resolveGlobalRateLimitPolicy({ strict: limiterIsRequired });
    ...
    try {
      ...
      const limiter = rateLimit({
        ...
        legacyHeaders: true,
        // A6 — explicit client-key resolution instead of the default req.ip.
        // Hosted reads a validated X-Real-IP and ignores client-supplied XFF;
        // every mode canonicalizes IPv4-mapped addresses and aggregates native
        // IPv6 to /64, preserving this policy instead of the v8 default /56. See
        // hostedClientIp.ts for why this is not Express `trust proxy`.
        keyGenerator: createClientIpKeyGenerator({ hosted: env.HOSTED_DEMO }),
        // The XFF validator warns when a proxy appears to be present but
        // identity is derived from the socket. Disable it ONLY when hosted,
        // where we deliberately ignore XFF and read X-Real-IP instead, so the
        // warning would be noise. Non-hosted keeps ordinary `req.ip`
        // semantics, which DO depend on proxy handling — a self-hosted
        // deployment behind a reverse proxy wants that diagnostic, so leaving
        // it disabled everywhere would silence a real misconfiguration signal.
        validate: { xForwardedForHeader: !env.HOSTED_DEMO },
      });

      this.app.use((req, res, next) => {
        // Unchanged by A6, deliberately: the local-demo bypass short-circuits
        // the whole limiter before any matching, so demos and the e2e portals
        // are never throttled.
        if (isDemo() && !env.HOSTED_DEMO) return next();

        if (policy.isExempt(req.path)) {
          return next();
        }

        // The global limiter is deliberately IP-keyed and pre-auth — its job
        // is bounding unauthenticated flood traffic, which by definition has
        // no user identity. Keep it that way: do NOT re-key it by user or move
        // it behind authentication. The authenticated, user-keyed family
        // limiters run later and stack with it.
        //
        // (The key itself comes from createClientIpKeyGenerator above, not the
        // express-rate-limit default — see hostedClientIp.ts.)
        return limiter(req, res, next);
      });
    } catch (error) {
      // A6 — fail CLOSED where the limiter is required.
      //
      // This catch was written to tolerate express-rate-limit being absent in
      // a trimmed local install. It also swallowed every initialization and
      // configuration error, logged a warning that did not include the error,
      // and let startup continue completely unthrottled. Now that hosted and
      // production-strength runtimes are expected to mount the limiter, that
      // path turns a misconfiguration or an API mismatch into a silent
      // security regression — the same failure mode as the original no-op,
      // just reached a different way.
      //
      // Hosted/production: log the underlying error and refuse to start.
      // Local dev/test: keep the tolerant behavior, but log the cause.
      if (limiterIsRequired) {
        logger.error(
          '[middleware] Global rate limiter failed to initialize in a hosted/production runtime — refusing to start unthrottled',
          { error },
        );
        throw error;
      }
      logger.warn('[middleware] Rate limiting not available, skipping', { error });
    }
  }

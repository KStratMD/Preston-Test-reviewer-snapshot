...
export interface RoutePolicyMatch {
  ...
  pathPrefix: string;
  /** Exact route or path-component prefix; omitted preserves prefix behavior. */
...
export function routePrefixMatches(reqPath: string, prefix: string): boolean {
  return routePathMatches(reqPath, prefix, 'prefix');
}
...
function policyMatches(p: RoutePolicy, reqPath: string, method: HttpMethod): boolean {
  if (!routePathMatches(reqPath, p.match.pathPrefix, p.match.matchMode ?? 'prefix')) return false;
  if (p.match.methods && !p.match.methods.includes(method)) return false;
  if (p.match.subpath) {
    const remainder = normalizeRoutePath(reqPath).slice(normalizeRoutePath(p.match.pathPrefix).length) || '/';
...
export const ROUTE_POLICY_MANIFEST: readonly RoutePolicy[] = Object.freeze(
  ([
    ...
    // B8: the eleven mock dashboard fallbacks from
    // `src/routes/mockDashboardAPIs.ts`, mounted only under isDemo() by a
    // prefixless router. These are exact leaves: a future route below one of
    // them must not inherit hosted_demo_public by accident.
    // Posture matches the sibling demo families; these serve static mock
    // payloads only.
    { match: { pathPrefix: '/api/cache/demo', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/health/demo', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/optimization/demo', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/dlq/messages', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/system/status', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/test-template-endpoint', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/mapping-studio/component-status', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/mapping-studio/mappings', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/mapping-studio/share-data', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/mapping-studio/shared-templates', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    { match: { pathPrefix: '/api/mapping-studio/validate', matchMode: 'exact' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'mock dashboard fallback (isDemo only)' },
    ...
    { match: { pathPrefix: '/api/mappings' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'demo-only flat-file store (Task A3)' },
    ...
    { match: { pathPrefix: '/api/dashboard/api/mappings' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'demo-only flat-file store (Task A3)' },
    { match: { pathPrefix: '/api/dashboard/mappings' }, auth: 'hosted_demo_public', lifecycle: 'not_applicable', rateProfile: 'anonymous_demo', notes: 'demo-only flat-file store (Task A3)' },

...
import { normalizeRoutePath, routePathMatches, type RouteMatchMode } from './routePathSemantics';
...
export interface RouteEntry {
   ...
   */
  path: string;
  /** Exact route or path-component prefix; omitted entries preserve prefix behavior. */
...
export const ROUTE_MANIFEST: readonly RouteEntry[] = Object.freeze(([
  ...
  // B8: the eleven mock dashboard fallbacks from `src/routes/mockDashboardAPIs.ts`.
  // That router is mounted ONLY under isDemo(), at '/' with no prefix, so it
  // declares its own '/api/...' paths internally — invisible to any scan of
  // `app.use('/api/...')` call sites, which is why these surfaces reached
  // runtime undeclared and silently defaulted to 'system'. Declaring them
  // 'demo' is behaviour-preserving: only 'tenant_required' changes dispatch.
  //
  // Listed as exact leaves rather than as '/api/mapping-studio', '/api/dlq' or
  // '/api/system' family prefixes. A family prefix would silently hand the
  // 'demo' posture (and hosted_demo_public auth) to any FUTURE real route
  // added beneath it — plausible for DLQ, system-status and mapping-studio
  // surfaces. Residual, deliberately not fixed here: the matcher is
  // longest-PREFIX, so a leaf still captures its own descendants
  // ('/api/dlq/messages/:id/retry' would inherit 'demo'). Closing that needs
  // exact-vs-prefix semantics in both manifests, which belongs with the
  // registration-time enforcement lane, not here.
  { path: '/api/cache/demo', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/health/demo', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/optimization/demo', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/dlq/messages', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/system/status', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/test-template-endpoint', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/mapping-studio/component-status', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/mapping-studio/mappings', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/mapping-studio/share-data', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/mapping-studio/shared-templates', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  { path: '/api/mapping-studio/validate', matchMode: 'exact', classification: 'demo', notes: 'mock dashboard fallback (isDemo only)' },
  ...
  { path: '/api/mappings', classification: 'demo', notes: 'Task A3: global flat-file store, no tenant column; mounts only under isDemo(); not a Blueprint source' },
  ...
  { path: '/api/dashboard/api/mappings', classification: 'demo', notes: 'legacy double-/api/ prefix; mirrors RouteSetup.ts mount; demo-only since Task A3' },
  { path: '/api/dashboard/mappings', classification: 'demo', notes: 'mirrors the /api/mappings mount; demo-only since Task A3' },
...
/**
 * The single definition of "this path is covered by the manifest", returning
 * the longest (most specific) matching entry or null.
 *
 * Exported so the boot-time mount audit (`routeManifestAudit`) asks the exact
 * question the runtime dispatcher asks. `classifyRoute` cannot serve that
 * purpose: it answers 'system' both for a genuine system route and for a path
 * that matched nothing, so an auditor built on it could not tell coverage from
 * the default.
 */
export function findManifestEntry(reqPath: string): RouteEntry | null {
  let best: RouteEntry | null = null;
  for (const entry of ROUTE_MANIFEST) {
    const matches = routePathMatches(reqPath, entry.path, entry.matchMode ?? 'prefix');
    if (matches && (!best || normalizeRoutePath(entry.path).length > normalizeRoutePath(best.path).length)) {
      best = entry;
    }
  }
  return best;
...
export function classifyRoute(reqPath: string): RouteClassification {
  const best = findManifestEntry(reqPath);
  if (best) return best.classification;

  // Only /api paths are in scope for runtime classification — RouteSetup
  // short-circuits everything else before calling in. An unmatched NON-/api
  // path must therefore return the safe default silently: tracking it would
  // grow the dedup set against out-of-scope input, and logging it would emit a
  // message that says "unmatched /api path" about a path that is not one.
  // Manifest entries that are not under /api (/health, /ready, /docs,
  // /api-docs, /metrics) already returned above via findManifestEntry.
  // Case-insensitive for the same reason the central gate is: Express routing
  // is.
  if (!reqPath.toLowerCase().startsWith('/api/')) return 'system';
  ...
  try {
    ...
    // B8: this fires for any /api path matching no manifest entry. Runtime
    // probing showed that USUALLY means 404 traffic — a request for a path that
    // is not mounted at all — which is why ERROR was the wrong level: every bot
    // probing /api/wp-admin wrote a production error line for a non-error.
    //
    // It is NOT dropped to debug, though: mount coverage is not enforced
    // anywhere yet (`routeManifestAudit` is a best-effort diagnostic that can
    // find gaps but cannot prove their absence), so this stays the most visible
    // runtime hint.
    //
    // Do not overstate it. Classification runs BEFORE Express knows whether
    // anything matched, so this is a clue about unmatched REQUESTS — dominated
    // by 404 probes — and once the cap below fills, a genuinely undeclared
    // surface may log nothing at all. WARN is the honest level for a noisy
    // hint; it is not a dependable "undeclared surface is being served" alarm.
    // A response-aware or sampled aggregate would be, and belongs with the
    // enforcement lane.
    logger.warn(
      '[routeManifest] unmatched /api path — no manifest entry, treating as system',
      {
        path: reqPath,
        hint: 'usually a 404 for an unmounted path; if this path IS served, it is an undeclared surface — add an entry to ROUTE_MANIFEST',
      }
    );
  } catch (err: unknown) {

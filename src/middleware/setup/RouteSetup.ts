...
/**
 * Mount the SyncCentral + Automation route families behind the tenant-lifecycle
 * kill-switch gate (PR #879). SyncCentral is the likely first pilot surface, so
 * the gate (active/suspended/disabled/trial_expired) must cover these three
 * mounts the same way it covers the *-central families. `req.user` is populated
 * by the global `/api` optionalAuthMiddleware, so the gate resolves a real
 * tenantId for Bearer-authenticated callers and falls back to SYSTEM_IDENTITY
 * (pass-through) for anonymous ones. The gate is async; `.catch(next)` forwards
 * a rejection before its inner try/catch to Express instead of hanging.
 *
 * Extracted (not inlined in setupAPIRoutes) so the exact production wiring is
 * exercised by `tests/integration/syncCentralTenantStatusGate.routes.test.ts`
 * — same pattern as `mountCentralTenantGate`. Dropping a mount or the gate here
 * fails that test.
 */
export function mountSyncCentralRoutes(
  ...
  routers: {
    syncCentral: RequestHandler;
    syncOrchestrator: RequestHandler;
    automationLibraries: RequestHandler;
  },
): void {
  const tenantStatusGate = makeTenantStatusGate(tenantSvc);
  const syncCentralGate = (req: Request, res: Response, next: NextFunction): void => {
    tenantStatusGate(req, res, next).catch(next);
  };
  app.use('/api/sync-central', syncCentralGate, routers.syncCentral);
  app.use('/api/sync-orchestrator', syncCentralGate, routers.syncOrchestrator);
  app.use('/api/automation-libraries', syncCentralGate, routers.automationLibraries);
}
...
export class RouteSetup {
  ...
  private async setupAPIRoutes(): Promise<void> {
    ...
    if (this.config.enableConfiguration && this.configurationService) {
      await this.safeRouteSetup(async () => {
        ...
        // malformed JWT or upstream regression fails closed.
        this.app.use('/api/configurations', authMiddleware, createConfigurationRouter(this.configurationService!));
      }, 'Configuration routes');
    }
    ...
    // Sits behind the global /api optionalAuthMiddleware and resolves identity via
    // the canonical extractIdentityContext + isSystemIdentity. Shows the real user
    // only for a NON-system identity; a JWT that normalizes to SYSTEM_IDENTITY
    // (tenantId missing/invalid) and unauthenticated callers get the demo fallback.
    await this.safeRouteSetup(async () => {
    ...
    if (this.config.enableIntegration && this.integrationService) {
      await this.safeRouteSetup(async () => {
        ...
        // PR 13c-4 Task 6: gate with authMiddleware (was implicitly under the
        // global /api optionalAuthMiddleware). Anonymous callers now 401 at the
        // middleware; authenticated callers reach handlers with req.user.tenantId
        // populated. Handlers additionally narrow on req.user?.tenantId so a
        // malformed JWT or upstream regression fails closed.
        this.app.use('/api/integrations', erpWriteRateLimit, authMiddleware, createIntegrationRouter(this.integrationService!));
      }, 'Integration routes');
    }
  ...
  async setupSuiteCentralRoutes(): Promise<void> {
    ...
    await this.safeRouteSetup(async () => {
      ...
      this.app.use('/api/suitecentral/sync', erpWriteRateLimit, createSuiteCentralSyncRouter(integrationService, mockObservabilityService));
      this.app.use('/api/suitecentral/netsuite/sync', erpWriteRateLimit, createSuiteCentralNetSuiteSyncRouter(integrationService, mockObservabilityService));
      this.app.use('/api/squire/suitecentral/netsuite/sync', erpWriteRateLimit, createSquireSuiteCentralNetSuiteSyncRouter(integrationService, mockObservabilityService));
    ...
    await this.safeRouteSetup(async () => {
      ...
      this.app.use('/api/cost-transparency', centralAuthMiddleware, costTransparencyRouter);

      // PR 11: Reconciliation Center API. Same auth posture as the other
      // SuiteCentral routes — `centralAuthMiddleware` populates req.user from
      // the JWT, and the route's `extractIdentityContext` 401s when the
      // resolved identity is SYSTEM_IDENTITY. The repo chain depends on
      ...
      try {
        ...
        this.app.use(
          '/api/reconciliation-center',
          centralAuthMiddleware,
          reconciliationCenterRouter(reconciliationService),
        );
      } catch (err) {
        logger.warn(
      ...
      // via `getAsync`. Same auth posture as the Reconciliation Center
      // resolve route — `centralAuthMiddleware` populates req.user from the
      // JWT, and the route's `extractIdentityContext` 401s when tenantId
      // resolves to SYSTEM_IDENTITY OR userId is missing OR userId is one
      // of the synthetic sentinels (SYSTEM_IDENTITY.userId, "unknown" from
      // auth.ts). See lineageRoutes.ts SYNTHETIC_OPERATOR_USER_IDS for the
      // full set. The embedded UI is intentionally NOT shipped this PR
      // (Known Gap on the proof card).
      try {
    ...
    await this.safeRouteSetup(async () => {
      ...
      // Kill-switch parity: resolve the TenantLifecycleService singleton (the
      // gate instance in the "SuiteCentral Feature routes" block lives in a
      // separate closure) and mount all three SyncCentral surfaces behind the
      // gate via the shared mountSyncCentralRoutes helper (its wiring is pinned
      // by tests/integration/syncCentralTenantStatusGate.routes.test.ts).
      const container = (await import('../../inversify/inversify.config')).container;
      const TYPES = (await import('../../inversify/types')).TYPES;
      const { TenantLifecycleService } = await import('../../services/tenants/TenantLifecycleService');
      const tenantSvc = await container.getAsync<InstanceType<typeof TenantLifecycleService>>(TYPES.TenantLifecycleService);

      mountSyncCentralRoutes(this.app, tenantSvc, {

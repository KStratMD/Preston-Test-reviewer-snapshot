...
// The whitelist moved to its own module so the static-HTML policy and this
// no-cache pass share one list (src/middleware/staticHtmlPolicy.ts).
import { htmlFiles } from './htmlWhitelist';
import {
...
export function mountCentralTenantGate(
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    //
    // SECURITY (B8, found by Copilot on PR #1172): this scope check and the
    // classification below MUST be case-insensitive. Express routing is
    // case-insensitive by default, so `/API/mappings` reaches the handler
    // mounted at `/api/mappings` — but a case-sensitive `startsWith('/api/')`
    // short-circuits here, so the route was never classified, `tenant_required`
    // never applied, and `tenantIsolation` never ran. Verified end to end
    // before the fix: `/api/mappings` returned 403 tenant_required while
    // `/API/mappings` returned 200 with mapping data. Compare on a lowercased
    // copy and never mutate `req.path` — handlers, logs and downstream routing
    // must continue to see the original casing.
    const policyPath = req.path.toLowerCase();
    if (!policyPath.startsWith('/api/')) {
      return next();
    }
    const classification = classifyRoute(policyPath);
...
export class RouteSetup {
  ...
  private async setupAPIRoutes(): Promise<void> {
    ...
    // Mappings routes — DEMO-ONLY since Task A3 (tranche 2). The store is one
    // global flat file (config/mappings.json) with no tenant column, so the
    // family mounts under the same gate as the mock dashboard fallback and is
    // classified `demo` in ROUTE_MANIFEST. Outside demo it is simply absent (404).
    if (this.config.enableMappings && isDemo()) {
      await this.safeRouteSetup(async () => {
    ...
    if (this.config.enableIntegration && this.integrationService) {
      await this.safeRouteSetup(async () => {
        ...
        const TYPES = (await import('../../inversify/types')).TYPES;
        const tenantSvc = await container.getAsync<TenantLifecycleService>(TYPES.TenantLifecycleService);
        // A9: resolve the forced-retry service HERE and pass it in, rather than
        // letting the route module reach into the container — that keeps the
        // router constructible in tests without the whole DI graph. A failure
        // to resolve it must not take the entire integration router down, so it
        // degrades to undefined and the two forced-retry routes report 503.
        ...
        try {
          ...
          // Log it. Copilot review on #1132: swallowing this silently means the
          // forced-retry endpoints degrade to 503 with no startup signal
          // explaining why, which is the hardest kind of outage to diagnose.
          serializedAssetRetryOperations = undefined;
          logger.warn(
            'SerializedAssetRetryOperationService could not be resolved; forced-retry endpoints will report 503 on this replica.',
  ...
  private async safeRouteSetup(
    ...
    try {
      ...
      const msg = `[routes] Failed to setup ${routeName}`;
      logger.error(msg, { routeName, error });

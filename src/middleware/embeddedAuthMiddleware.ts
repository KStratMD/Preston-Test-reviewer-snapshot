 ...
 * gates on the service-token bearer instead. Browser-originated calls to
 * guest-context / session-teardown MUST carry an Origin matching the API's
 * own origin (rejecting cross-origin XHR/fetch).
 */
function isSameOriginRequest(req: Request): boolean {
  const origin = req.headers.origin;
  if (typeof origin !== 'string') {
    // Closes Copilot review round-2: missing Origin used to return true
    // (treating server-to-server / non-browser clients as same-origin),
    // which let any non-browser caller that knew a sessionId bypass the
    // only cross-site check on guest-context and session-teardown. All
    // affected routes are POST/DELETE — modern browsers always emit
    // Origin on those — so requiring Origin is safe and closes the gap.
    return false;
  }
  ...
  const requestHostname = (req.get('host') ?? '').split(':')[0];

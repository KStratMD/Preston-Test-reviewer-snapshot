// Shared route-layer error helper for the HITL approval queue (PR 3B).
//
// Centralizes the catch logic so a route calling a BaseProvider-derived or
// BaseConnector-derived write only has to ADD a single line to its existing
// catch block:
//
//   if (await handleApprovalQueueError(err, req, res, classification)) return;
//
// Returning `true` means the helper fully handled the response (the route
// must not write again); `false` means the route should fall through to its
// existing error handling (typically a 500). The helper NEVER throws —
// enqueue failures fall through to `false` so the route's existing 500 path
// still surfaces something to the client (per the spec §11 risk table — the
// resume worker / enqueue path failing should never crash the request).
//
// Route mapping (per spec §3 typed errors + §4 route catch pattern):
//   PendingApprovalError      → 202 with pendingApprovalId + pollUrl
//   GovernanceBlockedError    → 403 with findings (existing Tier-A path)
//   ApprovalNotFoundError     → 404
//   AlreadyDecidedError       → 409 currentStatus
//   ApprovalExpiredError      → 410
//
// The last three are reserved for PR 3C's operator API which calls
// ApprovalQueueService.approve/reject directly; they're included here so the
// helper is the single source of truth for governance-error → HTTP mapping,
// avoiding drift between PR 3B routes and PR 3C operator endpoints.

import type { NextFunction, Request, Response } from 'express';

import { container } from '../../inversify/inversify.config';
import { TYPES } from '../../inversify/types';
import { Logger } from '../../utils/Logger';
import {
  GovernanceBlockedError,
  PendingApprovalError,
} from '../../services/governance/OutboundGovernanceErrors';
import {
  AlreadyDecidedError,
  ApprovalExpiredError,
  ApprovalNotFoundError,
  SeparationOfDutiesError,
  UnredactedPayloadError,
} from '../../services/governance/ApprovalQueueErrors';
import type { ApprovalQueueService } from '../../services/governance/ApprovalQueueService';
import type { ApprovalOperationType } from '../../services/governance/ApprovalQueueRepository';
import { verifiedIdentity } from '../../services/governance/verifiedIdentity';
import {
  OwnershipPendingApprovalError,
  WriteBlockedError,
} from '../../governance/sourceOfTruth/ConflictResolutionPolicy';

export interface ApprovalClassification {
  operationType: ApprovalOperationType;
  resourceType: string;
  /**
   * Caller-supplied target id. Use 'new' for create operations where no id
   * exists yet (matches spec §4 template). For deeply nested operations
   * (e.g. fullPipelineDemo → NetSuiteConnector.create at L739) supply the
   * outermost meaningful id.
   */
  resourceId: string;
}

/**
 * Inspect a caught error; if it is one of the typed governance errors, write
 * the appropriate HTTP response and return `true`. Otherwise return `false`
 * so the caller falls through to its own catch handling.
 *
 * `classification` only affects the `PendingApprovalError` branch — the
 * enqueue requires {operationType, resourceType, resourceId}. The other
 * branches map purely from the error class to an HTTP status.
 *
 * NEVER throws. Enqueue INFRASTRUCTURE failures (DB unreachable, container
 * unresolvable, etc.) are logged and the helper returns `false` so the caller's
 * existing 500 path still produces a response — the alternative (helper throws,
 * caller's catch already ran, request hangs) is worse.
 *
 * An identity REFUSAL is different and does not fall through: a request with no
 * verified tenant and user is answered here with a distinct 500
 * `approval_identity_required`, so the governance-side reason survives instead
 * of collapsing into the caller's generic 500. See `enqueueAndRespond`.
 */
export async function handleApprovalQueueError(
  err: unknown,
  req: Request,
  res: Response,
  classification: ApprovalClassification,
): Promise<boolean> {
  // Headers already sent: nothing safe to do; let caller's catch decide.
  if (res.headersSent) return false;

  // OwnershipPendingApprovalError fires from guardedWrite AFTER a successful
  // queue_for_human enqueue — the governance_approvals row already exists,
  // we just need to return the 202 with the existing queueId. Copilot R2
  // (PR 13b): this was previously falling through to the route's generic
  // 500 handler because the helper only recognized PendingApprovalError
  // (which TRIGGERS enqueue) and WriteBlockedError (which hard-blocks).
  // Must be checked BEFORE WriteBlockedError since neither extends the
  // other and order matters for clarity.
  if (err instanceof OwnershipPendingApprovalError) {
    res.status(202).json({
      pendingApprovalId: err.queueId,
      pollUrl: `/api/governance/approvals/${err.queueId}`,
    });
    return true;
  }

  // WriteBlockedError subclasses (OwnershipViolationError, OwnershipBlockedError,
  // LoopDetectedError) are checked FIRST — they are more specific than the
  // approval-queue typed errors and must not fall through to the 500 handler
  // when routes catch errors from guardedWrite (Tasks 8-10).
  if (err instanceof WriteBlockedError) {
    res.status(409).json({
      error: err.code,
      ...err.detail,
    });
    return true;
  }

  if (err instanceof PendingApprovalError) {
    return enqueueAndRespond(err, req, res, classification);
  }
  if (err instanceof GovernanceBlockedError) {
    res.status(403).json({
      ok: false,
      code: 'governance_blocked',
      findings: err.decision.findings,
      riskLevel: err.decision.riskLevel,
    });
    return true;
  }
  if (err instanceof SeparationOfDutiesError) {
    // 403, not 409: the decision was refused because of WHO asked, not because
    // of the row's state. The reason travels so an operator can tell a missing
    // grant from a self-approval from an out-of-scope account.
    res.status(403).json({ ok: false, code: 'separation_of_duties', reason: err.reason });
    return true;
  }
  if (err instanceof ApprovalNotFoundError) {
    res.status(404).json({ ok: false, code: err.code, message: err.message });
    return true;
  }
  if (err instanceof AlreadyDecidedError) {
    res.status(409).json({
      ok: false,
      code: err.code,
      currentStatus: err.currentStatus,
      message: err.message,
    });
    return true;
  }
  if (err instanceof ApprovalExpiredError) {
    res.status(410).json({ ok: false, code: err.code, message: err.message });
    return true;
  }
  return false;
}

/**
 * Synchronous Express error-handler middleware for WriteBlockedError subclasses
 * and OwnershipPendingApprovalError.
 *
 * Mount BEFORE the existing handleApprovalQueueError async helper (or use
 * alongside it) so that ownership blocks (409), queue_for_human signals (202),
 * and loop-detection blocks (409) surface before the queue helper's other
 * branches run.
 *
 * Shapes:
 *   - OwnershipPendingApprovalError → 202 { pendingApprovalId, pollUrl }
 *   - WriteBlockedError subclasses → 409 { error: <subclass.code>, ...detail }
 *
 * Non-matching errors are forwarded to the next handler via next(err).
 */
export function approvalQueueErrorHandler(
  err: unknown,
  _req: Request,
  res: Response,
  next: NextFunction,
): void {
  if (err instanceof OwnershipPendingApprovalError) {
    res.status(202).json({
      pendingApprovalId: err.queueId,
      pollUrl: `/api/governance/approvals/${err.queueId}`,
    });
    return;
  }
  if (err instanceof WriteBlockedError) {
    res.status(409).json({
      error: err.code,
      ...err.detail,
    });
    return;
  }
  next(err);
}

/** Resolve the Logger, or null when the container isn't initialized. */
function tryGetLogger(): Logger | null {
  try {
    return container.get<Logger>(TYPES.Logger);
  } catch {
    return null;
  }
}

async function enqueueAndRespond(
  err: PendingApprovalError,
  req: Request,
  res: Response,
  classification: ApprovalClassification,
): Promise<boolean> {
  // F6 PR4 Stage 2 — fail closed BEFORE the durable write.
  //
  // This was the last request-path sentinel fallback preceding a durable row.
  // extractIdentityContext never failed: it always returned a {tenantId, userId}
  // pair, substituting the retired sentinel for whichever half the request could
  // not supply — BOTH halves for an unidentifiable request, and the userId half
  // alone for a request carrying a tenant but no attributable user (the
  // req.tenantContext bridge). Either way the governance_approvals row was
  // written under a sentinel. PR 3B knowingly accepted that gap; it closes here.
  //
  // Refusal, not fall-through: returning `false` here would surface the
  // caller's generic 500 and silently lose the governance-side reason, which
  // is the same silent-fallback shape the F-track exists to remove. The
  // distinct 500 mirrors the UnredactedPayloadError branch below — both mean
  // "governance refused to persist", not "the tenant boundary rejected you"
  // (that contract lands at the central gate in Stage 5).
  //
  // Deliberately ahead of EVERY container resolution, including the Logger:
  // an unresolvable Logger must not be able to downgrade this refusal into the
  // caller's generic 500, so the refusal logs best-effort and answers anyway.
  const identity = verifiedIdentity(req);
  if (identity === null) {
    tryGetLogger()?.error(
      'approvalQueueErrorHandler: refused to enqueue without verified tenant and user',
      new Error('verified tenant and user identity are required before a durable approval write'),
      { path: req.path, resourceType: classification.resourceType },
    );
    res.status(500).json({
      ok: false,
      code: 'approval_identity_required',
      message: 'governance refused to enqueue: verified tenant and user identity are required',
    });
    return true;
  }

  const logger = tryGetLogger();
  if (logger === null) {
    // Container not initialized (e.g. some unit-test fixtures). Fall through.
    return false;
  }

  let approvalQueue: ApprovalQueueService;
  try {
    approvalQueue = await container.getAsync<ApprovalQueueService>(TYPES.ApprovalQueueService);
  } catch (resolveErr) {
    logger.error(
      'approvalQueueErrorHandler: failed to resolve ApprovalQueueService',
      resolveErr instanceof Error ? resolveErr : new Error(String(resolveErr)),
      { path: req.path },
    );
    return false;
  }

  const { tenantId, userId: requesterUserId } = identity;

  try {
    const approvalId = await approvalQueue.enqueue({
      tenantId,
      requesterUserId,
      // verifiedIdentity() returned non-null, and it reads req.user, which
      // src/middleware/auth.ts populates only from a verified jwt.verify().
      // So this requester was identified by a JWT, and saying so is what keeps
      // the row approvable under the strict requester policy (Task B4b).
      requesterProvenance: 'jwt',
      operationType: classification.operationType,
      resourceType: classification.resourceType,
      resourceId: classification.resourceId,
      reason: { kind: 'governance', decision: err.decision },
    });
    res.status(202).json({
      ok: false,
      code: 'pending_approval',
      pendingApprovalId: approvalId,
      pollUrl: `/api/governance/approvals/${approvalId}`,
    });
    return true;
  } catch (enqueueErr) {
    // Fail-closed shape: if the queue itself refuses (most importantly
    // UnredactedPayloadError, which means upstream produced an invalid
    // OutboundDecision — never persist raw PII), surface a 500 here rather
    // than the caller's generic 500. The caller's 500 would silently lose
    // the governance-side reason.
    if (enqueueErr instanceof UnredactedPayloadError) {
      logger.error(
        'approvalQueueErrorHandler: refused to enqueue unredacted payload (fail-closed)',
        enqueueErr,
        { tenantId, path: req.path, resourceType: classification.resourceType },
      );
      res.status(500).json({
        ok: false,
        code: enqueueErr.code,
        message: 'governance refused to enqueue: upstream decision did not produce a redacted payload',
      });
      return true;
    }
    logger.error(
      'approvalQueueErrorHandler: enqueue failed (request will receive caller default 500)',
      enqueueErr instanceof Error ? enqueueErr : new Error(String(enqueueErr)),
      { tenantId, path: req.path, resourceType: classification.resourceType },
    );
    return false;
  }
}

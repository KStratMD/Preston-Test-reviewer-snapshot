...
import { DEMO_ACTOR_ID } from '../services/governance/identityContext';
import { verifiedUserId } from '../services/governance/verifiedIdentity';
...
export function createGovernanceMiddleware(options: GovernanceMiddlewareOptions) {
  ...
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      // F6 PR4 Stage 2: attribution is either the VERIFIED req.user id or the
      // explicit anonymous demo label — never the retired system sentinel.
      // This middleware is mounted router-wide on /api/ai/proxy, so it sees F2's
      // anonymous demo traffic; DEMO_ACTOR_ID names that traffic honestly rather
      // than dressing it as system work. It is an attribution label ONLY — it
      // never supplies tenant scope or authorization.
      // Scope of the change is narrow by design: this value reaches
      // GovernanceService.validateInput's context, which uses userId only for a
      // truthiness check and does not persist it (it logs sessionId instead), so
      // no query, dashboard, metric or authorization decision moves with it.
      // Narrowed from the previous three-source read to req.user alone: this
      // middleware only ever runs on the ai-proxy mount, where req.auth is never
      // populated (AuthenticationMiddleware is not mounted there).
      const userId = verifiedUserId(req) ?? DEMO_ACTOR_ID;

...
const createKeyGenerator = (prefix: string) => {
  return (req: Request): string => {
    // Match the hosted resolver's /64 policy while retaining the user dimension.
    const ip = ipKeyGenerator(req.ip || req.connection.remoteAddress || 'unknown', 64);
...
/**
 * Serialized-asset readiness/activation budget per (IP, user) per 15 minutes.
 *
 * Every request on these two routes costs a secret-manager read, a connector
 * initialization, and TWO live Salesforce describe round-trips, so an
 * unthrottled loop is Salesforce API-limit exhaustion plus credential-store
 * amplification. 20 is generous for an operator iterating on a draft's field
 * selections (save selections -> re-check readiness -> activate) while cutting
 * a scripted loop down to something that cannot exhaust an org's daily API
 * allowance. Exported so the route test asserts against the real number rather
 * than a hardcoded copy that could drift.
 */
export const SERIALIZED_ASSET_READINESS_MAX_REQUESTS = 20;

/**
 * Factory (not a shared instance): production builds exactly one configuration
 * router, so one instance backs both routes and they genuinely SHARE a budget;
 * tests get a fresh MemoryStore per app instead of tripping each other's
 * counters. Mount it AFTER authMiddleware and the tenant kill switch so
 * rejected traffic never consumes budget and the (IP, user) key is populated.
 */
export const createSerializedAssetReadinessRateLimit = (): RequestHandler => rateLimit({
  windowMs: 15 * 60 * 1000,
  max: SERIALIZED_ASSET_READINESS_MAX_REQUESTS,
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: createKeyGenerator('serialized-asset-readiness'),
  handler: rateLimitHandler,
  skip: skipRateLimit,
});

/**
 * Serialized-asset FORCED-RETRY budget per (IP, user) per 15 minutes (Task 9,
 * 2026-07-27 NetSuite serialized-asset sync plan).
 *
 * Every accepted request re-sweeps the tenant's whole deferred backlog
 * (bypassing `next_attempt_at`) and re-runs the live Salesforce readiness
 * check before touching anything, so it costs the same secret-manager read +
 * connector-initialization + describe round-trips as the readiness route,
 * PLUS a real Asset upsert per deferred unit. Smaller budget than readiness
 * (10 vs 20) because this route also does real outbound writes, not just a
 * read-only check, and is gated behind `requireIntegrationTenantAdmin` so the
 * caller pool is already narrow. Exported so the route test asserts against
 * the real number rather than a hardcoded copy that could drift.
 */
export const SERIALIZED_ASSET_FORCED_RETRY_MAX_REQUESTS = 10;

/**
 * Factory (not a shared instance) for the same per-router-instance MemoryStore
 * reason as `createSerializedAssetReadinessRateLimit`. Mount AFTER
 * `authMiddleware`, the tenant kill switch, AND `requireIntegrationTenantAdmin`
 * so rejected traffic (unauthenticated, non-admin, missing tenant) never
 * consumes this dedicated budget.
 */
export const createSerializedAssetForcedRetryRateLimit = (): RequestHandler => rateLimit({
  windowMs: 15 * 60 * 1000,
  max: SERIALIZED_ASSET_FORCED_RETRY_MAX_REQUESTS,
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: createKeyGenerator('serialized-asset-forced-retry'),
  handler: rateLimitHandler,
  skip: skipRateLimit,
});
...
export const createMcpSchemaRateLimit = () => rateLimit({
  ...
  keyGenerator: (req: Request): string =>
    `mcp-schema:${ipKeyGenerator(req.ip || req.connection.remoteAddress || 'unknown', 64)}`,
  handler: rateLimitHandler,
  skip: skipRateLimit,
});

// F2: anonymous hosted-demo traffic on the /api/ai/proxy demo allowlist.
// Stricter than the family-wide aiRateLimit (100/15min). Anonymous demo
// traffic is zero-provider-spend by construction (F2 rulings), so this
// bounds abuse of rule-based compute + fixtures, not provider cost. Own
// MemoryStore per instance for the same per-router reason as
// createTestingRunRateLimit above. Does not honor the global rate-limit
// disable switch — this limiter exists FOR demo runtimes.
export const createAiDemoRateLimit = (): RequestHandler => rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 30,
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: 'demo_rate_limited', detail: 'Anonymous demo rate limit exceeded; try again later.' },
});

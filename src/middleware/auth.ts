...
// The canonical tenant-id validator now lives in `src/utils/tenantId.ts`,
// which imports nothing. It was extracted from this module during A5 review:
// this file imports the inversify container above, so every consumer of the
// validator (including pure schema modules) was paying that import cost and
// risking a cycle. Re-exported here because `normalizeTenantIdClaim` has been
// part of this module's public surface since F3 — importers that predate the
// extraction keep working unchanged.
import { normalizeTenantIdClaim } from '../utils/tenantId';
export { normalizeTenantIdClaim };
...
// The organization claim is read defensively from either conventional spelling
// and normalized to a trimmed non-empty string, mirroring the tenantId claim's
// handling. It is deliberately NOT put through normalizeTenantIdClaim: no
// tenant-registration seam consumes it, so the `[A-Za-z0-9_-]` shape rule that
// protects tenants.id would reject legitimate organization identifiers for no
// safety gain. tenantIsolation's verified-user extraction forwards this onto
// TenantContext.organizationId, which is the only source req.tenantContext
// consumers (MappingRouter cost attribution, MCPRouter tenant fallback) have
// for it once the verified-user path outranks raw-JWT extraction.
function normalizeOrganizationIdClaim(decoded: Record<string, unknown>): string | undefined {
  return normalizeJwtTextClaim(decoded.org_id) ?? normalizeJwtTextClaim(decoded.organizationId);
}
  ...
  try {
    ...
    try {
      ...
      req.user = {
        ...
        organizationId: normalizeOrganizationIdClaim(decoded),

...
export class AuthenticationMiddleware {
   ...
   */
  private async authenticateOAuth(req: Request): Promise<{
    type: 'oauth';
    user: OIDCClaims;
    permissions: string[];
    tenantId?: string;
  ...
  private async logAuthFailure(
    ...
    try {
      const userId = (authResult as any)?.type === 'oauth'
        ? (authResult as any).user?.sub || 'unknown'
        : (authResult as any)?.apiKey?.id || 'unknown';

      await this.auditLogRepository.create({
        tenant_id: (authResult as any)?.tenantId ?? SYSTEM_IDENTITY.tenantId,
        user_id: userId,

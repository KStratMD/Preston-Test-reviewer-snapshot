...
export function globalErrorHandler() {
  return (error: Error, req: Request, res: Response, _next: NextFunction) => {
    // Log the error
    const normalizedPath = (req.originalUrl || req.path).split('?')[0].toLowerCase();
    const credentialRoute = normalizedPath === '/api/credentials'
      || normalizedPath.startsWith('/api/credentials/')
      || normalizedPath === '/api/connector-credentials'
      || normalizedPath.startsWith('/api/connector-credentials/');
    const sanitizedBody = error instanceof ValidationAppError || credentialRoute
      ? '[REDACTED]'
      : (req.body ? sanitizeLogData(req.body) : undefined);
    ...
    // Cardinality preflight/activation gate: explicit branch BEFORE the
    // generic fallback below. CardinalityViolationError extends AppError
    // directly (not any of the subclasses above), so without this branch it
    // would fall through to the generic 500 response, losing its typed 422
    // contract. The error owns its own sanitized serialization — the
    // boundary only layers on the request-scoped timestamp/path, matching
    // every other branch's convention.

...
function extractTenantFromVerifiedUser(req: Request): TenantContext | null {
  ...
  if (
    ...
    // isValidTenantId enforces `^[a-zA-Z0-9_-]{1,64}$`, which rejects every
    // padded value outright. The sentinel compare below therefore needs no
    // .trim(): unlike the sibling verifiedTenantId/verifiedAdmin helpers,
    // which trim precisely because they have no format regex to subsume it.
    !isValidTenantId(user.tenantId) ||

...
/**
 * F2: requirePlatformAdmin's decision as a boolean, for in-handler branching
 * where the middleware (which responds directly) cannot be composed. Single
 * source of truth — requirePlatformAdmin delegates to this.
 */
export function isPlatformAdminActor(user: AdminClaims | undefined): boolean {
  if (readActorId(user).length === 0) return false;
...
/**
 * Integration tenant-administrator authorization from verified JWT claims
 * (Task 9, 2026-07-27 NetSuite serialized-asset sync plan).
 *
 * Mount AFTER `authMiddleware`, on the tenant namespace
 * (`/api/integrations/:id/serialized-assets/retry-deferred`), where the
 * operated-on tenant is always the caller's own `req.user.tenantId`. Like
 * {@link requirePlatformAdmin} and {@link requireSuiteCentralTenantAdmin} this
 * responds directly rather than delegating to `next(err)`.
 *
 * A missing tenant claim is 401, not 403 — same rationale as
 * {@link requireSuiteCentralTenantAdmin}: tenant identity is the *target* of
 * the operation, so without it there is no scope to authorize against.
 *
 * Deliberately narrower than {@link requireSuiteCentralTenantAdmin}: ONLY the
 * `tenant_admin` role authorizes here. That guard's `suitecentral:admin`
 * permission fallback is NOT accepted — it authorizes SuiteCentral-specific
 * authority, unrelated to an ERP retry command, and accepting it would
 * silently grant SuiteCentral admins control over serialized-asset execution
 * (specifically, the `MAX_DEFERRAL_ATTEMPTS` bypass this guard exists to gate
 * — decision 11 of the 2026-07-27 NetSuite serialized-asset sync plan). The
 * platform wildcard (`admin` role / `*` permission) is also not accepted, for
 * the same reason `requireSuiteCentralTenantAdmin` excludes it: a platform
 * actor reaching a tenant's configuration through this guard would attribute
 * the forced retry to the wrong access mode instead of a real cross-tenant
 * platform path.
 */
export const requireIntegrationTenantAdmin: RequestHandler = (req, res, next) => {
  const user = req.user as AdminClaims | undefined;
  if (readActorId(user).length === 0 || readTenantId(user).length === 0) {
    res.status(401).json({ error: 'Verified tenant administrator identity required' });
    return;
  ...
  if (!roles.includes('tenant_admin')) {
    res.status(403).json({ error: 'Integration tenant administrator access required' });
    return;
  }

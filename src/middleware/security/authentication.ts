...
export function createJWTValidator(logger: Logger, jwtSecret: string) {
  ...
  return (req: Request, _res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // jwt.verify returns `string` for non-JSON payloads. A validly-signed
      // string-payload token must not become req.user — downstream code
      // expects an object with id/roles/tenantId (AuthService.verifyJWT
      // rejects string payloads the same way).

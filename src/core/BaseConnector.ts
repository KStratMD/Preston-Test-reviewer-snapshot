...
/**
 * An `AxiosRequestConfig` tagged with a pre-redacted log URL. Set only by
 * `makeSensitiveRequest` (Task 4, 2026-07-27 NetSuite serialized-asset sync
 * plan) — request/response interceptors read `safeLogUrl` when present and
 * fall back to the real `config.url` for every ordinary (untagged) request,
 * so this is additive and never changes existing connector behavior.
 */
...
export abstract class BaseConnector implements IConnector {
  ...
  private async requestWithAuthentication<T>(config: AxiosRequestConfig): Promise<AxiosResponse<T>> {
    const authenticateGuarded = async (): Promise<boolean> => {
      if (this.isAuthenticating) return false;
      this.isAuthenticating = true;
      try {
        if (!(await this.authenticate())) {
          this.isAuthenticated = false;
          throw new UnauthorizedAppError('Authentication failed');
        }
      } finally {
    ...
    if (!this.isAuthenticated || this.isTokenExpired()) await authenticateGuarded();
    const context = replayContext(config);
    const send = () => this.retry(() => this.httpClient.request<T>(config), undefined, undefined, true, context);
    try {
      return await send();
    } catch (error) {
      if (!(error instanceof ReauthRequiredError)) throw error;
      this.isAuthenticated = false;
      if (!(await authenticateGuarded())) throw error.cause;
  ...
  protected handleApiError(error: unknown): Error {
    if (error instanceof Error && this.isNonRetryableError(error)) {
      if (error instanceof UnauthorizedAppError) this.isAuthenticated = false;
      return error;
    }
   ...
   * `safeLogUrl` (a sensitive request), log only the axios-provided generic
   * `.message` plus the already-redacted URL — never the raw error object,
   * which for an `AxiosError` embeds `.config.url`, `.config.data`, and
   * `.response.data` (the exact leak `makeSensitiveRequest` exists to
   * prevent). Untagged requests keep the original raw-error log so existing
   * behavior for ordinary connectors is unchanged.
   */
  protected redactInterceptorError(error: unknown): unknown {
    if (axios.isAxiosError(error)) {
      ...
      if (safeLogUrl !== undefined) {
        return { message: error.message, safeLogUrl };
      }
    }
    return error;
  }

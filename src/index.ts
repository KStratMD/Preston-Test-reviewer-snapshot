...
import { startVerificationServer } from './server/verificationServer';
...
class Server {
  ...
  /**
   * Everything `start()` does before the port opens.
   *
   * Split out so a caller can build the real production surface without
   * accepting traffic. `App` alone is not that surface: `/api/integrations`
   * is mounted here after the async IntegrationService resolution (RouteSetup
   * skips it during construction, when the service is still undefined), and
   * `/api/help` is mounted by `initializeHelpChat()` further down. Both are
   * followed by their own error handler, because Express resolves `next(err)`
   * only against handlers registered AFTER the failing one -- so the order of
   * this method is behaviour, not style.
   *
   * Composition also starts background workers, so anything that composes
   * without listening must still be torn down with `stop()`.
   */
  public async compose(): Promise<void> {
    ...
    // A9: this LATE mount — not RouteSetup's — is the one production actually
    // uses, so the forced-retry service has to be passed here too. Codex review
    // on #1132 caught that wiring only RouteSetup left the feature returning
    // 503 for every real request while every unit test passed. Same class of
    // mistake as A6's Dockerfile: wire the path the platform runs, not the
    // first one you find.
    const { SerializedAssetRetryOperationService } = await import(
      './services/serializedAsset/SerializedAssetRetryOperationService'
    );
    let serializedAssetRetryOperations:
      | InstanceType<typeof SerializedAssetRetryOperationService>
      | undefined;
    try {
      serializedAssetRetryOperations = await container.getAsync<
        InstanceType<typeof SerializedAssetRetryOperationService>
      >(TYPES.SerializedAssetRetryOperationService);
    } catch (error) {
      serializedAssetRetryOperations = undefined;
      this.logger.warn(
        'SerializedAssetRetryOperationService could not be resolved; forced-retry endpoints will report 503.',
    ...
    // Validate route-manifest coverage after all route registrations,
    // including the late /api/help family, and before any background worker
    // or scheduler is started. Literal gaps are proven at registration time;
    // unresolved dynamic shapes remain diagnostic and do not block boot.
    {
      const { assertRouteManifestCoverage, reportUnclassifiedMounts } = await import(
        './middleware/setup/routeManifestAudit'
      );
      const expressApp = this.application.getExpressApp();
      reportUnclassifiedMounts(expressApp, (m) => this.logger.warn(m));
      assertRouteManifestCoverage(expressApp);
    }
    ...
    // A9: start the durable forced serialized-asset retry worker. Same shape
    // as the retention job above — idempotent start, stopped in Server.stop()
    // before HTTP close so an in-flight drain is awaited rather than cut.
    // getAsync because the job's dependency chain reaches the async-bound
    // DatabaseService.
    try {
      const { SerializedAssetRetryJob } = await import('./services/serializedAsset/SerializedAssetRetryJob');
      const job = await container.getAsync<InstanceType<typeof SerializedAssetRetryJob>>(TYPES.SerializedAssetRetryJob);
      job.start();
      this.logger.info('SerializedAssetRetryJob started');
    } catch (error) {
      // Non-fatal: reservations still persist, they simply wait for a replica
      // whose worker is running. Failing startup over this would take the whole
      // API down for a background drain.
      this.logger.warn('Failed to start SerializedAssetRetryJob; forced serialized-asset retries will not drain on this replica.', { error: (error as Error).message });
    }
  ...
  /** Resolve a port and bind. Assumes `compose()` has already run. */
  ...
  public async stop(): Promise<void> {
    ...
    // A9: stop the forced-retry worker before closing HTTP. stop() awaits the
    // in-flight drain, and a claim taken but not yet started is marked
    // terminally interrupted rather than left `running` for a full lease.
    try {
      const { SerializedAssetRetryJob } = await import('./services/serializedAsset/SerializedAssetRetryJob');
      if (container.isBound(TYPES.SerializedAssetRetryJob)) {
        const job = await container.getAsync<InstanceType<typeof SerializedAssetRetryJob>>(TYPES.SerializedAssetRetryJob);
        await job.stop();
      }
    } catch (e) {
      try { this.logger.warn('SerializedAssetRetryJob.stop failed', { error: e }); } catch (_) { /* ignore */ }
    }
  ...
  private async loadSampleDataIfNeeded(): Promise<void> {
    const configs = this.configService.getAllConfigurations();
    const processHostedDemo = (process.env.HOSTED_DEMO ?? '').trim().toLowerCase();
    const isHostedDemo = env.HOSTED_DEMO
      || ['1', 'true', 'yes', 'on'].includes(processHostedDemo);
    const isProductionStrength = process.env.NODE_ENV !== 'development'
      && process.env.NODE_ENV !== 'test';
    const sampleKeys = new Set(sampleConfigurations.map(config => `${config.tenantId}:${config.id}`));
    const hasCompleteHostedSampleSet = [...sampleKeys].every(key => configs.some(
      config => `${config.tenantId}:${config.id}` === key,
    ));
    const shouldReconcileSamples = configs.length === 0
      || (isHostedDemo && !hasCompleteHostedSampleSet);

    if (shouldReconcileSamples) {
      this.logger.info(
        configs.length === 0
          ? 'No configurations found, loading sample configurations...'
          : 'Hosted sample configuration set is incomplete, repairing missing drafts...',
      );
      const existingKeys = new Set(configs.map(config => `${config.tenantId}:${config.id}`));
      for (const config of sampleConfigurations.filter(
        candidate => !existingKeys.has(`${candidate.tenantId}:${candidate.id}`),
      )) {
        try {
          // Startup has no operator authorization for cardinality findings.
          // Persist production-strength samples as drafts so the deployment
          // retains the definitions without bypassing the active-save gate; an
          // operator can review evidence and explicitly activate them later.
          const seed = isProductionStrength
          ...
          await this.configService.saveConfiguration(seed, context);
        } catch (error) {
    ...
    if (isHostedDemo) {
      const persistedConfigs = this.configService.getAllConfigurations();
      const persistedKeys = new Set(persistedConfigs.map(config => `${config.tenantId}:${config.id}`));
      const missingKeys = [...sampleKeys].filter(key => !persistedKeys.has(key));
      const activeCount = persistedConfigs
        .filter(config => sampleKeys.has(`${config.tenantId}:${config.id}`) && config.isActive).length;
      process.env.HOSTED_CONFIG_EXPECTED_COUNT = String(sampleConfigurations.length);
      process.env.HOSTED_CONFIG_EXPECTED_KEYS = JSON.stringify([...sampleKeys]);
      process.env.HOSTED_CONFIG_PERSISTED_COUNT = String(sampleConfigurations.length - missingKeys.length);
      process.env.HOSTED_CONFIG_ACTIVE_COUNT = String(activeCount);
      process.env.HOSTED_CONFIG_INACTIVE_DRAFT_COUNT = String(
        sampleConfigurations.length - missingKeys.length - activeCount,
      );
      if (missingKeys.length > 0) {
        this.logger.error('Hosted sample configuration seeding incomplete', { missingConfigurationKeys: missingKeys });
        throw new Error('Hosted sample configuration seeding did not persist the expected configuration set');
      }
    }
  }
...
export const main = async () => {
  ...
  if (process.env.DB_READ_ONLY === '1' && process.env.DB_VERIFY_ONLY !== '1') {
    throw new Error('DB_READ_ONLY is reserved for DB_VERIFY_ONLY verification boot');
  }

  logger.debug('[BOOT] Constructing Server...');
  if (process.env.DB_VERIFY_ONLY === '1') {
    if (process.env.DB_TYPE !== 'postgres') throw new Error('DB_VERIFY_ONLY requires DB_TYPE=postgres');
    process.env.DB_READ_ONLY = '1';
    const database = await container.getAsync<DatabaseService>(TYPES.DatabaseService);
    const verification = await startVerificationServer(database, serverConfig.port);
    const shutdownVerification = async (code = 0): Promise<never> => {
      try { await verification.close(); } finally { process.exit(code); }
    };
    process.on('SIGTERM', () => void shutdownVerification(0));
    process.on('SIGINT', () => void shutdownVerification(0));
    logger.info('Verification-only server started');
  ...
  try {
    ...
    logger.error('[BOOT] server.start() failed', { error });
    await shutdown(1);
  }
};

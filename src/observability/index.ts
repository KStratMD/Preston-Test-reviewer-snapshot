...
export class ObservabilityService implements IObservabilityService {
  ...
  constructor(config: ObservabilityConfig) {
    ...
    if (TracingServiceImport) {
      // Map the app-level TracingConfig (types.ts) onto the service-level one
      // (tracing.ts) explicitly — the old `config.tracing as any` pass-through
      // silently dropped `endpoint` because the field names never matched.
      this.tracing = new TracingServiceImport(
        {

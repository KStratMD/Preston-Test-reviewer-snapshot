...
export class MigrationRunner {
  ...
  /** Verify that an already-initialized database has exactly this ordered history. */
  async verifyExactHistory(): Promise<void> {
    const result = await sql<{ name: string }>`
      SELECT name FROM migrations ORDER BY id
    `.execute(this.db);
    const actual = result.rows.map((row) => row.name);
    if (actual.length !== this.manifest.length || actual.some((name, index) => name !== this.manifest[index])) {
      throw new Error('Migration history does not exactly match the ordered application manifest');
    }
  }

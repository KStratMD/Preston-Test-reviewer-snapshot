...
export class SqliteTransferSource {
  ...
  rows(table: string, columns: readonly string[], sortColumns: readonly string[]): readonly Record<string, unknown>[] {
    ...
    // Keep small INTEGER values as numbers: canonicalBoolean uses strict 1/0 checks.
    // Preserve larger INTEGER values as bigint until exact string canonicalization.
    const rows = this.db.prepare(`SELECT ${select} FROM ${quote(table)} ORDER BY ${order}`)
      .safeIntegers(true).all() as readonly Record<string, unknown>[];

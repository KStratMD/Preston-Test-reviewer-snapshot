...
import { FORBIDDEN_SESSION_OPTION_KEYS } from './transfer/targetIdentity';
  ...
  async initialize(): Promise<void> {
    try {
      if (this.dbType === 'sqlite') {
        ...
        if (process.env.DB_READ_ONLY === '1' && env.DATABASE_URL) {
          try {
            const parsed = new URL(env.DATABASE_URL);
            if ([...parsed.searchParams.keys()].some((key) => FORBIDDEN_SESSION_OPTION_KEYS.has(key.toLowerCase()))) {
              throw new Error('DATABASE_URL session-option overrides are forbidden in read-only verification mode');
            }
          } catch (error) {
            if (error instanceof Error && error.message.includes('session-option')) throw error;
            throw new Error('DATABASE_URL is invalid for read-only verification mode', { cause: error });
          }
        }
        ...
        this.pool = new Pool(
          useConnectionString
            ? {
                ...
                ...(process.env.DB_READ_ONLY === '1' ? { options: '-c default_transaction_read_only=on' } : {}),
              }
            : {
                host: env.DB_HOST,
                port: env.DB_PORT,
      ...
      // Verification boot never mutates schema: it checks exact history while
      // the PostgreSQL session defaults every transaction to read-only.
      if (this.dbType === 'postgres' && process.env.DB_READ_ONLY === '1') {
        const readOnly = await sql<{ default_transaction_read_only: string }>`SHOW default_transaction_read_only`.execute(this.db);
        if (readOnly.rows[0]?.default_transaction_read_only !== 'on') throw new Error('PostgreSQL read-only session enforcement is not active');
        const runner = new MigrationRunner({ db: this.db, dbType: this.dbType, modules: MIGRATIONS, logger: this.logger });
        await runner.verifyExactHistory();
      } else {

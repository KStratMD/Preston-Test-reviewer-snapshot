...
export interface GovernanceApprovalsTable {
  ...
  apply_status: ColumnType<'not_started' | 'claimed' | 'succeeded' | 'failed', 'not_started' | 'claimed' | 'succeeded' | 'failed' | undefined, 'not_started' | 'claimed' | 'succeeded' | 'failed'>;
  applied_at: ColumnType<Date | string | null, Date | string | null | undefined, Date | string | null>;
  apply_failed_at: ColumnType<Date | string | null, Date | string | null | undefined, Date | string | null>;
  apply_error: string | null;
  /** JSON-serialized WriteDescriptor for ownership_write rows; NULL for legacy governance rows. */
  write_descriptor: string | null;
  /**
   * How the requester was identified when the approval was raised. Defaults to
   * 'unknown' so a row written by a path that predates migration 065 cannot be
   * mistaken for a verified request.
   */
  requester_provenance: ColumnType<string, string | undefined, string>;
  /** The scope an approver must match. Null for JWT and system requests. */
  requester_platform: string | null;
  requester_platform_account_id: string | null;
  /**
   * A8 dedupe key: HMAC-SHA-256 of the canonical write intent, 64 lowercase
   * hex characters. Set on ownership_write rows only; NULL for legacy rows and
   * for every other operation type, neither of which is deduped. A partial
   * unique index (migration 062) enforces one PENDING ownership row per
   * (tenant_id, request_fingerprint).
   *
   * Optional on insert so the non-ownership arms need no change.
   */
...
// Cost Rollup Tables
export interface CostRollupDailyTable {
  id: string;
  tenant_id: string;
  provider: string;
  date_utc: string;
...
export interface CostRollupPerFlowTable {
  id: string;
  tenant_id: string;
  flow_name: string;
  date_utc: string;
...
export type CostRollupDailyRow = Selectable<CostRollupDailyTable>;
export type NewCostRollupDaily = Insertable<CostRollupDailyTable>;
export type CostRollupDailyUpdate = Updateable<CostRollupDailyTable>;

export type CostRollupPerFlowRow = Selectable<CostRollupPerFlowTable>;
export type NewCostRollupPerFlow = Insertable<CostRollupPerFlowTable>;
export type CostRollupPerFlowUpdate = Updateable<CostRollupPerFlowTable>;

// ----- Reconciliation Center (PR 11) -------------------------------------

export interface ReconciliationExceptionsTable {
  id: string;
  tenant_id: string;
  source_system: string;
  target_system: string;
...
export interface ReconciliationSchedulesTable {
  id: string;
  tenant_id: string;
  name: string;
  cadence: 'hourly' | 'daily' | 'weekly';
  /** Reconciler discriminator (migration 052). v1: 'netsuite_business_central_invoice_reconciliation'. */
  handler_key: string;
  // Matches IntegrationConfigHistoryTable.is_active / TenantConfigurationsTable.is_active
  // / EmbeddedSessionsTable.is_active conventions. SQLite stores this as
  // INTEGER (0/1) per migration 048, but callers should treat reads as
  // boolean and coerce at the boundary if needed — Kysely's ColumnType
  // pattern is reserved for cases where the insert + select shapes diverge
  // beyond truthy coercion.
  active: boolean;
  next_run_at: string;
  /** FK-less reference to the tenant's NS↔BC IntegrationConfig (migration 055; NOT NULL since migration 056). Legacy null-config rows were backfilled to the '__unconfigured__' sentinel + deactivated. */
...
export type ReconciliationExceptionRow = Selectable<ReconciliationExceptionsTable>;
export type NewReconciliationExceptionRow = Insertable<ReconciliationExceptionsTable>;
export type ReconciliationExceptionUpdate = Updateable<ReconciliationExceptionsTable>;

export type ReconciliationScheduleRow = Selectable<ReconciliationSchedulesTable>;
export type NewReconciliationScheduleRow = Insertable<ReconciliationSchedulesTable>;
export type ReconciliationScheduleUpdate = Updateable<ReconciliationSchedulesTable>;

export type ReconciliationRunRow = Selectable<ReconciliationRunsTable>;
export type NewReconciliationRunRow = Insertable<ReconciliationRunsTable>;
export type ReconciliationRunUpdate = Updateable<ReconciliationRunsTable>;

// ----- Record-Level Lineage (PR 12) --------------------------------------

export interface LineageEventsTable {
  id: string;
  tenant_id: string;
  chain_id: string;
  sequence: number;
  ...
  // sqlite returns TEXT as string; node-postgres returns TIMESTAMPTZ as Date
  // for SELECTs. Repository methods normalize to ISO string before returning
  // outward (LineageEventView.occurredAt + LineageChainSeed.occurredAt are
  // both string). PR #846 R3.
  occurred_at: string | Date;
}

export type LineageEventRow = Selectable<LineageEventsTable>;
export type NewLineageEventRow = Insertable<LineageEventsTable>;
...
export interface Database {
  ...
  cost_rollup_daily: CostRollupDailyTable;
  cost_rollup_per_flow: CostRollupPerFlowTable;
  embedded_sessions: EmbeddedSessionsTable;
  embedded_role_grants: EmbeddedRoleGrantsTable;
  embedded_user_assertion_nonces: EmbeddedUserAssertionNoncesTable;
  ...
  reconciliation_exceptions: ReconciliationExceptionsTable;
  reconciliation_schedules: ReconciliationSchedulesTable;
  reconciliation_runs: ReconciliationRunsTable;
  lineage_events: LineageEventsTable;
  suitecentral_environments: SuiteCentralEnvironmentsTable;
  suitecentral_credential_profiles: SuiteCentralCredentialProfilesTable;
  suitecentral_templates: SuiteCentralTemplatesTable;
  suitecentral_monitoring_configs: SuiteCentralMonitoringConfigsTable;
  suitecentral_allowed_hosts: SuiteCentralAllowedHostsTable;
  deferred_serialized_units: DeferredSerializedUnitsTable;
  serialized_asset_sweep_cursors: SerializedAssetSweepCursorsTable;
  serialized_asset_retry_operations: SerializedAssetRetryOperationsTable;
}

/**
 * A9 (migration 063) — one durable row per forced serialized-asset retry, so
 * the operation outlives the HTTP request that reserved it.
 *
 * The result projection is deliberately narrow: bounded counters plus a fixed
 * error code and class. These rows are operator-readable and reach an HTTP
 * status endpoint, so there is intentionally nowhere to put a serial number, a
 * source payload, a raw connector error, or a cause chain (decision 8). Do not
 * add a free-text or JSON column here.
 */
export interface SerializedAssetRetryOperationsTable {
  id: string;
  tenant_id: string;
  ...
  requester_user_id: string;
  correlation_id: string;
  /** 'accepted' | 'running' | 'succeeded' | 'failed' | 'interrupted' — CHECK-constrained in 063. */
  status: ColumnType<string, string | undefined, string>;
  /** Worker instance holding the lease; NULL until first claim. */
  lease_owner: ColumnType<string | null, string | null | undefined, string | null>;
  /**
   * Monotonic per operation. Every claim and takeover increments it, and every
   * heartbeat and terminal write must match it — that is what stops a worker
   * whose lease expired from publishing a result over its successor's.
   */
  fencing_token: ColumnType<number, number | undefined, number>;
  lease_expires_at: ColumnType<Date | string | null, Date | string | null | undefined, Date | string | null>;
  heartbeat_at: ColumnType<Date | string | null, Date | string | null | undefined, Date | string | null>;
  created_at: ColumnType<Date | string, Date | string, Date | string>;
  started_at: ColumnType<Date | string | null, Date | string | null | undefined, Date | string | null>;
  finished_at: ColumnType<Date | string | null, Date | string | null | undefined, Date | string | null>;
  units_read: ColumnType<number | null, number | null | undefined, number | null>;
  units_upserted: ColumnType<number | null, number | null | undefined, number | null>;
  units_deferred: ColumnType<number | null, number | null | undefined, number | null>;
  units_quarantined: ColumnType<number | null, number | null | undefined, number | null>;
  units_failed: ColumnType<number | null, number | null | undefined, number | null>;
  /** Fixed vocabulary code, never a raw driver or connector message. */
  error_code: ColumnType<string | null, string | null | undefined, string | null>;
  /** Error CLASS name only — no message, no cause chain. */
  error_class: ColumnType<string | null, string | null | undefined, string | null>;
}

// ----- SuiteCentral control-plane (migration 057, PR-A) --------------------
// Durable, tenant-scoped replacement for the retired process-global config /
// monitoring maps. Secrets are never stored here — credential rows carry only a
// deterministic secret_ref resolved through SecretManager.

export interface SuiteCentralEnvironmentsTable {
  id: string;
  tenant_id: string;
  name: string;
  base_url: string;
  environment_tier: ColumnType<'sandbox' | 'production', 'sandbox' | 'production' | undefined, 'sandbox' | 'production'>;
  api_version: string | null;
  timeout_ms: ColumnType<number, number | undefined, number>;
  retry_attempts: ColumnType<number, number | undefined, number>;
  rate_limit_config: NullableJsonColumn;
  security_config: NullableJsonColumn;
  feature_config: NullableJsonColumn;
...
export interface SuiteCentralCredentialProfilesTable {
  id: string;
  tenant_id: string;
  ...
  scopes: NullableJsonColumn;
  is_active: ColumnType<boolean, boolean | number | undefined, boolean | number>;
  rotated_at: ColumnType<Date | string | null, Date | string | null | undefined, Date | string | null>;
  last_used_at: ColumnType<Date | string | null, Date | string | null | undefined, Date | string | null>;
  version: ColumnType<number, number | undefined, number>;
  created_by: string | null;
  updated_by: string | null;
  created_at: GeneratedDateColumn;
...
export interface SuiteCentralTemplatesTable {
  id: string;
  tenant_id: string;
  name: string;
  description: string | null;
  source_system: string;
  target_entities: NullableJsonColumn;
  field_mappings: NullableJsonColumn;
  business_rules: NullableJsonColumn;
  sync_settings: NullableJsonColumn;
  version: ColumnType<number, number | undefined, number>;
  created_by: string | null;
  updated_by: string | null;
  created_at: GeneratedDateColumn;
...
export interface SuiteCentralMonitoringConfigsTable {
  id: string;
  tenant_id: string;
  environment_id: string;
  enabled: ColumnType<boolean, boolean | number | undefined, boolean | number>;
  interval_ms: ColumnType<number, number | undefined, number>;
  thresholds: NullableJsonColumn;
  version: ColumnType<number, number | undefined, number>;
  created_by: string | null;
  updated_by: string | null;
  created_at: GeneratedDateColumn;
...
export interface SuiteCentralAllowedHostsTable {
  id: string;
  hostname: string;
  allowed_ports: NullableJsonColumn;
...
// ----- Deferred serialized-unit work (migration 058, NetSuite serialized- --
// asset sync Task 2) --------------------------------------------------------
// Durable tenant-scoped store for `inventorynumber` rows that could not yet be
// upserted as a Salesforce Asset (parent Product2 not yet synced, or a
// transient dependency failure). Unique by (tenant_id, configuration_id,
// inventory_number_id) — decision 9 of the plan; a repeat deferral of the same
// physical unit updates the row in place instead of accumulating duplicates.
// `normalized_payload` may carry the serial number (decision 8's one permitted
// exception) — every other column, and every log/metric/audit/exception
// surface built on top of this table, must not. `reason` is a closed enum for
// exactly that reason: it structurally cannot carry a serial number.
export interface DeferredSerializedUnitsTable {
  tenant_id: string;
  configuration_id: string;
  inventory_number_id: string;
  normalized_payload: JsonColumn;
  // Mirrors migration 058's CHECK constraint and
  // `DeferredSerializedUnitReason`. Closed by design: this column is a
  // decision-8 surface and structurally cannot carry a serial number.
  reason: 'parent_missing' | 'transient_dependency_failure' | 'write_failed';
  attempt_count: number;
...
export type DeferredSerializedUnitRow = Selectable<DeferredSerializedUnitsTable>;
export type NewDeferredSerializedUnitRow = Insertable<DeferredSerializedUnitsTable>;
export type DeferredSerializedUnitUpdate = Updateable<DeferredSerializedUnitsTable>;

/**
 * Durable source-sweep position per (tenant, configuration) — migration 059.
 * `next_offset` is where the NEXT run resumes; a run that exhausts the source
 * resets it to 0 so sweeps wrap and no source row is permanently unreachable.
 */
export interface SerializedAssetSweepCursorsTable {
  tenant_id: string;
  configuration_id: string;
  next_offset: number;
...
export type SerializedAssetSweepCursorRow = Selectable<SerializedAssetSweepCursorsTable>;
...
export interface EmbeddedSessionsTable {
  ...
  /**
   * How the user on this session was identified. 'host_asserted' is the
   * default and means the host named the user — enough to display, never
   * enough to authorize. 'squire_verified' means the user proved possession
   * of a grant secret (migration 065, Task B3b) and `verified_grant_id`
   * names the row that was proven. Defaulting to 'host_asserted' is what
   * stops a writer that predates this column from implying verification.
   */
  user_identity: ColumnType<
    'host_asserted' | 'squire_verified',
    'host_asserted' | 'squire_verified' | undefined,
    'host_asserted' | 'squire_verified'
  >;
  /** The `embedded_role_grants.id` proven for this session, else null. */
  verified_grant_id: string | null;
  created_at: Generated<Date | string>;
}
...
/**
 * One row per role grant. This is the only source of a privileged role — the
 * host-bootstrap body can no longer supply one.
 *
 * The partial unique index (migration 065) keeps at most one OPEN row per
 * (tenant, platform, account, user, role): `revoked_at IS NULL` is the open
 * predicate, so an expired-but-unrevoked row still holds the slot and renewal
 * must close the old row explicitly.
 */
export interface EmbeddedRoleGrantsTable {
  id: string;
  tenant_id: string;
  platform: string;
  ...
  source: 'squire_operator' | 'squire_invitation' | 'erp_verified';
  granted_by: string;
  granted_at: Date | string;
  /** A time claim. Expiry alone does NOT free the unique-index slot. */
  expires_at: Date | string | null;
  /** A decision. Setting it is what frees the slot for a re-grant. */
  revoked_at: Date | string | null;
  revoked_by: string | null;
  /**
   * The per-grant user secret as EncryptionService ciphertext. NOT NULL: a
   * grant with no secret can never be asserted against, so it would look like
   * authority while conferring none. The plaintext is never stored.
   */
  secret_enc: string;
}

export type EmbeddedRoleGrant = Selectable<EmbeddedRoleGrantsTable>;
export type NewEmbeddedRoleGrant = Insertable<EmbeddedRoleGrantsTable>;
export type EmbeddedRoleGrantUpdate = Updateable<EmbeddedRoleGrantsTable>;

/**
 * Replay ledger for user assertions. A separate table rather than a session
 * column because guest teardown deletes session rows on pagehide, and a replay
 * ledger that dies with the session does not prevent replay.
 */
export interface EmbeddedUserAssertionNoncesTable {
  nonce: string;
  grant_id: string;
  used_at: Date | string;
}

export type EmbeddedUserAssertionNonce = Selectable<EmbeddedUserAssertionNoncesTable>;
export type NewEmbeddedUserAssertionNonce = Insertable<EmbeddedUserAssertionNoncesTable>;

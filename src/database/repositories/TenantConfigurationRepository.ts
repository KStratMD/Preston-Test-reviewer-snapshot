...
export class TenantConfigurationRepository {
  ...
  /** Plaintext-only STRING read — the string analogue of `getBooleanStrict`,
   *  for callers that must distinguish "not configured" from "infra failure".
   *  `getString` turns a SecretManager failure (or a deterministic-name
   *  mismatch) on an encrypted row into `null`; a caller that treats `null` as
   *  "absent" would then read an outage as a deliberate absence. For an
   *  access-control input (e.g. the managed-system ownership registry) that
   *  collapse would turn an outage into a silent denial, indistinguishable
   *  from a real refusal.
   *
   *  Throws on encrypted rows. Lets DB infra errors propagate unchanged.
   *  Missing row → null (legitimate "not configured yet").
   *  Plaintext row → `setting_value` verbatim.
   *
   *  SecretManager MUST NOT be consulted on the throw path — same invariant as
   *  `getBooleanStrict`: the throw says "this row's at-rest posture is wrong",
   *  and a successful backend lookup would mask that misconfiguration. */
  async getStringStrict(tenantId: string, settingKey: string): Promise<string | null> {
    const row = await this.db.getDatabase()
      .selectFrom('tenant_configurations')
      .select(['setting_value', 'is_encrypted'])
      .where('tenant_id', '=', tenantId)
    ...
    if (row.is_encrypted) {
      throw new Error(
        `tenant_configurations.${settingKey} for tenant ${tenantId} must be stored as plaintext for the strict-read path; encrypted rows would collapse SecretManager outages into null/deny per the standard getString contract`,
      );
    }

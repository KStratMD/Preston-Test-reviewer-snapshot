...
export class AuditLogRepository {
   ...
   */
  /**
   * The row an insert actually writes.
   *
   * Extracted so `insertWithin` and `create` cannot drift. Everything here is
   * load-bearing — the id substitution, the JSON serialisation of the three
   * object columns, and the created_at normalisation — so a raw
   * `trx.insertInto('audit_logs')` elsewhere would silently skip all of it and
   * write a row Postgres rejects or a reader cannot parse.
   */
  private toInsertRow(auditLog: NewAuditLog): Record<string, unknown> {
    return {
      ...auditLog,
  ...
  async create(auditLog: NewAuditLog): Promise<AuditLog> {
    const result = await this.db
      ...
      .values(this.toInsertRow(auditLog) as never)
      .returningAll()
  ...
  /**
   * Insert an audit row inside a caller's transaction.
   *
   * Exists so a state change and the record of it can commit or fail together.
   * A separate insert afterwards can fail on its own and leave, for example, an
   * approved governance row with no success audit — the row says a decision
   * happened and nothing says who made it.
   *
   * Returns nothing: `returningAll` is deliberately not used, because the
   * caller is inside a transaction that may still roll back and a row read back
   * mid-transaction is not yet a fact.
   */
  async insertWithin(trx: Transaction<Database>, auditLog: NewAuditLog): Promise<void> {
    await trx.insertInto('audit_logs').values(this.toInsertRow(auditLog) as never).execute();
  }

...
export class PerformanceMonitor {
  ...
  private readonly sampler: ProcessResourceSampler;
  private readonly statisticsCpuUsageCalculator = new CpuUsageDeltaCalculator();
  private readonly periodicCpuUsageCalculator = new CpuUsageDeltaCalculator();
  ...
  constructor(
    ...
    @unmanaged() sampler: ProcessResourceSampler = new NodeProcessResourceSampler(),
  ) {
    this.logger = logger;
  ...
  getStatistics(periodMinutes = 60): PerformanceStats {
    const resourceUsage = this.getCurrentResourceUsage(
      this.sampler.sample(),
      this.statisticsCpuUsageCalculator,
    );

    return this.buildStatistics(periodMinutes, resourceUsage);
  }

  private buildStatistics(periodMinutes: number, resourceUsage: ResourceUsage): PerformanceStats {
  ...
  isSystemUnderStress(): {
    ...
    return this.getStressAssessment(stats);
  }

  private getStressAssessment(stats: PerformanceStats): {
    underStress: boolean;
    reasons: string[];
    severity: 'low' | 'medium' | 'high';
  } {
  ...
  generateReport(periodMinutes = 60): {
    ...
    const summary = this.buildStatistics(periodMinutes, resourceUsage);
    const stressCheck = this.getStressAssessment(this.buildStatistics(5, resourceUsage));
  ...
  clearMetrics(): void {
    ...
    this.statisticsCpuUsageCalculator.reset();
    this.periodicCpuUsageCalculator.reset();
  ...
  private collectResourceUsage(): void {
    const resourceUsage = this.getCurrentResourceUsage(
      this.sampler.sample(),
      this.periodicCpuUsageCalculator,
    );
  ...
  private getCurrentResourceUsage(
    ...
    cpuUsageCalculator: CpuUsageDeltaCalculator,
  ): ResourceUsage {
    const cpuPercent = cpuUsageCalculator.calculate(snapshot) ?? 0;

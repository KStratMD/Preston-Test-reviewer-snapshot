...
export class ServiceFactory {
  ...
  static async createCacheService(): Promise<CacheServiceConstructor | null> {
    // env.DISABLE_REDIS, not the raw process.env read this replaced. The raw
    // read was truthy for ANY non-empty string, so docker-compose's
    // DISABLE_REDIS=0 ("Redis enabled") suppressed the cache against a healthy
    // Redis while the queue subsystem connected to it — one process
    // disagreeing with itself about the same flag.

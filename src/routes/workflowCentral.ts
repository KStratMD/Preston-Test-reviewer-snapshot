...
import { resolveCentralActor, resolveCentralTenantId } from './central/centralTenant';
...
router.post('/instances', asyncHandler(async (req, res) => {
  const service = await getService();
  // F5b: the audit actor is always the verified JWT subject. Writes are never
  // on a demo allowlist, so the gate guarantees an authenticated caller, and
  // a body-supplied startedBy is ignored rather than honored — the pre-auth
  // demo body-actor branch is closed by the strict mount.
  const actor = resolveCentralActor(req, res);
  if (actor === null) return;
  const { tenantId, userId: startedBy } = actor;
...
router.post('/instances/:id/cancel', asyncHandler(async (req, res) => {
  const service = await getService();
  // F5b: cancel actor is the verified JWT subject; body-supplied cancelledBy
  // is ignored (see POST /instances).
  const actor = resolveCentralActor(req, res);
  if (actor === null) return;
  const { tenantId, userId: cancelledBy } = actor;
...
router.post('/tasks/:id/complete', asyncHandler(async (req, res) => {
  const { actionId, comment, data } = req.body ?? {};

  // Field validation per spec D5 F-08 — deliberately BEFORE identity so a
  // malformed body stays a 400 (the pre-F5b contract order).
  ...
  // F5b: completion actor is the verified JWT subject; body-supplied
  // completedBy is ignored (see POST /instances).
  const actor = resolveCentralActor(req, res);
  if (actor === null) return;
  const { tenantId, userId: completedBy } = actor;
...
router.post('/tasks/:id/delegate', asyncHandler(async (req, res) => {
  const service = await getService();
  // F5b: delegation actor is the verified JWT subject; body-supplied
  // delegatedBy is ignored (see POST /instances).
  const actor = resolveCentralActor(req, res);
  if (actor === null) return;
  const { tenantId, userId: delegatedBy } = actor;
  const { newAssigneeId, newAssigneeName } = req.body ?? {};

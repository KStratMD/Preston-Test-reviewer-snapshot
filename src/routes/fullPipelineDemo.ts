...
/**
 * Trio of governance services every guardedWrite call needs. Resolved once
 * in the route handler and threaded into the per-step helpers so the
 * extract/transform/load helpers stay free of DI-container calls. PR 13b
 * Stage A3 follow-up.
 */
interface GuardedWriteDeps {
...
/**
 * Identity + correlation envelope threaded from the route handler into the
 * helpers so guardedWrite gets request-scoped tenantId / userId / correlationId
 * without each helper re-reading `req`.
 */
interface PipelineGovernanceContext {
  tenantId: string;
...
/**
 * The ONE source of truth for this family's request defaults.
 *
 * `POST /execute` destructures from it and `GET /configurations` advertises a
 * copy, so advertised and applied defaults cannot drift — the drift is
 * impossible by construction rather than merely detected.
 *
 * Safe by default: an omitted body SIMULATES. A real SuiteCentral write needs
 * an explicit `dryRun: false`, and the NetSuite leg additionally needs an
 * explicit `includeNetSuiteSync: true`.
 *
 * "Dry run" bounds WRITE behavior only. It is not a promise of a
 * side-effect-free request: connector initialization, the Squire fixture
 * read, logging, and in-memory pipeline metrics all still happen.
 */
const FULL_PIPELINE_DEFAULTS = Object.freeze({
  batchSize: 10,
  includeNetSuiteSync: false,
  dryRun: true,
});

/** A body we can safely destructure: an object, and not an array. */
function isPlainObjectBody(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
}

/** The three safety-sensitive fields, resolved to concrete values. */
interface PipelineRequestOptions {
  batchSize: number;
  includeNetSuiteSync: boolean;
  dryRun: boolean;
}

/**
 * Validate the three safety-sensitive fields. Returns the caller-facing
 * message for the first invalid field, or null when the body is acceptable.
 * Applying the defaults is `resolvePipelineOptions`' job, immediately below.
 *
 * Values are NEVER coerced: `"false"` is invalid rather than truthy, because
 * a truthy `"false"` would silently turn a requested simulation into a real
 * run. Omission is not an error — it selects the shared default.
 *
 * batchSize's lower bound is the load-bearing one: both batch loops advance
 * with `i += batchSize`, so zero never terminates and the request hangs
 * holding a connector. The upper bound is ordinary sanity.
 *
 * Check order is fixed so a body with several invalid fields always produces
 * the same response.
 */
function validatePipelineRequest(body: Record<string, unknown>): string | null {
  const { batchSize, includeNetSuiteSync, dryRun } = body;
  ...
  if (
    ...
    return 'batchSize must be an integer between 1 and 100.';
  }
  if (includeNetSuiteSync !== undefined && typeof includeNetSuiteSync !== 'boolean') {
    return 'includeNetSuiteSync must be a boolean.';
  }
...
/**
 * Resolve the three fields against the shared defaults.
 *
 * Only called after `validatePipelineRequest` returned null, so each `typeof`
 * here is a TYPE-LEVEL narrowing rather than a second validation: every
 * wrong-typed value has already been rejected with a 400, which leaves the
 * else-branch reachable only for an OMITTED field. Written this way because
 * the repo compiles with `strictNullChecks: false`, where narrowing a
 * `Record<string, unknown>` field through the compound check above does not
 * survive into an assignment — and the alternative, asserting the types the
 * validator just proved, would be an unchecked cast.
 */
function resolvePipelineOptions(body: Record<string, unknown>): PipelineRequestOptions {
  const { batchSize, includeNetSuiteSync, dryRun } = body;
  return {
    batchSize: typeof batchSize === 'number' ? batchSize : FULL_PIPELINE_DEFAULTS.batchSize,
    includeNetSuiteSync:
      typeof includeNetSuiteSync === 'boolean'
        ? includeNetSuiteSync
        : FULL_PIPELINE_DEFAULTS.includeNetSuiteSync,
    dryRun: typeof dryRun === 'boolean' ? dryRun : FULL_PIPELINE_DEFAULTS.dryRun,
  };
}
...
export function createFullPipelineDemoRouter(): Router {
  ...
  router.post(
    ...
    asyncHandler(async (req, res): Promise<void> => {
      ...
      const rawBody: unknown = req.body === undefined ? {} : req.body;
      if (!isPlainObjectBody(rawBody)) {
        res.status(400).json({ error: 'invalid_request', message: 'body must be a JSON object.' });
        return;
      }
      const validationError = validatePipelineRequest(rawBody);
      if (validationError) {
        res.status(400).json({ error: 'invalid_request', message: validationError });
        return;
      }
      const { batchSize, includeNetSuiteSync, dryRun } = resolvePipelineOptions(rawBody);

      // `module` and `entityType` keep their existing untyped passthrough:
      // their validation is out of scope for this hardening (design section 4),
      // so narrowing them here would be a silent behavior change. Read through
      // the SAME `undefined`-normalized value as the shape check above — going
      // back to the raw `req.body` would re-introduce the TypeError that
      // normalization exists to prevent (Express leaves `req.body` undefined
      // when no body parser matched).
      const { module = 'SupplierCentral', entityType = 'vendors' } = req.body ?? {};
      ...
      try {
        ...
        // PR 13b Stage A3: resolve governance trio once + thread an identity/
        // correlation envelope into the per-step helpers so every direct
        // connector write routes through guardedWrite. OwnershipResolver +
        // ApprovalQueueService are async-bound via toDynamicValue; AuditService
        // is sync-bound.
        ...
        const governance: PipelineGovernanceContext = {
          tenantId: identity.tenantId,
          requesterUserId: identity.userId,
          correlationId:
            (req.headers['x-correlation-id'] as string | undefined) ?? pipelineId,
          deps: { ownershipResolver, auditService, approvalQueueService },
        };
  ...
  router.get(
    ...
    asyncHandler(async (req, res): Promise<void> => {
      const configurations = {
        ...
        defaultSettings: { ...FULL_PIPELINE_DEFAULTS },
        estimatedDuration: {
...
async function loadToSuiteCentral(
  ...
  dryRun: boolean,
  logger: Logger,
  governance: PipelineGovernanceContext,
): Promise<{
  status: 'completed' | 'failed';
  ...
  try {
    ...
    for (let i = 0; i < transformedData.length; i += batchSize) {
      ...
      try {
        ...
        for (const record of batch) {
          try {
            // PR 13b Stage A3: route through guardedWrite so the SuiteCentral
            // write surfaces ownership decisions. callerSystem is
            // 'operator_action' (this is an operator-triggered demo route);
            // targetSystem is the canonical SourceSystem 'squire' (the
            // manifest's name for SuiteCentral — see SOURCE_SYSTEM_TO_CONNECTOR_KEY).
            const syncedRecord = await guardedWrite(
              {
                context: {
                  ...
                  requesterUserId: governance.requesterUserId,
                  operation: 'create',
                },
                do: () => suiteCentralConnector.create(entityType, record),
              },
              governance.deps,
            );
            ...
            // PR 13b Stage A3: ownership-block + approval signals must escape
            // so the route's outer handleApprovalQueueError routes 409 / 202.
            // PendingApprovalError is the OutboundGovernanceService signal
            // emitted by the connector layer; the same rethrow keeps the
            // pre-PR-13b approval flow intact.
            if (
              error instanceof PendingApprovalError ||
              error instanceof WriteBlockedError ||
              error instanceof OwnershipPendingApprovalError
            ) {
              throw error;
            }
        ...
        // PR 13b Stage A3: rethrow governance signals at the batch level too
        // so the per-record rethrow above isn't reabsorbed here.
        if (
          error instanceof PendingApprovalError ||
          error instanceof WriteBlockedError ||
          error instanceof OwnershipPendingApprovalError
    ...
    // PR 13b Stage A3: governance signals escape loadToSuiteCentral entirely
    // so the route-level handleApprovalQueueError surfaces 409 / 202 instead
    // of the helper collapsing them into a 'SuiteCentral load failed' step.
    if (
      error instanceof PendingApprovalError ||
      error instanceof WriteBlockedError ||
      error instanceof OwnershipPendingApprovalError
...
async function syncToNetSuite(
  ...
  try {
    ...
    for (let i = 0; i < suiteCentralData.length; i += batchSize) {
      ...
      try {
        ...
        for (const record of netSuiteBatch) {
          try {
            // PR 13b Stage A3: route through guardedWrite. callerSystem is
            // 'operator_action' (operator-triggered demo route); targetSystem
            // is the canonical SourceSystem 'netsuite'. PendingApprovalError
            // is the OutboundGovernanceService signal raised by the connector;
            // it must still propagate to the route's top-level catch so the
            // 202+pendingApprovalId path fires. Ownership-block errors
            // (WriteBlockedError / OwnershipPendingApprovalError) thrown by
            // guardedWrite likewise propagate — the route's
            // handleApprovalQueueError will translate them at the outer catch.
            await guardedWrite(
              {
                context: {
                  tenantId: governance.tenantId,
                ...
                do: () => netSuiteConnector.create(entityType, record),
              },
              governance.deps,
            );
            syncedCount++;
            ...
            //
            // PR 13b Stage A3: same logic now applies to guardedWrite's
            // ownership signals — handleApprovalQueueError routes
            // WriteBlockedError → 409 and OwnershipPendingApprovalError →
            // 202, but only if the error escapes the per-record catch.
            if (
              error instanceof PendingApprovalError ||
              error instanceof WriteBlockedError ||
              error instanceof OwnershipPendingApprovalError
        ...
        //
        // PR 13b Stage A3: same rule applies to ownership-block signals so
        // the route's top-level handleApprovalQueueError surfaces 409 / 202
        // instead of the batch entry collapsing them into a 500.
        if (
          error instanceof PendingApprovalError ||
          error instanceof WriteBlockedError ||
          error instanceof OwnershipPendingApprovalError
    ...
    //
    // PR 13b Stage A3: ownership-block signals get the same treatment so
    // 409 (WriteBlockedError) / 202 (OwnershipPendingApprovalError) reach
    // the operator rather than collapsing to a generic step failure.
    if (
      error instanceof PendingApprovalError ||
      error instanceof WriteBlockedError ||
      error instanceof OwnershipPendingApprovalError

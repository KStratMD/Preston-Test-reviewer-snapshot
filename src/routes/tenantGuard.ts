 ...
 *     the verified actor id (`req.user.id`) is missing, empty, whitespace-only,
 *     or the case-insensitive literal `'unknown'` — non-negotiable #6:
 *     `'unknown'` is never an actor. The claim is trimmed before it is
 *     validated, and the trimmed value is what the returned context carries,
 *     matching the sibling `verifiedIdentity` contract so the same raw claim
 *     never resolves to two different actor strings across call sites.
...
export function requireConfigurationCommandContext(
  ...
  const rawActorUserId = req.user?.id;
  const actorUserId = typeof rawActorUserId === 'string' ? rawActorUserId.trim() : '';
  if (actorUserId.length === 0 || actorUserId.toLowerCase() === 'unknown') {

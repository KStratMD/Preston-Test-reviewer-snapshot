...
/**
 * Resolve the verified subject id for a personal settings mutation. These
 * routes sit behind `authMiddleware`, so a missing id is a wiring defect
 * rather than an anonymous caller — fail loudly instead of writing under a
 * synthetic identity.
 */
function requireVerifiedUserId(req: AuthedRequest): string {
  const id = req.user?.id;
  if (typeof id !== 'string' || id.length === 0) {
    throw new Error('Authenticated settings mutation reached handler without req.user.id');
  }
  return id;
}
...
export async function createSettingsRouter(): Promise<Router> {
  ...
  // NOTE: demo-mode is a process-global runtime setting. Its mutation moved to
  // the audited platform-admin endpoint POST /api/admin/settings/demo-mode
  // (see src/routes/adminSettings.ts). The public router keeps only the GET
  ...
  router.post('/ai/dataset', authMiddleware, async (req: AuthedRequest, res, next) => {
    /**
     * @openapi
     * /api/settings/ai/dataset:
     *   post:
  ...
  router.post('/mcp', authMiddleware, async (req: AuthedRequest, res, next) => {
    try {
      const userId = requireVerifiedUserId(req);
  ...
  router.post('/mcp/reset', authMiddleware, async (req: AuthedRequest, res, next) => {
    try {
      const userId = requireVerifiedUserId(req);

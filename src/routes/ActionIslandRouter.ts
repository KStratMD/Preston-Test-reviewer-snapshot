...
function extractContext(req: Request): ActionContext {
    ...
    // req.tenantContext). Body/header `userId` is no longer trusted for
    // cross-system action attribution (DocuSign, Jira, etc.) — an
    // anonymous caller could otherwise forge an arbitrary requester
    // identity into external systems. Anonymous callers map to undefined
    // (preserving the previous "no userId" semantics for that case).
    const { userId: resolvedUserId } = extractIdentityContext(req);
    return {
        ...
        userId: resolvedUserId === SYSTEM_IDENTITY.userId ? undefined : resolvedUserId
    };
}

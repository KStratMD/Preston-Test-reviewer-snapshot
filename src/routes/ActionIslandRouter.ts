...
function extractContext(req: Request): ActionContext {
    // Only authMiddleware-verified req.user identity may attribute an action.
    // Anonymous hosted-demo actions intentionally carry no userId.
    // (preserving the previous "no userId" semantics for that case).
    // Whitespace-only ids fail closed to undefined rather than attributing
    // the action to a blank operator. id may be a number at runtime (the
    // global Express.User type + some auth fixtures), so stringify before
    // trimming instead of assuming string.
    const rawId = req.user?.id;
    const userId = rawId == null ? undefined : String(rawId).trim() || undefined;
    return {

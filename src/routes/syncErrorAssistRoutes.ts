...
export {
  ...
  // A5: TENANT_ID_REGEX is gone — the tenant-id rule now lives only in
  // normalizeTenantIdClaim (src/utils/tenantId.ts). Nothing consumed this
  // re-export.
...
import { PlaintextSecretError } from '../database/repositories/TenantConfigurationRepository';
...
syncErrorAssistRoutes.post('/api/sync-error-assist/ingest', async (req: Request, res: Response) => {
  ...
  // C2 — Read the HMAC secret via the encrypted-only path. The full
  // error-vs-null landscape of `getSecretString` (R5 (Copilot) — the
  // earlier comment overclaimed 500 coverage on paths that actually
  // return null):
  //
  //   - plaintext row                    → THROWS PlaintextSecretError
  //   - missing row                      → returns null
  //   - encrypted row + name mismatch    → returns null + error log
  //                                          (anti-exfiltration guard
  //                                          in resolveStringForRow)
  //   - encrypted row + SecretManager
  //     throws                           → returns null + error log
  //   - DB connection error / unrelated  → THROWS (propagates raw)
  //
  // The try/catch discriminates on the typed `PlaintextSecretError`
  // (NOT a regex/message check, per [[feedback-copilot-typed-error-pressure]])
  // and collapses ONLY that class to `secret = null`. Any other thrown
  // error (genuine infra failure like a DB connection drop) propagates
  // to the outer handler-scope try/catch and returns 500. The
  // name-mismatch + SecretManager-failure paths intentionally return
  // null inside the repo (preserving the existing `getString` contract)
  // and end up surfaced as 401 unknown_tenant via the existing
  // `secret === null` branch — operationally the same as a missing row.
  // Plaintext-misconfig is the only path that emits the dedicated log
  // line cuing the operator to re-set with `upsert(..., { isEncrypted: true })`.
  let secret: string | null = null;
  try {
    secret = await tenantConfig.getSecretString(keyIdHeader, 'sync_error_assist.webhook_hmac_secret');
  } catch (err) {
    if (err instanceof PlaintextSecretError) {
      // R3 (Copilot) — log under `tenantId` for consistency with the rest of
      // the route. The PlaintextSecretError carries the original tenantId
      // (the value passed into `getSecretString(keyIdHeader, key)`), so
      // `err.tenantId === keyIdHeader`; using the typed field couples the
      // log shape to the error class for free.
      log.error(
        'webhook HMAC secret stored plaintext; refusing to use. Re-set with isEncrypted: true to restore ingest for this tenant.',
        err,
        { tenantId: err.tenantId, settingKey: err.settingKey },
      );
      // secret stays null — falls through to existing unknown_tenant branch
    } else {
      throw err;
    }
  }

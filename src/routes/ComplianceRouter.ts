...
router.get('/summary', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Compliance summary failed', { error: String(error) });
...
router.post('/export', async (req: Request, res: Response) => {
  try {
    ...
    const { logger } = await getServices();
    logger.error('Compliance export failed', { error: String(error) });
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});
...
router.get('/soc2-mapping', async (req: Request, res: Response) => {
  try {
    ...
    const { logger } = await getServices();
    logger.error('SOC2 mapping failed', { error: String(error) });
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});
...
router.get('/dlp-patterns', async (req: Request, res: Response) => {
  try {
    ...
    const { logger } = await getServices();
    logger.error('DLP patterns query failed', { error: String(error) });
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});

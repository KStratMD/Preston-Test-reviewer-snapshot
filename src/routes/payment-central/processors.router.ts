...
// operational data. The old handler aggregated processor status and 24h
// transaction counts/success rate and served them to anonymous callers;
// those metrics remain available on the authenticated /dashboard and
// /analytics surfaces (Codex review, PR #1081 R1).
router.get('/health', (_req, res) => {
  res.json({ status: 'healthy', service: 'payment-central', timestamp: Date.now() });
});

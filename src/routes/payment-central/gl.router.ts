...
router.post('/journal-entries/:entryId/approve', asyncHandler(async (req, res, next) => {
  const { entryId } = req.params;
  const body = req.body ?? {};
  const approvedBy = resolveActor(req, body.approvedBy) ?? 'system';
...
router.post('/journal-entries/:entryId/post', asyncHandler(async (req, res, next) => {
  const { entryId } = req.params;
  const body = req.body ?? {};
  const postedBy = resolveActor(req, body.postedBy) ?? 'system';
...
router.post('/batches', asyncHandler(async (req, res, next) => {
  ...
  const createdBy = resolveActor(req, body.createdBy) ?? 'system';
...
router.post('/batches/:batchId/process', asyncHandler(async (req, res, next) => {
  const { batchId } = req.params;
  const body = req.body ?? {};
  const processedBy = resolveActor(req, body.processedBy) ?? 'system';

...
router.post('/journal-entries/:entryId/approve', asyncHandler(async (req, res, next) => {
  ...
  try {
    const entry = await getPaymentService().approveJournalEntry(entryId, actorId);
    if (!entry) {
      res.status(404).json({ error: 'Journal entry not found' });
      return;
    }
...
router.post('/journal-entries/:entryId/post', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().postJournalEntry(entryId, actorId);
  if (!result.success) {
    res.status(400).json(result);
    return;
  }
...
router.post('/batches', asyncHandler(async (req, res, next) => {
  ...
  try {
    const batch = await getPaymentService().createPostingBatch(name, entryIds, actorId);
...
router.post('/batches/:batchId/process', asyncHandler(async (req, res, next) => {
  ...
  try {
    const batch = await getPaymentService().processPostingBatch(batchId, actorId);
    res.json(batch);
  } catch (error) {

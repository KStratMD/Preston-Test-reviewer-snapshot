...
router.get('/invoices', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().getInvoices(filters);
  res.json(result);
}));

router.post('/invoices', asyncHandler(async (req, res, next) => {
  const { vendorId, invoiceNumber, invoiceDate, dueDate, amount, taxAmount, currency, lineItems, notes, source } = req.body;
  ...
  const invoice = await getPaymentService().createInvoice(vendorId, {
    invoiceNumber,
...
router.get('/invoices/statistics', asyncHandler(async (req, res, next) => {
  const statistics = await getPaymentService().getInvoiceStatistics();
  res.json(statistics);
}));
...
router.get('/invoices/:invoiceId', asyncHandler(async (req, res, next) => {
  ...
  const invoice = await getPaymentService().getInvoice(invoiceId);
  if (!invoice) {
...
router.post('/invoices/:invoiceId/match', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().matchInvoiceToPO(invoiceId, poId, matchedBy || 'api');
  if (!result.success) {
...
router.post('/invoices/:invoiceId/auto-match', asyncHandler(async (req, res, next) => {
  const { invoiceId } = req.params;
  const result = await getPaymentService().autoMatchInvoice(invoiceId);
  res.json(result);
}));
...
router.post('/invoices/:invoiceId/approve', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().approveInvoice(invoiceId, approvedBy);
  if (!result.success) {
    res.status(400).json(result);
    return;
  }
...
router.post('/invoices/:invoiceId/dispute', asyncHandler(async (req, res, next) => {
  const { invoiceId } = req.params;
  const { reason, description, createdBy } = req.body;
  ...
  const result = await getPaymentService().createInvoiceDispute(invoiceId, reason, description, createdBy);
  if (!result.success) {
    res.status(400).json(result);
    return;
  }
...
router.get('/disputes', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().getDisputes({
    status: statusResult,
    vendorId: vendorId as string | undefined,
    limit: limitResult,
    offset: offsetResult,
...
router.post('/disputes/:disputeId/resolve', asyncHandler(async (req, res, next) => {
  ...
  const { type, description, adjustedAmount, resolvedBy } = req.body;
  ...
  const result = await getPaymentService().resolveDispute(disputeId, { type, description, adjustedAmount }, resolvedBy);
  if (!result.success) {
    res.status(400).json(result);
    return;
  }
...
router.get('/credit-memos', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().getCreditMemos({
    vendorId: vendorId as string | undefined,
    status: statusResult,
    limit: limitResult,
    offset: offsetResult,
...
router.post('/credit-memos', asyncHandler(async (req, res, next) => {
  const { invoiceId, amount, reason, createdBy } = req.body;
  ...
  const creditMemo = await getPaymentService().createCreditMemo(invoiceId, amount, reason, createdBy);

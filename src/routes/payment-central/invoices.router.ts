...
router.post('/invoices/:invoiceId/match', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().matchInvoiceToPO(tenantId, invoiceId, poId, actorId);
  if (!result.success) {
...
router.post('/invoices/:invoiceId/approve', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().approveInvoice(tenantId, invoiceId, actorId);
  if (!result.success) {
    res.status(400).json(result);
    return;
  }
...
router.post('/invoices/:invoiceId/dispute', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().createInvoiceDispute(tenantId, invoiceId, reason, description, actorId);
  if (!result.success) {
    res.status(400).json(result);
    return;
  }
...
router.post('/disputes/:disputeId/resolve', asyncHandler(async (req, res, next) => {
  ...
  const result = await getPaymentService().resolveDispute(tenantId, disputeId, { type, description, adjustedAmount }, actorId);
  if (!result.success) {
    res.status(400).json(result);
    return;
  }
...
router.post('/credit-memos', asyncHandler(async (req, res, next) => {
  ...
  if (!invoiceId || !amount || !reason) {
    res.status(400).json({ error: 'invoiceId, amount, and reason are required' });
    return;
  }

  const creditMemo = await getPaymentService().createCreditMemo(tenantId, invoiceId, amount, reason, actorId);

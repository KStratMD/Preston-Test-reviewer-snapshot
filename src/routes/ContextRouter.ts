...
contextRouter.get('/:system/:recordType/:recordId', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (isAnonymousRequest(req)) {
            const fallback = buildFallbackContext(normalizedRecordType, recordId, modules);
            return res.json({
                success: true,
        ...
        const contextData = await buildContextData(normalizedSystem, normalizedRecordType, recordId, modules, tenantId);
...
async function buildContextData(
    ...
    const liveData = await buildLiveContextData(recordType, recordId, services, modules, tenantId);
    if (liveData) {
        return {
            ...liveData,
            dataSource: 'service',
...
async function buildLiveContextData(
    ...
    try {
        switch (recordType) {
            case 'vendor':
                return buildVendorContextFromServices(recordId, services, modules, tenantId);
            case 'customer':
                // F5 deferred: unscoped process-global service; re-enable when tenant-scoped.
                return null;
            case 'invoice':
                return buildInvoiceContextFromServices(recordId, services, modules, tenantId);
            case 'purchaseorder':
                return buildPurchaseOrderContextFromServices(recordId, services, modules, tenantId);
            default:
                return null;
        }
    } catch (error) {
...
async function buildVendorContextFromServices(
    ...
    const vendor = await supplier.getVendorProfile(tenantId, vendorId);
    if (!vendor) {
        return null;
    }

    const poResult = await supplier.getPurchaseOrdersForVendor(tenantId, vendor.id, { limit: 100, offset: 0 });
...
async function buildInvoiceContextFromServices(
    ...
    const invoice = await payments.getInvoice(tenantId, invoiceId);
    if (!invoice) {
        return null;
    }
...
async function buildPurchaseOrderContextFromServices(
    ...
    const po = await supplier.getPurchaseOrder(tenantId, poId);
    if (!po) {
        return null;
    }

    const vendor = await supplier.getVendorProfile(tenantId, po.vendorId);
    if (!vendor) {

...
/**
 * Thrown by resolveTenantId when no verified source carries a tenant.
 * Typed (not string-matched) so an unrelated downstream error can never be
 * misclassified as the strict-tenant 403 (Copilot R1 on PR #1038).
 */
export class TenantRequiredError extends Error {
  constructor() {
    super('The authenticated identity carries no tenant; a tenant-scoped token is required.');
...
/**
 * F1 (design D4/D5-F1): canonical whole-source order matching
 * extractIdentityContext — req.auth → req.user.tenantId → req.tenantContext.
 * The old `req.user.id → tenantId` fallback is DELETED (a user id is never a
 * tenant id), and the `'default'` terminal bucket is gone: `/api/mcp` mounts
 * authMiddleware and is strict — an authenticated caller whose token carries
 * no tenant gets 403 `tenant_required`, not a shared pseudo-tenant.
 *
 * The former admin-gated x-tenant-id/x-organization-id header fallback is
 * DELETED too (design D4's "stays only if re-justified" clause): with the
 * kill-switch gate on the mount, an authenticated tenant-less token is
 * rejected fail-closed BEFORE the router, so the header path was
 * production-unreachable. Platform admins target another tenant via the
 * explicit, admin-gated `?tenantId=` / body override in
 * resolveRequestedTenantId below.
 */

...
function resolveAdminHeaderTenant(req: Request): string | undefined {
  ...
  // allow-identity-header-read: admin-role-gated fallback inside resolveAdminHeaderTenant, only reached after isAdminRequest(req) returns true
  const headerTenant = req.headers['x-tenant-id'];
  if (typeof headerTenant === 'string' && headerTenant.trim().length > 0) {
    return headerTenant.trim();
  }

  // allow-identity-header-read: parallel admin-gated fallback when x-tenant-id is absent

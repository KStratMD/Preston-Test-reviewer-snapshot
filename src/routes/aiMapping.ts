...
import type {
  ...
  CardinalityAdvisoryContext,
  ExecutionProfileHint,
  ExecutionProfileRecommendation,
} from '../services/ai/orchestrator/interfaces';
import type { CardinalityAdvisoryEvidence } from '../types/cardinality';
import { CARDINALITY_ANALYZER_VERSION } from '../services/cardinality/CardinalityPreflightService';
...
} from './ai-proxy/utils/executionProfileRecommendationBoundary';
...
/**
 * Resolves advisory (never authoritative) cardinality evidence for one
 * suggestion request via the shared coordinator — see
 * docs/superpowers/specs/2026-07-26-cardinality-preflight-design.md
 * "Suggestion-time integration". Returns undefined on ANY resolution problem
 * (missing binding, discovery failure) so a suggestion request degrades to
 * "no advisory context" rather than ever failing. `getAdvisoryEvidence`
 * itself never throws, but the DI resolution is defensively wrapped too.
 */
async function getCardinalityAdvisoryContext(
  sourceSystem: string,
  targetSystem: string,
  sourceEntity: string,
  targetEntity: string,
): Promise<CardinalityAdvisoryContext | undefined> {
  try {
    const coordinator = container.get<CardinalityAdvisoryEvidence>(TYPES.CardinalityPreflightService);
    const [sourceEvidence, targetEvidence] = await Promise.all([
      coordinator.getAdvisoryEvidence(sourceSystem, sourceEntity),
      coordinator.getAdvisoryEvidence(targetSystem, targetEntity),
    ]);
    return {
    ...
    logger.warn('Advisory cardinality context unavailable for suggestion request', {
...
router.post('/suggestions', asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (isAnonymousRequest(req)) {
      // F2 zero-spend + fixture-isolation rulings: anonymous demo sessions
      // get the PURE demo mapper — no orchestrator, provider, learned mapping
      // service, telemetry service, repository, or shared learning state.
      const suggestions = buildRuleBasedDemoMappings(
        normalizedSourceSchema.fields,
        normalizedTargetSchema.fields,
      );
      ...
      res.json({
        ...
        suggestions: suggestions.map((suggestion) => ({
          ...
          businessRulesSuggested: [] as AIFieldMappingSuggestion['businessRulesSuggested'],
          reason: suggestion.reasoning,
        })),
        metadata: {
          totalSuggestions: suggestions.length,
          highConfidenceSuggestions: suggestions.filter((suggestion) => suggestion.confidence > 0.8).length,
          mediumConfidenceSuggestions: suggestions.filter(
            (suggestion) => suggestion.confidence > 0.5 && suggestion.confidence <= 0.8,
          ).length,
          lowConfidenceSuggestions: suggestions.filter((suggestion) => suggestion.confidence <= 0.5).length,
          averageConfidence,
    ...
    let executionProfileRecommendation: ExecutionProfileRecommendation | undefined;
    ...
    try {
      ...
      // Advisory only (design doc, "Suggestion-time integration"): trusted
      // evidence flows through the SAME coordinator/normalization the
      // activation gate uses, so the agent never trusts a browser-supplied
      // relationship graph. A discovery failure never fails the suggestion
      // request — getCardinalityAdvisoryContext degrades to undefined.
      const cardinalityAdvisory =
        normalizedSourceSchema.systemName !== undefined &&
        normalizedTargetSchema.systemName !== undefined
          ? await getCardinalityAdvisoryContext(
              normalizedSourceSchema.systemName,
              normalizedTargetSchema.systemName,
              normalizedSourceSchema.recordType,
              normalizedTargetSchema.recordType,
            )
          : undefined;

      // Task 10 (decision 10): a deterministic, model-independent hint for
      // the netsuite_serialized_asset ADVISORY recommendation, built from the
      // SAME normalized request schema cardinalityAdvisory uses above —
      // never from AI/model output. Shared with MappingRouter.ts so the two
      // production entry points for this route family can never drift.
      const executionProfileHint: ExecutionProfileHint | undefined = buildExecutionProfileHint(
        normalizedSourceSchema.systemName,
        normalizedTargetSchema.systemName,
        normalizedSourceSchema.recordType,
        normalizedTargetSchema.recordType,
      ...
      const agentInput: unknown = {
        ...
        sampleData,
        ...(cardinalityAdvisory ? { cardinalityAdvisory } : {}),
      ...
      // Independently re-checks executionProfileHint against the exact-pair
      // rule — never trusts agentResult.data's shape validity alone.
      executionProfileRecommendation = sanitizeExecutionProfileRecommendation(
        (agentResult.data as { executionProfileRecommendation?: unknown } | undefined)?.executionProfileRecommendation,
        executionProfileHint,
      );
      ...
      // The advisory recommendation is orchestrator-derived; a fallback to
      // the rule-based service never recomputes it, so any partial result
      // from the failed orchestrator attempt must not survive.
...
router.post('/feedback', asyncHandler(async (req: Request, res: Response) => {
  // F2 fixture-isolation ruling: anonymous demo feedback must not reach the
  // shared training dataset or persisted telemetry — public traffic could
  // poison the learning signal. Acknowledge without recording.
  if (isAnonymousRequest(req)) {
    res.json({ success: true, demoFixture: true, recorded: false });
    return;
  }

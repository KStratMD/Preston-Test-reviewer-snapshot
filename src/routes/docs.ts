...
export interface DocsRouterOptions {
  /** Directory served under /docs (default: <repo>/docs). */
  docsRoot?: string;
  /** Directory whose top-level *.md files are the fallback (default: the repo root). */
  repoRoot?: string;
  /** Directory holding *.postman_collection.json files (default: first existing of src/postman, postman). */
  postmanRoot?: string;
}
 ...
 */
export function createDocsRouter(opts: DocsRouterOptions = {}): express.Router {
  const router = express.Router();
  const docsRoot = opts.docsRoot ?? path.join(__dirname, '../../docs');
  ...
  const postmanRoot = opts.postmanRoot
    ?? (fs.existsSync(postmanFirst) ? postmanFirst : (fs.existsSync(postmanSecond) ? postmanSecond : postmanFirst));
  ...
  // Single entry point for Markdown -> HTML: whichever renderer loaded, its output is sanitised
  // (marked never sanitises; docs content is repo-controlled but the route is unauthenticated).
  async function renderMarkdown(src: string): Promise<string> {
    const renderer = await loadMarked();
    return sanitizeDocsHtml(await renderer.parse(src));
  }
  ...
  function walk(dir: string, baseDir: string = docsRoot, visited = new Set<string>()) {
    // A directory link back to an ancestor (a subfolder link pointing at the docs root) would otherwise recurse until
    // ELOOP/ENAMETOOLONG and fail router startup: visit each real directory once.
    let realDir: string;
    ...
    for (const entry of fs.readdirSync(dir)) {
      ...
      // Never follow a link out of the root it was found under (search snippets would disclose it).
      if (!isRealPathInsideRoot(baseDir, full)) continue;
      const rel = path.relative(baseDir, full).replace(/\\/g, '/');
      let stat: fs.Stats;
      try { stat = fs.statSync(full); } catch { continue; } // one bad entry must not fail router startup
      if (stat.isDirectory()) {
        walk(full, baseDir, visited);
      ...
      for (const entry of fs.readdirSync(rootDir)) {
        if (entry.endsWith('.md')) {
          try {
            ...
            if (!isRealPathInsideRoot(rootDir, full)) continue;
    ...
    try {
      if (fs.existsSync(postmanRoot)) {
        ...
        for (const f of pFiles) {
          ...
          if (!isRealPathInsideRoot(postmanRoot, abs)) continue;
            try {
  ...
  if (process.env.NODE_ENV !== 'production' && !DOCS_WATCH_DISABLED) {
    try {
      // (uses the router-level docsRoot)
      if (fs.existsSync(docsRoot)) {
  ...
  // `title` is plain text: escaped here for each context it is placed in.
  function renderPage(title: string, bodyHtml: string, tocHtml = '', filePath = ''): string {
    const titleHtml = escapeHtml(title);
    const head = `<!DOCTYPE html><html lang=\"en\" class=\"h-full\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>${titleHtml} - Integration Hub Docs</title><link href=\"https://cdnjs.cloudflare.com/ajax/libs/tailwindcss/2.2.19/tailwind.min.css\" rel=\"stylesheet\"><link rel=\"stylesheet\" href=\"/vendor/fontawesome-6.0.0.min.css\" onerror=\"this.remove();(function(){var l=document.createElement('link');l.rel='stylesheet';l.href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css';l.crossOrigin='anonymous';l.integrity='';document.head.appendChild(l);})();\">`;
    const styles = `<style>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex justify-between items-center py-3 gap-4 flex-wrap">
            ...
            <div class="flex items-center gap-2">
              ...
              <div class="text-xs text-gray-500 dark:text-gray-400">${titleHtml}</div>
            </div>
          </div>
        </div>
      </div>`;
    ...
    const scripts = `
      <script>
        ...
        document.getElementById('pdfBtn')?.addEventListener('click', () => {
          ...
          document.title = ${jsStringLiteral(title)}.replace(/[^a-zA-Z0-9\\s-_]/g, '');
      ...
      <!-- Back button and ESC handling for docs pages (the only navigation layer here) -->
      <script>
        // Note: Client-side console.log intentionally kept for browser debugging
  ...
  router.get('/', (_req, res) => {
    try {
      ...
      // (uses the router-level docsRoot)
      // (entries that link outside their root are not listed; they would not be served either)
      const docsFiles = fs.existsSync(docsRoot)
        ? fs.readdirSync(docsRoot).filter(f => f.endsWith('.md') && isRealPathInsideRoot(docsRoot, path.join(docsRoot, f)))
        : [];

      // Get markdown files from root directory
      const rootDir = repoRoot;
      const rootFiles = fs.existsSync(rootDir)
        ? fs.readdirSync(rootDir).filter(f => f.endsWith('.md') && isRealPathInsideRoot(rootDir, path.join(rootDir, f)))
        : [];
      
      const important = ['ARCHITECTURE.md', 'AI_IMPLEMENTATION_GUIDE.md', 'INDEX.md', 'README.md', 'GETTING-STARTED.md', 'API-REFERENCE.md'];
      
      const toLink = (f: string) => ({ path: `/docs/${f}`, label: `${important.includes(f) ? '⭐ ' : ''}${f}` });
      const docsList = renderDocLinkList(docsFiles.sort().map(toLink));
      const rootList = renderDocLinkList(rootFiles.sort().map(toLink));
      ...
      try {
        if (fs.existsSync(postmanRoot)) {
          const pFiles = fs.readdirSync(postmanRoot).filter(f =>
            f.toLowerCase().endsWith('.postman_collection.json') && isRealPathInsideRoot(postmanRoot, path.join(postmanRoot, f)));
          if (pFiles.length) {
            const pList = renderDocLinkList(pFiles.map(f => ({ path: `/docs/postman/${f}`, label: f })));
  ...
  async function sendMarkdown(res: express.Response, safePath: string, req: express.Request) {
    ...
    const now = Date.now();
    // No hand-built ETag or Last-Modified: res.send() derives the ETag from the
    // rendered bytes and answers 304 via req.fresh, so a change to the page
    // template invalidates cached copies even when the markdown file did not change.
  ...
  function buildNotFound(rel: string): string {
    ...
    return [
      '<h1>Not Found</h1>',
      `<p>The documentation path <code>${escapeHtml(rel)}</code> was not found.</p>`,
      qBox,
  ...
  async function renderDirectory(rel: string, absPath: string): Promise<string> {
    // Drop links that leave the root or cannot be resolved (loops, dangling) before any stat.
    const entriesAll = fs.readdirSync(absPath)
      .filter(e => !e.startsWith('.') && isRealPathInsideRoot(docsRoot, path.join(absPath, e)));
    ...
    // Only add slash between path components if normalizedRel doesn't already end with one
    const separator = normalizedRel && !normalizedRel.endsWith('/') ? '/' : '';
    const links = entries.flatMap(e => {
      if (!isRealPathInsideRoot(docsRoot, path.join(absPath, e))) return [];
      const isDir = fs.statSync(path.join(absPath, e)).isDirectory();
      if (!isDir && !e.endsWith('.md')) return [];
      return [{ path: `/docs/${normalizedRel}${separator}${e}`, label: isDir ? `${e}/` : e }];
    });
    const items = renderDocLinkList(links, '');
    const upHref = escapeHtml(encodePathSegments(`/docs/${rel.split('/').slice(0, -1).join('/')}`));
    const upLink = rel ? `<a class="text-indigo-600 hover:underline text-sm" href="${upHref}">⬅ Up</a>` : '';
    let preface = '';
    if (readmeName && isRealPathInsideRoot(docsRoot, path.join(absPath, readmeName))) {
      try {
        const raw = fs.readFileSync(path.join(absPath, readmeName), 'utf8');
        const rendered = addHeadingIds(await renderMarkdown(raw));
    ...
    return `<h1>Directory: /${escapeHtml(rel || '')}</h1>${upLink}${preface}<h2 class="mt-6">Contents</h2><ul class="mt-4 space-y-1">${items || '<li class="text-gray-500">(empty)</li>'}</ul>`;
  }
  ...
  router.get(/^(.*)$/, async (req, res, next) => {
    ...
    if (req.path.startsWith('/postman/')) {
      ...
      if (!isRealPathInsideRoot(postmanRoot, abs)) return sendError(res, 400, { code: 'INVALID_PATH', message: 'Invalid path' }, req);
  ...
  if (!file.toLowerCase().endsWith('.postman_collection.json')) {
      ...
      try {
        ...
        const now = Date.now();
        // ETag is derived from the rendered bytes by res.send(); see sendMarkdown().
        const cached = renderCache.get(cacheKey);
        if (cached && (now - cached.mtimeMs) < CACHE_TTL_MS && cached.mtimeMs === stat.mtimeMs) {
          res.setHeader('Cache-Control', 'public, max-age=60');
        ...
        if (Array.isArray(data.item)) walk(data.item);
        const rawName = data.info?.name || file;
        const body = [
          `<h1>${escapeHtml(rawName)}</h1>`,
        ...
        // renderPage escapes the title per context; pass it raw.
        const full = renderPage(`Postman ${rawName}`, body, '');
        renderCache.set(cacheKey, { html: full, mtimeMs: stat.mtimeMs });
        res.setHeader('Cache-Control', 'public, max-age=60');
        return res.type('html').send(full);
      } catch (e) {
        return res.status(500).type('html').send(renderPage('Error', `<h1>Failed to render collection</h1><pre>${escapeHtml((e as Error).message)}</pre>`));
      }
    }
    ...
    const docsRootLocal = docsRoot;
    const safePath = path.normalize(path.join(docsRootLocal, rel));
    if (!isInsideRoot(docsRootLocal, safePath)) return res.status(400).json({ error: 'Invalid path' });
    ...
    try {
      ...
      if (!fs.existsSync(safePath)) {
        ...
        if (isInsideRoot(repoRoot, rootPath) && fs.existsSync(rootPath) && rootPath.endsWith('.md')) {
          actualPath = rootPath;
        } else {
      ...
      // Real-path containment: a link inside the root that points outside it is not served.
      if (!isRealPathInsideRoot(actualPath === safePath ? docsRootLocal : repoRoot, actualPath)) {
        return res.status(404).type('html').send(renderPage('Not Found', buildNotFound(rel)));
      }

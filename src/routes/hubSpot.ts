...
/**
 * F4: verified JWT identity only — the mount (mountHubSpotRoutes) runs
 * authMiddleware + requirePlatformAdmin, so a missing user here is not
 * purely a type-level defense: `isPlatformAdminActor` only checks the
 * trimmed actor id, never the `__system__` sentinel (that check lives in
 * `readTenantId`, which authorization never calls), so a validly signed
 * JWT claiming the sentinel in either claim passes both middlewares and
 * still reaches the handler in production — this is a real, reachable
 * path to the 401, not just a handler-only-test scenario. Unlike the F3
 * read-surface shape, BOTH claims are required (userId included): every
 * route here is a guardedWrite whose ownership/audit context demands a
 * concrete `requesterUserId` — an unattributable write is refused, not
 * recorded as the retired `__system__` sentinel.
 */
    ...
    const contact = await guardedWrite(
      {
        context: {
          ...
          // Derived, not defaulted (Task B4b): verifiedIdentity() proved a JWT subject.

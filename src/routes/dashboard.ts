...
// merged
  ...
  private async getCapabilities(_r: Request, res: Response) { const isMock = (o: unknown) => (o as any)?.constructor?.name?.startsWith('Mock'); res.json({ queues: isMock(this.queueProvider) ? 'mock' : 'real', traces: isMock(this.traceProvider) ? 'mock' : 'real', credentials: isMock(this.credentialProvider) ? 'mock' : 'real', aiMetrics: 'mock', export: 'mock', metrics: 'real', streaming: 'sse', mode: env.DISABLE_REDIS ? 'lightweight' : 'standard' }); }

 ...
 * This local interface captures the union of fields this router actually
 * reads, replacing the previous `(mapping as any).field` access pattern.
 */
  ...
  router.post('/mapping/suggestions', asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      try {
        ...
        const suggestions = mappings.map((mapping, index) => ({

...
} from './utils/executionProfileRecommendationBoundary';
 ...
 *
 * `sourceEntity`/`targetEntity` are Task 10 additions (2026-07-27 NetSuite
 * serialized-asset sync plan, decision 10): the record-type identifiers a
 * caller may optionally declare alongside `sourceSystem`/`targetSystem` so
 * the netsuite_serialized_asset ADVISORY recommendation can be detected.
 * Both are OPTIONAL — an existing caller that never sends them keeps
 * working exactly as before (`buildExecutionProfileHint` simply returns
 * undefined and no recommendation is attempted).
 */
const SuggestionsBodySchema = z.object({
  ...
  // Bounded to match the canonical schema's entity-name cap
  // (`configurationSchemas.ts`: min 1 / max 100). Left unbounded when Task 10
  // introduced them, which only became operator-reachable once Task 11 added
  // the editor's Record Type inputs and the URL deep-link — an unbounded string
  // from the query string would otherwise flow into the mapping context and the
  // agent prompt. Deferred item from the Task 11 review, now closed.
  sourceEntity: z.string().min(1).max(100).optional(),
  targetEntity: z.string().min(1).max(100).optional(),
});
...
export async function createMappingRouter(deps: MappingRouterDependencies): Promise<Router> {
   ...
   */
  router.post('/mapping/suggestions', asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!parsed.success) {
      // F2: anonymous validation failures must not persist telemetry.
    ...
    // F2 zero-spend ruling: anonymous demo requests are served by the
    // rule-based aiMapping router mounted after this one in aiProxy.ts —
    // the orchestrator (and any provider) is never invoked without identity.
    if (isAnonymousRequest(req)) {
      next();
      return;
    }

    // F6: verified req.user only — no system-sentinel fallback.
    // Anonymous callers never reach this line (delegated above).
    ...
    // Task 10 (decision 10): deterministic, model-independent hint for the
    // netsuite_serialized_asset ADVISORY recommendation — built ONLY from
    // caller-declared request fields, never from AI/model output. Undefined
    // for any caller that doesn't send sourceEntity/targetEntity.
    const executionProfileHint = buildExecutionProfileHint(sourceSystem, targetSystem, sourceEntity, targetEntity);
    ...
    try {
      ...
      try {
        ...
        // (plus, Task 10, the deterministic executionProfileHint).
        const agentInput = {
          sourceFields,
          targetFields,
          sampleData,
        ...
        // Task 10 (decision 10): whitelist reconstruction, never a
        // pass-through — `agentResult.data` is loosely typed, so this never
        // relays whatever shape happens to be sitting in
        // executionProfileRecommendation. Independently re-checks
        // executionProfileHint against the exact-pair rule (never trusts
        // shape validity alone) before returning anything.
        const executionProfileRecommendation = sanitizeExecutionProfileRecommendation(
   ...
   */
  router.post('/mapping/transformation/suggest', asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const startTime = Date.now();
    ...
    // F2 zero-spend ruling: anonymous demo requests delegate to aiMapping's
    // verified-pure suggestTransformations() path. The route schema leaves
    // field `type` loose for authenticated compatibility, so the demo path
    // performs the extra type check the pure helper needs.
    if (isAnonymousRequest(req)) {
      const sourceType = parsed.data.sourceField.type;
      const targetType = parsed.data.targetField.type;
      if (
        typeof sourceType !== 'string' || sourceType.trim().length === 0 ||
        typeof targetType !== 'string' || targetType.trim().length === 0
      ) {
        res.status(400).json({
          success: false,
          error: 'Anonymous transformation suggestions require sourceField.type and targetField.type',
        });
        return;
      }
      next();
  ...
  router.post('/suggestions/:suggestionId/accept', asyncHandler(async (req: Request, res: Response) => {
    ...
    // F2 fixture-isolation ruling: anonymous acceptance is acknowledged but
    // never persisted (telemetry.recordAISuggestionAccepted is a durable

...
export async function createAgentRouter(deps: AgentRouterDependencies): Promise<Router> {
  ...
  router.post('/orchestrate', asyncHandler(async (req: Request, res: Response) => {
    ...
    // F6: identity comes from the VERIFIED req.user only — never an
    // attacker-supplied `x-user-id` header, and no longer a system-sentinel
    // fallback for anonymous callers. The F2 policy gate guarantees a verified
    // user here, so the refusal below is unreachable in production and exists
    // to make a mount regression loud.
  ...
  router.post('/agents/:agentType', asyncHandler(async (req: Request, res: Response) => {
    ...
    // F6: verified req.user only; see the /orchestrate handler comment.
    const userId = verifiedUserId(req);
    if (userId === null) {
      res.status(401).json({ error: 'identity_required' });
      return;

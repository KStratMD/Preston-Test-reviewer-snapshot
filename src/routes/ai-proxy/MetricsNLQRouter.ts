...
export function createMetricsNLQRouter(): Router {
    ...
    router.post('/nlq', async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const { query, userId, sessionId, context } = req.body as Omit<NLQueryRequest, 'tenantId'>;
            // F4: the orchestrator tenant comes from the VERIFIED user only —
            // never the body (a body tenantId is ignored). The F2 mount gate
            // guarantees an authenticated, tenant-carrying caller here; this
            // narrow is type-level defense for handler-only mounts.
            const tenantId = typeof req.user?.tenantId === 'string' && req.user.tenantId.length > 0
                ? req.user.tenantId
                : null;
            if (tenantId === null) {

...
export async function createProviderRouter(deps: ProviderRouterDependencies): Promise<Router> {
  ...
  router.get('/providers', asyncHandler(async (req: Request, res: Response) => {
    // F2 zero-spend ruling (v4): several testConnection() implementations
    // issue real paid completions (e.g. OpenAIProvider). Anonymous demo
    // sessions get UNPROBED registry metadata; live probes require identity.
    if (isAnonymousRequest(req)) {
      const providers = registry.listProviders();
      res.json({
        success: true,
        providers: providers.map((p) => ({ ...p, available: null as boolean | null, probed: false, status: 'demo session — live connectivity probe requires authentication' })),
        timestamp: new Date().toISOString(),
      });
      return;
    }
    ...
    const providersWithStatus = await Promise.all(
      providers.map(async (p) => {
        ...
        if (provider) {
          ...
          return { ...p, available: test.ok, probed: true, status: test.message };
        }
        return { ...p, available: false, probed: true, status: 'Provider not found' };
      })
    );

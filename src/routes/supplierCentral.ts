...
router.post('/vendors', asyncHandler(async (req, res, next) => {
  ...
  // A non-plain-object body is refused HERE, before the service runs, because
  // it cannot carry the verified actor: assigning `createdBy` on a primitive
  // is a no-op (or throws in strict mode) and an array survives the spread
  // without the field. Forwarding one is not merely unattributed, it is
  // destructive — VendorDirectory.createVendorProfile (src/services/
  // supplier-central/VendorDirectory.ts:27-80) spreads the input, writes the
  // vendor via `this.vendors.set` (:63) and an activity row via
  // `recordActivity` (:66) BEFORE it dereferences `vendor.basicInfo.
  // companyName` (:75), so both records persist with no createdBy and the
  // caller still gets a 500.
  if (!profileData || typeof profileData !== 'object' || Array.isArray(profileData)) {
    res.status(400).json({ error: 'Request body must be an object' });
    return;
  }
  profileData.createdBy = actorId;
...
router.post('/vendors/:vendorId/approve', asyncHandler(async (req, res, next) => {
  ...
  await supplierService.approveVendor(tenantId, vendorId, actorId, notes);
  res.json({ success: true });
}));
...
router.post('/vendors/:vendorId/reject', asyncHandler(async (req, res, next) => {
  ...
  await supplierService.rejectVendor(tenantId, vendorId, actorId, reason);
  res.json({ success: true });
}));
...
// Health Check Route — plain liveness probe. F5b declares this exact path
// public in routePolicy.ts (anonymous monitoring probe), so it must not read
// tenant data. Pre-F5b this handler computed per-tenant onboarding stats via
// the SYSTEM_IDENTITY fallback — an anonymous tenant-data read, which is
// exactly the surface F5b retires; those stats stay behind GET /dashboard.
router.get('/health', (_req, res) => {
  res.json({ status: 'healthy', service: 'supplier-central', timestamp: Date.now() });
});

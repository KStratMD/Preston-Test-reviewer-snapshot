...
router.post('/vendors', asyncHandler(async (req, res, next) => {
  ...
  const vendorId = await supplierService.createVendorProfile(profileData);
...
router.get('/vendors', asyncHandler(async (req, res, next) => {
  ...
  const result = await supplierService.getVendorProfiles(filters);
  res.json(result);
}));
...
router.get('/vendors/:vendorId', asyncHandler(async (req, res, next) => {
  ...
  const vendor = await supplierService.getVendorProfile(vendorId);
  if (!vendor) {
...
router.put('/vendors/:vendorId', asyncHandler(async (req, res, next) => {
  ...
  await supplierService.updateVendorProfile(vendorId, req.body);
...
router.post('/vendors/:vendorId/documents', asyncHandler(async (req, res, next) => {
  ...
  const result = await supplierService.uploadDocument(vendorId, documentType, {
    fileName,
    fileSize: fileSize || 0,
...
router.post('/vendors/:vendorId/approve', asyncHandler(async (req, res, next) => {
  ...
  await supplierService.approveVendor(vendorId, approvedBy, notes);
  res.json({ success: true });
}));
...
router.post('/vendors/:vendorId/reject', asyncHandler(async (req, res, next) => {
  ...
  await supplierService.rejectVendor(vendorId, rejectedBy, reason);
  res.json({ success: true });
}));
...
router.get('/vendors/:vendorId/ai-assessment', asyncHandler(async (req, res, next) => {
  ...
  const result = await supplierService.assessVendorForApproval(vendorId);
...
router.post('/vendors/:vendorId/sync', asyncHandler(async (req, res, next) => {
  ...
  const result = await supplierService.syncVendorToBusinessCentral(vendorId);
...
router.get('/analytics/onboarding', asyncHandler(async (req, res, next) => {
  ...
  const stats = await supplierService.getOnboardingStats();
  res.json(stats);
}));
...
router.get('/dashboard', asyncHandler(async (req, res, next) => {
  ...
  // Get recent vendors
  const recentVendors = await supplierService.getVendorProfiles({
    limit: 10,
    createdAfter: Date.now() - (7 * 24 * 60 * 60 * 1000), // Last 7 days
  });

  // Get vendors pending approval
  const pendingApproval = await supplierService.getVendorProfiles({
    stage: ['compliance_review', 'documents_pending'],
    limit: 20,
  });

  // Get recent activity
  const recentActivity = await supplierService.getPortalActivity(undefined, 15);
...
router.post('/vendors/bulk-approve', asyncHandler(async (req, res, next) => {
  const supplierService = container.get<SupplierCentralService>(TYPES.SupplierCentralService);
  const { vendorIds, approvedBy, notes } = req.body;
...
router.get('/export/vendors', asyncHandler(async (req, res, next) => {
  ...
  const result = await supplierService.getVendorProfiles({
    ...filters,
...
router.get('/vendors/:vendorId/purchase-orders', asyncHandler(async (req, res, next) => {
  ...
  const result = await supplierService.getPurchaseOrdersForVendor(vendorId, filters);
  res.json(result);
}));
...
router.post('/purchase-orders', asyncHandler(async (req, res, next) => {
  ...
  const normalizedCreatedBy = normalizeOptionalString('createdBy', createdBy);
  if (normalizedCreatedBy === null) return;
...
router.get('/purchase-orders/:poId', asyncHandler(async (req, res, next) => {
  ...
  const po = await supplierService.getPurchaseOrder(poId);
  if (!po) {
...
router.post('/purchase-orders/:poId/acknowledge', asyncHandler(async (req, res, next) => {
  const supplierService = container.get<SupplierCentralService>(TYPES.SupplierCentralService);
  const { poId } = req.params;
  const { acknowledgedBy, notes, lineConfirmations } = req.body;
  ...
  const po = await supplierService.acknowledgePurchaseOrder(poId, {
    acknowledgedBy,
    notes,
    lineConfirmations,
  });
...
router.post('/purchase-orders/:poId/asn', asyncHandler(async (req, res, next) => {
  ...
  const asn = await supplierService.createAdvancedShippingNotice({
...
router.get('/vendors/:vendorId/asn', asyncHandler(async (req, res, next) => {
  ...
  const asns = await supplierService.getAdvancedShippingNoticesForVendor(vendorId);
...
router.get('/purchase-orders/:poId/asn', asyncHandler(async (req, res, next) => {
  ...
  const asns = await supplierService.getAdvancedShippingNoticesForPO(poId);
  res.json({ asns, totalCount: asns.length });
}));
...
router.patch('/asn/:asnId/status', asyncHandler(async (req, res, next) => {
  ...
  const asn = await supplierService.updateASNStatus(asnId, status, actualDeliveryDate);
  res.json(asn);
}));
...
router.post('/vendors/:vendorId/sync-netsuite', asyncHandler(async (req, res, next) => {
  ...
  const result = await supplierService.syncVendorToNetSuite(vendorId);
  if (!result.success) {
    res.status(400).json(result);
    return;
  }
...
router.post('/netsuite/batch-sync', asyncHandler(async (req, res, next) => {
  ...
  const result = await supplierService.batchSyncVendorsToNetSuite(vendorIds);
  res.json(result);
}));
...
router.get('/netsuite/sync-status', asyncHandler(async (req, res, next) => {
  const supplierService = container.get<SupplierCentralService>(TYPES.SupplierCentralService);
  const status = await supplierService.getNetSuiteSyncStatus();
  res.json(status);
}));
...
router.post('/purchase-orders/:poId/sync-netsuite', asyncHandler(async (req, res, next) => {
  ...
  const result = await supplierService.syncPurchaseOrderToNetSuite(poId);
  if (!result.success) {
    res.status(400).json(result);
    return;
  }
...
router.get('/netsuite/governance', asyncHandler(async (req, res, next) => {
  const supplierService = container.get<SupplierCentralService>(TYPES.SupplierCentralService);
  const metrics = supplierService.getGovernanceMetrics();
  res.json(metrics);
}));
...
router.patch('/netsuite/governance', asyncHandler(async (req, res, next) => {
  ...
  supplierService.updateGovernanceConfig(config);
  res.json({ success: true, config: supplierService.getGovernanceMetrics().config });
}));

...
//
// `GET /:id` is the polling endpoint for server-to-server callers that
// received a 202. F3 flipped it strict: `authMiddleware` (Bearer JWT
// required) + the tenant-lifecycle kill switch front it, and the handler
// reads the tenant from `req.user.tenantId` — the extractIdentityContext /
// SYSTEM_IDENTITY fallback is gone from this file. The operator UI gets
// full row data from the list endpoint and doesn't need single-row reads.
...
import { makeTenantStatusGate } from '../../middleware/tenantStatusGate';
...
import type { EmbeddedPrincipal } from './_governanceAuth';
import type { ApprovalDecider } from '../../services/governance/ApprovalQueueService';

/**
 * The decider for a decision, from the principal `requireApproverRole`
 * resolved.
 *
 * Falls back to no verified roles if the principal is somehow absent. That
 * fallback is refused by the service's separation-of-duties gate, which is the
 * right outcome: a decision whose authority cannot be established must not
 * proceed on the strength of a user id alone.
 */
function readDecider(res: express.Response, fallbackUserId: string): ApprovalDecider {
  const principal = res.locals.embeddedPrincipal as EmbeddedPrincipal | undefined;
  if (principal === undefined) {
    return { userId: fallbackUserId, verifiedRoles: [], scope: null };
  }
  return {
    userId: principal.userId,
    verifiedRoles: principal.verifiedRoles,
    scope: { platform: principal.platform, platformAccountId: principal.platformAccountId },
  };
}
import { validateGuestContext } from '../../middleware/embeddedAuthMiddleware';
import { requireActiveEmbeddedTenant } from '../../middleware/embeddedTenantStatusGate';
...
/**
 * F3: tenant-lifecycle kill switch for the JWT-authenticated poll route.
 * TenantLifecycleService binds via toDynamicValue(async) — resolve lazily per
 * request (container caches the singleton), same posture as
 * getApprovalQueueService above. makeTenantStatusGate fails closed with 403
 * tenant_id_missing for authenticated requests without a tenantId claim.
 */
function requireActiveJwtTenant(req: express.Request, res: express.Response, next: express.NextFunction): void {
  container
    .getAsync<TenantLifecycleService>(TYPES.TenantLifecycleService)
    .then((svc) => makeTenantStatusGate(svc)(req, res, next))
    .catch(next);
}
...
router.get('/:id', authMiddleware, requireActiveJwtTenant, asyncHandler(async (req, res) => {
  ...
  // F3: authMiddleware guarantees req.user (anonymous → 401 upstream; the
  // Codex 5.4 fail-closed contract now lives in the middleware chain), and
  // requireActiveJwtTenant fails closed on a missing tenantId claim (403
  // tenant_id_missing) or a blocked tenant (403 tenant_blocked). This narrow
  // is type-level defense — reachable only if the chain is mis-mounted,
  // e.g. in a handler-only test.
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const updated = await service.approve({
        ...
        // The principal the role gate resolved, carrying VERIFIED roles and the
        // account they were verified for. A bare session user id would say who
        // is asking without saying what they proved, and the service refuses
        // exactly that.
        approver: readDecider(res, session.user_id),
        reason,
      });

...
router.get('/cash-position', asyncHandler(async (req, res) => {
  ...
  const tenantId = resolveCentralTenantId(req, res);
  if (tenantId === null) return;
  const cashPosition = await service.getCashPosition(tenantId);
...
router.get('/ar-aging', asyncHandler(async (req, res) => {
  ...
  const tenantId = resolveCentralTenantId(req, res);
  if (tenantId === null) return;
  const arAging = await service.getARAgingReport(tenantId);
  res.json(arAging);
}));
...
router.get('/ap-aging', asyncHandler(async (req, res) => {
  ...
  const tenantId = resolveCentralTenantId(req, res);
  if (tenantId === null) return;
  const apAging = await service.getAPAgingReport(tenantId);
  res.json(apAging);
}));
...
router.get('/metrics', asyncHandler(async (req, res) => {
  ...
  const tenantId = resolveCentralTenantId(req, res);
  if (tenantId === null) return;
  const metrics = await service.calculateFinancialMetrics(tenantId);
  res.json(metrics);
}));
...
router.get('/cash-flow', asyncHandler(async (req, res) => {
  ...
  const forecast = await service.getCashFlowForecast(tenantId, weeks);
  res.json(forecast);
}));
 ...
 * Body: { comments? }
 * The approver is taken from the verified req.user id; a body-supplied
 * approverId is ignored.
 */
 ...
 * Body: { reason }
 * The rejecter is taken from the verified req.user id; a body-supplied
 * rejecterId is ignored.
 */
...
router.get('/gl-accounts', asyncHandler(async (req, res) => {
  ...
  const accounts = await service.getGLAccounts(tenantId, filters);
  res.json(accounts);
}));
...
router.get('/consolidated', asyncHandler(async (req, res) => {
  ...
  const tenantId = resolveCentralTenantId(req, res);
  if (tenantId === null) return;
  const summary = await service.getConsolidatedSummary(tenantId);
  res.json(summary);
}));
...
router.get('/documents', asyncHandler(async (req, res) => {
  ...
  const documents = await service.getFinancialDocuments(tenantId, filters);
  res.json(documents);
}));

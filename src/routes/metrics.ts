...
/**
 * V8's moving heap-total value is the denominator used by the health route.
 * Exposing the fixed heap ceiling alongside it lets operators distinguish
 * ordinary V8 growth from genuine headroom exhaustion.
 */
const heapSizeLimitGauge = new promClient.Gauge({
...
export const createMetricsRouter = (): Router => {
  ...
  router.get('/', metricsScrapeAuth, async (_req, res) => {
    heapSizeLimitGauge.set(getHeapStatistics().heap_size_limit);
  ...
  router.get('/review', async (_req, res) => {
    ...
    const build_sha = (envSha && envSha.length > 0) ? envSha : 'unknown';
    const payload = {
      schema_version: 2 as const,
      served_at: new Date().toISOString(),
      payload_loaded_at: parts.loaded_at,
      build_sha,
      metrics_git_sha: pickGitSha(parts.metrics) ?? 'unknown',

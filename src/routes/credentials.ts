...
const router = Router();
const getCredentialManager = (): Promise<SecureCredentialManager> =>
  container.getAsync<SecureCredentialManager>(TYPES.SecureCredentialManager);
const getSecureConfigService = (): Promise<SecureConfigurationService> =>
  container.getAsync<SecureConfigurationService>(TYPES.SecureConfigurationService);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const userId = req.user?.id;
    const secureConfigService = await getSecureConfigService();
...
router.post('/migrate', authMiddleware, rbacMiddleware(['admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
  const secureConfigService = await getSecureConfigService();
  const userId = req.user?.id;
  // Trusted command context (Task 8, 'secure_save'): the migration sweep is
  // cross-tenant, but the actor issuing it must still be a verified identity —
  // requireTenantId establishes the base tenant, requireConfigurationCommandContext
  // additionally requires a non-empty actor id (401 operator_identity_required)
  // before the sweep is attributed to anyone. migrateToSecureCredentials
  // re-scopes the tenant on the context per member it actually saves.
  const tenantId = requireTenantId(req, res);
  if (!tenantId) return;
  const context = requireConfigurationCommandContext(req, res, 'secure_save', tenantId);
  if (!context) return;

  // The secure migration service is a separate singleton from the runtime
  // ConfigurationService. Load its legacy-compatible view before sweeping, or
  // the endpoint would always see an empty in-memory map.
  await secureConfigService.loadConfigurations();
  const migrationResults = await secureConfigService.migrateToSecureCredentials(context);
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const secureConfigService = await getSecureConfigService();
    await secureConfigService.updateIntegrationCredentials(tenantId, integrationId, systemType, newCredentials);

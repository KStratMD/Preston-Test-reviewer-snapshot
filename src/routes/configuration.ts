...
type ExtendedConfigurationService = ConfigurationService & {
  ...
  exportConfiguration?: (id: string, format?: string) => Promise<unknown>;
...
export const createConfigurationRouter = (optsOrService: ConfigurationService | CreateConfigRouterOpts): Router => {
  ...
  const getAllHandler = asyncHandler(async (req, res) => {
    ...
    try {
      let configs = await Promise.resolve(configService.getAllConfigurations());
  ...
  const getByIdHandler = asyncHandler(async (req, res) => {
    ...
    const config = await Promise.resolve(configService.getConfiguration(id));
    if (!config) {
  ...
  const createHandler = asyncHandler(async (req, res) => {
    ...
    try {
      if (saved && typeof saved === 'object' && saved.id) {
        res.setHeader('Location', `${base}/${saved.id}`);
      }
    } catch {/* ignore header errors */}
    const responsePayload: Record<string, unknown> = {
      message: 'Configuration saved successfully',
      id: (saved && typeof saved === 'object' && saved.id) ? saved.id : body.id,
    };
  ...
  router.post(base, createHandler);
  ...
  const deleteHandler = asyncHandler(async (req, res) => {
    ...
    try {
      const result: unknown = await configService.deleteConfiguration(id);
  ...
  const exportHandler = asyncHandler(async (req, res) => {
    ...
    try {
      const result = await configService.exportConfiguration?.(id, format);

...
export function createMockDashboardAPIs(app?: Application): Router {
  ...
  // SuiteCentral Production endpoints - use real API where available
  if (!addRouteAlias('/api/suitecentral-prod/health', '/api/suitecentral/prod/health', 'GET')) {
    router.get('/api/suitecentral-prod/health', (req, res) => {
      res.json({
        status: 'healthy',
        environments: [
          { name: 'Production', status: 'online', health: 100 },
          { name: 'Staging', status: 'online', health: 100 }
        ]
      });
    });
  }

  if (!addRouteAlias('/api/suitecentral-prod/metrics', '/api/suitecentral/prod/metrics', 'GET')) {
    router.get('/api/suitecentral-prod/metrics', (req, res) => {
      res.json({
        requests: 150000,
        avgLatency: 125,
        errorRate: 0.1,

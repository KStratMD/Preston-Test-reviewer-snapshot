...
import { EXECUTABLE_TRANSFORMATIONS, DECLARED_ONLY_TRANSFORMATIONS, FieldMappingSchema, validateMappingSet, type FieldMapping } from '../domain/mapping/MappingContract';
...
export function createMockDashboardAPIs(app?: Application): Router {
  ...
  if (!addRouteAlias('/api/ai/field-mapping/validate', '/api/ai/mapping/validate', 'POST')) {
    router.post('/api/ai/field-mapping/validate', (req: Request, res: Response) => {
      ...
      // Codex round 13 on PR #1253: validating nothing answered success:true.
      if (mappings.length === 0) {
        return res.status(400).json({ success: false, error: 'Empty mappings' });
      }
      ...
      let totalScore = 0;
      // Codex round 6 on PR #1253: the RAW mapping is what the contract judges — no
      // coercion, no dropped keys — and the accepted ones must also validate as a SET.
      const accepted: FieldMapping[] = [];

      mappings.forEach((rawMapping, index) => {
        // Codex round 8 on PR #1253: a null or non-object entry threw on
        // destructuring and became a 500. It is an invalid mapping, scored 0.
        const mapping = rawMapping !== null && typeof rawMapping === 'object' && !Array.isArray(rawMapping) ? rawMapping : {};
        ...
        // Structural validation through the canonical contract (Codex rounds 4-5 on
        // PR #1253: the vocabulary check alone let 'format' pass, then a mapping
        // with no type or a lookup with no config score 1). Anything the
        // contract refuses is not executable and cannot count as passed.
        let executable = true;
        const candidate = FieldMappingSchema.safeParse(mapping);
        if (candidate.success) {
          // Copilot on PR #1253: decide on the VALIDATED value, not the raw one.
          const validatedType = candidate.data.transformationType;
          if (!(DECLARED_ONLY_TRANSFORMATIONS as readonly string[]).includes(validatedType)) {
            score += 0.1;
            accepted.push(candidate.data);
          } else {
            issues.push(`Transformation type '${validatedType}' is declared but not executable (allowed: ${EXECUTABLE_TRANSFORMATIONS.join(', ')})`);
            executable = false;
          }
        } else {
          for (const issue of candidate.error.issues) issues.push(`${issue.path.join('.') || 'mapping'}: ${issue.message}`);
          executable = false;
        }
        ...
        const finalScore = executable ? Math.max(0, Math.min(1, score)) : 0;
      ...
      // The set as a whole: duplicate target slots (any lodash spelling) fail every member.
      const setResult = validateMappingSet(accepted);
      if (!setResult.ok) {
        for (const r of validationResults) { r.score = 0; r.issues.push(...setResult.issues); }
        potentialIssues.push(...setResult.issues);
        totalScore = 0;
      }
      const overallScore = mappings.length > 0 ? totalScore / mappings.length : 0;

      return res.json({
        success: setResult.ok && validationResults.every((r) => r.score > 0),
        overallScore,
        validationResults,

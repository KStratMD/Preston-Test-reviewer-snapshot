...
// A5: the tenant-id rule is NOT restated here. This module used to own a
// `^[a-zA-Z0-9_-]{1,64}$` copy annotated "canonical (tenantIsolation.ts:61)" —
// a comment that had already gone stale (the line had moved) and a constant
// that could drift from the real validator. `normalizeTenantIdClaim` in
// `src/utils/tenantId.ts` is the single source of truth; see the tenantId
// field on WebhookPayloadSchema below.
...
export const WebhookPayloadSchema: z.ZodType<WebhookPayload> = z.object({
  ...
  // A5 — validate through the canonical normalizer and carry its RETURN VALUE
  // forward, so a padded claim is persisted only in its trimmed spelling and
  // can never create a second row that renders identically to an existing
  // tenant. The message states the rule without echoing the rejected value:
  // this payload is attacker-controlled and the error text reaches logs.
  // `.trim()` then validate, rather than a `.transform()` returning the
  // normalizer's output. The two are exactly equivalent — `normalizeTenantIdClaim`
  // only ever trims; every other rule is a reject, not a rewrite — but zod 4
  // infers any piped/transformed field as OPTIONAL in the object's output type,
  // which silently breaks the `z.ZodType<WebhookPayload>` contract this file
  // exists to enforce. `.trim()` is a ZodString check, so the key stays required.
  // The parsed value is therefore the canonical spelling, and a padded sentinel
  // ("  __system__  ") is rejected too — the old regex caught that only as a
  // side effect of disallowing spaces.
  tenantId: z.string()
    .trim()
    .refine((v) => normalizeTenantIdClaim(v) !== undefined, {
      message: 'tenantId must be 1-64 characters of [A-Za-z0-9_-]',
    })
    .refine((v) => v !== RESERVED_TENANT_SENTINEL, {

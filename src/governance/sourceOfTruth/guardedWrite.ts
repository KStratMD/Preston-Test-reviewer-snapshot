...
import type { ApprovalScope, RequesterProvenance } from '../../services/governance/ApprovalQueueRepository';
import {
  LoopDetectedError,
...
export interface GuardedWriteContext {
  ...
  /**
   * How the requester was identified, if the caller knows.
   *
   * Omitted is meaningful: it persists as 'unknown', which the approval
   * service marks `sodWeak`. Most callers reach guardedWrite from a flow or a
   * connector and genuinely cannot say how the person was authenticated, so
   * the honest answer is to leave this unset rather than assert 'system' and
   * have an unverified request read as a verified one.
   */
  requesterProvenance?: RequesterProvenance;
  /** The platform account the request came from, when there is one. */
...
export async function guardedWrite<T, TPayload extends Record<string, unknown>>(
  ...
  if (decision.allowed === false) {
    if (decision.reason === 'queue_required') {
      ...
      const derivedTargetSystemId = connectorKeyForSystem(context.targetSystem);
      ...
      const queueId = await deps.approvalQueueService.enqueue({
        ...
        // Passed through, not invented. Undefined persists as 'unknown' and
        // the decision is recorded as sodWeak, which is the truthful state for
        // a write whose requester was never verified.
        requesterProvenance: context.requesterProvenance,
        requesterScope: context.requesterScope ?? null,

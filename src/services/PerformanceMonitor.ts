...
} from '../performance/ProcessResourceSampler';
...
export class PerformanceMonitor extends EventEmitter {
  ...
  private readonly cpuUsageCalculator = new CpuUsageDeltaCalculator();
  ...
  constructor(sampler: ProcessResourceSampler = new NodeProcessResourceSampler()) {
    super();
  ...
  public startMonitoring(intervalMs = 10000): void {
    ...
    this.resetResourceSampling();
    const baselineSnapshot = this.sampler.sample();
    this.memoryBaseline = baselineSnapshot.memory.heapUsed;
  ...
  private collectMetrics(): void {
    ...
    const resourceSnapshot = this.sampler.sample();
    const memUsage = resourceSnapshot.memory;
    const cpuUsage = this.cpuUsageCalculator.calculate(resourceSnapshot) ?? 0;
  ...
  private resetResourceSampling(): void {

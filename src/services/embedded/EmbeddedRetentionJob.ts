...
import { UserAssertionNonceRepository } from './UserAssertionNonceRepository';
 ...
 *  - Four queries per tick (round-8 finding #5, extended by migration 065):
 ...
 *      (d) DELETE user-assertion nonces older than 15m — nothing else removes
 *          them, and the ledger grows once per bootstrap
...
export class EmbeddedRetentionJob {
  ...
  /**
   * How long a spent user-assertion nonce is kept.
   *
   * 15 minutes is three times the ±300 s skew window the assertion verifier
   * allows. A nonce older than that cannot be replayed at all — the timestamp
   * check rejects the header before the ledger is ever consulted — so keeping
   * the row longer would grow the table without buying any replay protection.
   * Without this purge the ledger grows once per bootstrap, forever.
   */
  static readonly ASSERTION_NONCE_GRACE_MS = 15 * 60 * 1000;
  ...
  constructor(
    ...
    @inject(TYPES.UserAssertionNonceRepository)
    private readonly assertionNonces: UserAssertionNonceRepository,
  ...
  /**
   * Tick exposed for tests + manual invocation. The four retention queries
   ...
   * fail without poisoning the others (matches the broader app's
   * "best-effort cleanup" stance).
   *
   * Each query runs in its own try/catch so a failure in one (e.g. a session
   * DELETE blocked by a stray FK) does not prevent the others from
  ...
  async tick(now: Date = new Date()): Promise<{
    ...
    assertionNoncesPurged: number;
  }> {
    let sessionsDeleted = 0;
    let versionsRetired = 0;
    let versionsPurged = 0;
    let assertionNoncesPurged = 0;
    try {
    ...
    try {
      // The nonce ledger is a separate table from embedded_sessions on purpose
      // — guest teardown deletes session rows on pagehide, so a nonce stored
      // there would not survive to prevent replay. That independence also
      // means nothing else ever removes these rows, which is why this query
      // has to exist.
      assertionNoncesPurged = await this.assertionNonces.purgeOlderThan(
        new Date(now.getTime() - EmbeddedRetentionJob.ASSERTION_NONCE_GRACE_MS),
      );
    } catch (err) {
      this.logger.error('[EmbeddedRetentionJob] assertionNonces.purgeOlderThan failed', {
        error: err instanceof Error ? err.message : String(err),
      });
    }
    if (
      ...
      versionsRetired > 0 ||
      versionsPurged > 0 ||
      assertionNoncesPurged > 0
    ) {
      this.logger.info('[EmbeddedRetentionJob] tick complete', {
        sessionsDeleted,
        versionsRetired,
        versionsPurged,
        assertionNoncesPurged,
      });
    }
    return { sessionsDeleted, versionsRetired, versionsPurged, assertionNoncesPurged };
  }

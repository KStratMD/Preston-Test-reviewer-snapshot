...
export class MCPAggregatorService {
  ...
  async callTool(
    ...
    try {
      ...
      const normalized = this.normalizeToolResult(
        (piiScan.redactedData as MCPToolResult | undefined) ?? toolResult
      );
      ...
      normalized.structuredContent = {
        ...
        pii: {
          detected: piiScan.detected,
          riskLevel: piiScan.riskLevel,
          findingsCount: piiScan.findings.length,
        },
      };

      this.recordAudit(context, namespacedName, piiScan.detected, requestSessionId);

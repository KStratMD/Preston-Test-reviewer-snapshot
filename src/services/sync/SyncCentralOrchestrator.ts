...
import { addressesForbiddenSegment } from '../../domain/mapping/MappingContract';

const SYNC_SYSTEM_TO_SOURCE_SYSTEM: Record<string, SourceSystem> = {
    netsuite: 'netsuite',
    NetSuite: 'netsuite',
    ...
    // spelling so configs already in manifest vocab don't fall through to
    // 'squire' and misattribute Business Central writes.
...
function toSourceSystem(systemName: string, targetEntity?: string): SourceSystem {
    const mapped = SYNC_SYSTEM_TO_SOURCE_SYSTEM[systemName];
    if (mapped) {
        return mapped;
    }
    // Unmapped system name. Fail fast ONLY when the write targets a canonical
    // entity governed by a SourceSystem ownership policy: there, attributing
    // an unmapped connector to 'squire' would corrupt the ownership decision
    ...
    // no_policy_declared (allowed regardless of caller), so 'squire' is
    // cosmetic audit-only attribution; preserving it keeps valid-but-unmapped
    // connectors (Dynamics365/SAP/Oracle/SuiteCentral — registered but absent
    // from the SourceSystem union) syncing for non-canonical entities.
    if (targetEntity !== undefined && isCanonicalEntity(canonicalEntityFor(targetEntity))) {
        throw new Error(
            `toSourceSystem: unmapped system name '${systemName}' for canonical entity ` +
            `'${canonicalEntityFor(targetEntity)}' — add it to SYNC_SYSTEM_TO_SOURCE_SYSTEM ` +
...
export class SyncCentralOrchestrator {
     ...
     */
    async executeSync(operationId: string, requesterUserId: string): Promise<SyncResult> {
            ...
            try {
                ...
                if (existingRecord) {
                    ...
                    if (conflictingFields.length > 0) {
                        ...
                        if (operation.conflictResolution === 'ai-resolve') {
                            ...
                            if (conflict.aiSuggestion.confidence >= 0.8) {
                                await this.guardedConnectorWrite(
                                    targetConnector, 'update',
                                    operation.entityType, sourceRecord.id,
                                    conflict.aiSuggestion.suggestedResolution,
                                    operation.sourceSystem,
                                    operation.targetSystem,
                                    `sync-${operation.id}-${sourceRecord.id}-ai-resolve`,
                                    requesterUserId,
                                );
                                updated++;
                                continue;
                            ...
                            await this.guardedConnectorWrite(
                                ...
                                operation.sourceSystem,
                                operation.targetSystem,
                                `sync-${operation.id}-${sourceRecord.id}-source-wins`,
                                requesterUserId,
                            );
                            updated++;
                            continue;
                        ...
                        await this.guardedConnectorWrite(
                            ...
                            operation.sourceSystem,
                            operation.targetSystem,
                            `sync-${operation.id}-${sourceRecord.id}-update`,
                            requesterUserId,
                        );
                        updated++;
                    }
                    ...
                    await this.guardedConnectorWrite(
                        ...
                        operation.sourceSystem,
                        operation.targetSystem,
                        `sync-${operation.id}-${sourceRecord.id}-create`,
                        requesterUserId,
                    );
                    created++;
                }
    ...
    /**
     * Routes a connector write through guardedWrite for ownership audit. The
     * `do:` arrow function is a direct AST descendant of the guardedWrite()
     * call expression, satisfying check-guarded-writes.mjs's parent-walk.
     *
     * Copilot R4 on PR #851: the prior `callerSystem: 'sync_orchestrator'`
     * caused `guardedWrite` to skip `detectLoop` — the loop-detection step
     * only runs when `callerSystem` is a real `SourceSystem` (via
     * `isSourceSystem` narrowing). Since this helper is invoked from the
     * sync engine with `operation.sourceSystem` available at the callsite,
     * thread the mapped source system through as `callerSystem` so
     * reciprocal-write loops between source ↔ target ARE detected.
     */
    private async guardedConnectorWrite(
        connector: IConnector,
        ...
        const ownershipResolver = this.ownershipResolver;
        const auditService = this.auditService;
        const approvalQueueService = this.approvalQueueService;
        if (!ownershipResolver || !auditService || !approvalQueueService) {
            throw new Error(
                'SyncCentralOrchestrator: OwnershipResolver, AuditService, and ApprovalQueueService are required for connector writes',
            );
        }
        await guardedWrite(
            {
                context: {
                    // PR3: platform-global governance scope, NOT an
                    // unidentifiable-request fallback. /api/sync-orchestrator
                    // is platform-admin-only and its operations carry no
                    // tenantId (SyncOperation has no such field), so there is
                    // no tenant to name here. The ACTOR is now the verified
                    // admin — only the scope stays system-level.
                    tenantId: SYSTEM_IDENTITY.tenantId,
                    callerSystem: toSourceSystem(sourceSystemName, entityType),
                    targetSystem: toSourceSystem(targetSystemName, entityType),
                    // Copilot R10 on PR #851: normalize connector record type
                    // (e.g. 'Customer', 'customers') to the canonical manifest
                    // entity ('customer') so OwnershipResolver evaluates the
                    // policy rather than falling to no_policy_declared.
                    entity: canonicalEntityFor(entityType) as CanonicalEntity,
                    recordId,
                    correlationId,
                    requesterUserId,
                    operation,
                ...
                do: () => operation === 'create'
                    ? connector.create(entityType, data)
                    : connector.update(entityType, recordId!, data),
            },
            { ownershipResolver, auditService, approvalQueueService },
        );
    }

    /** The words transformRecord() implements; anything else in the legacy union fails closed. */
    private static readonly EXECUTABLE_TRANSFORMATIONS: ReadonlySet<string> = new Set(['none', 'uppercase', 'lowercase', 'date', 'number']);
    ...
    private transformRecord(record: DataRecord, mappings: FieldMapping[]): DataRecord {
        ...
        for (const mapping of mappings) {
            // Codex round 3 on PR #1253: decide whether the transformation is
            // executable BEFORE looking at the value. Inside the value condition
            // the check was skipped whenever the source and default were absent,
            // and the record was written as a success with an undefined field.
            if (mapping.transformation !== undefined && !SyncCentralOrchestrator.EXECUTABLE_TRANSFORMATIONS.has(mapping.transformation)) {
                throw new Error(
                    `transformation '${String(mapping.transformation)}' is declared but not executable (mapping ${mapping.sourceField} -> ${mapping.targetField})`,
                );
            }
            // Codex round 10 on PR #1253: a targetField of '__proto__' wrote the
            // output object's PROTOTYPE (then shipped to the connector as a
            // success), and a sourceField of 'constructor' read Object.prototype.
            // The legacy mapper indexes plain objects by name, so it refuses the
            // same names the canonical contract refuses.
            // Codex round 11 on PR #1253: an EMPTY sourceField never matched a
            // record field, fell through to its defaultValue and was written as
            // a success; an empty targetField wrote the key "". Neither is a
            // field name (the canonical contract requires min length 1).
            if (!mapping.sourceField?.trim() || !mapping.targetField?.trim()) {
                throw new Error(
                    `field names must not be empty (mapping '${String(mapping.sourceField)}' -> '${String(mapping.targetField)}')`,
                );
            }
            if (addressesForbiddenSegment(mapping.sourceField) || addressesForbiddenSegment(mapping.targetField)) {
                throw new Error(
                    `field names must not address __proto__, constructor or prototype (mapping ${mapping.sourceField} -> ${mapping.targetField})`,
                );
            }
            // Codex round 13 on PR #1253: a plain-object read walks the prototype,
            // so a source named 'toString' the record did not own resolved to a
            // function. Only an OWN property is a source value.
            const fields = record.fields ?? {};
            let value = (Object.prototype.hasOwnProperty.call(fields, mapping.sourceField) ? fields[mapping.sourceField] : undefined) ?? mapping.defaultValue;
            // Codex round 8 on PR #1253: `required` was never read — a missing
            // required source wrote undefined and counted as a success.
            if (mapping.required && (value === undefined || value === null)) {
                throw new Error(`required source field '${mapping.sourceField}' is missing (mapping -> ${mapping.targetField})`);
            }
            ...
            if (value !== undefined && mapping.transformation) {
                switch (mapping.transformation) {
                    ...
                    case 'number':
                        value = Number(value);
                        // Codex rounds 4 and 5 on PR #1253: a non-numeric source became NaN,
                        // then 'Infinity' passed the NaN-only guard; both followed the create
                        // path as a success. Only a finite number is a number here.
                        if (!Number.isFinite(value)) {
                            throw new Error(`transformation 'number' produced a non-finite value (${String(value)}) for ${mapping.sourceField} -> ${mapping.targetField}`);
                        }
                        break;
                    case 'none':
                        break;
                    default:
                        // Unreachable: the executability check above runs first. Kept so
                        // a new word added to the union without a branch cannot pass through.
                        throw new Error(
                            `transformation '${String(mapping.transformation)}' is declared but not executable (mapping ${mapping.sourceField} -> ${mapping.targetField})`,
                        );
                }
            ...
            // An optional source that is absent produces NO field, not an own property
            // holding undefined (the canonical engine behaves the same way; Codex round 9's
            // last probe before its credits ran out showed the orchestrator writing one).
            if (transformed.fields && value !== undefined) {

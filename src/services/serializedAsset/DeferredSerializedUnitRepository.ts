...
/**
 * Fixed, content-free text for a driver that ran `touchAttempt`'s conditional
 * UPDATE without reporting how many rows it matched. Names a storage-layer
 * condition and carries no row data, same contract as the message above.
 */
const TOUCH_ATTEMPT_ROWCOUNT_UNAVAILABLE = 'deferred_serialized_units.touchAttempt: driver reported no updated-row count';
...
export class DeferredSerializedUnitRepository {
   ...
   *
   * COMPARE-AND-SWAP on `attempt_count`. `expectedAttemptCount` is the count
   * the CALLER read in the snapshot it is acting on, and it is part of the
   * update predicate alongside the three key columns. This is what makes the
   * touch stale-safe rather than merely key-scoped.
   *
   * Key-only matching was wrong across concurrent runs. Run A reads a row at
   * count 1; run B re-defers the same key through `upsertDeferred`, replacing
   * the payload and rescheduling to a LATER time; run A's touch then lands on
   * that replacement, double-incrementing it and — because run A's `now`
   * predates the replacement — dragging `next_attempt_at` backwards into the
   * past. Near `MAX_DEFERRAL_ATTEMPTS` that tips a legitimately rescheduled
   * unit into `attempts_exhausted` quarantine, which is an operator-visible
   * give-up on work that was never actually abandoned.
   *
   * A single conditional UPDATE, not a transaction: the predicate IS the
   * concurrency control, so there is no read to keep consistent with a write.
   *
   * Returns the new `attempt_count` when exactly one row matched. Returns
   * `null` otherwise — the unit was never deferred, a concurrent run already
   * cleared it after a successful upsert, or the row was replaced since the
   * snapshot. All three are benign, and the caller must NOT reread and retry
   * against whatever row now occupies the key: doing so reintroduces exactly
   * the stale write this predicate exists to refuse.
   */
  async touchAttempt(
    tenantId: string,
    configurationId: string,
    ...
    // A driver that does not report a row count cannot be coalesced to zero:
    // "zero rows matched" and "I do not know how many rows matched" are
    // different facts, and collapsing them would report a successful UPDATE as
    // a no-op — the silent skipped back-off this method exists to make
    // impossible. Both dialects this repository runs on populate the count
    // (asserted per engine in the durability suite), so an absent count means
    // the storage layer changed underneath and is raised, not guessed. The
    // caller already treats a throw here as non-fatal and logs it.
    const updatedRows = Number(result.numUpdatedRows);
    if (!Number.isFinite(updatedRows)) {
      throw new Error(TOUCH_ATTEMPT_ROWCOUNT_UNAVAILABLE);
    }

    return updatedRows === 1 ? attemptCount : null;
  }

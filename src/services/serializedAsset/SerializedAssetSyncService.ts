...
/**
 * Stands in for an error class name when a back-off is refused because the
 * caller carried no durable-row snapshot. Fixed text, like every other value
 * on this logging path (decision 8) — it names a code path, not any datum.
 */
const MISSING_ATTEMPT_SNAPSHOT = 'MissingDeferredAttemptSnapshot';
...
export interface SerializedAssetDeferredStore {
   ...
   *
   * `expectedAttemptCount` is a compare-and-swap guard: it must be the count
   * from the snapshot the caller is acting on, so a touch cannot land on a row
   * that a concurrent run replaced in the meantime. Returns `null` when it
   * does not match — see the repository method for why that must not be
   * retried.
   */
  touchAttempt(
    tenantId: string,
...
interface SerializedAssetWorkItem {
  ...
  /**
   * `attempt_count` as READ from the durable row that backs this item, carried
   * so a later back-off can compare-and-swap against it rather than blind-
   * writing the key (see `DeferredSerializedUnitRepository.touchAttempt`).
   *
   * Deliberately the LISTING's value, never a fresh read: a value re-read at
   * back-off time would always match and the guard would be vacuous.
   * `undefined` exactly when `fromDeferred` is false.
   */
  expectedAttemptCount?: number;
}
...
export class SerializedAssetSyncService {
  ...
  private async mergeDeferredRows(
    ...
    for (const row of deferredRows) {
      ...
      if (!options.forceDeferredRetry && row.attemptCount >= MAX_DEFERRAL_ATTEMPTS) {
        ...
        await this.advanceDeferredAttempt(ctx, row, row.attemptCount, abandonedRef);
        continue;
      }
      ...
      if (existing) {
        ...
        existing.expectedAttemptCount = row.attemptCount;
      } else {
  ...
  private async quarantineUndecodableRows(
    ...
    for (const row of rows) {
      ...
      await this.advanceDeferredAttempt(ctx, row, row.attemptCount, unitRef);
    }
  }
    ...
    try {
      ...
      if (resolution.status === 'ambiguous') {
        ...
        if (item.fromDeferred) {
          await this.advanceDeferredAttempt(ctx, unit, item.expectedAttemptCount, unitRef);
        }
        return;
      }
   ...
   *
   * `expectedAttemptCount` is the count from the durable row THIS run read —
   * the abandoned/undecodable row itself, or the `SerializedAssetWorkItem`
   * snapshot `mergeDeferredRows` folded in. It is a compare-and-swap guard, so
   * it must never be re-read here: a fresh read would always match, and the
   * guard would silently become a blind key write again.
   *
   * A missing count means a caller reached this path claiming a durable row it
   * never actually read. That is a caller bug, not a storage condition, and it
   * fails SAFE — nothing is written, because the only alternative is the
   * unguarded write this method exists to prevent. It reuses the existing fixed
   * diagnostic rather than a new message so the operator vocabulary is
   * unchanged.
   *
  ...
  private async advanceDeferredAttempt(
    ...
    if (expectedAttemptCount === undefined) {
      this.logger?.warn(LOG_DEFERRAL_TOUCH_FAILED, {
        ...
        configurationId: ctx.config.id,
        unitRef,
        errorName: MISSING_ATTEMPT_SNAPSHOT,
      });
      return;
    }
    try {

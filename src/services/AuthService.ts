...
export class AuthService {
  ...
  async authenticateOAuth2(credentials: AuthCredentials, options: { cache?: 'bypass' } = {}): Promise<TokenInfo> {
    // Connector lifecycles own their single-flight and valid-token reuse.
    // A fresh credential check must neither join nor populate this service's
    // legacy client-id cache, including an exchange already pending there.
    if (options.cache === 'bypass') {
      try {
        const tokenInfo = this.shouldSimulateOAuth2(credentials)
          ? this.createDemoOAuth2Token(credentials)
          : await this.requestOAuth2Token(credentials, true);
        const expiresAt = tokenInfo.expiresAt.getTime();
        if (typeof tokenInfo.accessToken !== 'string' || !tokenInfo.accessToken.trim() ||
            !Number.isFinite(expiresAt) || expiresAt <= Date.now()) {
          throw new OAuth2Error('Invalid OAuth2 token response');
        }
        return tokenInfo;
      } catch (error) {
        // A token endpoint can echo credentials in its response. Preserve the
        // existing public authentication-failure status without that body/cause.
        this.logger.error('OAuth2 authentication failed', { status: error instanceof OAuth2Error ? error.statusCode : undefined });
        throw new OAuth2Error('OAuth2 authentication failed');
      }
    }
  ...
  private async requestOAuth2Token(credentials: AuthCredentials, validateExpiry = false): Promise<TokenInfo> {
    ...
    return this._makeOAuth2TokenRequest(token_url, params, validateExpiry);
  }
   ...
   * @private
   */
  private async _makeOAuth2TokenRequest(tokenUrl: string, params: URLSearchParams, validateExpiry = false): Promise<TokenInfo> {
    ...
    // Fresh connector exchanges reject present-but-invalid expiry values before
    // normalization. An omitted expiry retains the existing one-hour fallback;
    // it is a local reuse deadline, not evidence of the vendor's token lifetime.
    if (validateExpiry && Object.prototype.hasOwnProperty.call(data, 'expires_in') &&
        (typeof data.expires_in !== 'number' || !Number.isFinite(data.expires_in) || data.expires_in <= 0)) {
      throw new OAuth2Error('Invalid OAuth2 token expiry');
    }

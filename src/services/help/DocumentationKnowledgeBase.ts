...
export class DocumentationKnowledgeBase {
  ...
  /**
   * Zero-spend retrieval seam for anonymous help. Uses only the already
   * populated in-memory chunk map and never invokes embeddings, vector search,
   * or the documentation indexer.
   */
  findSimilarChunksLocalOnly(query: string, limit = 3): DocumentChunk[] {
    if (limit <= 0) {
      return [];
    }
    ...
    const scored: { chunk: DocumentChunk; score: number; insertionOrder: number }[] = [];
    let insertionOrder = 0;
    for (const chunk of this.chunks.values()) {
      const chunkTokens = new Set(chunk.content.toLowerCase().match(/[a-z0-9]+/g) ?? []);
      let score = 0;
      ...
      if (score > 0) {
        scored.push({ chunk, score, insertionOrder });
      }
      insertionOrder++;
    }

    return scored
      .sort((left, right) => right.score - left.score || left.insertionOrder - right.insertionOrder)
      .slice(0, limit)

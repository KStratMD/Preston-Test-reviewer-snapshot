...
export class WorkflowCentralService {
  ...
  /**
   * DLP redact a cancellation reason. Fail-closed per spec §3.2 modes (a)+(b).
   * Returns:
   *   - null when caller passed undefined/null reason.
   *   - REDACT_PLACEHOLDER when scanText throws OR returns findings without
   *     redactedData (defensive — never echo the raw reason on either path).
   *   - scan.redactedData (the redacted text) when findings exist and redactedData
   *     is populated.
   *   - original reason when no findings (string was scan-clean).
   *
  ...
  private async redactCancellationReason(
    ...
    if (scan.findings.length > 0 && scan.redactedData == null) {
      this.logger.warn('cancellation reason DLP failed-closed (findings without redactedData)', {
        tenant_id: tenantId,
        instance_id: instanceId,
    ...
    return scan.redactedData ?? reason;
  }

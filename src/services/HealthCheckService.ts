...
export class HealthCheckService {
  ...
  private readonly runtimeFeatureOverrides: Record<string, boolean> = {};
  ...
  private async checkFeatureFlags(): Promise<HealthCheckDetail> {
    ...
    // effectiveFeatures(), not deploymentConfig.features. Reading the raw
    // config here would list a feature as enabled purely because it was
    // configured on — which is the exact defect this override exists to fix,
    // reappearing on the other surface that reports features.
  ...
  /**
   * Record the runtime availability of a feature, overriding the configured
   * default.
   *
   * Needed because a feature can be configured on and still be unusable — the
   * batch queue is enabled by config whenever DISABLE_REDIS is unset, but a
   * Redis that refuses connections leaves it `unavailable`. Startup calls this
   * after initialization so the health payload reports what is actually true
   * rather than what was requested.
   */
  public setFeatureAvailability(feature: string, available: boolean): void {
    this.runtimeFeatureOverrides[feature] = available;
  }

  /**
   * Configured features with runtime availability applied.
   *
   * Single accessor on purpose. Features are surfaced on two separate
   * surfaces — `getDeploymentInfo()` and the `features` health check — and
   * the first version of this override only reached one of them, so
   * `/health/detailed` could still report a feature as enabled while the
   * subsystem behind it was unreachable. Every reader goes through here.
   */
  private effectiveFeatures(): Record<string, boolean> {
    return { ...this.deploymentConfig.features, ...this.runtimeFeatureOverrides };
  }
...
export const defaultDeploymentConfig: DeploymentConfig = {
  ...
  features: {
    ...
    rateLimiting: true,
    compression: true,
    // Configured capability, not a hardcoded claim. This was `true`
    // unconditionally while the subsystem was unreachable — the health payload
    // advertised batch processing that no route or binding could serve.
    // Startup refines this via setFeatureAvailability() once the queue's real
    // availability is known, so an enabled-but-unreachable Redis reports false.
    batchProcessing: !env.DISABLE_REDIS,

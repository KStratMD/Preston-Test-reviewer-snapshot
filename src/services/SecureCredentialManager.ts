...
function safeCredentialFailureMessage(error: unknown, fallback: string, safeDetail = 'credential operation failed'): string {
  const message = error instanceof Error ? error.message : '';
  if (message === 'Encryption key not configured') {
    return `${fallback}: Encryption key not configured`;
  }
  if (message.startsWith('Unsupported credential type:')) {
    return `${fallback}: Unsupported credential type`;
  }
  return `${fallback}: ${safeDetail}`;
}
...
export class SecureCredentialManager {
  ...
  async storeCredentials(
    ...
    try {
      ...
      throw new Error(
        safeCredentialFailureMessage(error, `Failed to store credentials for ${systemType}:${systemId}`),
        { cause: error },
      );
    }
  }
  ...
  async getCredentials(systemType: string, systemId: string): Promise<AuthConfig> {
    ...
    try {
      ...
      throw new Error(
        safeCredentialFailureMessage(
          error,
          `Failed to retrieve credentials for ${systemType}:${systemId}`,
          'Invalid credentials',
        ),
        { cause: error },
      );
  ...
  async deleteCredentials(systemType: string, systemId: string): Promise<void> {
    ...
    try {
      await this.secretManager.deleteSecret(credentialKey);
  ...
  async migrateFromEnvironment(): Promise<{ migrated: number; errors: string[] }> {
    ...
    for (const mapping of environmentMappings) {
      try {
        ...
        const errorMsg = `Failed to migrate ${mapping.systemType}:${mapping.systemId}`;
        errors.push(errorMsg);
        this.logger.error('Failed to migrate environment credentials', {
          systemType: mapping.systemType,
          systemId: mapping.systemId,
          error,
        });

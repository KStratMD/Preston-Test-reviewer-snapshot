...
export class IntegrationServiceExample {
  ...
  constructor(
    ...
    // Prerequisite PR B — required by ConnectorManager so a managed
    // (secret_manager/environment) system reference is actually resolvable.
    credentialResolver: ConnectorCredentialResolver,
  ) {
    this.logger = logger;

    // Create specialized services
    this.connectorManager = new ConnectorManager(logger, authService, outboundGovernance, credentialResolver);

...
export class IntegrationExecutor {
  ...
  async executeSync(config: IntegrationConfig, options: SyncOptions = {}): Promise<SyncResult> {
    ...
    // sync plan): this class is a SEPARATE, parallel execution engine from
    // `IntegrationService` and never went through that class's
    // `executionProfile` dispatch. Refuse rather than silently run the
    // generic per-record loop below against a specialized config — see
    // `SerializedAssetExecutionNotSupportedError`'s doc comment for why.
    // Thrown BEFORE the try/catch below so it propagates as a real
    // exception instead of being swallowed into a generic 'failed' result.
    if (config.executionProfile === 'netsuite_serialized_asset') {
      throw new SerializedAssetExecutionNotSupportedError(config.id, 'batch sync execution');
    }
    ...
    try {
      // Get connectors. sourceSystemType/targetSystemType (raw getSystemType
      // output) stay in the SourceSystem-mapping/logging shape below; the
      // connector cache/creation key MUST be the connector-registry key
      // (connectorKeyForSystem) — the same projection ConnectorManager itself
      // uses in initializeConnectorsForConfig — or a mixed-case config (e.g.
      // 'NetSuite') retrieves/creates a SECOND, never-initialized connector
      // instance instead of the one ConnectorManager already cached.
  ...
  async syncSingleRecord(
    ...
    // See executeSync's identical guard above for rationale.
    if (config.executionProfile === 'netsuite_serialized_asset') {
    ...
    try {
      // Get connectors. See executeSync's comment above the equivalent block:
      // the connector key must be connectorKeyForSystem()'s registry
      // projection, not the raw getSystemType() spelling.
      const sourceSystemType = getSystemType(config.sourceSystem);
      const targetSystemType = getSystemType(config.targetSystem);
      const sourceConnectorKey = connectorKeyForSystem(config.sourceSystem);
      const targetConnectorKey = connectorKeyForSystem(config.targetSystem);
  ...
  async testSync(config: IntegrationConfig, sampleSize = 5): Promise<{
    ...
    try {
      // Test connectivity. Only used for the connector cache/creation key
      // here, so project straight through connectorKeyForSystem() (see
      // executeSync's comment above the equivalent block).
      const sourceConnectorKey = connectorKeyForSystem(config.sourceSystem);
      const targetConnectorKey = connectorKeyForSystem(config.targetSystem);

      const sourceConnector = await this.connectorManager.getConnector(
        sourceConnectorKey,
        `${sourceConnectorKey}_test`
      );
      const targetConnector = await this.connectorManager.getConnector(
        targetConnectorKey,
        `${targetConnectorKey}_test`
      );

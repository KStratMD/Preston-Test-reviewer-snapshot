...
export class BatchProcessingService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.info('Batch processing service initialized', {
        queueAvailability: this.queueService.getAvailability(),
      });
    } catch (error) {
      // Boot-and-degrade, matching QueueService: startup must survive an
      // unreachable Redis because the one machine-required check boots the
      // production image without one. QueueService.initializeQueue already
      // absorbs connection failures and records `unavailable`; this catch is
      // the backstop for anything else, and it must not rethrow into
      // Server.start(). submitBatch() refuses on its own availability check,
      // so degrading here cannot silently accept work.
      this.logger.error('Failed to initialize batch processing service; batch processing unavailable', {
  ...
  async submitBatch(
    ...
    // Availability is checked BEFORE the small-batch branch on purpose. If it
    // came after, a caller running with Redis disabled would get a synthetic
    // success for any dataset under the batch size and a refusal for anything
    // over it — the same operation reporting two different contracts based on
    // input size. Refusing uniformly is the honest answer: this service is a
    // queue-backed API, and the queue is not available.
    if (!this.queueService.isReady()) {
      throw new ServiceUnavailableAppError(
        `Batch processing is unavailable (queue subsystem is ${this.queueService.getAvailability()})`,
      );
    }

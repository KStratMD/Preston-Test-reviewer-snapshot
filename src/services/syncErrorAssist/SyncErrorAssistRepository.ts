...
export class SyncErrorAssistRepository {
  ...
  /**
   * Read-only cross-domain accessor: the tenant's recorded ERP account, from
   * the newest non-retired `embedded_service_token_versions` row (owned by
   * `EmbeddedServiceTokenRepository`; written at provisioning by
   * `scripts/rotate-embedded-service-token.ts`). That row is currently the
   * ONLY per-tenant record binding a tenant to a NetSuite account — there is
   * no per-tenant ERP credential store yet — so the PR #966 Codex-P1
   * tenant-account guard (`buildNetSuiteEnvAuthConfigForTenant`) reads it
   * from here rather than adding a 15th constructor dependency to both Sync
   * Error Assist services. Newest-by-valid_from wins so a rotation's overlap
   * row (older valid_from, same 24h window) can't mask an account change.
   * Retirement is the only liveness filter: an expired-but-unretired row
   * still records the binding, which is what the guard compares against.
   */
  async getEmbeddedPlatformAccountId(tenantId: string, platform: string): Promise<string | null> {
    const row = await this.db.getDatabase()
      ...
      .limit(1)
      .executeTakeFirst();
    return row?.platform_account_id ?? null;
  }
}

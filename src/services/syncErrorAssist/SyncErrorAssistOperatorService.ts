...
export class SyncErrorAssistOperatorService {
  ...
  async accept(args: {
    ...
    let connectorFailureStage: 'getConnector_threw' | 'initialize_threw' = 'getConnector_threw';
    try {
      connector = await this.connectorManager.getConnector('netsuite', `netsuite_${args.tenantId}`);
      connectorFailureStage = 'initialize_threw';
      // ConnectorManager.getConnector() only constructs+caches a connector — it
      // never calls initialize() (see SyncErrorAssistService's runCycle call site
      // for the fuller gotcha writeup). Without this, ensureAuthenticated() throws
      ...
      // — or a tenant-account mismatch (Codex P1, PR #966), which surfaces under
      // the same 'initialize_threw' stage with its typed name in error_message —
      // degrades to the SAME 'connector_unavailable' outcome as a getConnector throw.
      if (connector) {
        await connector.initialize(await buildNetSuiteEnvAuthConfigForTenant(args.tenantId, this.repo));
      }
    } catch (err) {
      ...
      await this.safeAudit({
        ...
        details: {
          ...
          connector_error: connectorFailureStage,
        },
      });

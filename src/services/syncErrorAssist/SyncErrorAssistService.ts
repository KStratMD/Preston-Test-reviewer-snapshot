...
export class SyncErrorAssistService {
  ...
  private async runSingleErrorCycle(args: RunSingleErrorCycleArgs): Promise<ProcessClaimedRecordOutcome> {
    ...
    const sanitizeOpts = { dlpService: this.dlpService, correlationId, tenantId, logger: log, metrics: this.metrics };
    ...
    try {
      ...
      created = await withTimeout(
        nsConnector.create('customrecord_suitecentral_fix_suggestion', {
      ...
      try {
        ...
        // When findings exist but redactedData is missing (or the wrong type), fail-safe to
        // a placeholder rather than persisting/logging the raw PII-bearing message.
        const hasFindings = dlpResult.findings.length > 0;
        redactedMessage = typeof dlpResult.redactedData === 'string'
          ? dlpResult.redactedData
          : (hasFindings ? '[redaction-unavailable]' : errorMessage);
      } catch (dlpErr) {
      ...
      this.processClaimedRecord({
        ...
        void (async () => {
          ...
          try {
            ...
            // Decision tree:
            //   - scan.findings.length > 0 without scan.redactedData: DLP found PII but
            //     autoRedact:true didn't populate redactedData (downstream policy stage
            //     cleared it, or a custom producer returned findings without a redacted
            //     string). The raw message contains the unredacted PII → placeholder.
            //   - scan.findings.length > 0 with scan.redactedData: use the redacted string.
            //   - default (no findings): no PII found, the raw message is safe to keep.
            //   - scanText() throws on engine failure → caught by surrounding try/catch.
            const detected = scan.findings.length > 0;
            if (detected) {
              safeMessage = scan.redactedData ?? '[redaction-unavailable]';
            }
          } catch {

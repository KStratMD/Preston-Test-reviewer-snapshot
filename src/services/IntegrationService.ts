...
export class IntegrationService {
  ...
  async runIntegration(configId: string, options: SyncOptions = {}): Promise<SyncResult> {
    ...
    const config = this.configService.getConfiguration(configId);
    if (!config) {
  ...
  async syncSingleRecord(configId: string, recordId: string): Promise<SyncResult> {
    ...
    try {
      ...
      // Write to target
      await targetConnector.create(config.targetEntity, transformResult.transformedData);
      ...
      const processRecord = async (sourceRecord: DataRecord): Promise<void> => {
        try {
          ...
          if (options.dryRun) {
            ...
            await this.syncRecord(targetConnector, config.targetEntity, transformResult.transformedData);
            recordsSuccessful++;
          }
        } catch (error) {
          recordsFailed++;
  ...
  private async syncRecord(
    ...
    if (existingRecord) {
      ...
      await targetConnector.update(targetEntity, existingRecord.id!, transformedRecord);
      this.logger.debug(`Updated record ${existingRecord.id} in target system`);
    } else {
      // Create new record
      const createdRecord = await targetConnector.create(targetEntity, transformedRecord);
      this.logger.debug(`Created record ${createdRecord.id} in target system`);
    }
  }

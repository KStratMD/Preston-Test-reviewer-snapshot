...
import { BadRequestAppError, ConflictAppError, ServiceUnavailableAppError } from '../errors/AppError';
import { ConnectorCredentialResolutionError } from './integration/ConnectorCredentialResolver';
import { SerializedAssetExecutionNotSupportedError } from '../errors/SerializedAssetExecutionNotSupportedError';
...
import type {
  SerializedAssetSyncActor,
  SerializedAssetSyncOptions,
  SerializedAssetSyncResult,
  SerializedAssetSyncService,
} from './serializedAsset/SerializedAssetSyncService';
...
/**
 * Maps the ordinary `SyncOptions` bag (as passed to `runIntegration`/
 * `runIntegrationForTenant`) to the specialized profile's bounded options
 * (Task 8, 2026-07-27 NetSuite serialized-asset sync plan).
 *
 * `forceDeferredRetry` is deliberately NEVER read from `options` here and is
 * always `false`: the existing `POST /api/integrations/:id/run` route spreads
 * `req.body` directly into `SyncOptions` with no schema validation, so
 * honoring a caller-supplied flag at this layer would let any ordinary tenant
 * caller bypass a deferred unit's `next_attempt_at` (decision 11) today, well
 * before Task 9's verified tenant-administrator endpoint exists to be the
 * actual trust boundary for that bypass. `SerializedAssetSyncService.run()`
 * bounds `batchSize`/`concurrency` again internally, so no upper bound needs
 * to be enforced a second time here.
 */
function normalizeSerializedAssetOptions(options: SyncOptions): SerializedAssetSyncOptions {
  return {
    batchSize: options.batchSize ?? INTEGRATION_CONSTANTS.DEFAULT_BATCH_SIZE,
    concurrency: options.concurrency || parseInt(process.env.MAX_RECORD_CONCURRENCY || '5', 10),
...
/**
 * Derives the specialized profile's `guardedWrite`/audit actor from
 * SERVER-OWNED context ONLY (review-round-two fix, 2026-07-27 NetSuite
 * serialized-asset sync plan). `options.actorUserId`/`.correlationId` are
 * declared on `SyncOptions` for a future authenticated caller to eventually
 * populate, but neither is read here — deliberately, by construction, not by
 * discipline: the existing `POST /api/integrations/:id/run` route spreads
 * `req.body` directly into `SyncOptions` with no schema validation, so
 * honoring either field would let an ordinary caller write an
 * ATTACKER-CHOSEN principal into `governance_approvals.requester_user_id`
 * and `AuditService.logDataAccess` — the exact chokepoints decisions 5 and 8
 * rest on. This is not merely "audit misattribution": it corrupts the
 * governance/DLP audit trail those decisions depend on being trustworthy.
 *
 * Until a verified-identity path is threaded through this service (not yet
 * plumbed — `IntegrationService` never sees `req.user`; that would need a new,
 * properly-typed constructor input, not a generic options bag), the actor's
 * `userId` is always `SYSTEM_IDENTITY.userId` and its `correlationId` is
 * always the server-generated `syncId`. `tenantId` is the one field genuinely
 * safe to derive from caller-adjacent state, because it comes from the
 * ALREADY tenant-scope-resolved `config` (resolved via `getConfigurationForTenant`
 * before this is ever called), not from `options`.
 */
function deriveSerializedAssetActor(config: IntegrationConfig, syncId: string): SerializedAssetSyncActor {
  return {
...
/**
 * Adapts `SerializedAssetSyncService`'s rich, mode-discriminated result to the
 * generic `SyncResult` shape the rest of `IntegrationService` (status
 * tracking, metrics, logging, `IntegrationStatus.lastSyncResult`) already
 * expects — so none of that shared machinery needs to become aware of a
 * second result shape. Nothing is lost: the full specialized result (counts,
 * `mode`, `truncated`) is preserved verbatim under `metadata`, and — per
 * `SerializedAssetSyncResult`'s own privacy guarantee (decision 8) — it never
 * carries a serial number or any other unit datum, so embedding it here is
 * safe.
 */
function mapSerializedAssetResultToSyncResult(
  config: IntegrationConfig,
  syncId: string,
  startTime: Date,
  result: SerializedAssetSyncResult,
): SyncResult {
  const recordsSuccessful = result.mode === 'executed' ? result.upserted : result.wouldUpsert;
  const recordsFailed = result.failed;
  const status: SyncResult['status'] =
    recordsFailed === 0 ? 'success' : recordsSuccessful > 0 ? 'partial' : 'failed';
  ...
  return {
    ...
    recordsProcessed: result.unitsRead,
    recordsSuccessful,
    recordsFailed,
    errors: result.failures.map(failure => failure.category),
    startTime,
    endTime: new Date(),
    dryRun: result.mode === 'previewed',
    metadata: { serializedAssetResult: result },
  };
}
...
export interface SyncOptions {
  ...
  concurrency?: number;
  /**
   * Reserved for a future verified-identity actor for the
   * `netsuite_serialized_asset` execution profile's guardedWrite/audit
   * attribution (Task 8, 2026-07-27 NetSuite serialized-asset sync plan).
   * **NOT currently honored** — see `deriveSerializedAssetActor`, which never
   * reads this field. The existing `POST /api/integrations/:id/run` route
   * spreads `req.body` directly into `SyncOptions` with no schema validation,
   * so trusting this field here would let an ordinary caller write an
   * attacker-chosen principal into the governance/DLP audit chain (review
   * round two, IMPORTANT 2). A future verified-identity path must thread a
   * properly-typed value through this service, not through this generic bag.
   */
  actorUserId?: string;
  /**
   * Reserved for a future server-derived correlation id. **NOT currently
   * honored** — see `deriveSerializedAssetActor`, which always uses the
   * locally-generated `syncId` instead, for the same reason as
   * `actorUserId` above.
   */
  correlationId?: string;
  /**
   * Decision 11 (2026-07-27 NetSuite serialized-asset sync plan): an early
   * retry that bypasses a deferred unit's `next_attempt_at`. This field is
   * NEVER honored directly by `IntegrationService` — see
   * `normalizeSerializedAssetOptions`, which always forces it to `false`.
   * The existing `/api/integrations/:id/run` route spreads `req.body`
   * directly into `SyncOptions` with no schema validation, so trusting a
   * caller-supplied value here would let any ordinary caller bypass the
   * retry ceiling today. Task 9's verified tenant-administrator endpoint is
   * the only intended trust boundary for this bypass and must set it through
   * a path that does not go through the ordinary run dispatch options bag.
   */
...
export class IntegrationService {
  ...
  // `netsuite_serialized_asset` profile dispatch's own connector/execution
  // dependencies. Both required (fail-closed) so a DI wiring gap can never
  // silently fall through to standard execution for a specialized config.
  private readonly connectorManager: ConnectorManager;
  private readonly serializedAssetSyncService: SerializedAssetSyncService;
  ...
  constructor(
    ...
    @optional() @inject(TYPES.ConnectorManager) connectorManager?: ConnectorManager,
    @optional() @inject(TYPES.SerializedAssetSyncService) serializedAssetSyncService?: SerializedAssetSyncService,
  ) {
    ...
    if (!connectorManager) {
      throw new Error('ConnectorManager is required for IntegrationService specialized-profile dispatch');
    }
    this.connectorManager = connectorManager;
    if (!serializedAssetSyncService) {
      throw new Error('SerializedAssetSyncService is required for IntegrationService specialized-profile dispatch');
    }
    this.serializedAssetSyncService = serializedAssetSyncService;
  ...
  private async testIntegrationWithOptionalTenant(tenantId: string | undefined, configId: string): Promise<{
    ...
    try {
      ...
      // Test connector connections. Both sides are resolved and initialized by
      // the manager chokepoint before either is tested (A2).
      const { sourceConnector, targetConnector } = await this.resolveStandardConnectorPair(config);
  ...
  private async syncSingleRecordWithOptionalTenant(tenantId: string | undefined, configId: string, recordId: string): Promise<SyncResult> {
    ...
    // Review round two, IMPORTANT 1 (2026-07-27 NetSuite serialized-asset
    // sync plan): this method is a SECOND execution surface — reachable from
    // the authenticated `POST /api/integrations/:id/sync-record` — that never
    // went through `executeSync`'s profile dispatch. Without this check it
    // reads `config.sourceAuthentication`/`.targetAuthentication` inline (the
    // exact managed-credentials bypass Task 8's dispatch was built to avoid)
    // and, if the connectors happened to be initialized (e.g. a config
    // restored by `ConfigurationService.loadConfigurations`, which does NOT
    // re-validate against the schema that would otherwise forbid inline auth
    // beside a managed reference), issues a generic `create()` — prohibited
    // by decision 4, bypassing the decision-7 readiness gate and the
    // decision-8 privacy construction, with a serial number in the payload.
    // Refuse unconditionally rather than rely on the schema/uninitialized-
    // connector accident as the only mitigation.
    if (config.executionProfile === 'netsuite_serialized_asset') {
      throw new SerializedAssetExecutionNotSupportedError(config.id, 'single-record sync');
    }

    // A config can declare a managed credential reference WITHOUT the
    // specialized profile gated above. A2 routes this surface through the
    // manager chokepoint, so such a reference is resolved (and its tenant
    // ownership asserted) rather than refused.
    const { sourceConnector, targetConnector } = await this.resolveStandardConnectorPair(config);

    // Governance attribution reads the RAW system spelling, deliberately NOT
    // the projected connector key. `toSourceSystem()` maps into the
    // SourceOfTruth manifest vocabulary and names the unmapped value in its
    // refusal; projecting first would rewrite 'Dynamics365' to the registry key
    // 'dynamics' and change both the ownership decision's inputs and the text
    // an operator reads. Connector SELECTION projects, ownership attribution
    // does not — the two keys are allowed to differ (A2).
      ...
      await guardedWrite(
        {
          context: {
            ...
            // Derived, not defaulted (Task B4b): the requester is the process itself.
  ...
  /**
   * Dispatch point (Task 8, 2026-07-27 NetSuite serialized-asset sync plan).
   * Everything shared — config resolution, tenant isolation, active-state and
   * running-state checks, status tracking, and observability — lives in
   * `runIntegrationWithOptionalTenant` and runs identically for both branches
   * BEFORE this method is ever called. Only the execution itself forks here,
   * so a `netsuite_serialized_asset` config never falls through to the
   * generic per-record connector loop below, and a `standard`/`undefined`
   * config never touches the specialized orchestrator.
   */
  private async executeSync(config: IntegrationConfig, options: SyncOptions): Promise<SyncResult> {
    if (config.executionProfile === 'netsuite_serialized_asset') {
      return this.executeSerializedAssetSync(config, options);
    }
    return this.executeStandardSync(config, options);
  }

  /**
   * The specialized `netsuite_serialized_asset` profile's execution path.
   *
   * Connectors are RETRIEVED — not initialized — through `ConnectorManager`
   * here, never through this class's own `getConnector`/registry-factory path
   * (that path only ever reads `config.sourceAuthentication`/
   * `.targetAuthentication` inline and never resolves a managed
   * `secret_manager` reference, which this profile requires). Initialization
   * itself is deliberately left to `SerializedAssetSyncService.run()`'s
   * non-overrideable readiness re-check (decision 7), which unconditionally
   * calls `ConnectorManager.initializeConnectorsForConfig` — the single
   * funnel that resolves managed credentials and enforces
   * `TenantSystemCredentialRegistry`'s tenant-ownership check — before any
   * read reaches the network (see `SerializedAssetReadinessService.discover`'s
   * doc comment). Calling it again here would just repeat that authentication
   * for no benefit, and would do so even on a run that ultimately fails
   * readiness and never reads anything.
   */
  private async executeSerializedAssetSync(config: IntegrationConfig, options: SyncOptions): Promise<SyncResult> {
    const startTime = new Date();
    const syncId = `serialized_asset_sync_${Date.now()}_${Math.random().toString(36).slice(2, 2 + INTEGRATION_CONSTANTS.SYNC_ID_LENGTH)}`;
    ...
    const sourceConnector = await this.connectorManager.getConnector(
      sourceConnectorKey,
      `${sourceConnectorKey}_${config.id}`,
    );
    const targetConnector = await this.connectorManager.getConnector(
      targetConnectorKey,
      `${targetConnectorKey}_${config.id}`,
    );

    // Fail fast on a misconfigured target system before assembling the run
    // input: `SerializedAssetSyncService.run()` asserts this too, but doing it
    // here as well keeps the failure attributable to connector shape rather
    // than something deeper in the orchestrator.
    assertSalesforceSerializedAssetReadCapabilities(targetConnector);

    const result = await this.serializedAssetSyncService.run({
      config,
      sourceConnector,
      targetConnector,
      options: normalizeSerializedAssetOptions(options),
      actor: deriveSerializedAssetActor(config, syncId),
    });

    return mapSerializedAssetResultToSyncResult(config, syncId, startTime, result);
  }

  /**
   * Task 9 (2026-07-27 NetSuite serialized-asset sync plan): the ONLY
   * production caller that ever constructs a `SerializedAssetSyncOptions`
   * with `forceDeferredRetry: true` (decision 11).
   *
   * Deliberately NOT implemented by threading a caller-supplied flag through
   * `runIntegrationForTenant`'s `SyncOptions` bag. `POST
   * /api/integrations/:id/run` spreads `req.body` directly into that bag with
   * no schema validation, and `normalizeSerializedAssetOptions` therefore
   * unconditionally forces the flag to `false` — see that function's doc
   * comment. Reusing the same bag here would either (a) require relaxing that
   * hard force, handing every ordinary tenant caller the
   * `MAX_DEFERRAL_ATTEMPTS` bypass this method exists to gate, or (b) require
   * this method to smuggle the flag past the normalizer some other way, which
   * is just this method by a different name. So this is a genuinely separate
   * execution surface: its own tenant/active/running checks, its own call
   * into `SerializedAssetSyncService.run()`, never routed through
   * `executeSync`'s profile dispatch.
   *
   * `actor` MUST be built by the caller from verified JWT claims only
   * (`req.user.id` for `userId`) — never from the request body. The route
   * handler (`POST /api/integrations/:id/serialized-assets/retry-deferred`)
   * is the only production caller and is gated by
   * `requireIntegrationTenantAdmin`.
   */
  /**
   * A9 — the request-time half of the forced-retry checks, split out so the
   * durable reservation can run them WITHOUT starting the drain.
   *
   * Only the checks whose answer is a property of the request itself live here:
   * does the configuration exist for this tenant, is it the right execution
   * profile, is it active. Those must still fail synchronously with the route's
   * bounded 404/400 vocabulary, because the caller asked for something that
   * cannot succeed no matter when it runs.
   *
   * The "already running" check deliberately does NOT live here — see
   * `retryDeferredSerializedAssetsForTenant`, which still makes it at execution
   * time where it is actually meaningful.
   */
  assertForcedRetryEligible(tenantId: string, configId: string): void {
    const config = this.resolveConfiguration(configId, tenantId);
    if (!config) {
      // NotFoundError so the route maps cross-tenant/missing configs to 404,
      // consistent with every other tenant-scoped entry point in this class.
      throw new NotFoundError(`Configuration ${configId} not found`);
    }
    if (config.executionProfile !== 'netsuite_serialized_asset') {
      // This surface exists ONLY to bypass the deferred-retry ceiling of the
      // specialized profile (decision 11) — it has no meaning for any other
      // config and must not silently fall through to a generic run.
      throw new BadRequestAppError(
        `Configuration ${configId} does not use the netsuite_serialized_asset execution profile`,
      );
    }
    if (!config.isActive) {
      throw new BadRequestAppError(`Configuration ${configId} is not active`);
    }
  }

  async retryDeferredSerializedAssetsForTenant(
    tenantId: string,
    configId: string,
    actor: { userId: string; correlationId: string },
  ): Promise<SyncResult> {
    this.assertForcedRetryEligible(tenantId, configId);
    const config = this.resolveConfiguration(configId, tenantId)!;
    if (this.runningIntegrations.has(configId)) {
      throw new ConflictAppError(`Integration ${configId} is already running`);
    }
    ...
    try {
      const result = await this.executeSerializedAssetForcedRetry(config, actor);
      this.updateIntegrationStatus(configId, {
        isRunning: false,
        lastSync: new Date(),
        lastSyncResult: result,
  ...
  /**
   * The forced-retry execution itself. Mirrors `executeSerializedAssetSync`'s
   * connector resolution and readiness-capability check, but builds its own
   * `SerializedAssetSyncOptions` (`forceDeferredRetry: true`, decision 11) and
   * its own actor from the VERIFIED caller passed in — never
   * `deriveSerializedAssetActor`'s `SYSTEM_IDENTITY` default, and never
   * `normalizeSerializedAssetOptions`, which would force the flag back to
   * `false`.
   */
  private async executeSerializedAssetForcedRetry(
    config: IntegrationConfig,
    actor: { userId: string; correlationId: string },
  ): Promise<SyncResult> {
    const startTime = new Date();
    const syncId = `serialized_asset_forced_retry_${Date.now()}_${Math.random().toString(36).slice(2, 2 + INTEGRATION_CONSTANTS.SYNC_ID_LENGTH)}`;
    ...
    const options: SerializedAssetSyncOptions = {
      batchSize: INTEGRATION_CONSTANTS.DEFAULT_BATCH_SIZE,
      concurrency: parseInt(process.env.MAX_RECORD_CONCURRENCY || '5', 10),
      dryRun: false,
  ...
  private async executeStandardSync(config: IntegrationConfig, options: SyncOptions): Promise<SyncResult> {
    // Standard and specialized profiles now share ONE credential-resolution
    // route (A2). The specialized profile still keeps its own dispatch above
    // because it hands initialization to `SerializedAssetSyncService.run()`'s
    // non-overrideable readiness re-check; what it no longer has is a
    // privileged claim on managed references.
    const { sourceConnector, targetConnector } = await this.resolveStandardConnectorPair(config);
  ...
  /**
   * The single chokepoint every STANDARD execution path (run, test,
   * single-record, boot initialize) uses to obtain its source/target connector
   * pair. PR A2 (deployment-readiness Tranche A).
   *
   * Before A2 those paths created connectors from this class's own registry
   * projection and initialized them from `config.sourceAuthentication` /
   * `.targetAuthentication` inline. That could neither resolve a managed
   * (`secret_manager`) reference nor enforce tenant ownership, so
   * `assertNoManagedCredentialReference` refused managed configurations
   * outright — and refusing was correct, because the alternative (skipping
   * `initialize()`) would have served the run from a per-process cached
   * connector still holding the configuration's OLD inline credentials, with no
   * ownership assertion anywhere.
   *
   * Routing through `ConnectorManager` removes the cause rather than the
   * symptom: it is the single funnel that resolves through
   * `ConnectorCredentialResolver` (which asserts
   * `TenantSystemCredentialRegistry` ownership BEFORE fetching any secret) and
   * that also applies `assertDistinguishableManagedSides`. It is the same route
   * the specialized `netsuite_serialized_asset` profile already takes, so there
   * is exactly one credential-resolution pipeline in this class, not two.
   *
   * ORDER IS LOAD-BEARING: initialize first, retrieve second. A resolver
   * rejection therefore propagates before any connector is handed to a caller,
   * so a refused configuration can never reach a read or a write.
   */
  private async resolveStandardConnectorPair(config: IntegrationConfig): Promise<{
    sourceConnector: IConnector;
    targetConnector: IConnector;
  }> {
    // Projected BEFORE the guarded call so an unrecognized system spelling
    // keeps surfacing as its own unmapped-system failure rather than being
    // recategorized as a credential-resolution outage below.
    const sourceConnectorKey = connectorKeyForSystem(config.sourceSystem);
    const targetConnectorKey = connectorKeyForSystem(config.targetSystem);

    await this.initializeConnectorPairThroughManager(config);
    ...
    return { sourceConnector, targetConnector };
  }

  /**
   * Credential resolution reaches a real secret store, so an unclassified
   * failure from it can carry secret material in its message — and this
   * class's callers put thrown text into client-visible results, logs, and
   * audit payloads. Bound it here, once.
   *
   * Everything that is NOT an unclassified resolver failure passes through
   * untouched: `CrossTenantCredentialError` (403),
   * `ServiceUnavailableAppError` (503) and `ConflictAppError` (409) are the
   * deliberate, value-free refusals `ConnectorCredentialResolver`,
   * `TenantSystemCredentialRegistry` and `assertDistinguishableManagedSides`
   * already emit; re-wrapping them would turn an actionable authorization
   * answer into an "upstream is down" lie. Anything else becomes a fixed 503
   * naming only the configuration id, and the log records the CAUSE's class
   * name rather than any message so a failure is still diagnosable without a
   * value ever being written down. The cause, not the wrapper: every failure
   * reaching here is a `ConnectorCredentialResolutionError` by construction,
   * so logging the wrapper's own name would be a constant string and tell an
   * operator nothing.
   */
  private async initializeConnectorPairThroughManager(config: IntegrationConfig): Promise<void> {
    try {
      await this.connectorManager.initializeConnectorsForConfig(config);
    } catch (error) {
      // ONLY an unclassified credential-resolution failure is masked. An
      // `IConnector.initialize()` failure travels this same call, but its text
      // ('Token expired') is an ordinary connector/auth diagnostic that callers
      // have always received, and A2 has no mandate to start hiding it —
      // masking it would also misreport it, since the credentials resolved
      // fine and the connector rejected them.
      if (!(error instanceof ConnectorCredentialResolutionError)) throw error;

      this.logger.error('Connector credential resolution failed', {
        configurationId: config.id,
        tenantId: config.tenantId,
        errorClass: error.cause instanceof Error ? error.cause.name : 'Unknown',
      });
      throw new ServiceUnavailableAppError(
        `Connector credentials for configuration ${config.id} could not be resolved`,
      );
    }
  }
  ...
  private async initializeConnectorsForConfig(config: IntegrationConfig): Promise<void> {
    try {
      await this.resolveStandardConnectorPair(config);

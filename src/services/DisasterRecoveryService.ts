...
export class DisasterRecoveryService extends EventEmitter {
   ...
   *
   * `context` is REQUIRED (Task 8, `bulk_restore` operation kind): a restore can
   * reactivate configurations, so it crosses the same cardinality activation
   * gate as `ConfigurationService.importAll`. There is no HTTP request behind
   * this admin/background operation, so — unlike a route — nothing here can
   * derive a trusted context on its own; the caller (an authenticated
   * administrative-job/operator surface) must supply one built from verified
   * identity.
   */
  public async restoreFromBackup(backupId: string, context: ConfigurationCommandContext): Promise<void> {
    ...
    try {
      ...
      if (backupData.configurations) {
        await this.restoreConfigurations(backupData.configurations, context);
   ...
   *
   * `context` is REQUIRED (Task 8) — threaded to the `restoreFromBackup` call
   * failover performs internally; see that method's doc for why this operation
   * cannot derive trusted identity on its own.
   */
  public async performFailover(targetEnvironment: string, context: ConfigurationCommandContext): Promise<void> {
    ...
    try {
      ...
      if (latestBackup) {
        await this.restoreFromBackup(latestBackup.id, context);
      }
  ...
  private async restoreConfigurations(data: unknown, context: ConfigurationCommandContext): Promise<void> {
    await this.configService.importAll(data, context);
  }

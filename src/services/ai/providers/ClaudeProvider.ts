 ...
 * Anthropic Claude 3.5 Sonnet integration for premium tier routing
...
export class ClaudeProvider extends BaseProvider implements AIProvider {
  ...
  async chat(messages: ChatMessage[], options?: {
    ...
    try {
      ...
      if (result.usage) {
        const inputCost = result.usage.input_tokens * this.getCostPerToken();
        const outputCost = result.usage.output_tokens * this.getCostPerToken() * 3;
        this.lastTokenUsage = {
  ...
  private async callClaude(prompt: string, options: { maxTokens: number; temperature: number }, ctx?: IdentityContext): Promise<ClaudeResponse> {
    ...
    if (result.usage) {
      const inputCost = result.usage.input_tokens * this.getCostPerToken();
      const outputCost = result.usage.output_tokens * this.getCostPerToken() * 3; // Output tokens cost 3x more
      this.lastTokenUsage = {
        inputTokens: result.usage.input_tokens,
        outputTokens: result.usage.output_tokens,
        totalTokens: result.usage.input_tokens + result.usage.output_tokens,

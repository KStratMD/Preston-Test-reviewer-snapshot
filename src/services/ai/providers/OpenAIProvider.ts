 ...
 * Enhanced with current OpenAI model support and cost tracking
...
function openAICompletionTokenLimitParam(model: string, maxTokens: number): { max_tokens: number } | { max_completion_tokens: number } {
...
export class OpenAIProvider extends BaseProvider implements AIProvider {
  ...
  constructor(
    ...
    // Normalize the legacy 'gpt-4' alias to gpt-4o at the single construction
    // chokepoint, so every caller (DI binding, factories, init paths) is
    // consistent without each duplicating the check. Mirrors setModel().
    if (this.config.model === 'gpt-4') {
  ...
  async chat(messages: ChatMessage[], options?: {
    ...
    try {
      ...
      const body = {
        ...
        ...openAICompletionTokenLimitParam(this.config.model, options?.maxTokens || this.config.maxTokens || 1000),
      ...
      if (result.usage) {
        this.lastTokenUsage = {
          ...
          estimatedCost: this.estimateUsageCostUSD(result.usage.prompt_tokens, result.usage.completion_tokens)
        };
      }
   ...
   * every cost site so the two request paths cannot diverge — divergence
   * between them was a real cost-understatement bug.
   */
  private estimateUsageCostUSD(promptTokens: number, completionTokens: number): number {
    const accurate = tableCostUSD(this.config.model, promptTokens, completionTokens);
    if (accurate !== null) return accurate;
    const perToken = this.getCostPerToken();
    return promptTokens * perToken + completionTokens * perToken * 2;
  }
  ...
  private getCostPerToken(): number {
    ...
    if (m.includes('gpt-5.4-nano')) return 0.0000002;
    if (m.includes('gpt-5.4-mini')) return 0.00000075;
    if (m.includes('gpt-5.4')) return 0.0000025;
  ...
  private async callOpenAI(prompt: string, options: { maxTokens: number; temperature?: number }, ctx?: IdentityContext): Promise<OpenAIResponse> {
    ...
    const body = {
      ...
      ...openAICompletionTokenLimitParam(this.config.model, options.maxTokens),

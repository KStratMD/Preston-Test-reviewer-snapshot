...
export class NetSuiteSchemaIntelligence {
  ...
  getRecordRelationships(recordType: string): NetSuiteRelationship[] {
    ...
    case 'inventorynumber':
      // Task 3 (2026-07-27 NetSuite serialized-asset sync plan): the only
      // catalogued edge for this entity — an inventorynumber row's parent
      // item is a mandatory (non-nullable) NetSuite reference field.
      relationships.push(
        { field: 'item', relatedRecord: 'item', type: 'parent', required: true },
      );
      break;
    }

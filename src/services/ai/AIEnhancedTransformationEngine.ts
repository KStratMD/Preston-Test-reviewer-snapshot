      ...
      if (aiOptions.enableAutoMapping) {
        ...
        if (aiOptions.autoAcceptHighConfidence) {
          ...
          if (autoApplied.count > 0) {
            ...
            // Codex on PR #1253: a failed re-transformation (an auto-generated
            // mapping that duplicates an existing target, a structural error)
            // used to be merged and reported as success. Fail closed: carry
            // its errors, mark the result failed, and merge nothing.
            baseResult.warnings.push(...updatedResult.warnings);
            if (!updatedResult.success) {
              baseResult.success = false;
              baseResult.errors.push(...updatedResult.errors);
              learningInsights.push(
                `Auto-applied mappings were rejected by the transformation engine: ${updatedResult.errors.map((e) => `${e.field}: ${e.message}`).join('; ')}`,
              );
            } else {
              // Merge results

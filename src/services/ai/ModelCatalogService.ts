...
export class ModelCatalogService {
  ...
  const data = await resp.json() as { data?: { id: string }[] };
    ...
    const allowed = ['gpt', 'o', 'text', 'gpt-4', 'gpt-4o'];
    ...
    const recommended = ['gpt-4o', 'gpt-4o-mini'];
    for (const r of recommended) {
      if (!models.find(m => m.id === r)) models.push({ id: r, provider: 'openai', family: 'gpt-4o', recommended: true });
    }
    return models;
  }
    ...
    switch (provider) {
      ...
      case 'anthropic':
        return [
          ...
          { id: 'claude-3-5-sonnet-20241022', provider: 'anthropic', family: 'claude-3-5', recommended: true, releasedAt: '2024-10-22' },

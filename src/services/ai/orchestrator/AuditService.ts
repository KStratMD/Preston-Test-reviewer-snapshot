...
// ---------------------------------------------------------------------------
// Cardinality preflight/activation gate audits (docs/superpowers/specs/
// 2026-07-26-cardinality-preflight-design.md, "Activation semantics" +
// "Audited override"). Two distinct action types per that design:
//   - `cardinality_activation_decision`: the mandatory pre-decision record,
//     written once PER DIRECTION before any active-state mutation.
//   - `cardinality_activation_outcome`: a separate, later record of whether
//     the resulting save actually succeeded or failed.
// Neither ever carries sample data, raw key values, or credentials — only
// the report fingerprint, finding KEYS, and (for an override) its reason and
// scope, all sourced from server-trusted command context.
// ---------------------------------------------------------------------------

/** The policy outcome the pre-decision audit row records. */
export type CardinalityActivationDecision = 'allowed' | 'blocked' | 'overridden';

/** Server-authored override reason + finding-key scope, never client text alone. */
export interface CardinalityDecisionOverrideMetadata {
  reason: string;
  scope: string[];
}

export interface CardinalityActivationDecisionInput {
  tenantId: string;
  actorUserId: string;
  ...
  findingKeys: string[];
  decision: CardinalityActivationDecision;
  override?: CardinalityDecisionOverrideMetadata;
}

export interface CardinalityActivationOutcomeInput {
  tenantId: string;
  actorUserId: string;
  correlationId: string;
  configurationId: string;
  ...
  /** Non-sensitive failure reason (e.g. `'disk_write_rejected'`); never raw error text that might embed sample data. */
  reason?: string;
}
...
export class AuditService {
  ...
  constructor(
    ...
    // A2 (defense-in-depth): fail at wiring time, not at persist time, if the
    // outbound-governance dependency is missing — mirrors the BaseProvider /
    // BaseConnector constructor invariant so a DI regression can't silently
    // ship audit rows past the egress DLP chokepoint.
    if (!this.outboundGovernance) {
      throw new Error('OutboundGovernanceService is required for AuditService payload governance');
    }
  ...
  /**
   * Persists the mandatory pre-decision cardinality activation audit row for
   * ONE direction. Bidirectional configurations call this once per direction
   * (design: "one audited decision" per direction, never a combined row).
   *
   * `outcome.success` on the persisted row means the POLICY DECISION WAS
   * RECORDED — it is `true` even when `decision: 'blocked'`. It is never a
   * verdict on whether the subsequent disk save succeeded; that is the
   * separate row `logCardinalityOutcome` writes.
   *
   * Deliberately has no try/catch around the underlying write: a persistence
   * failure here MUST propagate so the caller (activation) can refuse to
   * proceed rather than mutate state without a durable decision record.
   */
  async logCardinalityDecision(data: CardinalityActivationDecisionInput): Promise<string> {
    const auditId = this.generateAuditId();

    const auditLog: AuditLog = {
      id: auditId,
      timestamp: this.now(),
      sessionId: data.configurationId,
      userId: data.actorUserId,
      event: {
        type: 'configuration_change',
        action: 'cardinality_activation_decision',
        resource: 'cardinality_activation',
        details: {
          ...
          ...(data.override
            ? { override: { reason: data.override.reason, scope: data.override.scope } }
            : {}),
        },
      },
      context: {
      ...
      outcome: {
        success: true,
        resultSummary: `Cardinality activation decision recorded: ${data.decision} (${data.direction})`,
        riskLevel: data.decision === 'overridden' ? 'high' : data.decision === 'blocked' ? 'medium' : 'low',
        governanceFlags: data.override ? ['cardinality_override'] : [],
        errors: [],
        warnings: [],
      },
      compliance: this.determineComplianceInfo('configuration_change'),
      retention: this.calculateRetentionInfo('configuration_change'),
    };
  ...
  /**
   * Persists a cardinality activation OUTCOME row for one direction: whether
   * the save that followed the decision actually succeeded or failed. Always
   * a separate row from `logCardinalityDecision` — never merged — so the
   * mandatory pre-decision record survives even if the outcome write fails.
   */
  async logCardinalityOutcome(data: CardinalityActivationOutcomeInput): Promise<string> {
    const auditId = this.generateAuditId();
    const succeeded = data.outcome === 'succeeded';
    ...
    const auditLog: AuditLog = {
      ...
      event: {
        type: 'configuration_change',
        action: 'cardinality_activation_outcome',
        resource: 'cardinality_activation',
        details: {
          correlationId: data.correlationId,
          direction: data.direction,
      ...
      outcome: {
        success: succeeded,
        resultSummary: `Cardinality activation ${data.outcome} (${data.direction})`,
        riskLevel: succeeded ? 'low' : 'high',
        governanceFlags: [],
        errors: succeeded ? [] : [data.reason ?? 'cardinality_activation_failed'],
        warnings: [],
      },
      compliance: this.determineComplianceInfo('configuration_change'),
      retention: this.calculateRetentionInfo('configuration_change'),

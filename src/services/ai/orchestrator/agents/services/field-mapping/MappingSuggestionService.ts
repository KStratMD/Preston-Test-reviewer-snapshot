...
export class MappingSuggestionService {
  ...
  private async generateProviderSuggestions(context: MappingContext): Promise<MappingSuggestionResult> {
    ...
    try {
      ...
      // NOTHING derived from a field name is logged here. `rawSuggestions` is
      // untouched provider output, and the `extractFieldName` call below exists
      // precisely because the model sometimes returns a sample VALUE inside a
      // field name (see its comment) — so this log, which runs BEFORE that
      // cleaning, is where the one known echo mechanism in this file emptied.
      // `sampleData` is real customer data on every integration, not only the
      // serialized-asset profile whose decision-8 rule forbids values in logs.
      //
      // Digesting the names was the first attempt and is not sufficient: an
      // echoed value is not reliably high-entropy (serials, phone numbers,
      // dates and structured ids are all dictionary- or range-recoverable), and
      // a plain SHA-256 is deterministic enough to correlate across tenants.
      // The confidence distribution is what this log is actually FOR — it says
      // how well the provider thought it did — and it derives from no field
      // value at all.
      this.logger.info(`✅ AI provider (${providerId}) returned ${rawSuggestions.length} suggestions`, {
        providerId,
        suggestionCount: rawSuggestions.length,
        topConfidencePcts: rawSuggestions.slice(0, 5).map(s => Math.round(s.confidence * 100)),
      });

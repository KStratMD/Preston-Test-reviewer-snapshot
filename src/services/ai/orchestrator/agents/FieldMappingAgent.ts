...
import { matchesSerializedAssetAdvisoryPair } from '../../../serializedAsset/SerializedAssetProfileValidator';
import { isReaderResolvableSourceField } from '../../../serializedAsset/NetSuiteSerializedUnitReader';
...
export class FieldMappingAgent extends BaseAgent {
  ...
  /** Task 10 (decision 10): the closed, fixed role vocabulary the netsuite_serialized_asset advisory recommendation always carries. */
  private static readonly SERIALIZED_ASSET_REQUIRED_ROLES: readonly ('inventory_number_id' | 'serial_number' | 'parent_item_id')[] = ['inventory_number_id', 'serial_number', 'parent_item_id'];
  private static readonly SERIALIZED_ASSET_OPTIONAL_ROLES: readonly ('status' | 'location')[] = [
    'status',
    'location',
  ];
  ...
  protected async executeInternal(
    ...
    try {
      ...
      // Task 10 (decision 10, advisory-only): deterministic, model-independent
      // — built ONLY from `input.executionProfileHint`, never from
      // `mappingSuggestions`/`enhancedMappings`/any model output, so profile
      // authority never lives in the model prompt.
      const executionProfileRecommendation = this.buildExecutionProfileRecommendation(
        input.executionProfileHint,
      );

      // Once this exact pair is recommended, drop any suggestion whose
      // sourceField the Task-3 reader (`NetSuiteSerializedUnitReader`) could
      // never resolve — an operator who later accepted such a path into the
      // profile's fieldMappings would see every record silently rejected at
      // runtime. Without a log, a drop here would be exactly as undiagnosable
      // as the runtime failure it exists to prevent.
      //
      // The log carries a COUNT and nothing derived from a field name. An
      // earlier version logged the raw pair, reasoning that decision 8 forbids
      // record VALUES while these are field NAMES — but that is an assumption
      // about model output that nothing enforces. `MappingValidationService`
      // copies `suggestion.sourceField` through verbatim with no check that it
      // names a real source field, and this agent is given `sampleData`. This
      // branch fires precisely WHEN the string is not field-shaped, so it is
      // the most likely place for an echoed sample value to arrive, not the
      // least.
      //
      // Digesting the pair was the next attempt and is also not enough: an
      // echoed value is not reliably high-entropy (serials, phone numbers,
      // dates and structured ids are dictionary- or range-recoverable), and a
      // plain SHA-256 correlates across tenants. A count answers the question
      // the log exists for — did the advisory silently shrink? — and is derived
      // from no model text whatsoever.
      let droppedUnresolvableCount = 0;
      const finalMappings = executionProfileRecommendation
        ? enhancedMappings.filter((mapping) => {
            const resolvable = isReaderResolvableSourceField(mapping.sourceField);
            if (!resolvable) {
              droppedUnresolvableCount += 1;
            }
            return resolvable;
          })
        : enhancedMappings;
      if (droppedUnresolvableCount > 0) {
        this.logger.warn(
          'Dropped reader-unresolvable sourceFields from netsuite_serialized_asset advisory recommendation',
          { droppedCount: droppedUnresolvableCount, retainedCount: finalMappings.length },
        );
      }

      // Advisory only: the activation-time cardinality gate stays authoritative.
      const cardinalityWarnedCount = finalMappings.filter(
        (mapping) => (mapping.cardinalityWarnings?.length ?? 0) > 0,
      ).length;

      // Calculate overall quality score
      const overallQuality = this.qualityService.calculateOverallQuality(finalMappings);

      // Generate recommendations
      const recommendations = this.qualityService.generateRecommendations(finalMappings, input);

      const output: FieldMappingOutput = {
        mappings: finalMappings,
        qualityScore: overallQuality,
        recommendations,
        alternatives: finalMappings.flatMap(m => m.alternatives || []),
      ...
      const reasoning = this.mergeReasoning([
        ...
        cardinalityWarnedCount > 0
          ? `Flagged ${cardinalityWarnedCount} mapping(s) with advisory cardinality warnings (advisory only; activation preflight is authoritative)`
          : '',
  ...
  /**
   * Task 10 deterministic projector (decision 10). Reads ONLY `hint` — a
   * caller-supplied, non-AI-derived system/entity pair (see
   * `ExecutionProfileHint`'s doc comment) — never `mappingSuggestions` or any
   * other model output, so a hostile/hallucinated suggestion can never
   * influence whether this recommends, or what it recommends. Delegates the
   * actual match rule to `matchesSerializedAssetAdvisoryPair`, the same
   * projection `evaluateSerializedAssetProfile`'s system/entity contract
   * uses, so the advisory surface can never drift from the activation-time
   * gate. Every returned value is a fixed literal or a member of a
   * closed-set role array — there is no property here an invented Salesforce
   * field name or an activation instruction could ever occupy.
   */
  private buildExecutionProfileRecommendation(
    hint: FieldMappingInput['executionProfileHint'],
    ...
    return {
      ...
      targetEntity: 'Asset',
      requiredMappingRoles: [...FieldMappingAgent.SERIALIZED_ASSET_REQUIRED_ROLES],
      optionalMappingRoles: [...FieldMappingAgent.SERIALIZED_ASSET_OPTIONAL_ROLES],
    };
  }

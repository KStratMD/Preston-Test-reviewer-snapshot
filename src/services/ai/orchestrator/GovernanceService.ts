...
export class GovernanceService {
  ...
  async validateInput(data: unknown, context: AgentExecutionContext): Promise<GovernanceResult> {
    try {
      ...
      if (this.config.enablePIIDetection) {
        ...
        if (piiResult.hasPII) {
          result.flags.push('pii_detected');
          result.riskLevel = 'high';

          if (this.config.autoRedactPII) {
            result.redactedData = this.redactPIIFromData(data, piiResult);
            result.flags.push('pii_auto_redacted');
          } else if (this.config.strictMode) {
            result.approved = false;
            result.reason = 'PII detected in input data';
          }
        }
      }
  ...
  async validateOutput(data: unknown, context: AgentExecutionContext): Promise<GovernanceResult> {
    try {
      ...
      if (this.config.enablePIIDetection) {
        const piiResult = await this.detectPII(data);
        if (piiResult.hasPII) {
          result.flags.push('output_pii_detected');
          result.riskLevel = 'high';

          // Always flag PII in output as it shouldn't be there
          if (this.config.strictMode) {
            result.approved = false;
            result.reason = 'PII found in agent output';
          }
        }
      }
  ...
  private async validateOutputQuality(
    ...
    if (typeof data === 'object' && (data as any).success === false && !(data as any).errors) {
      result.checks.push({

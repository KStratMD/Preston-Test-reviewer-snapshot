...
export interface ProposedAction {
    id: string;
    /** F4: owning tenant, stamped at propose; every store accessor scopes by it. */
    tenantId: string;
    /** F4: verified approver user id, retained on the in-memory action for audit/response. */
    approvedBy?: string;
    intent: ParsedIntent;
    apiCall: {
...
// Capacity is enforced PER TENANT with self-eviction only (Codex R2 on
// PR #1055): the previous global evict-oldest let one tenant's flood evict
// another tenant's pending/approved actions. Global size is structurally
// bounded at (active tenants × PER_TENANT_MAX_PENDING) — tenantIds come from
// verified JWTs, so an attacker cannot mint tenants — and entries expire
// within minutes via the periodic sweep. MAX_PENDING_ACTIONS is a soft cap
// that now only warns; it never evicts across tenants.
const MAX_PENDING_ACTIONS = 1000;
export const PER_TENANT_MAX_PENDING = 100;

/**
 * Thrown by proposeAction when the tenant's capacity is held entirely by
 * approved actions awaiting execution (Codex R3 on PR #1055) — evicting those
 * would silently break the approve → execute contract, so the NEW proposal is
 * refused instead. Router maps this to 429 capacity_exceeded.
 */
export class ActionCapacityExceededError extends Error {
    constructor(public readonly tenantId: string) {
        super(`NL action capacity exceeded for tenant (${PER_TENANT_MAX_PENDING} retained actions, all approved)`);
        this.name = 'ActionCapacityExceededError';
    }
}
...
export class NLActionGateService {
    ...
    private cleanupExpiredActions(): void {
        ...
        // Enforce the per-tenant cap (defense-in-depth — proposeAction already
        // self-evicts at the cap) and NEVER evict across tenants: tenant A's
        // flood must not remove tenant B's pending/approved actions (Codex R2
        // on PR #1055). Map iteration is insertion order, so each tenant's
        // list is oldest-first.
        // Status-aware (Codex R3): the expired/terminal prune above already
        // ran, so retained entries are pending or approved. Evict only
        // PENDING entries — approved actions awaiting execution are never
        // removed by capacity pressure, only by expiry.
        const byTenant = new Map<string, { total: number; pendingIds: string[] }>();
        for (const [actionId, action] of this.pendingActions) {
            const entry = byTenant.get(action.tenantId) ?? { total: 0, pendingIds: [] };
            entry.total++;
            if (action.status === 'pending') entry.pendingIds.push(actionId);
            byTenant.set(action.tenantId, entry);
        }
        for (const [tenantId, entry] of byTenant) {
            while (entry.total > PER_TENANT_MAX_PENDING && entry.pendingIds.length > 0) {
                this.pendingActions.delete(entry.pendingIds.shift() as string);
                entry.total--;
                removed++;
            }
            if (entry.total > PER_TENANT_MAX_PENDING) {
                this.logger.warn('[NLActionGate] Tenant over cap with only approved actions retained — no eviction', { tenantId, total: entry.total });
            }
        }
        if (this.pendingActions.size > MAX_PENDING_ACTIONS) {
            this.logger.warn('[NLActionGate] Store over global soft cap; per-tenant caps respected — no cross-tenant eviction', {
                size: this.pendingActions.size,
     ...
     * F4: `tenantId` is the VERIFIED caller tenant — every later store
     * accessor scopes by it (cross-tenant = not-found, never revealed).
     */
    proposeAction(intent: ParsedIntent, tenantId: string): ProposedAction {
        ...
        // Per-tenant capacity with SELF-eviction only: a tenant at its cap
        // evicts its own oldest entries; other tenants' records are never
        // touched (Codex R2 on PR #1055 — the old global evict-oldest let one
        // tenant flood another tenant's actions out of the store). Eviction is
        // STATUS-AWARE (Codex R3): expired/terminal entries are pruned first,
        // then only PENDING entries are evicted oldest-first — an approved
        // action awaiting execution is never silently removed. If the cap is
        // held entirely by approved actions, the NEW proposal is refused
        // instead (typed error → 429 at the router).
        const now = Date.now();
        const ownPendingIds: string[] = [];
        let ownRetained = 0;
        for (const [existingId, existing] of this.pendingActions) {
            if (existing.tenantId !== tenantId) continue;
            const terminal = existing.status !== 'pending' && existing.status !== 'approved';
            if (terminal || existing.expiresAt.getTime() < now) {
                this.pendingActions.delete(existingId);
                continue;
            }
            ownRetained++;
            if (existing.status === 'pending') ownPendingIds.push(existingId);
        }
        while (ownRetained >= PER_TENANT_MAX_PENDING) {
            const evictId = ownPendingIds.shift();
            if (evictId === undefined) {
                // Every retained entry is approved-awaiting-execute; refuse the
                // new proposal rather than evict an approved action.
                throw new ActionCapacityExceededError(tenantId);
            }
            this.pendingActions.delete(evictId);
            ownRetained--;
            this.logger.warn('[NLActionGate] Per-tenant capacity reached — evicted own oldest PENDING action', { tenantId, evictId });
        }
     ...
     * F4: tenant-scoped — a cross-tenant actionId behaves exactly like a
     * missing one (null), never revealing the action's existence.
     */
    approveAction(actionId: string, approverUserId: string, tenantId: string): ProposedAction | null {
        ...
        action.approvedBy = approverUserId;
     ...
     * F4: tenant-scoped — cross-tenant behaves like not-found (null).
     */
    rejectAction(actionId: string, tenantId: string, reason?: string): ProposedAction | null {
        const action = this.pendingActions.get(actionId);
        if (!action || action.tenantId !== tenantId) return null;

        // Only pending actions are rejectable — an approved/executed action must
        // not be retroactively invalidated via the reject endpoint (Copilot R1,
        // PR #1055). Mirrors approveAction's guard; router surfaces null as 404.
        if (action.status !== 'pending') {
            this.logger.warn('[NLActionGate] Action not pending:', { actionId, status: action.status });
            return null;
        }
     ...
     *
     * `ctx` is the request-scoped verified identity — passed through to
     * dispatch so backend operator services see the real caller (Codex R1
     * BM-3). F4: a tenant match against the action is REQUIRED — a missing
     * ctx or a cross-tenant ctx fails closed as not_found BEFORE the status
     * check, so cross-tenant probes cannot distinguish action states.
     */
    async executeAction(actionId: string, ctx?: IdentityContext): Promise<ActionResult> {
        const action = this.pendingActions.get(actionId);
        if (!action || !ctx || action.tenantId !== ctx.tenantId) {
     ...
     * F4: tenant-scoped — only the caller tenant's actions are returned.
     */
    getPendingActions(tenantId: string): ProposedAction[] {
        const now = new Date();
        const pending: ProposedAction[] = [];

        for (const [, action] of this.pendingActions) {
            if (action.tenantId === tenantId && action.status === 'pending') {
                if (now > action.expiresAt) {
                    action.status = 'expired';
                } else {
                    pending.push(action);

...
const MODEL_PRESETS: Record<AIProviderType, Record<string, ModelPreset>> = {
  openai: {
    "gpt-5.4-mini": {
      name: "GPT-5.4 Mini",
      description: "Current balanced model for strong quality at controlled cost",
      inputCostPer1K: 0.00075,
      outputCostPer1K: 0.0045,
      contextWindow: 400_000,
      tags: ["recommended", "budget", "reasoning"],
    },
    "gpt-5.4-nano": {
      name: "GPT-5.4 Nano",
      description: "Lowest-cost current OpenAI model for lightweight use cases",
    ...
    "gpt-5.4": {
      name: "GPT-5.4",
      description: "Higher-quality current OpenAI model for complex tasks",
  ...
  claude: {
    "claude-sonnet-4-6": {
      ...
      description: "Current Claude model for stronger production reasoning quality",
    ...
    "claude-haiku-4-5-20251001": {
      ...
      description: "Default fast, lower-cost Claude model for lighter workloads",
      inputCostPer1K: 0.001,
      outputCostPer1K: 0.005,
      contextWindow: 200_000,
      tags: ["recommended", "budget", "fast"],
    },
    "claude-opus-4-8": {
      name: "Claude Opus 4.8",
      description: "Premium Claude model for the most complex reasoning tasks",
    ...
    "claude-3-5-sonnet-20241022": {
      ...
      description: "Legacy Claude model with high reasoning quality",
      inputCostPer1K: 0.003,
      outputCostPer1K: 0.015,
      contextWindow: 200_000,
...
export class AIConfigurationService {
  ...
  async saveProviderConfig(config: AIProviderConfig): Promise<AIProviderConfig> {
    try {
      ...
      if (config.providerType === "lmstudio") {
        // Normalize caller-provided values too — a whitespace-only or
        // trailing-slash endpointUrl must not bypass the resolver.
        config.endpointUrl = resolveLMStudioBaseUrl(
          config.endpointUrl?.trim() || process.env.LMSTUDIO_BASE_URL,
        );
      }
  ...
  async getProviderConfigs(userId: number, providerType?: AIProviderType): Promise<AIProviderConfig[]> {
    try {
      ...
      // Dynamic Override: keep saved LM Studio configs usable at runtime.
      // - Operator pinned LMSTUDIO_BASE_URL -> that value (resolved) wins, so an
      //   explicit choice is never rewritten and saved non-local endpoints are
      //   not clobbered by a default.
      // - Env unset -> canonicalize the saved value: legacy rows persisted
      //   before the WSL-aware resolver hold loopback URLs that cannot reach
      //   the Windows-side LM Studio from WSL (NAT); non-loopback values and
      //   non-WSL/mirrored environments pass through unchanged.
      const envLMStudioBaseUrl = process.env.LMSTUDIO_BASE_URL?.trim();
      for (const config of configs) {
        if (config.providerType === 'lmstudio') {
          const resolvedLMStudioBaseUrl = envLMStudioBaseUrl
            ? resolveLMStudioBaseUrl(envLMStudioBaseUrl)
            : canonicalizeLMStudioBaseUrl(config.endpointUrl);
          config.endpointUrl = resolvedLMStudioBaseUrl;
          // Also update the nested configuration object if it exists to keep UI consistent
          if (config.configuration) {
            config.configuration.baseURL = resolvedLMStudioBaseUrl;
            config.configuration.baseUrl = resolvedLMStudioBaseUrl; // Handle both casings
          }
        }
      }

...
import { FIXTURE_DATA_SOURCE } from '../../utils/dataProvenance';

// Query request from user
export interface NLQueryRequest {
    query: string;
    userId: string;
    /** F4: verified tenant from the JWT — REQUIRED so write-intent proposals land tenant-scoped. Never body-sourced. */
    tenantId: string;
    sessionId?: string;
...
export class NLQueryOrchestrator {
    ...
    /**
     * Execute a capability using fixture data.
     */
    private async executeCapability(resolution: QueryResolution): Promise<NLQExecutionResult> {
        ...
        const response = this.getSimulatedResponse(capability.id, extractedParameters);
     ...
     * This is the sole data source — there is no live API path.
     */
    ...
    private async handleWriteIntent(
        ...
        // Propose the action (requires human approval), scoped to the
        // verified caller tenant (F4).
        const proposedAction = this.nlActionGateService.proposeAction(intent, request.tenantId);

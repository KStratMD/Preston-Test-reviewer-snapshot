...
/**
 * Raised when a value that is about to be folded into a fingerprint hash
 * (a sample digest or a directional report fingerprint) is not a well-formed
 * lowercase 64-character SHA-256 hex digest. Every digest this module
 * produces is generated exclusively via `sha256Hex`, which always satisfies
 * this shape — so in normal operation this only fires if something upstream
 * is broken (a stubbed/placeholder value, truncation, wrong casing). Failing
 * closed here is cheaper than silently combining a corrupted component into
 * a fingerprint that then looks valid.
 */
export class InvalidCardinalityFingerprintError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'InvalidCardinalityFingerprintError';
  }
}

const HEX64_PATTERN = /^[a-f0-9]{64}$/;

function assertHex64(value: string, message: string): void {
  if (!HEX64_PATTERN.test(value)) {
    throw new InvalidCardinalityFingerprintError(message);
  }
}
...
export function computeReportFingerprint(input: ReportFingerprintInput): string {
  assertHex64(input.sampleDigest, 'sampleDigest is not a valid lowercase 64-character hex digest');
...
export function computeCombinedFingerprint(reports: DirectionFingerprint[]): string {
  for (const report of reports) {
    assertHex64(
      report.fingerprint,
      'Directional report fingerprint is not a valid lowercase 64-character hex digest',
    );
  }

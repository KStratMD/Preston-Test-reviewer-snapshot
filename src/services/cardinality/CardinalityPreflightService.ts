...
import { computeCombinedFingerprint, InvalidCardinalityFingerprintError } from './fingerprint';
...
export class CardinalityPreflightService implements CardinalityPreflight, CardinalityAdvisoryEvidence {
  ...
  private async run(
    ...
    const combinedFingerprint = this.composeCombinedFingerprint(reports);

    return { reports, blocking, combinedFingerprint };
  }

  /**
   * Every fingerprint this coordinator combines is produced internally by the
   * pure analyzer, which always emits well-formed digests — so a malformed
   * directional fingerprint here means something upstream is broken, not a
   * request the caller can fix. That is the same "coordinator cannot decide"
   * class as a discovery transport failure, so it maps to the identical 503
   * inability-to-decide contract. Only the typed `InvalidCardinalityFingerprintError`
   * is translated; any other failure (a genuine programming fault) propagates
   * unchanged rather than being reinterpreted as a policy result.
   */
  private composeCombinedFingerprint(reports: CardinalityReport[]): string {
    try {
      return computeCombinedFingerprint(
        reports.map((report) => ({ direction: report.direction, fingerprint: report.fingerprint })),
      );
    } catch (error) {
      if (error instanceof InvalidCardinalityFingerprintError) {

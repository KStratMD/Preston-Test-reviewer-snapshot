...
import {
  NOOP_DLP_SCAN_METRICS,
  type DLPScanKind,
  type DLPScanMetricsRecorder,
  type DLPScanOutcome,
} from './DLPScanMetrics';
...
export class DLPService {
  ...
  constructor(
    ...
    // No @optional(): B2.2 limits the NOOP fallback to DIRECT unit
    // construction. Through the container, a missing binding must fail loudly
    // rather than silently disable telemetry (found adversarially in B2
    // review — an @optional() resolution succeeded with the NOOP recorder).
    @inject(TYPES.DLPScanMetrics) private readonly metrics: DLPScanMetricsRecorder = NOOP_DLP_SCAN_METRICS,
  ) {
  ...
  async scanForPII(data: unknown, policy?: DLPPolicy): Promise<PIIDetectionResult> {
    ...
    let outcome: DLPScanOutcome = 'failed';
    ...
    try {
      ...
      outcome = normalizedFindings.length > 0 ? 'findings' : 'clean';
      this.logScanCompletion('structured', outcome, normalizedFindings.length, riskLevel, normalizedFindings);

      return result;

    } catch (error) {
      this.logger.error('PII scan failed', { errorName: this.sanitizeErrorName(error) });
      return {
        detected: false,
        piiTypes: [],
        findings: [],
      ...
      this.metrics.recordScanOutcome('structured', outcome);
    }
  }
  ...
  public async scanText(text: string, policy?: DLPPolicy): Promise<{
    ...
    try {
      ...
      this.logScanCompletion('text', outcome, findings.length, this.assessRiskLevel(findings), findings);
      ...
      this.logger.error('PII text scan failed', { errorName: this.sanitizeErrorName(error) });
      throw error;
    } finally {
      this.metrics.recordScanOutcome('text', outcome);
    }
  }

  private logScanCompletion(
    kind: DLPScanKind,
    outcome: DLPScanOutcome,
    ...
    const metadata = {
      findingsCount,
      riskLevel,
      categoryCounts: this.groupFindingsByCategory(findings),
    };
    const message = kind === 'structured' ? 'PII scan completed' : 'PII text scan completed';
  ...
  private groupFindingsByCategory(findings: PIIFinding[]): Partial<Record<PIICategory | 'unknown', number>> {
    const categoryCounts: Partial<Record<PIICategory | 'unknown', number>> = {};
    const categories = new Map(this.piiPatterns.map(pattern => [pattern.type, pattern.category]));

    for (const finding of findings) {
      const category = categories.get(finding.type as DLPPIIType) ?? 'unknown';
  ...
  private static readonly SAFE_ERROR_NAMES: ReadonlySet<string> = new Set([
    'Error',
    'TypeError',
    'RangeError',
    'ReferenceError',
  ...
  private sanitizeErrorName(error: unknown): string {
    if (!(error instanceof Error)) return 'NonErrorThrown';
    // Closed allowlist, not shape validation: `error.name` is writable, so an
    // identifier-shaped value like "SSN123456789" passes any syntax check and
    // would put attacker-controlled (or PII-bearing) content into the log.
    // Only names from a fixed set may appear verbatim; everything else
    // collapses to 'Error' (found adversarially in B2 review).
    return DLPService.SAFE_ERROR_NAMES.has(error.name) ? error.name : 'Error';
  }
   ...
   *
   * PR-C3.1a R1 — exposed `public` (was `private`) so per-tenant posture
   * consumers can re-redact a scanned payload using a NARROWED `findings`
   * array (e.g. filtered to `posture.piiTypes`). The internal callers at
   * `scanForPII` (L530) and `scanText` (L611) continue to pass the full
   * unfiltered findings; external callers may pass a subset to honor
   * tenant-configured per-finding selectivity. The function itself is
   * pure (no side effects, no mutation of input) so exposure carries no
   * additional risk surface.
   */
  public redactData(data: unknown, findings: PIIFinding[], currentPath = ''): unknown {

...
import { isHostedCredentialCustodyRequired, validateHostedCredentialCustody } from './hostedCredentialCustody';
...
export class ConfigurationService {
  ...
  public async loadConfigurations(): Promise<void> {
    try {
      ...
      this.refreshHostedConfigurationHealth();
  ...
  private async loadSingleConfiguration(fileName: string, seenKeys: Set<string>, target: Map<string, IntegrationConfig>): Promise<void> {
    ...
    if (this.shouldEnforceHostedCredentialCustodyOnLoad()) {
      this.assertHostedCredentialCustody(config);
    }
    ...
    if (this.shouldEnforceConfigurationSchemaOnLoad()) {
  ...
  public getConfigurationForTenant(tenantId: string, id: string): IntegrationConfig | undefined {
    const config = this.configurations.get(storageKey(tenantId, id));
   ...
   *
   * Filters the Map by tenantId BEFORE cloning (Copilot review, PR #1127) —
   * delegating to `getAllConfigurations().filter(...)` would clone every
   * tenant's configs and then discard most of them, an avoidable per-request
   * cost that compounds with downstream cloning (e.g. `toExternalIntegrationConfig`).
   * Each returned record is still cloned exactly once.
   */
  public getAllConfigurationsForTenant(tenantId: string): IntegrationConfig[] {
    ...
    for (const config of this.configurations.values()) {
      if (config.tenantId === tenantId) {
        matches.push(structuredClone(config));
      }
    }
    return matches;
  }
  ...
  public getAllConfigurations(): IntegrationConfig[] {
    return Array.from(this.configurations.values(), config => structuredClone(config));
  }
   ...
   *
   *   Class-wide invariant (A4): every public accessor that hands out a stored
   *   configuration — this method's return value, `getConfiguration`,
   *   `getConfigurationForTenant`, `getAllConfigurations`, and
   *   `getAllConfigurationsForTenant` (filters the Map by tenantId, then
   *   clones only the matches — see its own doc comment) —
   *   returns a `structuredClone`, never the live Map value. A caller can
   *   never mutate tenant-scoped storage through a read, and storage can never
   *   be mutated by a caller holding an earlier read across an unrelated save
   *   failure. `exportAll` is the one exception by design: it iterates
   *   `this.configurations` directly and clones once, inside the external
   *   serializer (`toExternalIntegrationConfig`), rather than cloning twice by
   *   sourcing from `getAllConfigurations`. Internal persistence helpers
   *   (`persistWithRollback`, `saveConfigurationToFile`, etc.) retain direct
   *   Map access — this invariant covers only what crosses a public accessor.
   */
  ...
  /**
   * The ordinary runtime reader is fail-closed in production-strength modes.
   * SecureConfigurationService overrides this only for its explicit legacy
   * migration reader, which must first load inline credentials and then replace
   * them with managed references before persisting.
   */
  protected shouldEnforceHostedCredentialCustodyOnLoad(): boolean {
    return true;
  }

  /**
   * Production-strength runtime loads use the current schema. Development and
   * test loads preserve the fixture/migration compatibility of the old reader.
   */
  protected shouldEnforceConfigurationSchemaOnLoad(): boolean {
    return isHostedCredentialCustodyRequired(process.env.NODE_ENV);
  }

  private refreshHostedConfigurationHealth(): void {
    const hostedDemo = ['1', 'true', 'yes', 'on'].includes((process.env.HOSTED_DEMO ?? '').trim().toLowerCase());
    if (!hostedDemo || !process.env.HOSTED_CONFIG_EXPECTED_KEYS) return;

    let expectedKeys: string[];
    try {
      const parsed: unknown = JSON.parse(process.env.HOSTED_CONFIG_EXPECTED_KEYS);
    ...
    const hostedConfigs = [...this.configurations.values()]
      .filter(config => expected.has(`${config.tenantId}:${config.id}`));
    const activeCount = hostedConfigs.filter(config => config.isActive).length;
    process.env.HOSTED_CONFIG_EXPECTED_COUNT = String(expected.size);
    process.env.HOSTED_CONFIG_PERSISTED_COUNT = String(hostedConfigs.length);
    process.env.HOSTED_CONFIG_ACTIVE_COUNT = String(activeCount);
    process.env.HOSTED_CONFIG_INACTIVE_DRAFT_COUNT = String(hostedConfigs.length - activeCount);
  }
  ...
  private async persistWithRollback(config: IntegrationConfig): Promise<void> {
    ...
    try {
      ...
      // Only roll back if this write still owns the in-memory slot. A newer
      // concurrent save for the same tenant/id must not be erased by an older
      // write that finishes later.
      if (this.configurations.get(key) === config) {

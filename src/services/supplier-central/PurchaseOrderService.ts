...
export class PurchaseOrderService {
   ...
   *
   * @param vendors - Seeded vendor list from VendorDirectory.
   * @param nowMs - Pinned seed time from TenantSandbox (same value passed to
   *   VendorDirectory.seedDemoData). PO IDs use a fixed prefix + index so every
   *   tenant's clone is byte-identical — mirroring the vendor ID strategy.
   */
  seedDemoPurchaseOrders(vendors: VendorProfile[], nowMs: number): void {

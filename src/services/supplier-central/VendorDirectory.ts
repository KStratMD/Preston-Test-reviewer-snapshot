...
export class VendorDirectory {
   ...
   *
   * @param nowMs - Pinned seed time from TenantSandbox. Used for relative timestamps
   *   (e.g. "30 days ago"). Vendor IDs use a fixed SEED_EPOCH + index so every tenant's
   *   clone is byte-identical — TenantSandbox calls this.now() freshly per tenant, so
   *   nowMs differs across tenants and cannot be used in IDs if cross-tenant identity
   *   of the seed is required.
   */
  seedDemoData(nowMs: number): void {
    ...
    const defaultTemplate: OnboardingTemplate = {
      ...
      isActive: true,
      isDefault: true,
      createdAt: nowMs - (30 * 24 * 60 * 60 * 1000),
      updatedAt: nowMs - (7 * 24 * 60 * 60 * 1000),
    };
    ...
    sampleVendors.forEach((sample, index) => {
      // Use index-only ID (no time component) so every tenant's clone is byte-identical.
      // TenantSandbox calls this.now() freshly per tenant, so nowMs differs across tenants;
      // embedding nowMs in the ID would produce different IDs per tenant.
      const id = `vendor_demo_${index}`;

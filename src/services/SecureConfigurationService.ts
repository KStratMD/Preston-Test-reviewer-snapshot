...
export class SecureConfigurationService extends ConfigurationService {
  ...
  // This service's load path is intentionally a legacy migration reader. It
  // must inspect inline credentials so migrateToSecureCredentials can replace
  // them with secret-manager references; the ordinary ConfigurationService
  // runtime reader remains fail-closed.
  protected override shouldEnforceHostedCredentialCustodyOnLoad(): boolean {
    return false;
  }

  protected override shouldEnforceConfigurationSchemaOnLoad(): boolean {
    return false;
  }
  ...
  async migrateToSecureCredentials(context: ConfigurationCommandContext): Promise<{
    ...
    try {
      ...
      for (const integration of integrations) {
        try {
          ...
          const detail = error instanceof Error
            && error.message === 'Managed credential migration requires an explicit systemId'
            ? error.message
            : 'credential migration failed';
          const errorMsg = `Failed to migrate integration ${integration.id}: ${detail}`;
          migrationResult.errors.push(errorMsg);
          this.logger.error('Failed to migrate integration during secure credential migration', {
            integrationId: integration.id,
            error,
          });
        }
      ...
      this.logger.error('Secure credential migration failed', { error });
      throw error;
    }
  }

...
export type ApprovalOperationType = 'ai_call' | 'connector_write' | 'audit_log';
...
export class ApprovalQueueRepository {
   ...
   */
  async countPendingForTenant(tenantId: string, nowIso?: string): Promise<number> {
  ...
  async claimForApply(args: {
    ...
    const result = await db
      .updateTable('governance_approvals')
      .set({ apply_idempotency_key: args.idempotencyKey })
      .where('tenant_id', '=', args.tenantId)
      .where('id', '=', args.id)

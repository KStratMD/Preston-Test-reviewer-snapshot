...
import type { Database, GovernanceApproval, NewAuditLog } from '../../database/types';
 ...
 */
/** How the requester on an approval row was identified when it was raised. */
...
/** The platform account a request came from, and that an approver must match. */
...
export interface NewPendingApprovalRow {
  ...
  /**
   * How the requester was identified. Omitted means 'unknown' — a caller that
   * never considered provenance must not read as a verified one.
   */
  requesterProvenance?: RequesterProvenance;
  /**
   * The scope an approver must match. Null for JWT and system requests, which
   * belong to no platform account and are approvable by any verified approver
   * in the tenant.
   */
  requesterScope?: ApprovalScope | null;
  /**
   * A8 dedupe key — 64 lowercase hex characters from
   * `computeApprovalFingerprint`. Set on ownership_write rows only; omitted
   * (NULL) everywhere else, which is what keeps the partial unique index from
   * constraining arms that were never deduped.
   */
...
export interface PersistedApproval {
  ...
  tenantId: string;
  requesterUserId: string;
  /** How the requester was identified when this row was raised. */
  requesterProvenance: RequesterProvenance;
  /** The scope an approver must match, or null for JWT/system requests. */
  requesterPlatform: string | null;
  requesterPlatformAccountId: string | null;
  operationType: ApprovalOperationType;
  resourceType: string;
  resourceId: string;
  riskLevel: ApprovalRiskLevel;
...
/**
 * Anything that can run a query: the pooled connection or an open transaction.
 * Threading this through lets a decision and its audit share one transaction
 * without duplicating the decision logic.
 */
export type ApprovalExecutor = Kysely<Database> | Transaction<Database>;
...
export type DecideOutcome =
  ...
  /**
   * The row's requester was never verified, so it cannot be APPROVED — decision
   * D-SoD, enforced here at the sink and not only in the service (Task B4b).
   * Rejection is unaffected: refusing an unverifiable request is not an act of
   * trust in it.
   */
  | { outcome: 'unverified_requester'; row: PersistedApproval };
...
/**
 * Name of the A8 partial unique index from migration 062. Postgres reports it
 * verbatim as `error.constraint`, which is what lets the conflict predicate
 * below identify THIS constraint rather than any unique violation.
 */
export const PENDING_OWNERSHIP_FINGERPRINT_INDEX =
  'uq_governance_approvals_pending_ownership_fingerprint';

/**
 * True only for a violation of the pending-ownership dedupe index.
 *
 * Identified by DRIVER CODE plus a known identifier, never by pattern-matching
 * a raw database message into control flow. Treating any unique violation as a
 * dedupe hit would make an unrelated constraint failure silently return some
 * other approval's id.
 *
 * The two engines report it differently, and both shapes were confirmed
 * against the real drivers rather than assumed:
 *  - Postgres: SQLSTATE `23505` with `constraint` set to the index name.
 *  - better-sqlite3: `SQLITE_CONSTRAINT_UNIQUE`, and NO constraint field — it
 *    names the conflicting COLUMNS instead. `(tenant_id, request_fingerprint)`
 *    carries exactly one unique index, so that column pair identifies it.
 *
 * CAUTION for callers: the Postgres error's `detail` contains the conflicting
 * VALUES, i.e. the fingerprint itself. Never log one of these errors whole.
 */
export function isPendingOwnershipFingerprintConflict(error: unknown): boolean {
  if (typeof error !== 'object' || error === null) return false;
  const candidate = error as { code?: unknown; constraint?: unknown; message?: unknown };
  const code = typeof candidate.code === 'string' ? candidate.code : '';

  if (code === '23505') {
    return candidate.constraint === PENDING_OWNERSHIP_FINGERPRINT_INDEX;
  }
  if (code === 'SQLITE_CONSTRAINT_UNIQUE') {
    const message = typeof candidate.message === 'string' ? candidate.message : '';
    return (
      message.includes('governance_approvals.tenant_id') &&
      message.includes('governance_approvals.request_fingerprint')
    );
  }
  return false;
}
...
export class ApprovalQueueRepository {
  ...
  async insertPending(row: NewPendingApprovalRow): Promise<PersistedApproval> {
    ...
    await db
      ...
      .values({
        ...
        // How the requester was identified, and the scope an approver must
        // match. Defaulting to 'unknown' rather than trusting the caller means
        // a path that never thought about provenance cannot pass for a
        // verified one.
        requester_provenance: row.requesterProvenance ?? 'unknown',
        requester_platform: row.requesterScope?.platform ?? null,
        requester_platform_account_id: row.requesterScope?.platformAccountId ?? null,
      })
      .execute();
   ...
   */
  async decide(args: DecideArgs): Promise<DecideOutcome> {
    return this.decideOn(this.db.getDatabase(), args);
  }

  /**
   * Decide, and write the audit record in the SAME transaction.
   *
   * `decide` plus a separate audit insert can leave an approved row with no
   * success audit if the second write fails: the row then says a decision
   * happened and nothing says who made it or on what basis. Here the two
   * commit together or neither does.
   *
   * The audit row is written only for an `updated` outcome. not_found,
   * expired and already_decided changed nothing, so there is nothing to
   * record — and the service turns them into 404/410/409 rather than an
   * audited event.
   *
   * Returns `DecideOutcome` exactly as `decide` does, because the service maps
   * those variants onto HTTP status codes and a different shape here would
   * silently change them.
   */
  async decideWithAudit(
    args: DecideArgs,
    auditRow: NewAuditLog,
    auditLogs: AuditLogRepository,
  ): Promise<DecideOutcome> {
    return this.db.getDatabase().transaction().execute(async (trx) => {
      const outcome = await this.decideOn(trx, args);
      if (outcome.outcome === 'updated') {
        await auditLogs.insertWithin(trx, auditRow);
      }
      return outcome;
    });
  }
  ...
  /**
   * Provenance values that mean "nobody verified this requester": the host
   * asserted an identity and nothing checked it, or the row predates provenance
   * tracking / the caller never said. Must stay identical to `sodWeak` in
   * ApprovalQueueService; the agreement is asserted in the repository tests.
   */
  private static readonly UNVERIFIED_PROVENANCE = ['host_asserted', 'unknown'] as const;

  /** Today's `decide` body, parameterised on the executor. */
  private async decideOn(executor: ApprovalExecutor, args: DecideArgs): Promise<DecideOutcome> {
    // Read ONCE. `args` is caller-supplied, so `args.decision` could be a getter
    // that answers 'approved' when the predicate consults it and 'rejected' when
    // the SET clause does — writing an approval while skipping the provenance
    // check. Codex found exactly that against the first version of this block,
    // reproducing status:"approved" for a host_asserted row. A snapshot removes
    // the second read; the validation makes an unexpected value fail loudly
    // rather than silently taking the rejection path.
    const decision = args.decision;
    if (decision !== 'approved' && decision !== 'rejected') {
      throw new Error(`decide() got an unsupported decision: ${String(decision)}`);
    }
    const result = await executor
      ...
      // Approving an unverifiable requester is refused by the WRITE, not by a
      // prior read, so there is no window between the two (Task B4b, limit (i)
      // from the Codex review of #1250). Rejection keeps the old predicate set.
      .$if(decision === 'approved', (qb) =>
        qb.where('requester_provenance', 'not in', [...ApprovalQueueRepository.UNVERIFIED_PROVENANCE]),
      )
      .executeTakeFirst();

    // numUpdatedRows is a bigint in kysely; coerce for arithmetic.
    if (Number(result.numUpdatedRows ?? 0n) === 1) {
      const row = await this.getByIdInternal(args.tenantId, args.id, executor);
      if (!row) {
    ...
    const existing = await this.getByIdInternal(args.tenantId, args.id, executor);
    if (!existing) {
    ...
    // Still pending and unexpired, but the approve predicate excluded it: the
    // requester was never verified. Reported as itself rather than falling
    // through to already_decided, which would be a false explanation.
    if (
      decision === 'approved' &&
      existing.status === 'pending' &&
      (ApprovalQueueRepository.UNVERIFIED_PROVENANCE as readonly string[]).includes(
        existing.requesterProvenance,
      )
    ) {
      return { outcome: 'unverified_requester', row: existing };
    }
  ...
  /**
   * A8 — load the row currently occupying a fingerprint's dedupe slot.
   *
   * Scoped to exactly the partial index's domain (tenant + fingerprint +
   * ownership + pending) so it can only ever return the row that actually
   * blocked an insert. It deliberately does NOT filter on `expires_at`: the
   * caller needs to distinguish a live blocker (return its id — the enqueue
   * was a duplicate) from a stale unreaped one (expire it and retry), and a
   * TTL filter here would erase that distinction by hiding the stale row.
   */
  async findPendingOwnershipByFingerprint(
    tenantId: string,
    fingerprint: string,
  ): Promise<PersistedApproval | null> {
    const db = this.db.getDatabase();
    const row = await db
      ...
      .where('operation_type', '=', 'ownership_write')
      .where('status', '=', 'pending')
      .executeTakeFirst();
  ...
  /**
   * A8 — compare-and-swap a STALE pending row out of the dedupe slot.
   *
   * The partial unique index cannot express `expires_at > now` (a partial
   * index predicate must be immutable), so a pending row past its TTL keeps
   * blocking new intent until the background sweep reaches it. This is the
   * targeted, tenant-scoped version of that sweep.
   *
   * Every condition is in the predicate, not in a preceding read: a live row
   * and a terminal row must both survive untouched even if a concurrent
   * transaction decides or refreshes the row between the caller's read and
   * this write. `expires_at <= nowIso` is the CAS.
   */
  async expirePendingFingerprintIfStale(args: {
    tenantId: string;
    fingerprint: string;
    nowIso: string;
  }): Promise<boolean> {
    ...
    const result = await db
      .updateTable('governance_approvals')
      .set({ status: 'expired', decided_at: args.nowIso })
      .where('tenant_id', '=', args.tenantId)
      .where('request_fingerprint', '=', args.fingerprint)
      .where('operation_type', '=', 'ownership_write')
      .where('status', '=', 'pending')
      .where('expires_at', '<=', args.nowIso)
      .executeTakeFirst();

    // Same rule as A7's touchAttempt: "zero rows" and "the driver did not
    // report a count" are different facts and must not be coalesced. The
    // caller does not depend on this boolean for correctness — it retries the
    // insert either way — so an unknown count reports false rather than
    // throwing, and the retry settles the truth.
  ...
  private async getByIdInternal(
    tenantId: string,
    id: string,
    executor: ApprovalExecutor = this.db.getDatabase(),
  ): Promise<PersistedApproval | null> {
...
function rowToPersistedApproval(row: GovernanceApproval): PersistedApproval {
  return {
    ...
    // Rows written before migration 065 have no provenance; they read as
    // 'unknown', which is the conservative answer rather than a claim.
    requesterProvenance: (row.requester_provenance ?? 'unknown') as RequesterProvenance,
    requesterPlatform: row.requester_platform ?? null,
    requesterPlatformAccountId: row.requester_platform_account_id ?? null,

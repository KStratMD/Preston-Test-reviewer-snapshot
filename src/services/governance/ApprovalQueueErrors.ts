 ...
 */
/**
 * Raised when a decision would violate separation of duties.
 *
 * Lives here rather than beside the service so the route-layer error handler
 * can import it as a VALUE without pulling in ApprovalQueueService — that edge
 * created a runtime import cycle which left `TYPES` uninitialised at decorator
 * time and broke every entry point reaching the handler.
 *
 * Four reasons rather than one, because they mean different things to whoever
 * is looking: an unverified approver is a missing grant, a self approval is one
 * person acting as both parties, and a scope mismatch is an approver reaching
 * outside their own platform account.
 */
export class SeparationOfDutiesError extends Error {
  ...
  constructor(
    public readonly reason:
      ...
      | 'self_approval'
      | 'scope_mismatch'
      | 'unverified_requester',
    public readonly approvalId: string,
  ) {
    super(`separation of duties violated (${reason}) for approval ${approvalId}`);

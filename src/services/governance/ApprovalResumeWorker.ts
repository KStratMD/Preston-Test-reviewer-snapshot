...
export class ApprovalResumeRegistry {
  ...
  /**
   * Default handlers keyed by operationType (PR 13b Stage B).
   * Registered via `setDefault(operationType, handler)` and used as a
   * fallback when `resolve(operationType, resourceType)` finds no exact match.
   * The sentinel resourceType '*' on OwnershipResumeHandler is never stored
   * as an exact-match key — the handler is only reachable via this map.
   */
  private readonly defaultsByOperationType = new Map<ApprovalOperationType, ApprovalResumeHandler>();
  ...
  /**
   * Register a fallback handler for all resourceTypes under a given
   * operationType (PR 13b Stage B). `resolve()` returns this handler
   * when no exact (operationType, resourceType) key matches.
   *
   * Throws on duplicate `setDefault` for the same operationType —
   * silently overwriting would cause two handlers to fight for the
   * same fallback slot with no audit trail.
   */
  setDefault(operationType: ApprovalOperationType, handler: ApprovalResumeHandler): void {
    if (this.defaultsByOperationType.has(operationType)) {
      throw new Error(
        `ApprovalResumeRegistry: duplicate setDefault for operationType=${operationType}`,
      );
    }
    this.defaultsByOperationType.set(operationType, handler);
  }

  /**
   * Resolve a handler for (operationType, resourceType).
   * Exact key wins; default-by-operationType is the fallback.
   */
  resolve(
    operationType: ApprovalOperationType,
    resourceType: string,
  ): ApprovalResumeHandler | null {
    const exact = this.handlers.get(ApprovalResumeRegistry.makeKey(operationType, resourceType));
    if (exact) return exact;
    return this.defaultsByOperationType.get(operationType) ?? null;
  }

  /** Test/debugging hook: how many exact handlers are registered. */
  size(): number {
    return this.handlers.size;
  }

  /** Test/debugging hook: list registered exact keys. */
...
export class ApprovalResumeWorker {
  ...
  async resume(approval: PersistedApproval): Promise<ResumeOutcome> {
    ...
    try {
      ...
      try {
        await this.repo.markApplySucceeded({
          tenantId: approval.tenantId,
          id: approval.id,
        ...
        this.logger.error(
          'ApprovalResumeWorker.resume — markApplySucceeded failed after handler.apply succeeded',
          markErr instanceof Error ? markErr : new Error(markMessage),
          { approvalId: approval.id },
        );
      }
      ...
      try {
        await this.repo.markApplyFailed({
          tenantId: approval.tenantId,
          id: approval.id,
          error: message,
        ...
        const markMessage = markErr instanceof Error ? markErr.message : String(markErr);
        this.logger.error(
          'ApprovalResumeWorker.resume — markApplyFailed failed after handler.apply threw',
          markErr instanceof Error ? markErr : new Error(markMessage),
          { approvalId: approval.id },
        );
      }
      // Apply failure lifecycle (PR 13c-3 Task 9): `markApplyFailed` (just
      // above) persists the lifecycle state — `apply_status='failed'`,
      // `apply_failed_at`, `apply_error` — via migration 051's columns on
      // governance_approvals. The CAS claim has already set
      // apply_idempotency_key, so subsequent resume() invocations return
      // `skipped: 'already_claimed'`. Recovery: an admin clears the claim
      // via `POST /api/governance/approvals/:id/reset-claim` (requires
      // non-empty reason, gated by ADMIN_ROLES, audited with adminUserId +
      // reason + outcome), after which the next resume() picks the row up
      // again. Without that reset there is no automatic retry path for
      // transient failures.
      //
      // We additionally emit a structured ERROR-level log with the full
      // triage triplet (approvalId, operationType, resourceType) + tenant
      // so operators can spot failures via the log pipeline (in addition
      // to querying by apply_status). The surface-tag `apply_failed` makes
      // log queries grep-friendly.
      this.logger.error(

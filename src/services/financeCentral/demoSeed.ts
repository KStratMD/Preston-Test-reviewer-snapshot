 ...
 * dashboard reads the same source as the operator service. Calls are gated by
 * the shared `shouldSeedDemoData` predicate: ordinary production starts never
 * insert demo rows and jest never does, but a HOSTED_DEMO=1 deployment seeds so
 * the hosted demo dashboard shows content (F5b-3). Tests opt in by exercising
 * the repo directly.
 *

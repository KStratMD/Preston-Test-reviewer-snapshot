...
export class WorkflowEngineService {
   ...
   * inserts these rows inside a TX via repo.insertInstanceIfMissing, so a
   * re-seed skips ids that already exist instead of raising.
   *
   * tenantId comes from the caller (F5b-3: the demo tenant). It was hardcoded
   * to the system tenant per spec D6 until the demo fixtures moved onto their
   * own tenant; a required parameter is what stops that from regressing.
   * pausedFromStatus is null for all 3 (none are paused).
   */
  getDemoInstanceRows(tenantId: string): NewInstanceRow[] {
    const now = new Date();
    return [
      {
        id: 'INST-1000',

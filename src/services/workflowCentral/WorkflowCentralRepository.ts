...
export class WorkflowCentralRepository {
  ...
  /**
   * Column mapping for a new task row. Shared by insertTask and
   * insertTaskIfMissing so the two write paths cannot drift.
   */
  private taskRowToColumns(row: NewTaskRow): Insertable<WorkflowCentralTaskTable> {
    return {
      id: row.id,
  ...
  async insertTask(tx: Kysely<Database>, row: NewTaskRow): Promise<void> {
    await tx
      ...
      .values(this.taskRowToColumns(row))
      .execute();
  }

  /**
   * Conflict-tolerant INSERT for fixed-id seed rows. Returns true when the row
   * was inserted, false when an id already existed.
   *
   * F5b-3: the demo seeds previously wrapped insertTask in try/catch to fake
   * ON CONFLICT DO NOTHING. On Postgres that is unsafe — the first caught
   * constraint violation aborts the surrounding transaction, so every later
   * insert in the same TX fails with "current transaction is aborted". A real
   * onConflict clause never raises, so re-seeding stays a clean no-op.
   */
  async insertTaskIfMissing(tx: Kysely<Database>, row: NewTaskRow): Promise<boolean> {
    // RETURNING (not the driver's affected-row count) is the portable signal:
    // a conflict-skipped insert yields no row on both SQLite and Postgres,
    // whereas numInsertedOrUpdatedRows is typed optional per dialect.
  ...
  async insertInstance(
    ...
    const result = await tx
      ...
      .values(this.instanceRowToColumns(row))
      .returningAll()
      .executeTakeFirstOrThrow();
  ...
  /**
   * Column mapping for a new instance row. Shared by insertInstance and
   * insertInstanceIfMissing so the two write paths cannot drift.
   */
  private instanceRowToColumns(row: NewInstanceRow): Insertable<WorkflowCentralInstanceTable> {
  ...
  /**
   * Conflict-tolerant INSERT for fixed-id seed rows. Returns true when the row
   * was inserted, false when an id already existed. See insertTaskIfMissing for
   * why the demo seeds need a real onConflict clause rather than try/catch.
   */
  async insertInstanceIfMissing(tx: Kysely<Database>, row: NewInstanceRow): Promise<boolean> {
    const inserted = await tx
      .insertInto('workflow_central_instances')
      .values(this.instanceRowToColumns(row))
      .onConflict((oc) => oc.column('id').doNothing())

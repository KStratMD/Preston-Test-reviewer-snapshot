...
export const observabilityConfig: ObservabilityConfig = {
  tracing: {
    ...
    // OTLP/HTTP trace endpoint (e.g. http://jaeger:4318/v1/traces), using the
    // OTel-standard env var. The legacy JAEGER_ENDPOINT var is intentionally
    // NOT read: it was never actually wired through to the exporter
    // (field-name mismatch swallowed by an `as any`), and its historical
    // values were Jaeger *thrift* URLs (:14268/api/traces) that would be
    // wrong for the OTLP exporter.

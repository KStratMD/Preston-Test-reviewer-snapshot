...
export const buildQueueConnectionOptions = (): BullMqConnectionOptions => {
  if (env.REDIS_URL) {
    ...
    // ioredis uses an embedded URL password ahead of this separate option.
    // Rotating REDIS_PASSWORD alone therefore works only when the URL omits
    // its password. The retained precedence finding and follow-up are recorded
    // in docs/review/2026-09-12-maintenance-ioredis.md.
    return {
      url: env.REDIS_URL,

...
/**
 * Tenant-setting key for the per-tenant half of the NetSuite serialized-asset
 * sync activation gate (Task 1, 2026-07-27 NetSuite serialized-asset sync
 * plan — decision 7: activation requires BOTH this tenant setting AND
 * `NETSUITE_SERIALIZED_ASSET_SYNC_ENABLED`, and both default closed).
 */
export const NETSUITE_SERIALIZED_ASSET_SYNC_SETTING_KEY =
  'integration.netsuite_serialized_asset.enabled';

/**
 * Env half of the NetSuite serialized-asset sync activation gate. Default
 * closed — unset/falsy keeps the profile unreachable at activation
 * regardless of the per-tenant setting (Task 6 combines both halves).
 */
export function isNetSuiteSerializedAssetSyncGloballyEnabled(): boolean {
  return isEnabled(process.env.NETSUITE_SERIALIZED_ASSET_SYNC_ENABLED);
}

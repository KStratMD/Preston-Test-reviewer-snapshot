...
describe('AI Mapping Suggestions - dataset weighting flow', () => {
  ...
  const previousFullAppMode = process.env.FORCE_FULL_APP_MODE;

  beforeAll(async () => {
    // The anonymous suggestions request below uses the demo fixture route.
    // Pin its runtime posture instead of inheriting the demo-mode setting.
  ...
  afterAll(async () => {
    try {
      ...
      if (previousFullAppMode === undefined) delete process.env.FORCE_FULL_APP_MODE;
      else process.env.FORCE_FULL_APP_MODE = previousFullAppMode;
    }
  });
  ...
  test('anonymous demo suggestions accept datasetId and return fixture suggestions', async () => {

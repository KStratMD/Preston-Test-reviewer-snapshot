...
import { classifyHttpFailure } from '../core/http/RetryPolicy';

function isHttpFailure(error: unknown): boolean {
  if (!error || typeof error !== 'object') return false;
  const candidate = error as { response?: { status?: unknown }; request?: unknown };
  return typeof candidate.response?.status === 'number' || ('request' in candidate && !candidate.response);
}
...
export interface RetryConfig {
  ...
  /** Generic operations are unsafe to replay unless the caller opts in. */
  replay?: 'safe' | 'unsafe';
}
...
export class RetryService {
  ...
  async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
      ...
      try {
        ...
        // Keep the custom veto first; non-HTTP policies retain their behavior.
        const httpFailure = isHttpFailure(error) ? classifyHttpFailure(error) : undefined;
        if (httpFailure && (httpFailure.kind !== 'retryable' ||
            (config.replay !== 'safe' && !httpFailure.rejectedBeforeProcessing))) {
          throw error;
        }
        ...
        const calculatedDelay = this.calculateDelay(attempt, config);
        const delay = httpFailure
          ? Math.min(httpFailure.retryAfterMs ?? calculatedDelay, config.maxDelay)
          : calculatedDelay;
  ...
  /**
   * Get retry ceilings and vetoes for common scenarios. HTTP replay remains
   * unsafe by default, including network/api presets: a caller that can prove
   * replay safety must pass { ...preset, replay: 'safe' }. A retryCondition
   * permits a failure class but cannot establish operation replay safety.
   */

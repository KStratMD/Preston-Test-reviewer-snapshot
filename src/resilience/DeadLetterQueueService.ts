...
export class DeadLetterQueueService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // NOTE: this service remains orphaned/unbound per the #1102 PR 1
      // contract; the only change here is the minimum needed to typecheck once
      // the ambient bullmq type shim is deleted. That shim declared
      // `declare module 'bullmq'` with no top-level import or export, which
      // made it a global declaration file — so it REPLACED the package's type
      // surface instead of augmenting it, and these lines had never been
      // checked against the actual API.
      //
      // Described rather than linked by path deliberately: the file is deleted
      // in this PR, and the blocking inbound-link audit resolves path-shaped
      // references in comments. Naming a deleted file would send a reader
      // looking for something that is gone.
      //
      // `retryDelayOnFailover` is not an ioredis option at all —
      // it was silently ignored at runtime, so dropping it changes no
      // behaviour. The hardcoded localhost is deliberately left as-is for
      // whichever future PR actually wires this service.
      ...
      this.deadLetterQueue = new Queue('dead-letter-queue', {
        // Cast: bullmq bundles its own nested ioredis (5.10.1) whose Redis type
        // is structurally incompatible with the top-level ioredis (5.11.1).
  ...
  async sendToDeadLetter(
    ...
    try {
      const deadLetterRecord: DeadLetterRecord = {
        ...
        originalQueue,
        // Type-only, same category as the casts above: real BullMQ types
        // `Job.id` as `string | undefined`, where the deleted shim declared it
        // `string`. This line is unchanged from before this PR — removing the
        // shim revealed the true type rather than introducing a defect, and it
        // is the sole reason the strict-null ratchet moved. Asserted rather
        // than defaulted (`?? ''`) deliberately: a default would silently
        // change what this orphaned service would write if the id were ever
        // absent, and the mandate here is strictly no behaviour change.

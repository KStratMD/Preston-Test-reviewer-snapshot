...
module.exports = {
  ...baseConfig,
  testMatch: [
    '<rootDir>/tests/unit/core/ConnectorAuthentication.entrypoints.test.ts',
    '<rootDir>/tests/unit/core/ConnectorAuthentication.lifecycle.test.ts',
    '<rootDir>/tests/unit/core/ConnectorAuthentication.reconfiguration.test.ts',
    '<rootDir>/tests/unit/core/ConnectorAuthentication.oauthCache.test.ts',
    ...
    // PR-F5b-3: demo-tenant seeding — covers the conflict-safe upserts
    // (insertInstanceIfMissing / insertTaskIfMissing) and the tenant-
    // parameterized instance producer.
    '<rootDir>/tests/unit/services/workflowCentral/demoSeed.test.ts',
    ...
    // F2 (PR-F2): the /api/ai/proxy policy gate joins the tenant/governance
    // enforcement set — same rationale as tenantStatusGate/tenantIsolation.
    '<rootDir>/tests/unit/middleware/aiProxyPolicyGate.test.ts',
    ...
    // A8: the ownership enqueue idempotency protocol lives in
    // ApprovalQueueService.ts, so its suite belongs here per the note above —
    // omitting it measures the new conflict-protocol branches with none of
    // their tests and reports a coverage REGRESSION that is purely an artifact
    // of the profile, not a real loss.
    '<rootDir>/tests/unit/services/governance/approvalEnqueueIdempotency.test.ts',
    // The separation-of-duties gate lives in ApprovalQueueService.ts and its
    // transactional decide in ApprovalQueueRepository.ts, so this suite belongs
    // here for the same reason as the A8 entry above: omitting it measures the
    // new branches with none of their tests and reports a regression that is an
    // artifact of the profile rather than a real loss.
    '<rootDir>/tests/unit/services/governance/ApprovalQueueService.separationOfDuties.test.ts',
    // Task B4b's requester policy lives in the same two files, so it belongs
    // here for exactly the reason the note above gives: without it the profile
    // measures the new branches with none of their tests and reports a
    // regression that is an artifact of the profile, not a real loss. That is
    // precisely what happened on #1250 before this line was added.
    '<rootDir>/tests/unit/services/governance/ApprovalQueueService.requesterPolicy.test.ts',
    ...
    // PR4 Stage 1: resolveActor's whole contract is delegated to
    // verifiedIdentity, so the ratchet must measure that module too.
    '<rootDir>/tests/unit/services/governance/verifiedIdentity.test.ts',
  ...
  collectCoverageFrom: [
    ...
    // Task 10: tenant/governance ratchet expansion
    // F2 (PR-F2): first runtime consumer of the F0 route policy manifest.
    'src/middleware/aiProxyPolicyGate.ts',
    ...
    'src/services/governance/verifiedIdentity.ts',
